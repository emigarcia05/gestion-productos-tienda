...
- **Área Marketing** (`/marketing/*`): SSOT rutas **`MARKETING_ROUTES`** (`src/lib/marketingRoutes.ts`). Entrada modal / `/marketing` → redirect a **`/marketing/publicaciones/calendario`**. **Calendario De Publicaciones** (`MarketingCalendarioPageClient`): header con **Gestionar Redes** y **Gestionar Tipo Publicaciones** (solo `editor`, `Settings2`); ambos abren el mismo modal estético **`GestionarMktCatalogoNombreModal`** (`kind: "red" | "tipo"`) — listado + alta inline + editar/borrar (confirmación), nombres en MAYÚSCULAS. Cuerpo: **`MktCalendarioPublicacionesGrid`** — grilla **5 semanas × 7 días** (columnas **LUN.–DOM.**); la 1.ª fila = semana ancla (al cargar = semana actual AR); días del **mes calendario actual** con tinte `bg-primary/8`; **hoy** con anillo + badge primary; navegación **← / →** (±1 semana) + **Ir A Esta Semana**. Helpers SSOT: `@/lib/mktCalendarioPublicaciones.ts`. **Ideas**: placeholder. `PERMISOS.marketing.acceso` (solo `editor`).
...
*Última actualización (2026-07-13): **Área Marketing** — cuarta opción en modal **Áreas De La Aplicación** (`MAIN_APP_AREAS` + ícono `Megaphone`); sidebar **PUBLICACIONES** con **Calendario de Publicaciones** e **Ideas**; rutas `MARKETING_ROUTES`; `PERMISOS.marketing.acceso` (solo editor). Calendario: **Gestionar Redes** / **Gestionar Tipo Publicaciones** → `GestionarMktCatalogoNombreModal` (CRUD); grilla **5 semanas** (`MktCalendarioPublicacionesGrid`).*
