...
- **`/finanzas/analisis-mc/costos-financieros`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Costos Financieros** (módulo sidebar **ANALISIS M.C.**). **Filtros**: **TERMINAL**, **PAGO**, **HABILITADO** + contador **X COMBINACIÓN(ES)** + **`LimpiarFiltrosButton`**. Tabla **`TablaFinAnaCosFina`**: **HABILITADO** e **IMP. CHEQUE** (`tabla-check-toggle`, editable solo `editor`); **TERMINAL** / **PAGO** (lectura); **DÍAS ACRED.**, **ARANCEL**, **CX FINANCIERO** (inputs; % con **`.input-mascara-sufijo`**); **CX TERMINAL C/ IVA** y **CX IMP. CHEQUE** (calculadas, `readOnly` + `%` fijo). **CX IMP. CHEQUE**: `0` si **IMP. CHEQUE** es false; si true → `(1 − CX TERMINAL C/ IVA) × 0,012` (ver `cxImpChequeFinAnaCosFina`). Mutación: **`actualizarFinAnaCosFinaAction`**. Ver `BACKEND_GUIDELINES` §2.5h.
