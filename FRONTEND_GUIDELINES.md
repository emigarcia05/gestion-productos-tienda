...
- **Layout:** `area-page-shell` + `ClassicFilteredTableLayout`. **FilterBar** `filtros-contenedor-tienda bg-card` (mismo patrón que **Px Competencia** / **Cx Compra**): fila 1 `FilaFiltrosDesplegables` — **MARCA**, **RUBRO**, **SUB-RUBRO**, **ACTUALIZAR** (SI/NO: margen manual guardado distinto o igual al margen DUX) en `FiltroIndividualContainer` + `FILTER_SELECT_WRAPPER_CLASS`, `SelectTrigger` **`input-filtro-unificado`**, `SelectContent` `select-content-filtro`; sin ítems «TODAS/TODOS» (estado vacío = `value` `undefined` + placeholder en trigger); cascada marca → limpia rubro/sub-rubro, rubro → limpia sub-rubro. Fila 2: `FilterRowSearch` `flex-1` + `FiltroBusquedaInput` + `LimpiarFiltrosButton` + `FILTER_COUNT_CLASS` (`X PRODUCTO(S)`).
...
- **Exportación DUX:** botón **Act. Px.** → un Excel **por** `nombre_lista` del catálogo (`prod_tienda_listas_precios`); columnas **CODIGO** (`cod_tienda`) + **PORC UTILIDAD** (margen manual guardado); **solo ítems con diff** de PORC UTILIDAD respecto al margen calculado desde precio DUX (`margenesPorcUtilidadDifieren`, 4 decimales); si no hay filas en ninguna lista → **`ModalSinProductosExportar`**: **No hay actualizaciones para mostrar** (sin descarga); archivo **`Act. {nombre_lista} dd-mm hh-mm.xls`**; `exportarPxListasMargenAction` + `exportPxListasMargenExcelClient.ts`. Sin filtros de página (catálogo completo por lista).
- **Tabla:** clase `tabla-px-listas-listado tabla-px-listas-precios` (scroll horizontal si hay muchas listas). **DESCRIPCIÓN** + por cada fila de `prod_tienda_listas_precios`: subcabecera con `nombre_lista` (MAYÚSCULAS), columnas **PX.** (solo lectura; entero calculado desde margen manual o precio DUX) y **MARGEN** (4 decimales + `%`; editable). **PX** = `Cx × (1 + Margen/100) × 1.21` (`calcPxListaDesdeMargenSinIvaPct`); recálculo instantáneo en cliente al tipear margen. Filas **`--tabla-body-row-min-height` (2rem)**. Celdas numéricas: **`celda-datos celda-numero`** + `celda-px-lista-col` / `celda-marcacion-col`. **MARGEN:** `<input>` directo en celda (`readOnly` hasta foco). Clase **`px-lista-input--edicion`** = negrita si hay margen manual en `prod_tienda_margen_edicion`. Guardado en blur → `guardarPxListaMargenEdicionAction`. **Resaltado** (`.celda-px-listas-actualizar`, tinte `accent2`): **DESCRIPCIÓN** + columnas PX./MARGEN de cada lista donde margen manual ≠ margen DUX.
