...
- **`/finanzas/venc-por-fecha`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Venc. por fecha**. Datos solo desde **hoy** (Argentina) hasta **hoy + 120 días** inclusive (`listarVencimientosEnRango` en servidor); sin query **`?mes=`** ni flechas de mes. Encabezado interno **Próximos 120 días** + rango **`DD/MM/YYYY — DD/MM/YYYY`** (`formatIsoYmdDdMmYyyyArgentina`). Tabla: **`contenedor-tabla-gestion no-scroll-x`**, **`Table variant="compact"`**. Columnas (anchos): **MES (15%)** texto **`formatMesAnioMayusculasDesdeIsoYmd`** + **`uppercase`** en celda; **DÍA (5%)**; **VENCIMIENTO DEL DÍA (20%)** total del día; **VTOS ACUMULADOS (20%)** suma corrida de vencimiento del día en el listado; **CAJA DISPONIBLE (20%)** (placeholder `0` hasta origen); **SALDO (20%)** acumulativo en la ventana: primera fila `CAJA − vencimiento del día`, siguientes `saldo anterior − vencimiento del día`. **Doble clic** → **`AppModal`** solo lectura (detalle por proveedor); tabla del modal con **`no-scrollbar`**. `PERMISOS.finanzas.acceso`.
