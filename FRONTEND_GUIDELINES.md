...
7. **Nuevo modal con tabla**  
   ...
   - **`PedidoHistoriaDetalleModal`:** `AppModal` `sm:max-w-[62.4rem]` (≈ +30% sobre 48rem). Tabla **5% + 55% + 10% + 15% + 15%**: columna **lista de verificación** (icono **Check** en cabecera; por fila input + OK o **Enter** → ícono check; estado solo en cliente hasta definir reglas/persistencia), **DESCRIPCIÓN**, **CANT. PEDIDA**, **CANT. RECIBIDA**, **ACCIONES**; sin **COD. TIENDA** (`title` en descripción con código si existe). (1) Resumen: hueco **5%** alineado al check; bloque proveedor **65%** (55+10); **CANT. RECIBIDA** **15%**; fecha **15%** (`GRID_CAPAS_SUP_PEDIDO_HISTORIA` = `5fr_65fr_15fr_15fr`). (2) Fila **Agregar producto**: **tres columnas** **70% + 15% + 15%** (**Agregar un producto** + input | **Cant.** + input | botón **Agregar**), sin alinear columna a columna con la tabla (`GRID_FILA_AGREGAR_PEDIDO_HISTORIA` = `70fr_15fr_15fr`, **`sm:items-center`**); cada columna hija: **`flex flex-col items-center justify-center gap-1.5 p-2`**; primera columna etiqueta/input **`text-left`**, **Cant.** **`text-center`**. Sección con padding más compacto **`p-2 sm:p-3`** (el resumen superior sigue **`p-3 sm:p-4`**). (3) **Ítems del pedido** + tabla: `MODAL_RESUMEN_PANEL_CLASS` / `MODAL_SECTION_CARD_CLASS` = `bg-transparent`. `fechaRecepcion` sin persistencia backend hasta definir campo.
