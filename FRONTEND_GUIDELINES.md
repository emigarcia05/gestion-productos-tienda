...
- **Altura de fila (obligatoria en este grid)**: **todas** las filas (incluida la cabecera de columnas) usan la misma altura fija **`h-10` (2,5 rem)**, tomada como referencia de la fila **Resultado operativo** (una línea `text-sm` sin botones). Celdas: `flex items-center`, `py-0`, `px-3`. En columnas de **sucursal**, el sublayout interno es **`30%` + `70%`** (`grid-cols-[30%_70%]`): el **30%** izquierdo es una **rejilla de dos ranuras fijas** (`grid-cols-2`) para **histórico** | **desglose/edición**; el **70%** derecho muestra el importe alineado a la derecha. Botones: `TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS` + `TABLE_ROW_ACTION_ICON_CLASS` (`@/lib/ui-classes`) con override compacto **`!h-7 !w-7 !p-1`**. Constantes `CLASE_FILA_BALANCE_MENSUAL_GRID` / `CLASE_CELDA_BALANCE_MENSUAL` / `CLASE_BOTON_ACCION_BALANCE_MENSUAL` en el componente.
