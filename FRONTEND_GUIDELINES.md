...
- **`/finanzas/balance/gastos`**: **`ClassicFilteredTableLayout`** `contentWidth="full"`, título **Balance**, subtítulo **Gastos**. **Filtros** (`FilterBar`): **fila 1** (`FilterRowSelection`) — **RUBRO**, **GASTO** (opciones acotadas si hay rubro), **SUCURSAL**, **PROVEEDOR**; **Pagado** con opción **NO** = solo filas con `montoDevengadoPendiente > 0`; contador `FILTER_COUNT_CLASS`. **Fila 2** (`FilterRowSelection`) — Select **Año** y **Mes**: opciones **solo** desde `listarPeriodosConImputacionesEnDb` (prop `periodosOpciones`); el mes listado es el subconjunto de meses que existen en BD para el año elegido. Si la URL pide un periodo sin filas y hay otros periodos en BD, el servidor **redirige** al más reciente. `FilterRowNoSearchActions` + `LimpiarFiltrosButton`. Solo `editor`: **Cargar Datos Mes.** llama `cargarFinBalGastoMensualMesAction({ mes, anio })`. Tabla **`TablaGastos`**: columnas de datos + **Acc.** (`bg-muted/25`, `Pencil` / `Trash2`); **Editar monto** → `EditarMontoFinBalGastoMensualModal` + **Repetir Ult. Monto**; **Eliminar** → `EliminarFinBalGastoMensualModal`. Pie **TOTAL** `colspan` 7. `PERMISOS.finanzas.acceso`.
...
*Última actualización (2026-04-21): **`/finanzas/balance/gastos`** — año/mes en **segunda fila** de filtros y opciones de periodo **solo desde BD** (`listarPeriodosConImputacionesEnDb`); rubro/gasto/sucursal/proveedor/pagado **NO**; acciones y modales de monto; ver `BACKEND_GUIDELINES` §2.5e.*
