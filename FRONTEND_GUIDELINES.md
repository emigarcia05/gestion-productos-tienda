...
| `TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS` (`@/lib/ui-classes`) | Botones **solo ícono** en **filas de tabla** (y equivalentes en modales de grilla): fondo **`#0072BB`**, ícono blanco, `size="icon"` + `variant="ghost"`, alto acoplado a la fila (`!h-full`, `min-h-[var(--tabla-body-row-min-height)]`, `self-stretch`). Sustituye el patrón prohibido outline + icon-xs neutro. |
