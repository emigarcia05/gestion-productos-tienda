...
- **Layout Finder (catálogos jerárquicos):** componentes compartidos en `@/components/shared/catalogo-finder/` — **`CatalogoFinderColumn`** (header **`bg-primary`** como `TableHead`, título **`CATALOGO_FINDER_COLUMN_HEADER_TITLE_CLASS`** centrado en grilla `1fr / auto / 1fr`; botón **`+`** **`CATALOGO_FINDER_COLUMN_NOVO_BUTTON_CLASS`** sobre fondo azul), **`CatalogoFinderRow`**, **`CatalogoFinderEmpty`**. Usado en **Balance · Catálogo Gastos** (5 columnas) y **Comp. Categorias · Categorias** (3 columnas).
