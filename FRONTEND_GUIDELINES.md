...
- **`/finanzas/balance/gastos`**: **`ClassicFilteredTableLayout`** título **Balance**, subtítulo **Gastos** (submódulo **Gastos** dentro del módulo **BALANCE**). Botón **Nuevo Gasto** (solo `editor`) en **`actions`** del **`ClassicPageHeader`** (`h-10 px-4`, `Plus`) abre **`NuevoGastoModal`** (`AppModal`, título **Crear Gasto**, `sm:max-w-xl`) con campos **TIPO GASTO** (`EFECTIVO | BANCO | CHEQUE`), **NOMBRE** (persistido en MAYÚSCULAS), **SUCURSAL** (FK a `Sucursal`, desplegable con `nombre`) y **MONTO** (`Input type="number" step="0.01"`, dos decimales). Guarda con `crearMovimientoFinanzasAction` y `router.refresh()`. Tabla **`TablaGastos`** (sin filtros aún): columnas **TIPO GASTO**, **NOMBRE** (`celda-destacado` + `truncate`) y **MONTO** (alineado a derecha, `$` + `fmtPrecio`); fila de pie **TOTAL** suma la columna **MONTO** (colspan 2). Datos desde `listarMovimientosFinanzas` (modelo Prisma `MovimientoFinanzas` = `movimientos_finanzas`), orden por `createdAt` descendente, nombre mostrado en MAYÚSCULAS. Sin paginación. `PERMISOS.finanzas.acceso`.
