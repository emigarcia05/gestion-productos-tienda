...
8. **Tablas (encabezado fijo + paginación)**  
   ...
   - **Pie de totales fijo** (patrón `contenedor-tabla-gestion--pie-fijo`, dos `<table>`): `scrollbar-gutter: stable` en scroll y pie (`globals.css`). Para alineación exacta con la barra vertical u otros anchos útiles distintos, usar **`usePieFijoColumnWidthsSync`** (`src/lib/hooks/usePieFijoColumnWidthsSync.ts`): ref en `.contenedor-tabla-gestion--pie-fijo-scroll`, ref en la `<table>` del pie, mismo número de `<col>` que de `th`; pasar firma de columnas si cambia el layout (ej. **ACCIONES**). Referencia: **`TablaGastos`**, **`TablaTesoreriaCajas`**.
...
- **`/finanzas/venc-por-fecha`**: página servidor + cliente **`FinanzasVencPorFechaPageClient`** (`src/components/finanzas/FinanzasVencPorFechaPageClient.tsx`). **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Flujo De Fondo**; los filtros van en la prop **`filters`** (mismo patrón que **Tesorería** / **Control Comprobantes**): **FilterBar** una fila con Select **PROVEEDOR** + **`LimpiarFiltrosButton`** en **`FILTER_INLINE_ACTION_SLOT_CLASS`**. Datos solo desde **hoy** (Argentina) hasta **hoy + 150 días** inclusive (`listarVencimientosEnRango` en servidor); sin query **`?mes=`** ni flechas de mes. **Tabla principal** (mismo cascarón que **`TablaDeudaProveedores`** / **`TablaControlComprobantes`**): tarjeta **`contenedor-tabla-gestion flex min-h-0 flex-1 flex-col overflow-hidden rounded-md border border-border bg-card`** → hijo scroll **`flex-1 min-h-0 min-w-0 overflow-x-auto overflow-y-auto`** → **`Table variant="compact"`** **`scrollX={false}`** **`table-fixed`** + **`<colgroup>`** en **20%** × cinco columnas; celdas numéricas **`TH_NUM` / `TD_NUM`** (`celda-datos text-right tabular-nums`). La grilla lista **todos los días calendario** de la ventana (incluye días con **VENCIMIENTO DEL DÍA** en `$0,00` cuando no hay comprobantes vencidos ese día). Columnas: **FECHA** (`formatMesDiaMayusculasDesdeIsoYmd`, **`uppercase`** en celda); **VENCIMIENTO DEL DÍA** (si hay proveedor seleccionado, solo ese proveedor por día); **VTOS ACUMULADOS** = total de saldos con vencimiento **anterior a hoy** (`sumarSaldoVencimientosConFechaVencAnteriorA`) + suma corrida del día; **CAJA DISPONIBLE**: primera fila suma `fin_tesoreria_cajas`, desde la segunda `max(SALDO fila anterior, 0)`; **SALDO** = **`CAJA DISPONIBLE - VTOS ACUMULADOS`**. Montos formateados con **`fmtMontoAr`** (locale **es-AR**, 2 dec.). Advertencia: si hay **SALDO** negativo en alguna fila, **`tabla-venc-por-fecha-alerta`** en **`<Table>`**; filas negativas **`venc-saldo-negativo`** + cebra (`globals.css`). Paginación **`PaginacionTabla`** (**100** filas, query **`pagina`**). **Doble clic** en fila → **`AppModal`** detalle por proveedor; tabla del modal usa el **mismo patrón tarjeta + scroll** y **`no-scrollbar`** en el scrollport. `PERMISOS.finanzas.acceso`.
