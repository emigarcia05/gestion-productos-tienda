...
7. **Nuevo modal con tabla**  
   ...
   - **`PedidoHistoriaDetalleModal`:** `AppModal` `sm:max-w-[62.4rem]` (≈ +30% sobre 48rem). Tabla **60% + 10% + 10% + 20%** (**DESCRIPCIÓN** / **CANT. PEDIDA** / **CANT. RECIBIDA** / **ACCIONES**); sin columna **COD. TIENDA** (el código sigue en `title` de la celda de descripción). (1) Resumen: microetiqueta **Nombre proveedor**, nombre en negrita; línea **Sucursal — fecha/hora pedido · estado**; derecha **Fecha de emisión factura** + `type="date"` (`GRID_CAPAS_SUP_PEDIDO_HISTORIA` = `70fr_10fr_20fr`; celda fecha solo `CELDA_TABLA_PADDING_X`, sin sombra). (2) Fila alta: **Agregar un producto** + input (60%), celda vacía alineada a CANT. PEDIDA (10%), **Cant.** + input (10%), botón **Agregar** (20%) (`GRID_TABLA_PEDIDO_HISTORIA_4COL` = `60fr_10fr_10fr_20fr`). (3) **Ítems del pedido** + tabla: contenedores de grilla `MODAL_RESUMEN_PANEL_CLASS` / `MODAL_SECTION_CARD_CLASS` = `bg-transparent`, sin borde ni sombra. Estado `fechaRecepcion` (emisión factura en UI) sin persistencia backend hasta definir campo.
