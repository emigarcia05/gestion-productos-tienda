...
3. **Input de búsqueda en filtros (reutilización)**
  ...
  - **Regla UX anti-race (typing + recarga):** al sincronizar `qActual` desde URL, si el input de búsqueda sigue enfocado y el usuario ya escribió un valor más nuevo localmente, **no** sobrescribir ese texto con el valor de una navegación previa. Esto evita que se borre lo tipeado cuando el usuario hace una pausa corta y vuelve a escribir.
   ...
   - **Pedido Urgente** (`PedidoUrgentePageClient`): **solo SUCURSAL** es obligatoria para listar productos; **PROVEEDOR**, filtro **PEDIDO** y búsqueda acotan. Filtro **PEDIDO** (orden): `CUALQUIER TIPO PEDIDO` (`cant_pedir > 0`), `PEDIDO URGENTE` (`urgente_cant_pedir > 0`) y `PEDIDO REPOSICION` (`reposicion_cant_pedir > 0`) en **`prod_ped_merc`**. Mensaje sin sucursal: *«Seleccioná una sucursal para ver los productos.»* En cabecera solo el botón **Generar Pedido** (`GenerarPedidoToolbarButton`); **no** hay **Guardar Cambios**. Cantidades: modal de cantidad o cesto (`upsertPedidoUrgenteMercaderiaItemAction`). Debajo de la tabla **no** mostrar texto resumen tipo **“Mostrando X de Y”**; si hay más de una página, renderizar solo `PaginacionTabla` alineada a la derecha. **Doble click en fila:** si el ítem está vinculado a `prod_precios_tienda` y existe otro proveedor habilitado más barato para el mismo `id_lista_precios_tienda`, abrir modal **Proveedor Con Menor Costo**: **Pedir A Este Proveedor** abre el modal de cantidad con el `lista_precios_provee.id` del **menor costo**; **Continuar Con Proveedor Actual** abre el modal de cantidad con el `lista_precios_provee.id` de la **fila clickeada** (no reutilizar el id del menor costo en ambos botones). Si no hay alternativa más barata, abrir el modal de cantidad directo.
...
- **Altura de fila (obligatoria en este grid)**: **todas** las filas (incluida la cabecera de columnas) usan la misma altura fija **`h-10` (2,5 rem)**, tomada como referencia de la fila **Resultado operativo** (una línea `text-sm` sin botones). Celdas: `flex items-center`, `py-0`, `px-3`. En columnas de **Global y sucursal**, el sublayout interno es **`25%` + `75%`** (`grid-cols-[25%_75%]`): el **25%** izquierdo muestra solo el botón **histórico** (ícono **BarChart2**, “Evolución mensual del gasto”) en **Costo variable** y **Costo fijo**; el **75%** derecho muestra el importe alineado a la derecha. **No** hay botón directo al desglose por rubro en la grilla: el modal **BalanceMensualDetallePorRubroModal** se abre **solo** tras abrir **`BalanceMensualGastoHistoricoModal`** y **hacer clic en la barra** del mes deseado; los datos de ese periodo se cargan con **`cargarFilasBalanceMensualPeriodoAction`** (`@/actions/finBalGastoMensualBalance`). Si el historial se abre desde una fila de detalle (**`BalanceMensualDetalleGastosRubroModal`**) no se guarda contexto de desglose: las barras del gráfico **no** abren el detalle por rubro. Botones: `TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS` + `TABLE_ROW_ACTION_ICON_CLASS` (`@/lib/ui-classes`) con override compacto **`!h-7 !w-7 !p-1`**. Constantes `CLASE_FILA_BALANCE_MENSUAL_GRID` / `CLASE_CELDA_BALANCE_MENSUAL` / `CLASE_BOTON_ACCION_BALANCE_MENSUAL` en el componente.
...
- **Ventas en Balance mensual**: la fila **Ventas** es **solo lectura**: los montos salen de **`fin_bal_vtas`** según mes/año. Cargar o ajustar ventas solo en **Finanzas → Balance · Ventas** (`/finanzas/balance/vtas`): **`CrearFinBalVtasModal`**, **`crearFinBalVtasAction`** y grilla de registros (`FinBalVtasPageClient`); no hay lápiz ni modal de edición en Balance mensual.
...
- **Jerarquía canónica de URLs (2026-03):** para el área **Gestión Productos** usar siempre prefijo **`/gestion-productos`** con estructura **área/módulo/submódulo**:
  ...
  - **Comp. Proveedores** (`src/app/tienda/page.tsx`, URL canónica **`/gestion-productos/tienda/comp-proveedores`**): cabecera **`ClassicFilteredTableLayout`** con acción **Exportar Prov. Menor Costo**; requiere selección múltiple en la grilla (`TablaTienda`, checkbox por fila + seleccionar todos visibles). Al ejecutar, **no modifica base de datos** y solo exporta dos Excel: **Act. Proveedor** (`CODIGO`, `CODIGO EXTERNO`, `PROVEEDOR`, `COSTO`) y **Act. Margen** (`CODIGO`, `IMPORTE`).
