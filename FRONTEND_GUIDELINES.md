...
- **Columnas** (sin `tabla-bloque-*`; `<colgroup>` 5/15/45/15/10/10% con editor, 5/15/55/15/10% sin editor):
  1. **BASE** (`5%`, header con ícono `Check`): `<input type="checkbox">` por fila; solo una marcada a la vez. Refleja **`cx_px_cx_cod_ext`** (la fila tildada es la **base de comparación** para la columna VARIACION y a la vez el costo elegido en Cx & Px Tienda). Click persiste vía `establecerCostoListaTiendaAction(codTienda, codExt | null)` (`vinculos.ts`): cliquear una fila no marcada → setea esa como base; cliquear la ya marcada → destilda (FK a `null` = Cx. Prom.). Deshabilitado para no editores y mientras hay `isPending`.
...
- **Jerarquía canónica de URLs (2026-03):** para el área **Gestión Productos** usar siempre prefijo **`/gestion-productos`** con estructura **área/módulo/submódulo**:
  ...
  - **Cx Compra** (antes *Vinculacion Con Prov.*; `src/app/tienda/page.tsx`, URL canónica **`/gestion-productos/tienda/comp-proveedores`**): al abrir sin query de filtros carga **todos** los ítems de **`prod_precios_tienda`** paginados (`getTiendaPageData`); los filtros en URL **reducen** el listado. Cabecera **`ClassicFilteredTableLayout`** título **Cx Compra**; **`FiltrosTienda`** (orden): **MARCA** → **RUBRO** → **SUB-RUBRO** → **PROV. VINC.** (`?proveedor=`, proveedores mercadería) → **VINCULADO** (`?vinculado=no|si`); paginación con **`PAGE_SIZE`**. Grilla **`TablaTienda`**: **COD. TIENDA**, **DESCRIPCIÓN**, **VINCULACIÓN**, bloque secundario **CX PROD.** (`tabla-bloque-secundario-head-divider` / `tabla-bloque-secundario-cell-divider`; celda **`CeldaCxProdTienda`** compartida con Cx & Px Tienda). Doble clic en fila (salvo celda CX PROD.) abre **`VincularModal`** si permiso `vinculos`. Edición CX PROD.: `PERMISOS.cxPxTienda.acceso` + `guardarCostoCxProdTiendaAction`.
...
**ANALISIS DE PRECIOS** (rol **editor**, orden): `Cx Compra` → `Px Competencia` → `Cx & Px Tienda` → `Comp. Por Cat.`. **`Cx & Px Tienda`**: al abrir **sin** query de filtros carga **todos** los ítems de **`prod_precios_tienda`** paginados (`PAGE_SIZE`). **`FiltrosCxPxTienda`**: **MARCA** (campo `prod_precios_tienda.marca`) → **CX VINCULADO** (`SIN PROV. (SIN VINC.)` / `UN PROV.` / `+ 1 PROV.`) → **CX. PROV.** (`prom` o proveedor configurado en **CX PROD.**) → **PX LISTA** (`prom` o proveedor asociado a la competencia configurada en PX LISTA) → **MARCACION** (`marcacionOrden=menor_mayor` \| `mayor_menor`: orden global por columna calculada) + **`q`**. Grilla **`TablaCxPxTienda`**: `<colgroup>` **45%** DESCRIPCIÓN + **22%** CX PROD. + **22%** PX LISTA + **11%** MARCACION; dentro de CX PROD. y PX LISTA grid **50/50** (lista desplegable + valor numérico centrado); **MARCACION** con `tabla-bloque-secundario-*-divider` (bloque secundario). Header **`ExportarCxButton`** (control costos): compara **`costo_compra` (DUX)** con **CX PROD.** (proveedor elegido o **promedio** si **CX. PROM.**). Excel **CODIGO** + **COSTO** solo si hay diferencia. Sin filas → **`ModalSinProductosExportar`**. **`ExportarPxButton`** (control precios): compara **`px_lista_tienda` (DUX)** con **PX LISTA** (tolerancia **0,02 %** relativa al mayor de los dos precios). Excel **CODIGO** + **PORC UTILIDAD** solo si hay diferencia. Si no hay filas para exportar, **`ModalSinProductosExportar`** («No hay productos para importar»). Ver `BACKEND_GUIDELINES` §1.10b.
...
- **TablaTienda / Cx Compra** (`TablaTienda.tsx`, clase `tabla-tienda-listado` en `globals.css`): cuatro columnas — **COD. TIENDA** **12%**, **DESCRIPCIÓN** **46%**, **VINCULACIÓN** **12%**, **CX PROD.** **30%** (bloque secundario con divisor; **`CeldaCxProdTienda`** en `@/components/shared/CeldaCxProdTienda.tsx`). **VINCULACIÓN**: **`-`** o contador; badge **PROPIO** si `esProductoPropio`. Doble clic en fila abre **`VincularModal`** (`stopPropagation` en celda CX PROD.). **`ItemTiendaParaTabla.cxProd`**: datos de costo producto (misma lógica que Cx & Px Tienda).
- **CeldaCxProdTienda** (`@/components/shared/CeldaCxProdTienda.tsx`): grid 2 columnas — `Select` (**CX. PROM.** + proveedores vinculados) + monto `$` con `fmtPrecio`; `guardarCostoCxProdTiendaAction` + `router.refresh()`. Reutilizado en **`TablaTienda`** y **`TablaCxPxTienda`**.
...
## 5.1 Módulo Px Competencia (`/precios-competencia`)
...
- **Sidebar:** **ANALISIS DE PRECIOS** agrupa **Cx Compra**, **Px Competencia**, **Cx & Px Tienda** y **Comp. Por Cat.** (sin módulo lateral propio para Cx & Px).
