...
7. **Nuevo modal con tabla**  
   ...
   - **`PedidoHistoriaDetalleModal`:** `AppModal` `sm:max-w-[62.4rem]` (≈ +30% sobre 48rem); proporciones de grillas y tabla sin cambio. Tabla **8% + 61% + 10% + 10% + 11%** (**COD. TIENDA** / **DESCRIPCIÓN** / **CANT. PEDIDA** / **CANT. RECIBIDA** / **ACCIONES**; columna ACCIONES ~50% más angosta que el diseño previo). (1) Resumen: microetiqueta **Nombre proveedor**, nombre en negrita; línea **Sucursal — fecha/hora pedido · estado**; derecha **Fecha de emisión factura** + `type="date"` (`GRID_CAPAS_SUP_PEDIDO_HISTORIA` = `79fr_10fr_11fr`; celda fecha solo `CELDA_TABLA_PADDING_X`, sin sombra). (2) Fila alta: **Agregar un producto** + input, **Cant.** + input, botón **Agregar** (`GRID_TABLA_PEDIDO_HISTORIA_5COL` = `8fr_61fr_10fr_10fr_11fr`). (3) **Ítems del pedido** + tabla: contenedores de grilla `MODAL_RESUMEN_PANEL_CLASS` / `MODAL_SECTION_CARD_CLASS` = `bg-transparent`, sin borde ni sombra. Estado `fechaRecepcion` (emisión factura en UI) sin persistencia backend hasta definir campo.
