...
*Última actualización (2026-05-19): **`/finanzas/tesoreria`** — **TIPO CAJA** (`BANCO` \| `BILLETERA_DIGITAL` \| `CHEQUE` \| `EFECTIVO`), columnas **TIPO VALOR** y **DISPONIBILIDAD**; opciones de modal vía `OPCIONES_TIPO_CAJA_TESORERIA_UI` (`cajasTesoreriaTipos.ts`); filtro **TIPO CAJA** con `etiquetaTipoCajaEnPantalla`. Ver §1 **`/finanzas/tesoreria`**.*
