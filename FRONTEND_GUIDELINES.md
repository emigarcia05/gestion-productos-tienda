...
- **`/finanzas/analisis-mc/margen-contribucion`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Margen Contribución**. **Configuración** (barra de filtros): **TERMINAL** (opcional; sin filtro = promedio de terminales habilitadas por forma de pago), **TIPO COMPROBANTE** (`FACTURA_A` / `FACTURA_C` / `FACTURA_X`; **C** no calcula IVA ni IIBB; **A** y **X** sí), **EVALUAR** (`Un producto` | `Porc. Utilidad`). Si **Un producto**: botón **ELEGIR PRODUCTO** → **`ElegirProductoMargenContribucionModal`** (búsqueda en `prod_tienda`, doble clic; carga PX LISTA de lista principal DUX y porc. utilidad derivado del costo); PX LISTA en tabla solo lectura. Si **Porc. Utilidad**: **`PorcentajeCentInput`** en filtros (alimenta **CX MERCADERÍA**); PX LISTA editable en tabla. Contador **X FORMA(S) DE PAGO** + **`LimpiarFiltrosButton`**. Tabla **`TablaFinAnaMargenContribucion`**: columnas = formas de pago (`FIN_ANA_COS_FINA_PAGOS` + **EFECTIVO**). **Secciones de filas** (`FIN_ANA_MC_LAYOUT`): bloque precio (**PX LISTA**, **DESCUENTO** con **`PorcentajeEnteroMaskInput`** entero 0–100 + `%` fijo no seleccionable, **PX VENTA**), fila espacio, bloque costos (**IVA**, **IIBB**, **CX MERCADERÍA**, **CX FINANCIERO**), línea subtotal costos, **M.C** (por columna). Clases globales: `.tabla-fila-mc-subtotal`, `.tabla-fila-mc-espacio` en `globals.css`. Fórmulas y tipos de comprobante en `@/lib/finAnaMargenContribucion.ts`. Actions: `@/actions/finAnaMargenContribucion.ts`.
- **`/finanzas/analisis-mc/costos-financieros`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Costos Financieros** (módulo sidebar **ANALISIS M.C.**). **`actions`**: **Cálculo Cx. Total** (`Calculator`, botón primario `h-10 px-4`) → **`CalculoCxTotalFinAnaCosFinaModal`** (fórmulas **CX TOTAL S/ IVA** y **CX TOTAL C/ IVA** + componentes); **Gestionar Terminales** (`Settings2`, botón primario `h-10 px-4`) → **`GestionarTerminalesFinAnaCosFinaModal`** (listado, **Crear**, editar y eliminar con íconos **Pencil** / **Trash2** en **`TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS`**; mutaciones solo `editor`). **Filtros**: **TERMINAL** (catálogo dinámico `fin_ana_cos_fina_terminales`), **PAGO**, **HABILITADO** + contador **X COMBINACIÓN(ES)** + **`LimpiarFiltrosButton`**. Tabla **`TablaFinAnaCosFina`** (orden de columnas): **HABILITADO** → **TERMINAL** → **PAGO** → **DÍAS ACREDITACIÓN** → **ARANCEL** → **CX FINANCIERO** → **IMP. CHEQUE** (`tabla-check-toggle`, editable solo `editor`) → **CX TOTAL S/ IVA** → **CX TOTAL C/ IVA** (calculadas, solo lectura: texto centrado `text-xs tabular-nums` + `%`, sin `Input`; encabezados en **dos líneas** `CX TOTAL` / `S|C IVA`). Encabezados con **`leading-tight`** y **`text-center`** alineados a columnas centradas. Inputs % editables: **`.input-mascara-sufijo`**. Fórmulas: ver `BACKEND_GUIDELINES` §2.5h (`cxTotalSinIvaFinAnaCosFina`, `cxTotalConIvaFinAnaCosFina`). Mutación grilla: **`actualizarFinAnaCosFinaAction`**.
