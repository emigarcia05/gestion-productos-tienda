...
- **`/finanzas/venc-por-fecha`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Venc. por fecha**. Datos solo desde **hoy** (Argentina) hasta **hoy + 150 días** inclusive (`listarVencimientosEnRango` en servidor); sin query **`?mes=`** ni flechas de mes. Encabezado interno **Próximos 150 días** + rango **`DD/MM/YYYY — DD/MM/YYYY`** (`formatIsoYmdDdMmYyyyArgentina`). Tabla: **`contenedor-tabla-gestion no-scroll-x`**, **`Table variant="compact"`**. Columnas (cinco, **20%** c/u): **FECHA** (`formatMesDiaMayusculasDesdeIsoYmd`, ej. **`ABRIL 1`**) + **`uppercase`** en celda; **VENCIMIENTO DEL DÍA** total del día; **VTOS ACUMULADOS** = total de saldos con vencimiento **anterior a hoy** (`sumarSaldoVencimientosConFechaVencAnteriorA`) + suma corrida del vencimiento de cada día en la ventana; **CAJA DISPONIBLE** (placeholder `0` hasta origen); **SALDO** acumulativo en la ventana: primera fila `CAJA − vencimiento del día`, siguientes `saldo anterior − vencimiento del día`. **Doble clic** → **`AppModal`** solo lectura (detalle por proveedor); tabla del modal con **`no-scrollbar`**. `PERMISOS.finanzas.acceso`.
