...
- **Layout Finder (catálogos jerárquicos):** componentes compartidos en `@/components/shared/catalogo-finder/` — **`CatalogoFinderColumn`** (header **`bg-primary`**, título centrado; botón **`+`** con **`nuevoLado`**: default **`end`** (derecha) o **`start`** (izquierda, p. ej. Marketing · Ideas); clases **`CATALOGO_FINDER_COLUMN_NOVO_BUTTON_*`**), **`CatalogoFinderRow`**, **`CatalogoFinderEmpty`**. Usado en **Balance · Catálogo Gastos** (5 columnas), **Comp. Categorias** y **Marketing · Ideas** (2 columnas **SECCIONES** → **DETALLE**).
...
*Última actualización (2026-07-14): **Marketing · Ideas** — Finder 2 cols SECCIONES/DETALLE; CRUD; Red/Tipo multi-select (N:M); `CatalogoFinderColumn.nuevoLado`.*
