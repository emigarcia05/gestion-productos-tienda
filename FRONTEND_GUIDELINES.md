...
| `.tabla-fila-seccion-subencabezado`, `.tabla-fila-seccion-subencabezado-celda` | Subencabezado de bloque en **`tbody`**: fondo **`muted` 70%**, borde inferior, **franja izquierda `primary` (3px)**, texto **MAYÚSCULAS** (`text-xs`, `font-semibold`). Componente **`TablaSubencabezadoSeccionRow`** (`@/components/shared/TablaSubencabezadoSeccionRow`): props **`titulo`**, **`colSpan`**, **`totalBloque?`** (total del bloque a la derecha; 0 → **—**). Usar en **Pedido Urgente** y modales Balance (**`BalanceMensualDetallePorRubroModal`**, sección por tipo de gasto). |
