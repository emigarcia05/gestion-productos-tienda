...
- **Jerarquía canónica de URLs (2026-06-19):** SSOT en **`src/lib/gestionProductosRoutes.ts`** (`GP_ROUTES`). Prefijo **`/gestion-productos/{módulo}/{agrupador}/{submódulo}`** alineado al sidebar. Rewrites en `next.config.ts` sirven páginas internas (`src/app/pedidos`, `proveedores`, `tienda`, …); redirects permanentes desde URLs legacy y canónicas anteriores.
  ...
  - **Análisis de Precios:** `/gestion-productos/analisis-precios/lista-proveedores/{lista-precios|reglas-descuentos|lista}`, `/gestion-productos/analisis-precios/cx-y-px-tienda/{cx-compra|px-listas}`, `/gestion-productos/analisis-precios/px-competencia`, `/gestion-productos/analisis-precios/comp-categorias/{comparacion|categorias}`.
...
**ANALISIS DE PRECIOS** (rol **editor**, orden): agrupador desplegable **`LISTA PROVEEDORES`** (sin ruta; ícono `Handshake`) → **Lista Precios** (toolbar: **`CotizacionUsdListaPreciosControl`**, **`ReglasDescuentosListaPrecioModal`** — cotización USD y CRUD reglas descuentos; permisos `listaPrecios.acciones.gestionarCotizacionUsd` / `gestionarReglasDescuentos`), **Lista Proveedores**; agrupador **`Cx y Px Tienda`** (sin ruta; ícono `Layers`) → **Cx Compra** + **Px Listas**; luego **Px Competencia** (`/gestion-productos/tienda/cx-px-tienda`); agrupador **`Comp. Categorias`** (sin ruta; ícono `FolderTree`) → **Comparacion** (`/gestion-productos/proveedores/comparacion-categorias`) + **Categorias** (`/gestion-productos/proveedores/comparacion-categorias/categorias`, permiso `comparacionCategorias.editar`). Los agrupadores se abren al click y auto-expanden si la ruta activa es un hijo. **Categorias** (`ComparacionCategoriasCatalogoPageClient`): layout **Finder** de **3 columnas** (`grid-cols-3`) — **CATEGORÍA** → **SUBCATEGORÍA** → **PRESENTACIÓN** — con componentes compartidos `@/components/shared/catalogo-finder/*` (misma estética que **Balance · Catálogo Gastos**). Cascada por click; `+ Nuevo` por columna; filas con editar/eliminar al hover (`CatalogoFinderRow`). Modales `CrearEditarComparacionCategoriaModal` / `EliminarComparacionCategoriaModal`. La asignación de productos sigue en **Comparacion**. **`Cx Compra`**: grilla con **CX PROD.** + **`ActCxButton`** (Excel costos + modal opcional PDF informe aumentos). **`Px Competencia`** (`/gestion-productos/tienda/cx-px-tienda`, rewrite → `/tienda/cx-px`): listado paginado **solo** de productos con **`comparar_competencia = true`**; header **`Prod. Comparar`** + **`Gestionar Competidores`** + **`Comparar Precios Competencia`** (`SincronizarCompetenciaModal`; permiso `competenciaPrecios.editar`). **`CompetenciaSyncProgresoBanner`** sobre filtros. Filtros **`FiltrosPxListas`**: **MARCA**, **RUBRO**, **PX PROMEDIO** (DIF TIENDA vs promedio; query `filtroPxPromedio`) + búsqueda. Grilla **`TablaPxListas`**: **DESCRIPCIÓN**, **PX PROMEDIO**, **DIF TIENDA**, **ACCIONES** (detalle expandido por competidor; asociar/relevar URLs; quitar de comparación). DIF TIENDA compara `px_lista_tienda` (DUX) vs promedio competidores. **Eliminado (2026-05-28, reimplementar):** columna **DET PRECIO**, **PX LISTA**, **MARCACION**, filtros `detPrecio` / `ordenMarcacion`, **`ExportarPxButton`**, `guardarPxListaTiendaAction`. Componentes conservados para la próxima versión (no usados en grilla): `PxListaCxPxCelda`, `MarcacionPxListaCelda`. Permiso `PERMISOS.cxPxTienda.acceso`. `@/components/px-listas/`.
...
- **Sidebar:** **ANALISIS DE PRECIOS** agrupa **`LISTA PROVEEDORES`** (desplegable: **Lista Precios**, **Lista Proveedores**; reglas descuentos en modal desde toolbar Lista Precios), **`Cx y Px Tienda`** (desplegable: **Cx Compra**, **Px Listas**), **Px Competencia** y **`Comp. Categorias`** (desplegable: **Comparacion**, **Categorias**).
...
*Última actualización (2026-06-19): **Reglas descuentos — modal en Lista Precios** — `ReglasDescuentosListaPrecioModal` en toolbar de `/gestion-productos/proveedores/lista-precios` (`listaPrecios.acciones.gestionarReglasDescuentos` + `esEditor()` en actions). `AppModal` `size="xl"` + `sm:max-w-[72rem]`; **filtros** en `FilterBar` (`FilaFiltrosDesplegables` ×5: **TIPO DESC.**, **PROVEEDOR**, **MARCA**, **RUBRO**, contador `FILTER_COUNT_CLASS`; cada `Select` en `FiltroIndividualContainer`, `input-filtro-unificado`, `select-content-filtro`; sin `LimpiarFiltrosButton` global — patrón modal); filtrado en cliente por `campo` / `idProveedor` / `idMarca` / `idRubro`. Tabla `tabla-vinculos-modal` con `colgroup` 14/8/54/8/16; scroll interno `max-h-[min(58vh,32rem)]`. Sin ítem sidebar **Reglas Descuentos**; ruta legacy `/reglas-descuentos` → redirect Lista Precios. CRUD anidado: `CrearEditarReglaDescuentoListaPrecioModal` / `EliminarReglaDescuentoListaPrecioModal`.*
