...
| `.tabla-gestion-compacta.tabla-precios-competencia` | **Precios Competencia**: filas de datos = alto global **2rem**; botones **ACCIONES** a altura de celda (`celda-acciones-competencia` + `tabla-precios-competencia-acciones`). Subfila detalle: **`tabla-fila-altura-auto`**. |
...
*Última actualización (2026-05-21): **Alto de fila unificado** — **`--tabla-body-row-min-height: 2rem`** en todas las tablas (referencia Lista Px Proveedores); retirado **`tabla-balance-gastos`** (2.875rem); deuda proveedores sin alto automático; excepción **`tabla-fila-altura-auto`** para subencabezados y detalle expandido.*
