...
- **`/finanzas/control-comprobantes`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Control Comprobantes**. Tabla **`TablaControlComprobantes`** con columnas: **PROVEEDOR**, **ID SUCURSAL EMPRESA**, **COMPROBANTE**, **TOTAL**, **MONTO APLICADO**, **VENCIMIENTO** y **CONTROLADO**. En **VENCIMIENTO** se muestra monto solo si existe saldo vencido según regla backend (`vencimientoSaldo > 0`); caso contrario la celda queda vacía. **CONTROLADO** usa `button.tabla-check-toggle` (alto autoajustado por variables de tabla) y renderiza ícono **Check** cuando está activo; al click invoca `actualizarControladoComprobanteAction` y refresca la grilla (`router.refresh()`). El toggle está habilitado solo para rol `editor`; en rol `simple` se muestra deshabilitado.
