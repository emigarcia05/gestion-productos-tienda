...
**ANALISIS DE PRECIOS** (rol **editor**, orden): `Cx Compra` → `Px Listas` (incluye comparación competencia; ex **Px Competencia**) → `Comp. Por Cat.`. **`Cx Compra`**: grilla con **CX PROD.** + **`ExportarCxButton`**. **`Px Listas`** (`/gestion-productos/tienda/cx-px-tienda`, rewrite → `/tienda/cx-px`): listado paginado; header **`ExportarPxButton`** (mismo slot que **Exportar Cx** en Cx Compra). Filtros **`FiltrosPxListas`** (fila de 4 desplegables + búsqueda): **MARCA**, **RUBRO**, **DET PRECIO** (competidores con al menos un vínculo en `prod_precios_competencia`), **MARCACION** (orden mayor→menor / menor→mayor sobre la columna calculada; query `ordenMarcacion`). Grilla **`TablaPxListas`**: 7 columnas — **DESCRIPCIÓN**, **DET PRECIO**, **PX LISTA**, **MARCACION** | separador | **PX PROMEDIO** (media de todos los competidores con precio, scraping o **Px Sugerido**), **DIF TIENDA** (`%` de **PX LISTA** vs promedio; `competenciaPreciosFilaResumen`) | separador | **ACCIONES** (detalle: cada competidor con precio y su `%` vs **PX LISTA**; fallos solo con URL y sin sugerido). **Asociar URLs**: competidor con Px Sugerido muestra texto **Px Sugerido** (sin input URL). `/gestion-productos/precios-competencia` redirige aquí. **DESCRIPCIÓN**, **DET PRECIO** (`Select`: siempre **PX MANUAL** + competidores del ítem con **precio registrado** (`px` &gt; 0 tras sugerido/scraping); sin config: prioridad al competidor guardado, al del `px_vta_sugerido` o al primero; si el usuario eligió **PX MANUAL** en BD, se respeta), **PX LISTA** y **MARCACION** en modo **PX MANUAL**: inputs enlazados (`PxListaCxPxCelda` + **`MarcacionPxListaCelda`**; editar uno recalcula el otero en vivo; persistencia al blur). Competencia → solo lectura en ambas columnas. **MARCACION** (`calcMarcacionPxLista` / inversa `calcPxListaDesdeMarcacionPxLista`, 5 decimales; persistida en BD al guardar). **Exportar Px**: Excel **CODIGO** + **IMPORTE** si marcación DUX ≠ persistida. Permiso `PERMISOS.cxPxTienda.acceso`. Componentes `@/components/px-listas/` + `@/components/tienda/PxListaCxPxCelda.tsx`.
