...
- **`/finanzas/venc-por-fecha`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Flujo De Fondos**. Datos solo desde **hoy** (Argentina) hasta **hoy + 150 días** inclusive (`listarVencimientosEnRango` en servidor); sin query **`?mes=`** ni flechas de mes. Encabezado interno **Próximos 150 días** + rango **`DD/MM/YYYY — DD/MM/YYYY`** (`formatIsoYmdDdMmYyyyArgentina`). Tabla: **`contenedor-tabla-gestion no-scroll-x`**, **`Table variant="compact"`**. La grilla lista **todos los días calendario** de la ventana (incluye días con **VENCIMIENTO DEL DÍA** en `$0,00` cuando no hay comprobantes vencidos ese día). Columnas (cinco, **20%** c/u): **FECHA** (`formatMesDiaMayusculasDesdeIsoYmd`, ej. **`ABRIL 1`**) + **`uppercase`** en celda; **VENCIMIENTO DEL DÍA** total del día; **VTOS ACUMULADOS** = total de saldos con vencimiento **anterior a hoy** (`sumarSaldoVencimientosConFechaVencAnteriorA`) + suma corrida del vencimiento de cada día en la ventana; **CAJA DISPONIBLE**: primera fila usa la suma de montos de `cajas_tesoreria`, y desde la segunda fila usa `max(SALDO fila anterior, 0)`; **SALDO** = **`VTOS ACUMULADOS - CAJA DISPONIBLE`** en cada fila. Paginación estándar por URL con **`PaginacionTabla`**: **100 filas por página** (`PAGE_SIZE`) usando query **`pagina`**. **Doble clic** → **`AppModal`** solo lectura (detalle por proveedor); tabla del modal con **`no-scrollbar`**. `PERMISOS.finanzas.acceso`.
