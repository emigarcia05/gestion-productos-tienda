...
- **Tabla** (`CompetenciaPreciosTabla`): subfila **`tabla-fila-detalle-competencia`**: bloque visual solo en **PRECIO TIENDA + PX PROMEDIO + DIF TIENDA** (`tabla-fila-detalle-competencia-celda`; resto `*-hueca` transparente); borde inferior **#0072bb** + **`padding-bottom`** en **`--cierre`** (última fila del detalle; separación respecto a la fila siguiente). Nombre competidor en **PRECIO TIENDA** (`text-right`), **px** en **PX PROMEDIO**, **dif.** en **DIF TIENDA**. `<colgroup>` **45% + 9%×5 + 10%**.
