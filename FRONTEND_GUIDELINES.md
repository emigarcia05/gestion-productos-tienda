...
- **Tabla:** clase `tabla-px-listas-listado tabla-px-listas-precios` (scroll horizontal de listas en `.contenedor-tabla-gestion`). **Columnas fijas** (`sticky left`, clases `.celda-px-listas-col-fija-desc` / `-cat`): **DESCRIPCIÓN** + **CATEGORÍA MARGEN** (solo lectura; resuelta con `resolverCategoriaMargenPxListas` / rangos `fin_ana_mc_cat` sobre el **PORC. UTILIDAD** de la lista **`1 - GENERAL`** — `encontrarIdListaGeneralPxListas` en `@/lib/pxListasPreciosCategoria`; se actualiza en vivo al editar PX./PORC. de esa lista). **Columnas desplazables:** por cada fila de `prod_tienda_listas_precios`, subcabecera con `nombre_lista` (MAYÚSCULAS) y subcolumnas **PX.** (**`PxListaEnteroInput`**, máscara POS entero, miles `.`; `@/lib/pxListaEnteroMask`) y **PORC. UTILIDAD** (antes **MARG. MAN.**; **`PorcentajeCentInput`** + **`.input-mascara-sufijo`**; máscara POS 2 decimales, `%` fijo; tope **`MARGEN_PX_LISTA_MAX_CENTS`** → hasta **9.999,99 %**, puede superar 100 %). Edición bidireccional en vivo: PORC. → PX (`calcPxListaDesdeMargenSinIvaPct`); PX → PORC. (`margenDesdePrecioDux`). Al blur se persiste en **`prod_tienda_precios_edicion`** (solo la celda/lista editada). Clase **`px-lista-input--edicion`** = negrita si hay staging (`pxEdicion`). **Resaltado** (`.celda-px-listas-actualizar`): filas/celdas con staging pendiente de **Act. Px**.
