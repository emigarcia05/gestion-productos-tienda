...
- **Simetría visual con FECHA FACTURA:** el botón **Agregar Producto** debe usar `h-9 w-full min-w-0` con bordes/ring `#0072bb`, `rounded-md`, tipografía `text-sm` y padding compacto (`px-3 py-1`) para mantener el mismo volumen visual que el input de fecha.
...
- **Fila de acciones de alta (Recepción):** en `PedidoHistoriaDetalleModal`, filtro por descripción + botón cesto + botón **Agregar Producto** deben renderizarse en una sola fila (`grid-cols-[minmax(0,1fr)_auto_auto]`), reduciendo el ancho efectivo del filtro para conservar separación entre los tres controles.
- **Separación funcional en fila de alta:** agrupar **filtro + cesto** como bloque izquierdo y dejar **Agregar Producto** como bloque derecho con separación horizontal mayor (`gap-x-4`) para distinguir búsqueda/limpieza de la acción de alta.
...
- **Ancho/alto del botón Agregar:** el botón **Agregar Producto** ocupa toda la segunda columna (`w-full min-w-0`) en una grilla `85fr_15fr`, para equiparar ancho con la columna de **FECHA FACTURA**; altura fija `h-9` (igual al input de filtro).
...
- **Assets del instructivo de recepción:** las capturas del carrusel se toman desde `public/Importar Compra - Paso1.png` a `public/Importar Compra - Paso5.png` (una por paso, en ese orden), para mantener consistencia visual con el flujo real de DUX.
- **Unificación de instructivos post-exportación:** los tres modales tutoriales (precios, stock y recepción) usan el mismo título visible: **Instructivo: Importar El Archivo Exportado** y navegación homogénea de carrusel (**Paso Anterior** / **Paso Siguiente**).
