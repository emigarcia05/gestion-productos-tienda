...
- **`/finanzas/tesoreria`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Tesorería**. Botón **Nueva Caja** (solo `editor`) en **`actions`** del **`ClassicPageHeader`** (`h-10 px-4`, `Plus`) abre **`NuevaCajaTesoreriaModal`**. Modal (`AppModal`) con alta por tres campos: **NOMBRE CAJA**, **TITULAR** y **TIPO CAJA** (`Digital | Efectivo | Cheque`); guarda con `crearCajaTesoreriaAction` y `router.refresh()`. **TITULAR** se elige por desplegable fijo (sin texto libre) con estos valores: `Suc. Guaymallen`, `Suc. Maipu`, `Walter Garcia`, `Fernando Panaia`, `Emiliano Garcia`, `Vanesa Garcia`. La misma restricción aplica al flujo de edición (validación backend compartida). Tabla **`TablaTesoreriaCajas`**: columnas **NOMBRE CAJA**, **TITULAR**, **TIPO CAJA** (`TipoCajaTesoreria`), **MONTO** (`$` + `fmtPrecio`, entero), **ÚLT. ACTUALIZACIÓN** (`ult_actualizacion` con `formatFechaHoraCompletaArgentina`). Contador **X CAJA(S)**. Sin paginación. Datos desde `listarCajasTesoreria` (servidor). `PERMISOS.finanzas.acceso`.
...
- **Estados**
  ...
  - **Sync lista precios en curso** (polling): **`MensajeProceso`** **`SINCRONIZANDO PROD.`** + detalle **X de Y**.
