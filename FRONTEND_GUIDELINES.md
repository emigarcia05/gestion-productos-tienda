...
3. **Input de búsqueda en filtros (reutilización)**
  ...
  - **Px. Vta. Sugeridos** (`/gestion-productos/proveedores/sugeridos`): en la columna de descripción (`DESC. PROVEEDOR`), la UI debe mostrar descripción efectiva por `cod_ext`: primero **`descripcion_tienda`** (si existe en `precios_tienda`), y como fallback **`descripcion_proveedor`** (`precios_proveedores`).
