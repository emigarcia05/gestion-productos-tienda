...
- **`/finanzas/analisis-mc/margen-contribucion`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Margen Contribución**. **`actions`**: **Gestionar Pagos** (`Settings2`, botón primario `h-10 px-4`) → **`GestionarPagosFinAnaCosFinaModal`**. **Configuración** (`FilterBar`, una fila): **TERMINAL**, **TIPO DE COMPROBANTE** (`FACTURA A` | `FACTURA C` | `FACTURA X`; **C** no calcula IVA ni IIBB), **PORC. UTILIDAD** (`PorcentajeCentInput` → CX MERCADERÍA), **`LimpiarFiltrosButton`**. Recuadro **Variables** (`FinAnaMcFormulasPanel`, bajo filtros): **PX LISTA C/ IVA** editable, **PX LISTA S/ IVA** y **FACTOR IVA** solo lectura, **IVA** / **IIBB** alícuotas en `%` (persistencia `fin_ana_mc_formulas`). Simulador **solo PORC. UTILIDAD**: PX LISTA base desde fórmulas. Tabla **`TablaFinAnaMargenContribucion`**: columnas = formas de pago con `en_margen_contribucion`; secciones **INGRESO** (**solo DESCUENTO** editable por columna) → **COSTOS** (IVA · IIBB · CX MERCADERÍA · CX FINANCIERO) → **MARGEN** (M.C · M.C PONDERADO). Montos en **`N%`**. Ver BACKEND §2.5i.
