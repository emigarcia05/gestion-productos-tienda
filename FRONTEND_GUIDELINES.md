...
- **Tabla** (`CompetenciaPreciosTabla`): clase **`tabla-precios-competencia`** (`globals.css`: filas principales alto fijo `--tabla-body-row-min-height` **2rem**; subfila detalle alto automático). `<colgroup>` **45% + 9%×5 + 10%** = **DESCRIPCIÓN**, **PRECIO TIENDA**, **PX PROMEDIO**, **DIF TIENDA**, **MENOR PRECIO**, **MAYOR PRECIO**, **ACCIONES** (dos botones `#0072BB` **Detalle** + **Asociar URL**, altura = celda vía `tabla-precios-competencia-acciones` + `tabla-row-btn-filled-brand`). Bloques secundarios `tabla-bloque-secundario-*`. Cálculos en `competenciaPreciosFilaResumen.ts`.
