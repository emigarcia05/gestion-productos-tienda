...
| `.tabla-gestion-compacta.tabla-deuda-proveedores` | Solo **Finanzas / Deuda Proveedores**: desactiva el overlay de `td:hover` para que el hover resalte la **fila completa**. |
...
- **`/finanzas/venc-por-fecha`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Venc. por fecha**. Calendario mensual (`?mes=YYYY-MM`, default mes actual AR): grilla **LUN→DOM**, celdas con **`aspect-[4/5]`** (más alto que ancho); **parte superior** = número de día; **parte inferior** = tarjetas por vencimiento (nombre proveedor + monto). Navegación mes anterior/siguiente. Datos: `listarVencimientosEnRango` (misma regla de vencimiento que deuda). `PERMISOS.finanzas.acceso`.
