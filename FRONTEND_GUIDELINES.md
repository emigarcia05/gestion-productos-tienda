...
- **Jerarquía canónica de URLs (2026-03):** para el área **Gestión Productos** usar siempre prefijo **`/gestion-productos`** con estructura **área/módulo/submódulo**:
  ...
  - **Cx Compra** (…): **`ActCxButton`** en header exporta Excel (**CODIGO**, **COSTO**, archivo **`Act Cx dd-mm hh-mm.xls`**); toast al descargar; si no hay diferencias → **`ModalSinProductosExportar`**: **No hay actualizaciones para mostrar** (sin descarga); tras exportar Excel, modal **¿Desea exportar el informe de aumento?** (**No** / **Sí, Exportar** → PDF resumen vía `exportarResumenAumentosPxAction` + `descargarPdfResumenAumentosPx`). …
...
**ANALISIS DE PRECIOS** (rol **editor**, orden): `Cx Compra` → **Px Listas** (`/gestion-productos/tienda/px-listas`) → **Px Competencia** (`/gestion-productos/tienda/cx-px-tienda`) → `Comp. Por Cat.`. **`Cx Compra`**: grilla con **CX PROD.** + **`ActCxButton`** (Excel costos + modal opcional PDF informe aumentos). **`Px Competencia`** (`/gestion-productos/tienda/cx-px-tienda`, rewrite → `/tienda/cx-px`): listado paginado; header **`Gestionar Competidores`** + **`Comparar Precios Competencia`** (`SincronizarCompetenciaModal`; permiso `competenciaPrecios.editar`). **`CompetenciaSyncProgresoBanner`** sobre filtros. Filtros **`FiltrosPxListas`**: **MARCA**, **RUBRO**, **PX PROMEDIO** (DIF TIENDA vs promedio; query `filtroPxPromedio`) + búsqueda. Grilla **`TablaPxListas`**: **DESCRIPCIÓN**, **PX PROMEDIO**, **DIF TIENDA**, **ACCIONES** (detalle expandido por competidor; asociar/relevar URLs). DIF TIENDA compara `px_lista_tienda` (DUX) vs promedio competidores. **Eliminado (2026-05-28, reimplementar):** columna **DET PRECIO**, **PX LISTA**, **MARCACION**, filtros `detPrecio` / `ordenMarcacion`, **`ExportarPxButton`**, `guardarPxListaTiendaAction`. Componentes conservados para la próxima versión (no usados en grilla): `PxListaCxPxCelda`, `MarcacionPxListaCelda`. Permiso `PERMISOS.cxPxTienda.acceso`. `@/components/px-listas/`.
