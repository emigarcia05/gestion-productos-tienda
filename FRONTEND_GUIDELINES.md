...
- **`/finanzas/analisis-mc/margen-contribucion`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Margen Contribución**. **Filtros**: **TERMINAL** (opcional; sin filtro = promedio de terminales habilitadas por forma de pago) + contador **X FORMA(S) DE PAGO** + **`LimpiarFiltrosButton`**. Tabla **`TablaFinAnaMargenContribucion`**: columnas = formas de pago de Costos Financieros (`FIN_ANA_COS_FINA_PAGOS`) + **EFECTIVO**; filas = **PX LISTA** (`PxListaEnteroInput`), **DESCUENTO** (`PorcentajeCentInput`, 1–100 %), **PORC. UTILIDAD** (`PorcentajeCentInput`, puede superar 100 %), **PRECIO VENTA**, **IVA**, **IIBB**, **CX MERCADERÍA** (calculadas; mismas en todas las columnas), **CX FINANCIERO** (por columna: **CX TOTAL C/ IVA** de Costos Financieros; **EFECTIVO** = 0 %). Fórmulas en `@/lib/finAnaMargenContribucion.ts`; datos CX financiero vía `mapCxFinancieroPorFormaPago` (`finAnaMargenContribucion.service.ts`). Inputs editables solo `editor`; sin persistencia en BD (simulador en cliente).
