...
- **Tabla:** clase `tabla-px-listas-listado tabla-px-listas-precios` (scroll horizontal si hay muchas listas). **DESCRIPCIÓN** + por cada fila de `prod_tienda_listas_precios`: subcabecera con `nombre_lista` (MAYÚSCULAS), columnas **PX.** y **MARGEN**. Filas con alto fijo `--tabla-body-row-min-height` (2rem), igual que el resto de tablas compactas. Celdas editables: `celda-px-lista-col`, `celda-marcacion-col`, envoltorio **`px-lista-celda-shell`** (fondo **transparente**, borde **#0072BB**, texto **negro** `#000`, alto **1.75rem** dentro de la celda); el `<input>` va **dentro** del shell sin borde propio. Modificador **`px-lista-celda-shell--edicion`** = negrita cuando hay override en `prod_tienda_precios_edicion`. Guardado en blur → `guardarPxListaPrecioEdicionAction`. Vaciar PX con override activo elimina fila en `prod_tienda_precios_edicion`.
- **Clases globales:** `.tabla-px-listas-precios` — `table-layout: auto`, `width: max-content`; estilos PX/MARGEN en `globals.css` bajo `.tabla-px-listas-listado tbody td.celda-px-lista-col` / `celda-marcacion-col`.
