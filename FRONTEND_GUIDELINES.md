...
- **`/finanzas/balance/gastos`**: **`ClassicFilteredTableLayout`** `contentWidth="full"`, título **Balance**, subtítulo **Gastos**. **Filtros** (`FilterBar` `filtros-contenedor-tienda bg-card`): **fila 1** `FilterRowSelection` + `FilaFiltrosDesplegables` — **RUBRO**, **GASTO** (opciones acotadas si hay rubro), **SUCURSAL**, **PROVEEDOR**, **PAGADO** (opción **NO** = solo filas con `montoDevengadoPendiente > 0`); patrón global de Select (`input-filtro-unificado`, `SelectContent` `position="popper"` `side="bottom"` `align="start"` `select-content-filtro`, primera opción **TODOS** con valor `none`). **Fila 2** — misma retícula de cinco columnas: **Año**, **Mes** y, en `col-span-3` (`FILTER_INLINE_ACTION_SLOT_CLASS`), contador **`FILTER_COUNT_CLASS`** (**X GASTO(S)**) + **`LimpiarFiltrosButton`**. Opciones de periodo **solo** desde `listarPeriodosConImputacionesEnDb` (prop `periodosOpciones`); el mes listado es el subconjunto de meses que existen en BD para el año elegido. Si la URL pide un periodo sin filas y hay otros periodos en BD, el servidor **redirige** al más reciente. Solo `editor`: **Cargar Datos Mes.** llama `cargarFinBalGastoMensualMesAction({ mes, anio })`. Tabla **`TablaGastos`**: columnas de datos + **Acc.** (`bg-muted/25`, `Pencil` / `Trash2`); **Editar monto** → `EditarMontoFinBalGastoMensualModal` + **Repetir Ult. Monto**; **Eliminar** → `EliminarFinBalGastoMensualModal`. Pie **TOTAL** `colspan` 7. `PERMISOS.finanzas.acceso`.
