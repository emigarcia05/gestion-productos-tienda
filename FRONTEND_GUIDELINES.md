...
- **Guías post-exportación (PROCESOS):** las guías paso a paso para importar en DUX **no** se abren automáticamente al exportar Excel. Están centralizadas en **`/gestion-productos/procesos`** (`ProcesosPageClient` + `PROCESOS_INSTRUCTIVOS` en `src/lib/procesos-instructivos.ts`). Layout: grid **`25% | 75%`**. Columna izquierda: bloque superior **25%** alto con título **Módulo** y `Select` (`MODULOS_PROCESOS`; por ahora solo **Importacion Datos en Dux**); bloque inferior **75%** con listado de procesos (`labelCorto`: **Imp. Stock**, **Imp. Compra**). Columna derecha: carrusel `ProcesoInstructivoCarrusel`. Imágenes: `importar_stock_{1..5}.png`, `importar_compra_{1..5}.png`. Sidebar: enlace directo **PROCESOS** (sin submódulos). Cada proceso se lista solo si el rol tiene permiso del módulo de origen (stock / pedidos).
...
- **Carrusel de procesos** (`ProcesoInstructivoCarrusel`): **`gap-4`** único; sin `flex` duplicado en el contenedor del carrusel.
