...
| `.tabla-gestion-compacta` | **Diseño único** de tablas (referencia: Comp. Proveedores). Usar siempre `<Table>` de `@/components/ui/table`; no usar otra clase. **Encabezado fijo obligatorio**: al hacer scroll los encabezados no desaparecen (`position: sticky` en `globals.css`). **`thead th`**: altura global **`--tabla-thead-height`** calculada como **(2 líneas de texto + padding vertical corto)**; títulos centrados horizontal y verticalmente, hasta **2 líneas** con recorte del excedente; sin `nowrap` en encabezados. `TableHead` mantiene `text-xs` y `font-normal`. **Inputs y Select** en celdas: fondo transparente, recuadro #0072bb. **Select en tablas**: texto en negro, sin bold (`globals.css`). **Hover de celda (global):** en `tbody td:hover` el contenido puede sobresalir sobre celdas vecinas (`overflow: visible` + `z-index`) para leer textos truncados sin cambiar anchos de columna. |
...
- **`/finanzas/deuda-proveedores`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Deuda Proveedores**. Tabla **`TablaDeudaProveedores`**: proveedores con **Σ (total − monto_aplicado) > 0** en `comprobantes_proveedor` (servicio `listarDeudaProveedores`); columnas **PROVEEDOR**, **ID PROV. DUX**, **DEUDA** (formato `$` + `es-AR` 2 dec.); contador **X PROVEEDOR(ES) CON DEUDA** en MAYÚSCULAS. Sin paginación (listado completo). `PERMISOS.finanzas.acceso`.
