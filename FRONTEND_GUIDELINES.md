...
- **Layout:** `area-page-shell` + `ClassicFilteredTableLayout`. **FilterBar** `filtros-contenedor-tienda bg-card`: **MARCA**, **RUBRO**, **SUBRUBRO** (`FilaFiltrosDesplegables`) + búsqueda (`FiltroBusquedaInput`) + contador `PRODUCTO(S)`.
- **Componentes:** `src/components/px-listas-precios/*` — `FiltrosPxListasPrecios`, `TablaPxListasPrecios`, `PxListasPreciosPageClient`.
- **Tabla:** clase `tabla-px-listas-listado tabla-px-listas-precios` (scroll horizontal si hay muchas listas). **DESCRIPCIÓN** + por cada fila de `prod_tienda_listas_precios`: subcabecera con `nombre_lista` (MAYÚSCULAS), columnas **PX.** y **MARGEN**. Celdas editables (`celda-px-lista-col`, `celda-marcacion-col`, shell `px-lista-celda-shell`); guardado en blur → `guardarPxListaPrecioEdicionAction`. Editar **MARGEN** recalcula PX en cliente y persiste precio; editar **PX** recalcula margen al refrescar estado. PX con override manual se muestra en **negrita**. Vaciar PX con override activo elimina fila en `prod_tienda_precios_edicion` (vuelve baseline DUX).
- **Clase global nueva:** `.tabla-px-listas-precios` — `table-layout: auto`, `width: max-content` (ver `globals.css`).
