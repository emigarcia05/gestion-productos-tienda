...
- **`/finanzas/venc-por-fecha`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Venc. por fecha**. Vista en **tabla** (no calendario), con columnas y anchos: **MES (20%)**, **DÍA (5%)**, **A PAGAR (25%)**, **CAJA DISPONIBLE (25%)**, **SALDO (25%)**. `A PAGAR` se consolida por día desde `listarVencimientosEnRango` (sin discriminar proveedor). En la implementación actual, `CAJA DISPONIBLE` se muestra en `0` y `SALDO` = `CAJA DISPONIBLE - A PAGAR`. Navegación mes anterior/siguiente por `?mes=YYYY-MM`. `PERMISOS.finanzas.acceso`.
