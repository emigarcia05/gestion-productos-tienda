...
- **Layout Finder (catálogos jerárquicos):** componentes compartidos en `@/components/shared/catalogo-finder/` — **`CatalogoFinderColumn`** (sección con header `bg-muted/60`, `+ Nuevo`), **`CatalogoFinderRow`** (selección en cascada, acciones editar/eliminar al hover), **`CatalogoFinderEmpty`**. Usado en **Balance · Catálogo Gastos** (5 columnas) y **Comp. Categorias · Categorias** (3 columnas).
...
**Columna Referencia competencia** (`ComparacionReferenciaCompetenciaColumn`): grilla **COMPETIDOR 10 %** · **PRODUCTO 80 %** · **PX. 10 %**; filas clickeables con selección única — clases **`CATALOGO_FINDER_ROW_SELECTED_CLASS`** (`bg-primary/10`, igual que `CatalogoFinderRow`); **+ Nuevo** agrega; pie **Quitar referencia seleccionada**.
