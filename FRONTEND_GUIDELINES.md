...
- **Vínculo manual:** botón **Asociar URLs** por fila → `AsociarUrlsCompetenciaModal` (todas las URLs del producto; **Guardar** solo persiste competidores con cambios; si `vinculosPorCompetencia[*].urlBloqueadaPorPxSugerido` (competidor con proveedor y `px_vta_sugerido` en lista proveedor para ese `cod_tienda`), el campo URL y tipo de página quedan deshabilitados y se muestra leyenda de Px. sugerido). `RelevamientoUltimoMensaje` bajo cada competidor: último intento, fecha y mensaje copiable desde `error_mensaje` o guía `SIN_PRECIO` (no se muestra cuando la URL está bloqueada por sugerido). Guía en modal: fallo en un producto → URL; fallos masivos del mismo competidor → **Configuracion Competidor**. Detalle expandido: precios OK + filas `ERROR`/`SIN_PRECIO` con el mismo componente.
