...
- **Tabla** (`CompetenciaPreciosTabla`): columnas **MENOR PRECIO** y **MAYOR PRECIO** muestran `global_proveedores.prefijo` del proveedor asociado al competidor (`prefijoProveedor` en `CompetenciaParaCliente`; `title` con nombre completo del competidor). Subfila **`tabla-fila-detalle-competencia`**: fondo **`var(--gris-inset)`** en todas las celdas; **alto fijo** = **`--tabla-body-row-min-height`** (igual que filas principales); **sin** franja izquierda ni bordes en celdas huecas. Bloque de datos solo en cols 2–4 (**PRECIO TIENDA**, **PX PROMEDIO**, **DIF TIENDA**): divisor **#0072bb** **solo entre** subfilas consecutivas (`tr + tr` → `border-top` en `*-celda`). **`--cierre`**: misma altura que el resto de subfilas (sin padding inferior extra). `<colgroup>` **45% + 9%×5 + 10%**.
