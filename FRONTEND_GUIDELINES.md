...
| `.tabla-fin-ana-margen-contribucion`, `.tabla-mc-col-forma`, `.tabla-mc-celda-dual`, `.tabla-mc-dual-grid`, `.tabla-mc-dual-pad`, `.tabla-mc-dual-pesos`, `.tabla-mc-dual-pct` | **Margen Contribución**: tabla **`width: max-content`** (evita que `w-full` + `table-layout: fixed` ensanche formas). Columna forma **`--tabla-mc-forma-width` (8rem)**. Par $/%: **`CeldaSubcolumnasMontoPct`**, grid `10% \| 65fr \| 10% \| 35fr \| 10%`. |
...
| `CeldaSubcolumnasMontoPct` + CVA (`@/components/shared/CeldaSubcolumnasMontoPct.tsx`) | Par **$ / %**: grid `10% \| 65fr \| 10% \| 35fr \| 10%` (pads = gap). Props: `monto`, `pct`, `density`, `weight`. Requiere columna anclada (no estirada por `w-full` de tabla). |
...
### `CeldaSubcolumnasMontoPct` (`src/components/shared/CeldaSubcolumnasMontoPct.tsx`)

Par **monto $ / porcentaje %** como subcolumnas visuales dentro de **una** celda de tabla (Server Component).

- **Cuándo usarlo**: grillas densas donde `$` y `%` forman un solo grupo visual (ej. **Margen Contribución** modo **PRODUCTO**). No usar dos `<td>` independientes si el ritmo horizontal debe ser simétrico.
- **Props**
  - **`monto`**: `string` — texto ya formateado (ej. `$12.407`).
  - **`pct`**: `string` — texto ya formateado (ej. `13%`).
...
- **Layout (globals)**: grid `10% | 65fr | 10% | 35fr | 10%` → pad \| $ \| gap \| % \| pad. Pads **porcentuales fijos** (no `fr`, no se comprimen). Pistas de valor **65:35** del resto. Valores centrados con **flex**. Cinco hijos DOM (pads vacíos + monto + pct).
- **Requisito de columna**: la tabla/columna no debe estirarse con el sobrante de `width: 100%` (en MC: `width: max-content` + **`.tabla-mc-col-forma`** / `--tabla-mc-forma-width`). Si la celda se ensancha, el par vuelve a verse asimétrico.
...
- **Ejemplo**
  ...
  <TableCell className={cn("celda-datos celda-numero tabular-nums", "tabla-mc-celda-dual", "tabla-mc-col-forma")}>
    <CeldaSubcolumnasMontoPct monto="$12.407" pct="13%" />
  </TableCell>
  ```
...
La app se divide en **cuatro áreas** de alto nivel; el resto de rutas actuales pertenecen a **Gestión Productos** (comportamiento por defecto).
...
- **`/finanzas/analisis-mc/margen-contribucion`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Margen Contribución**. **`actions`**: **Gestionar Pagos** (`Settings2`, botón primario `h-10 px-4`) → **`GestionarPagosFinAnaCosFinaModal`** (CRUD catálogo `fin_ana_cos_fina_pagos`; **reordenar** filas con arrastre desde ícono **`GripVertical`** — persiste `orden` y actualiza columnas de la tabla; mutaciones solo `editor`; `router.refresh()` + `key` por `id:orden` en página servidor). **Configuración** (`FilterBar`, dos filas): **fila 1** — **TERMINAL**, **TIPO DE COMPROBANTE** (`FACTURA A` | `FACTURA C` | `FACTURA X`; **C** no calcula IVA ni IIBB), **`LimpiarFiltrosButton`**; **fila 2** — **TIPO DE ANÁLISIS** (`PRODUCTO` | `PORC. UTILIDAD`) + input según modo (**ELEGIR PRODUCTO** → modal `prod_tienda` | **`PorcentajeCentInput`** margen). Si **PRODUCTO**: PX LISTA solo lectura en tabla (**repetido en cada forma de pago**); si **PORC. UTILIDAD**: PX LISTA fijo **$100** (solo lectura, **también repetido por columna**) y margen desde el input de filtros. Tabla **`TablaFinAnaMargenContribucion`**: envuelta en **`.contenedor-tabla-gestion`** → scrollport interno **`overflow-x-auto overflow-y-auto`** (barra horizontal cuando hay muchas formas de pago); columna **CONCEPTO** fija (`sticky left-0`, clase **`.tabla-mc-col-concepto`** en `globals.css`; esquina thead `z-30` + fondo `primary`); separadores entre formas **`.tabla-mc-sep-forma`** (borde vertical 2px más marcado). Columnas = formas de pago del catálogo (`en_margen_contribucion`). **Formato de celdas**: **PORC. UTILIDAD** → una columna por forma, montos como `%` (`fmtCeldaMontoMargenContribucion`, ej. `10%`; base PX LISTA = 100). **PRODUCTO** → **una columna** por forma (**`.tabla-mc-col-forma`**, `--tabla-mc-forma-width: 8rem`, tabla con **`width: max-content`** para que no absorba el sobrante de `w-full`) con **`CeldaSubcolumnasMontoPct`** en **`.tabla-mc-celda-dual`**: grid `10% | 65fr | 10% | 35fr | 10%` (pads exteriores = gap; valores centrados con flex). Helpers `fmtPesosMargenContribucion` / `fmtPctSobrePxListaMargenContribucion`. **DESCUENTO** siempre % firmado (− / +; input centrado en la columna). **CX FINANCIERO** = costo en pesos (misma regla), no la tasa cruda. **Secciones de filas** (`FIN_ANA_MC_LAYOUT`): bloque precio (**PX LISTA**, **DESCUENTO** con input entero −100…100 **por forma de pago**, **PX VENTA** en negrita por columna), fila espacio, bloque costos, subtotal costos, **M.C**. Fórmulas en `@/lib/finAnaMargenContribucion.ts`.
...
- **Tabla:** clase `tabla-px-listas-listado tabla-px-listas-precios` (scroll horizontal si hay muchas listas). **DESCRIPCIÓN** + por cada fila de `prod_tienda_listas_precios`: subcabecera con `nombre_lista` (MAYÚSCULAS), columnas **PX. CALC.** (**`PxListaEnteroInput`**, máscara POS entero, miles `.`; `@/lib/pxListaEnteroMask`) y **MARG. MAN.** (**`PorcentajeCentInput`** + **`.input-mascara-sufijo`**; máscara POS 2 decimales, `%` fijo; tope **`MARGEN_PX_LISTA_MAX_CENTS`** → hasta **9.999,99 %**, puede superar 100 %). Edición bidireccional en vivo: margen → PX (`calcPxListaDesdeMargenSinIvaPct`); PX → margen (`margenDesdePrecioDux`). Al blur se persiste en **`prod_tienda_precios_edicion`** (solo la celda/lista editada). Clase **`px-lista-input--edicion`** = negrita si hay staging (`pxEdicion`). **Resaltado** (`.celda-px-listas-actualizar`): filas/celdas con staging pendiente de **Act. Px**.
- **Clases globales:** `.tabla-px-listas-precios` — `table-layout: auto`, `width: max-content`; inputs PX y MARGEN en `globals.css` bajo `.tabla-px-listas-precios tbody td.celda-px-lista-col` / `.celda-marcacion-col`; `.celda-px-listas-actualizar` para pendientes de actualizar en DUX.
