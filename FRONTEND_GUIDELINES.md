...
- **Área Marketing** (`/marketing/*`): SSOT rutas **`MARKETING_ROUTES`** (`src/lib/marketingRoutes.ts`). Entrada modal / `/marketing` → redirect a **`/marketing/publicaciones/calendario`**. **Calendario De Publicaciones** (`MarketingCalendarioPageClient`): header con **Gestionar Redes** y **Gestionar Tipo Publicaciones** (solo `editor`, `Settings2`); ambos abren el mismo modal estético **`GestionarMktCatalogoNombreModal`** (`kind: "red" | "tipo"`) — listado + alta inline + editar/borrar (confirmación), nombres en MAYÚSCULAS. **Ideas**: placeholder. `PERMISOS.marketing.acceso` (solo `editor`).
