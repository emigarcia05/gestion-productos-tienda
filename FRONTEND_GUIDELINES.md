...
7. **Nuevo modal con tabla**  
   ...
   - **`PedidoHistoriaDetalleModal`:** tabla **8% + 50% + 10% + 10% + 22%** (**COD. TIENDA** / **DESCRIPCIÓN** / **CANT. PEDIDA** / **CANT. RECIBIDA** / **ACCIONES**). **Cabecera** y **fila de herramientas** comparten `GRID_CAPAS_SUP_PEDIDO_HISTORIA` = `sm:grid-cols-[68fr_10fr_22fr]` (68 = 8+50+10). Cabecera: proveedor + fecha generación (col 1, `CELDA_TABLA_PADDING_X`), celda vacía `hidden sm:block` alineada a **CANT. RECIBIDA**, **FECHA RECEPCIÓN** + `type="date"` en col 3 con `COLUMNA_ACCIONES_EXTERNA_CLASS`. Fila herramientas: **SELECCIONAR PRODUCTO** + input (producto elegido o `readOnly` con placeholder `BUSCAR POR DESCRIPCIÓN O CÓDIGO...` que abre el modal), **CANT.**, botón **Agregar** (`w-full`, title case según guía). Controles `h-9`, fila herramientas con `sm:items-end`. Estado `fechaRecepcion` sin persistencia backend hasta definir campo.
