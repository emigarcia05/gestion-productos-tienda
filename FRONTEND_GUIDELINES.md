...
- **Jerarquía canónica de URLs (2026-03):** para el área **Gestión Productos** usar siempre prefijo **`/gestion-productos`** con estructura **área/módulo/submódulo**:
  ...
  - **Cx Compra** (antes *Vinculacion Con Prov.*; `src/app/tienda/page.tsx`, URL canónica **`/gestion-productos/tienda/comp-proveedores`**): al abrir sin query de filtros carga **todos** los ítems de **`prod_precios_tienda`** paginados (`getTiendaPageData`); los filtros en URL **reducen** el listado. Cabecera **`ClassicFilteredTableLayout`** título **Cx Compra**; acciones **`ExportarCxButton`** (`PERMISOS.cxPxTienda.acceso`) — control de costos DUX vs **CX PROD.**; Excel **CODIGO** + **COSTO** solo diferencias; sin filas → **`ModalSinProductosExportar`**. **`FiltrosTienda`** (`modoFiltroTercero="cxCompra"`): **MARCA** → **RUBRO** → **CX COMPRA** (proveedores con ítems en **CX PROD.**; query `cxCompra`) → **PROV. VINC.** → **VINCULADO**; paginación **`PAGE_SIZE`**. Grilla **`TablaTienda`**: **COD. TIENDA**, **DESCRIPCIÓN**, **VINCULACIÓN**, **CX PROD.** (`CeldaCxProdTienda`), **ACCIONES** (expandir vínculos + vincular proveedor). Subfilas **`CxCompraVinculosDetalle`** (mismo contenido que el antiguo modal de vínculos: proveedor, precio, variación, BASE, desvincular). Botón **Link** → **`SeleccionarProductoModal`** (“Vincular Nuevo Producto”). Sin **`VincularModal`**. Edición CX PROD.: `guardarCostoCxProdTiendaAction`.
...
- **Vínculos en grilla** (antes modal `VincularModal.tsx`, eliminado): lógica en **`CxCompraVinculosDetalle`** + **`SeleccionarProductoModal`**.
...
- **Pantalla canónica:** comparación en **`TablaPxListas`** (`/gestion-productos/tienda/cx-px-tienda`). Panel lateral **`ListaCompetidoresPxListas`**: input filtra por nombre; tabla **COMPETIDOR** | **DÍAS SIN ACT.** (`getDiasSinActividadCompetencia` sobre `ultimaComparacionAt`) | **Actualizar** (`POST /api/sync-competencia-precios` por fila; permiso `competenciaPrecios.editar`). Layout `contentWidth="full"`. **`/gestion-productos/precios-competencia`** (y legacy `/proveedores/competencia-precios`) **redirigen** a Px Listas. La grilla standalone **`CompetenciaPreciosTabla`** queda como referencia; componentes compartidos en `src/components/precios-competencia/*` (`AsociarUrlsCompetenciaModal`, `RelevamientoUltimoMensaje`, etc.).
...
- **Acciones (editor):** **Gestionar Competidores** + **Comparar Precios Competencia** (`SincronizarCompetenciaModal`: **Select COMPETIDOR** con opción **TODOS** (compara todos los competidores con URL cargada o con sugerido del proveedor asociado, en secuencia, progreso global) + pie **Cancelar** / **Comparar Precios**; en curso **Detener Comparación** y cartel en el cuerpo del modal **`{processed} de {total}`** (`useCompetenciaSyncStatusPoll`); progreso detallado también en `CompetenciaSyncProgresoBanner` sobre la grilla).
...
*Última actualización (2026-05-28): **Retiro Cx & Px Tienda** — eliminadas columnas `px_lista_cx_px`, `es_producto_propio`, `cx_px_px_comp_ref` y todo el flujo de comparación tienda↔competencia en `prod_precios_tienda` (migración **`20260528210000`**). **Cx Compra** concentra vínculos + **CX PROD.** + **Exportar Cx**.*
