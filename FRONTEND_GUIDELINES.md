...
- **Fila de acciones de alta (Recepción):** en `PedidoHistoriaDetalleModal`, **filtro + cesto** + **Agregar Producto** en `flex` (`sm:flex-row sm:justify-between sm:items-center sm:gap-x-10`): bloque izquierdo acotado (`sm:max-w-[18rem]`) para que el input de búsqueda no estire todo el ancho; bloque derecho **Agregar Producto** con `sm:w-auto sm:shrink-0` para dejar hueco visual claro entre filtrar y alta.
