...
3. **Texto en mayúscula inicial (title case)**  
   ...
   - **Sidebar**: nombre del **módulo** en MAYÚSCULAS (ej. "LISTA PROVEEDORES", "PEDIDO MERCADERÍA"). Nombre del **submódulo**: primera letra de cada palabra en mayúscula (title case) (ej. "Lista Precios", "Vinc. Con Prov.", "Pedido Urgente", "Px. Vta. Sugeridos").  
...
- Sidebar por rol: **simple** → **PEDIDO DE MERCADERÍA** + **AYUDA VENDEDOR**; **editor** → los cuatro módulos. **Vinc. Con Prov.** solo **editor** (`PERMISOS.tienda.acceso`).
...
- **TablaTienda / Vinculacion Con Prov.** (`TablaTienda.tsx`, clase `tabla-tienda-listado` en `globals.css`): tres columnas — **COD. TIENDA** **15%**, **DESCRIPCIÓN** **67%**, **VINCULACIÓN** **18%** (muestra **`-`** si `_count.productos === 0`, si no el **número** de vínculos). Doble clic en fila (si permiso `vinculos`) abre **`VincularModal`**. El payload **`ItemTiendaParaTabla`** incluye costos/prefijos para el modal aunque no se listen en la grilla.
...
*Última actualización (2026-05-28): **Cx & Px Tienda — anchos y PX LISTA editable** — `<colgroup>` 50/20/20/10; **`PxListaCxPxCelda`** (`$` + miles con punto, persistencia en blur vía `guardarPxListaCxPxValorAction`).*

*Última actualización (2026-05-28): **Marcación Cx & Px** — cálculo y UI con **5 decimales** (`CX_PX_MARCACION_DECIMALES`, `roundMarcacionCxPx`, `fmtMarcacionCxPxPct`); export Px usa el mismo redondeo en **PORC UTILIDAD**.*
...
- **Sidebar:** **ANALISIS DE PRECIOS** agrupa **Vinc. Con Prov.**, **Px Competencia**, **Cx & Px Tienda** y **Comp. Por Cat.** (sin módulo lateral propio para Cx & Px).
