...
3. **Input de búsqueda en filtros (reutilización)**
   ...
   - En **Pedido Urgente** y **Pedido Tintométrico**, `EnviarPedidoButton` (texto por defecto **Generar Pedido**, prop opcional `label`; en urgente: **Generar Pedido Urgente**; en tintométrico: **Generar Pedido Tintométrico**) se muestra sólo cuando están seleccionados `SUCURSAL` y `PROVEEDOR`; el tipo de pedido lo determina el módulo. La página **enviar** también usa el mismo componente según filtros.
...
Listado **Historial Pedidos** (`/pedidos/historial`). Última columna **ACCIONES** (`tabla-bloque-secundario-*` alineado al patrón de tabla gestión), celdas con **`flex items-center justify-center gap-2`**. Tres botones **`size="icon-xs"`** con **`Tooltip`**: **Recepción De Mercadería** (`PackageCheck`) → **`PedidoHistoriaDetalleModal`**; **Ver Detalles** (`Eye`) → **`PedidoHistoriaLecturaModal`** (solo lectura, título **Ver Pedido**; **`AppModal`** **`scrollBody={false}`** para que la card sea **`flex flex-col` `overflow-hidden`** y el **único scroll vertical** sea **`.contenedor-tabla-gestion`** bajo la cabecera fija — patrón equivalente a *header / `flex-1 overflow-y-auto` con tabla + `thead` sticky / footer*; cabecera del cuerpo: badge **Pedido** / **Recepcionado**, proveedor `text-base font-semibold text-foreground`, sucursal + fecha; **`.contenedor-tabla-gestion`** **`no-scrollbar`** **`no-scroll-x`**; tabla sin inputs); **Borrar** (`Trash2`, hover **destructive**) → **`PedidoHistoriaBorrarConfirmModal`** (texto de confirmación, **Cancelar** outline / **Sí, Borrar** destructive). Tras cerrar recepción o borrar, **`router.refresh()`** mantiene el listado al día.
