...
- **`/finanzas/venc-por-fecha`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Venc. por fecha**. Calendario mensual (`?mes=YYYY-MM`, default mes actual AR): grilla **LUN→DOM**, celdas con **`aspect-[4/5]`** (más alto que ancho); **parte superior** = número de día; **parte inferior** = **una sola tarjeta con el total consolidado de vencimientos del día** (sin discriminar por proveedor). La tarjeta de total diario usa fondo **`destructive` suavizado** (rojo no agresivo), con monto en **blanco**, **bold** y centrado. Para mantener contorno y separaciones visuales consistentes, cada celda se envuelve en un shell **`bg-[#0072BB] p-px`** (separador del mismo color del contorno principal) y el contenido interno (`bg-card`/`bg-muted`) va con radio interno. Navegación mes anterior/siguiente. Datos: `listarVencimientosEnRango` (misma regla de vencimiento que deuda). `PERMISOS.finanzas.acceso`.
