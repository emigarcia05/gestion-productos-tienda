...
1. **Estructura de página**
  ...
  - **Scroll vertical**: las páginas hacen scroll a nivel de `main` (no dentro de la tabla). `contenedor-tabla-gestion` y `contenedor-tabla-paginada` **no** tienen `overflow-y: auto`; los wrappers internos (cards, divs de tabla) tampoco. Esto evita cajas de scroll anidadas y hace que el encabezado sticky funcione con el scroll principal.
...
| `.tabla-bloque-secundario-head`, `.tabla-bloque-secundario-head-divider` | Estilo para encabezados de columnas de **información secundaria** (bloque a la derecha): fondo `bg-muted` suavizado, texto `text-muted-foreground`. La variante `*-divider` usa una línea vertical **blanca** (`border-left: 2px solid var(--primary-foreground)`) para cortar visualmente el bloque secundario dentro del encabezado. |
| `.tabla-bloque-secundario-cell`, `.tabla-bloque-secundario-cell-divider` | Estilo para celdas de cuerpo de **información secundaria** (ej. STOCK, CANT. A PEDIR en Pedido Reposición). Fondo y color alineados con los encabezados secundarios. La variante `*-divider` agrega la misma línea divisoria vertical pero en gris marcado (mezcla de `border` y `foreground`), diferenciando filas del header. |
