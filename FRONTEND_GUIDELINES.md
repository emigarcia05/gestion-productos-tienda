...
7. **Nuevo modal con tabla**  
   ...
   - **`PedidoHistoriaDetalleModal` — alineación con la tabla:** `GRID_ALINEACION_TABLA_PEDIDO_HISTORIA` = `sm:grid-cols-[9fr_62.5fr_10.5fr_10.5fr_15fr]` para **cabecera** (proveedor en `sm:col-span-4` + **FECHA RECEPCIÓN** en col 5 con `CELDA_TABLA_PADDING_X` + `sm:shadow-[inset_1px_0_0_#0072bb]` alineada a **ACCIONES**). **Fila de alta producto** (`GRID_FILA_ALTA_PEDIDO_HISTORIA`): **tres columnas** `sm:grid-cols-[82fr_10.5fr_15fr]` donde `82fr = 9+62.5+10.5` (mismo ancho conjunto que **COD. TIENDA** + **DESCRIPCIÓN** + **CANT. PEDIDA**), segunda columna = **CANT. RECIBIDA**, tercera = **ACCIONES** (**+ AGREGAR** `w-full`, mismo shell que fecha). Rótulo de la primera columna: **SELECCIONE PRODUCTO**; controles `h-9`, `sm:items-end`. Estado `fechaRecepcion` sin persistencia backend hasta definir campo. Calendario nativo.
