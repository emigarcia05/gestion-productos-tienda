...
*Última actualización (2026-05-11): **`ChequesCajaTesoreriaModal`** — solo filtro **Tenencia** (sin **Vista**); ícono **Transferir** (antes “acreditar”) solo en **ACTUALES** con `tenencia === TIENDA` y cheque no transferido; modal **Destino Cheque** (`DestinoChequeTesoreriaModal`): ambos botones CTA **`Button`** **default**; si el cheque está **al día** (`chequePuedeAcreditarsePorFechaArgentina`) se ofrecen **Acreditar En Cuenta Propia** y **Pago Proveedor**; si está **diferido**, aviso **`CALLOUT_WARNING_CLASS`** y solo **Pago Proveedor**. **Acreditar En Cuenta Propia** → **`AcreditarChequeTesoreriaModal`** (columna **ACCIONES**: `Button` **`size="icon"`** con **`BadgeCheck`**, **`Loader2`** al enviar; `transferirFinTesoreriaChequeAction`). **Pago Proveedor** en **Destino Cheque** ejecuta `marcarEntregaProveedorFinTesoreriaChequeAction` (sin segundo modal). **Detalles De Cheques**: `AppModal` `xl` + **`max-w-[calc(48rem*1.3)]`**; `colgroup` cinco/seis columnas (**TRANSFERIDOS**) o siete/ocho (**ACTUALES**) según filtro y rol.*

*Última actualización (2026-05-16): **`ChequesCajaTesoreriaModal`** — **ACTUALES**: tabla **RECIBIDO** … **DÍAS**, **ACCIONES** (**Transferir** / **Editar** / **Borrar**), orden **DÍAS** ↑; **TRANSFERIDOS**: **RECIBIDO**, **EMISOR**, **TRANSFERENCIA**, **TENEDOR** (`tenencia`), **MONTO**, **ACCIONES** (**Editar**/**Borrar**), orden **TRANSFERENCIA** ↓; **`AltaChequeTesoreriaModal`**: **Registrar Cheque**; **FECHA RECIBIDO**; doble clic fechas → `showPicker`; sin **Entrega Proveedor**.*
*Última actualización (2026-05-17): **Pago proveedor cheque** — baja **`PagoProveedorChequeTesoreriaModal`**; **Pago Proveedor** en **`DestinoChequeTesoreriaModal`** dispara directo `marcarEntregaProveedorFinTesoreriaChequeAction`.*
