...
| `.tabla-fin-ana-margen-contribucion`, `.tabla-mc-celda-dual`, `.tabla-mc-dual-grid`, `.tabla-mc-dual-pesos`, `.tabla-mc-dual-pct` | **Margen Contribución** (modo **PRODUCTO**): una columna por forma de pago; par $/% vía **`CeldaSubcolumnasMontoPct`**. **`.tabla-mc-celda-dual`** anula padding horizontal de **`.celda-datos`**. Grid SSOT: pads exteriores = gap; pistas de valor **65:35**. Variable **`--tabla-mc-dual-cols`**. |
...
| `CeldaSubcolumnasMontoPct` + CVA (`@/components/shared/CeldaSubcolumnasMontoPct.tsx`) | Par **$ / %** en subcolumnas visuales **65 % / 35 %** (grid `1fr 6.5fr 1fr 3.5fr 1fr`: pads exteriores = gap). Props: `monto`, `pct`, `density` (`table` \| `default`), `weight` del % (`bold` \| `normal`). Clases: `.tabla-mc-dual-grid`, `.tabla-mc-dual-pesos`, `.tabla-mc-dual-pct`; celda envoltorio **`.tabla-mc-celda-dual`**. |
...
- **Layout (globals)**: grid `1fr 6.5fr 1fr 3.5fr 1fr` → pad \| $ \| gap \| % \| pad. Pads exteriores = gap; pistas de valor en **65:35**. Valores centrados en su pista.
- **Celda envoltorio**: `className={cn("celda-datos celda-numero tabular-nums", "tabla-mc-celda-dual", …)}` (anula `padding-left/right` de `.celda-datos`).
...
- **Ejemplo**
  ...
  <TableCell className={cn("celda-datos celda-numero tabular-nums", "tabla-mc-celda-dual")}>
