...
9. **Al terminar un cambio**  
   ...
   - Si ajustas elementos de **slidenav/sidebar**, mantener componentes compactos y consistentes:
     ...
     - **Progreso import / sync** (`ImportStatusIndicator`, `SyncStatusIndicator`): **`MensajeProceso` `variant="sidebar"`** solo cuando hay **proceso en curso** (fondo/borde azul proceso en `globals.css`). `ImportStatusIndicator` solo visible con import activa. `SyncStatusIndicator` en **reposo** (últ. act. / disparar sync): **`<button>`** con `bg-sidebar-accent` y `text-sidebar-foreground`; en hover se refuerza con **elevación** (`hover:shadow-sm`) y **borde anular** (`hover:ring-sidebar-ring/50`), manteniendo el mismo fondo para coherencia visual. Segunda línea `text-sidebar-foreground/80` y misma altura compacta (~`min-h-[3.5rem]`, dos líneas centradas) para fundirse con la slidenav. En **sync en curso**: `<MensajeProceso mensaje="Sincronizando DUX" detalle={…} />`.
