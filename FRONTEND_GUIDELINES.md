...
7. **Nuevo modal con tabla**  
   ...
   - **`PedidoHistoriaDetalleModal`:** tabla **8% + 50% + 10% + 10% + 22%** (**COD. TIENDA** / **DESCRIPCIÓN** / **CANT. PEDIDA** / **CANT. RECIBIDA** / **ACCIONES**). Layout en **tres bloques** esquemáticos: (1) panel resumen `MODAL_RESUMEN_PANEL_CLASS` (`bg-muted/25`, borde) con **Datos del pedido** (`MODAL_MICRO_LABEL_CLASS`), proveedor/sucursal, fecha generación y **Badge** de estado; **Fecha recepción** alineada a columna ACCIONES (`GRID_CAPAS_SUP_PEDIDO_HISTORIA` + `COLUMNA_ACCIONES_EXTERNA_CLASS`). (2) tarjeta **Agregar ítem a la recepción** (`MODAL_SECTION_CARD_CLASS`) con `GRID_TABLA_PEDIDO_HISTORIA_5COL` y botón primario **Agregar**. (3) **Ítems del pedido** + tabla dentro de `MODAL_SECTION_CARD_CLASS` + `.contenedor-tabla-gestion`. Separador decorativo opcional (`Separator`) bajo el título de ítems. Estado `fechaRecepcion` sin persistencia backend hasta definir campo.
