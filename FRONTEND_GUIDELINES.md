...
10. **Al terminar un cambio**  
   ...
   - Si ajustas elementos de **slidenav/sidebar**, mantener componentes compactos y consistentes:
    - **Ritmo vertical** (`Sidebar.tsx`): **navegación** arriba (`pt-3 px-4`). Abajo (`mt-auto`, `px-4 pb-4`): **sync/import** → contenedor propio de **`SelectorRol` `compact`** (`w-full min-w-0 pt-3` + wrapper `rounded-lg p-2`, sin borde ni fondo) → un solo contenedor `flex flex-col pt-2` con **regla superior** (`mx-auto h-px w-[80%] bg-sidebar-foreground/70`) + **`SidebarMainAppArea`** (`className="pt-3"` en sidebar) + **regla inferior** (`mx-auto mt-2 h-px w-[80%]`, mismo estilo), sin mezclar usuario y área en el mismo contenedor visual.
...
- **Altura de fila (obligatoria en este grid)**: **todas** las filas (incluida la cabecera de columnas) usan la misma altura fija **`h-10` (2,5 rem)**, tomada como referencia de la fila **Resultado operativo** (una línea `text-sm` sin botones). Celdas: `flex items-center`, `py-0`, `px-3`. En columnas de **Global y sucursal**, el sublayout interno es **`25%` + `75%`** (`grid-cols-[25%_75%]`): el **25%** izquierdo es una **rejilla de dos ranuras fijas** (`grid-cols-2`) para **histórico** | **desglose/edición**; en **Global** se usa solo la ranura de histórico (la 2ª queda vacía). El **75%** derecho muestra el importe alineado a la derecha. Botones: `TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS` + `TABLE_ROW_ACTION_ICON_CLASS` (`@/lib/ui-classes`) con override compacto **`!h-7 !w-7 !p-1`**. Constantes `CLASE_FILA_BALANCE_MENSUAL_GRID` / `CLASE_CELDA_BALANCE_MENSUAL` / `CLASE_BOTON_ACCION_BALANCE_MENSUAL` en el componente.
...
- **Fila Ventas — edición**: si `puedeEditarVentas` y la columna tiene `sucursalId`, el **lápiz** va en la **ranura 2** del subgrid (junto al histórico en ranura 1); el monto sigue en el **70%** derecho.
...
- **`CrearEditarFinBalGastoFinalModal`:** campos con **`ModalMicroLabel`**; **GENERA IVA CRÉDITO** (`SIEMPRE` / `NUNCA` / `PREGUNTA`) ubicado debajo de **PLAZO DE PAGO** (columna `fin_bal_gasto_final.iva`).
