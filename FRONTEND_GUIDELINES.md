...
*Última actualización (2026-05-05): **Gasto eventual** (`GASTO MENSUAL` = NO) — en `CrearEditarFinBalGastoFinalModal` los campos **DÍA DEVENGADO** y **PLAZO DE PAGO** quedan bloqueados; al guardar se envían como `null` para persistirlos vacíos en DB.*
...
*Última actualización (2026-05-05): **`CrearEditarFinBalGastoFinalModal`** (Nuevo/Editar Gasto Final) reordena campos a: **TIPO DE GASTO (MENSUAL/EVENTUAL)**, **SUCURSAL**, **PROVEEDOR**, **DÍA DEVENGADO**, **PLAZO DE PAGO**. **DÍA DEVENGADO** usa `Select` 1..28 y **PLAZO DE PAGO** usa `Select` 0..30. Si **TIPO = EVENTUAL**, ambos quedan deshabilitados y se persisten como `NULL` en `fin_bal_gasto_final`.*
