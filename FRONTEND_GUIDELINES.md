...
- **`/finanzas/venc-por-fecha`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Venc. por fecha**. Calendario mensual (`?mes=YYYY-MM`, default mes actual AR): grilla **LUN→DOM**, celdas con **`aspect-[4/5]`** (más alto que ancho); **parte superior** = número de día; **parte inferior** = **una sola tarjeta con el total consolidado de vencimientos del día** (sin discriminar por proveedor). La tarjeta de total diario usa fondo **`destructive` suavizado** (rojo no agresivo), con monto en **blanco**, **bold** y centrado. Para mantener contorno y separaciones visuales consistentes, **todas** las celdas del calendario (con o sin fecha válida) usan borde fino **`#0072BB`**. Navegación mes anterior/siguiente. Datos: `listarVencimientosEnRango` (misma regla de vencimiento que deuda). `PERMISOS.finanzas.acceso`.
