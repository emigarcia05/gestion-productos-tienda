...
**ANALISIS DE PRECIOS** (rol **editor**, orden): `Vinc. Con Prov.` → `Px Competencia` → `Cx & Px Tienda` → `Comp. Por Cat.`. **`Cx & Px Tienda`**: al abrir **sin** query de filtros carga **todos** los ítems de **`prod_precios_tienda`** paginados (`PAGE_SIZE`). **`FiltrosCxPxTienda`**: **MARCA** → **CX. VINC.** → **CX. PROV.** → **PX LISTA** (`pxLista=prom` → `competencia_id_px_lista` nulo / Px. Prom.; `pxLista={competenciaId}` → FK a ese competidor) → **MARCACION** (`marcacionOrden=menor_mayor` \| `mayor_menor`: orden global por columna calculada) + **`q`**. Grilla **`TablaCxPxTienda`**: `<colgroup>` **46%** + **22%** + **22%** + **10%**; dentro de CX PROD. grid **16fr** (lista) + **9fr** (valor); PX LISTA **13fr** + **9fr**; **MARCACION** con `tabla-bloque-secundario-*-divider` (bloque secundario). Header **`ExportarCxButton`** / **`ExportarPxButton`**: Excel `.xls` — Cx (**CODIGO**, **COSTO**: `costo_compra` ≠ costo lista vinculado); Px (**CODIGO**, **IMPORTE** = marcación de la grilla; solo ítems con `px_lista_tienda` ≠ PX LISTA configurado). Ver `BACKEND_GUIDELINES` §1.10b.
