...
- **Layout Finder (catálogos jerárquicos):** componentes compartidos en `@/components/shared/catalogo-finder/` — **`CatalogoFinderColumn`** (sección con header `bg-muted/60`, botón **`+`** solo ícono `size-6` alineado al alto del título), **`CatalogoFinderRow`** (selección en cascada, acciones editar/eliminar al hover), **`CatalogoFinderEmpty`**. Usado en **Balance · Catálogo Gastos** (5 columnas) y **Comp. Categorias · Categorias** (3 columnas).
...
**Columna Referencia competencia** (`ComparacionReferenciaCompetenciaColumn`): grilla **COMP. 12 %** · **PRODUCTO 70 %** (`line-clamp-2`) · **PX. 13 %** · **basura 5 %** (encabezado con ícono `Trash2` como tablas de gestión; botón por fila); selección única con **`CATALOGO_FINDER_ROW_SELECTED_CLASS`**; **+ Nuevo** agrega referencia.
