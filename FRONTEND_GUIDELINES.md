...
7. **Nuevo modal con tabla**  
   ...
   - **`PedidoHistoriaDetalleModal`:** título visible del modal (AppModal): **Recepcion Del Pedido**. `AppModal` `sm:max-w-[62.4rem]` (≈ +30% sobre 48rem). Tabla **5% + 55% + 10% + 15% + 15%**: columna **lista de verificación** (icono **Check** en cabecera; por fila input + OK o **Enter** → ícono check; estado solo en cliente hasta definir reglas/persistencia), **DESCRIPCIÓN**, **CANT. PEDIDA**, **CANT. RECIBIDA**, **ACCIONES**; sin **COD. TIENDA** (`title` en descripción con código si existe). (1) Resumen: **dos columnas** en `sm`: bloque proveedor / metadatos **~85%** y columna **FECHA FACTURA** ~**15%** (`GRID_CAPAS_SUP_PEDIDO_HISTORIA` = `85fr_15fr`; sin `div` hueco `hidden sm:block`). (2) Contenedor **grid** que envuelve la fila de alta + tabla de ítems: **`grid-cols-1`** **`grid-rows-[auto_minmax(0,1fr)]`** **`gap-x-3`** **`gap-y-0`** (sin espacio vertical entre fila de búsqueda y tabla; **`gap-x-3`** = separación horizontal vía *column-gap*). Fila **Agregar producto**: **tres columnas** **70% + 15% + 15%** (input búsqueda | input cantidad con `aria-label="Cant. recibida (nuevo ítem)"` | botón **Agregar**), sin alinear columna a columna con la tabla (`GRID_FILA_AGREGAR_PEDIDO_HISTORIA` = `70fr_15fr_15fr`, móvil **`gap-y-3`** sin *column-gap*; en **`sm`**: **`gap-x-3`** **`gap-y-0`**, **`sm:items-center`**); columnas hijas: **`flex flex-col items-center justify-center gap-1.5`** — **primera** (búsqueda): **`py-0 pr-2 pl-0`** (sin padding vertical ni izquierdo); **cantidad y botón**: **`px-2 py-0`**. Búsqueda **`text-left`**, cantidad **`text-center`**. La sección de alta tiene título **AGREGAR PRODUCTO A LA RECEPCIÓN** (`<span>` `MODAL_MICRO_LABEL_CLASS` + **`p-0 m-0 mb-1 box-border block w-full font-bold`**); la `<section>` usa **`flex flex-col gap-0 p-0`** (sin hueco entre título y grid salvo el **`mb-1`** del título) y **`aria-labelledby`** al `<span>`. Contenedor **columna** principal del cuerpo del modal: **`gap-0`** entre el **resumen** y el **grid** (alta + tabla), para evitar hueco vertical excesivo. Panel resumen: **`px-3 pt-3 pb-0 sm:px-4 sm:pt-4 sm:pb-0`** (sin **`pb`** inferior). Sección **AGREGAR PRODUCTO A LA RECEPCIÓN**: **`flex flex-col gap-0 p-0`**; separación mínima título ↔ fila: **`mb-1`** en el `<span>`. Celdas del grid sin padding vertical (**`py-0`**). El acople con el resumen sigue con **`gap-0`** en la columna del modal. Los **`gap`** internos de **`GRID_CAPAS_SUP_PEDIDO_HISTORIA`** y **`GRID_FILA_AGREGAR_PEDIDO_HISTORIA`** no se alteran. (3) Bloque de tabla de ítems: `<section aria-label="Ítems del pedido">` (sin encabezado visual previo); `MODAL_RESUMEN_PANEL_CLASS` / `MODAL_SECTION_CARD_CLASS` = `bg-transparent`. `fechaRecepcion` sin persistencia backend hasta definir campo.
...
- **Resumen superior (columna proveedor):** sin micro-etiqueta “Nombre proveedor”; el nombre del proveedor es el primer nodo visible (`<p>` `text-sm font-semibold`). El `gap-2` del contenedor flex se conserva entre ese párrafo y la línea secundaria: solo **sucursal - dd/mm hh:mm** (`text-xs` `text-muted-foreground`), sin mostrar estado (PEDIDO/RECIBIDO) en esa línea.
