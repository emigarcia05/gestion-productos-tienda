...
## 5.1 Módulo Px Competencia

- **Pantalla canónica:** **`TablaPxListas`** / **`PxListasPageClient`** en `/gestion-productos/tienda/cx-px-tienda` (título de página y sidebar: **Px Competencia**). **`/gestion-productos/precios-competencia`** (y legacy `/proveedores/competencia-precios`) **redirigen** aquí. La grilla standalone **`CompetenciaPreciosTabla`** queda como referencia; componentes compartidos en `src/components/precios-competencia/*` (`AsociarUrlsCompetenciaModal`, `RelevamientoUltimoMensaje`, etc.).
...
- **Acciones (editor):** **Gestionar Competidores** + **Comparar Precios Competencia** (`SincronizarCompetenciaModal` `size="lg"`: filtro por nombre; lista con **DÍAS SIN ACT.** y **Actualizar** (`RefreshCw`) por competidor; en curso **Detener Comparación** + **`{processed} de {total}`**; `CompetenciaSyncProgresoBanner` sobre la grilla **Px Competencia**).
...
- **Sidebar:** **ANALISIS DE PRECIOS** agrupa **Cx Compra**, **Px Competencia** y **Comp. Por Cat.**
