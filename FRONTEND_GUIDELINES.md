...
| `TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS` (`@/lib/ui-classes`) | Botones **solo ícono** en **filas de tabla** (y equivalentes en modales de grilla): fondo **`#0072BB`**, ícono blanco (tamaño vía **`TABLE_ROW_ACTION_ICON_CLASS`**), `size="icon"` + `variant="ghost"`, alto acoplado a la fila (`!h-full`, `min-h-[var(--tabla-body-row-min-height)]`, `self-stretch`), **padding interno** `!px-2 !py-2` sobre el recuadro azul (el ícono no se agranda). Ancho mínimo **`min-w-10`**, `!w-auto` salvo que el componente añada **`!w-full`**. Sustituye el patrón prohibido outline + icon-xs neutro. |
...
*Última actualización (2026-05-05): **`TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS`** — `!px-2 !py-2` para más “aire” azul alrededor del ícono (`min-w-10`, `!w-auto`); el tamaño del glifo sigue en **`TABLE_ROW_ACTION_ICON_CLASS`**.*
