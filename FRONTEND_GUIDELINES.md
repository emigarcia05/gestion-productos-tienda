...
- **Navegación lateral por área activa** (`Sidebar.tsx`): los módulos (`PEDIDO DE MERCADERÍA`, `LISTA PROVEEDORES`, `LISTA TIENDA`) se muestran **solo** cuando el área activa es **Gestión Productos**. Con área **Finanzas** se muestra el bloque **`FINANZAS`** (`Landmark`) con **Deuda Proveedores** → `/finanzas/deuda-proveedores` y **Venc. por fecha** → `/finanzas/venc-por-fecha` (`CalendarDays`, `PERMISOS.finanzas.acceso`). En **Estadísticas Productos** no hay módulos en sidebar todavía (mensaje vacío).
...
- **`/finanzas/venc-por-fecha`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Venc. por fecha**. Misma envoltura de tabla que **`TablaDeudaProveedores`**: **`contenedor-tabla-gestion`** + **`rounded-md border border-border bg-card`** + **`overflow-hidden`**, scroll en hijo **`overflow-x-auto overflow-y-auto`** (no solo `no-scroll-x` suelto). Cabeceras numéricas con **`text-right`** (`TH_NUM`); celdas importes **`celda-datos text-right tabular-nums`** como en deuda. El **mes** del listado se muestra solo en el encabezado (**`tituloMes`** + **`mesYm`**); la tabla **no** incluye columna **MES** (cuatro columnas: **DÍA (10%)**, **A PAGAR (30%)**, **CAJA DISPONIBLE (30%)**, **SALDO (30%)**). `A PAGAR` se consolida por día desde `listarVencimientosEnRango` (sin discriminar proveedor). En la implementación actual, `CAJA DISPONIBLE` usa placeholder `0` hasta integrar origen; **`SALDO` es acumulativo en el mes** (filas por fecha): primera fila `CAJA − A PAGAR`, siguientes `saldo anterior − A PAGAR`. Filas con **`isoYmd` &lt; hoy** (Argentina): **`data-fecha-pasada="true"`**, `aria-label` con “fecha pasada”, y clases que **anulan** la cebra y el hover de **`TableRow`** (`!bg-muted/55`, `odd:!bg-muted/55`, `even:!bg-muted/55`, `hover:!bg-muted/62`, `text-muted-foreground`, `cursor-default`) para aspecto de fila bloqueada. **Doble clic** en una fila abre **`AppModal`** solo lectura: título **Detalle Del Día** + fecha larga AR (`formatFechaLargaNotaPedidoArgentina`), texto **Solo lectura**; tabla **PROVEEDOR** / **MONTO A PAGAR** (`detallesPorDia`). `title` en fila: pista de doble clic. Navegación mes anterior/siguiente por `?mes=YYYY-MM`. `PERMISOS.finanzas.acceso`.
