...
| `.tabla-gestion-compacta.tabla-deuda-proveedores` | Solo **Finanzas / Deuda Proveedores**: desactiva el overlay de `td:hover` para que el hover resalte la **fila completa** y habilita alto automático de fila + wrap en `.celda-proveedor-deuda` para textos largos. |
| `.tabla-gestion-compacta` | **Diseño único** de tablas (referencia: Comp. Proveedores). Usar siempre `<Table>` de `@/components/ui/table`; no usar otra clase. **Encabezado fijo obligatorio**: al hacer scroll los encabezados no desaparecen (`position: sticky` en `globals.css`). **`thead th`**: altura global **`--tabla-thead-height`** calculada como **(2 líneas de texto + padding vertical corto)**; títulos centrados horizontal y verticalmente, hasta **2 líneas** con recorte del excedente; sin `nowrap` en encabezados. `TableHead` mantiene `text-xs` y `font-normal`. **Inputs y Select** en celdas: fondo transparente, recuadro #0072bb. **Select en tablas**: texto en negro, sin bold (`globals.css`). **Hover de celda (global, solo `tbody`):** `td:hover` con `overflow: visible` y `z-index` **35**; **`::before`** opaco con **`width: max(100%, 42rem)`** para revelar contenido truncado y **tapar texto de celdas contiguas**. Para evitar diferencias visuales entre columnas, el fondo del `::before` toma una variable de fila (`--tabla-row-bg`) sincronizada con el estado de la fila (cebra `odd`/`even`, `hover`, `data-state="selected"`), manteniendo el mismo hover uniforme en toda la fila; los tonos de cebra y hover se definen como colores **opacos** (sin alpha sobre transparente) para evitar “doble capa” en la celda activa; hijos directos **`z-index: 36`**. |
...
- **`/finanzas/venc-por-fecha`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Venc. por fecha**. Calendario mensual (`?mes=YYYY-MM`, default mes actual AR): grilla **LUN→DOM**, celdas con **`aspect-[4/5]`** (más alto que ancho); **parte superior** = número de día; **parte inferior** = **una sola tarjeta con el total consolidado de vencimientos del día** (sin discriminar por proveedor). La tarjeta de total diario usa fondo **`destructive` suavizado** (rojo no agresivo), con monto en **blanco**, **bold** y centrado. Navegación mes anterior/siguiente. Datos: `listarVencimientosEnRango` (misma regla de vencimiento que deuda). `PERMISOS.finanzas.acceso`.
