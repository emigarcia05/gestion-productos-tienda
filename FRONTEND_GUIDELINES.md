...
7. **Nuevo modal con tabla**  
   ...
   - Si necesitás alinear un bloque inferior con las mismas columnas de la tabla, **no** usar `grid-cols` con porcentajes que superen 100% (ej. `9% + 62.5% + 10.5% + 10.5% + 15%`). Eso provoca desbordes y desplazamientos laterales. Usar `fr` proporcionales (ej. `grid-cols-[9fr_62.5fr_10.5fr_10.5fr_15fr]`) para mantener la proporción sin overflow, o preferir **`TableFooter` (`<tfoot>`) dentro del mismo `<Table>`** con `colSpan` en las columnas previas: el ancho de la última columna coincide exactamente con el cuerpo (ej. total alineado con **ACCIONES** en `PedidoHistoriaDetalleModal`).
   - **`PedidoHistoriaDetalleModal` — alineación con la tabla:** constante de layout `GRID_ALINEACION_TABLA_PEDIDO_HISTORIA` = `grid-cols-1 sm:grid-cols-[9fr_62.5fr_10.5fr_10.5fr_15fr]` (mismas proporciones que **COD. TIENDA** / **DESCRIPCIÓN** / **CANT. PEDIDA** / **CANT. RECIBIDA** / **ACCIONES**). **Cabecera:** cols 1–4 (`sm:col-span-4`) proveedor/sucursal y fecha generación + estado (centrado); col 5: **FECHA RECEPCIÓN** + `type="date"` `w-full`, envuelto con `CELDA_TABLA_PADDING_X` + `sm:shadow-[inset_1px_0_0_#0072bb]` (misma franja que **ACCIONES**). **Fila de alta (DESCRIPCIÓN / CANT. / + AGREGAR):** mismo grid con `sm:items-end`; col 1 y 3 reservadas (`hidden sm:block`) para alinear con COD. y CANT. PEDIDA; col 2 descripción `w-full`; col 4 **CANT.** (alineada con **CANT. RECIBIDA**); col 5 **+ AGREGAR** `w-full` con el mismo shell que la fecha. Así fecha, botón y columna **ACCIONES** comparten ancho y eje vertical. Estado `fechaRecepcion` sin persistencia backend hasta definir campo. Calendario nativo.
