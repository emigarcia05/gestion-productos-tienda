...
| `.tabla-bloque-secundario-head`, `.tabla-bloque-secundario-head-divider` | Estilo para encabezados de columnas de **información secundaria** (bloque a la derecha): fondo `bg-muted` suavizado, texto `text-muted-foreground`. La variante `*-divider` agrega una línea vertical más marcada (`border-left: 2px` con mezcla de `border` y `foreground`) para separar visualmente el bloque secundario del principal. |
| `.tabla-bloque-secundario-cell`, `.tabla-bloque-secundario-cell-divider` | Estilo para celdas de cuerpo de **información secundaria** (ej. STOCK, CANT. A PEDIR en Pedido Reposición). Fondo y color alineados con los encabezados secundarios. La variante `*-divider` agrega la misma línea divisoria vertical marcada. |
