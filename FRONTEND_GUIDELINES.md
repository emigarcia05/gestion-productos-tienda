...
| `.tabla-fila-seccion-subencabezado`, `.tabla-fila-seccion-subencabezado-celda` | Subencabezado de bloque en **`tbody`**: fondo **`var(--card)`** (blanco), **sin** bordes ni franja lateral; **alto fijo** = **`--tabla-thead-height`**. Componente **`TablaSubencabezadoSeccionRow`**: **`titulo`**, **`colSpan`**, **`totalBloque?`**. |
