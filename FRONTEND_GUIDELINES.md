...
- **Jerarquía canónica de URLs (2026-03):** para el área **Gestión Productos** usar siempre prefijo **`/gestion-productos`** con estructura **área/módulo/submódulo**:
  ...
  - **Cx Compra** (…): **`ActCxButton`** en header exporta Excel (**CODIGO**, **COSTO**, archivo **`Act Cx dd-mm hh-mm.xls`**); toast al descargar; modal si no hay diferencias; tras exportar Excel, modal **¿Desea exportar el informe de aumento?** (**No** / **Sí, Exportar** → PDF resumen vía `exportarResumenAumentosPxAction` + `descargarPdfResumenAumentosPx`). …
...
- **Exportación DUX:** botón **Act. Px.** → un Excel **por** `nombre_lista` del catálogo (`prod_tienda_listas_precios`); columnas **CODIGO** (`cod_tienda`) + **PORC UTILIDAD** (margen desde precio efectivo: edición ?? DUX, `calcMargenSinIvaPct`, 4 decimales); archivo **`Act. {nombre_lista} dd-mm hh-mm.xls`**; `exportarPxListasMargenAction` + `exportPxListasMargenExcelClient.ts`. Sin filtros de página (catálogo completo por lista).
