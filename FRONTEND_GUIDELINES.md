...
- **Tabla** (`CompetenciaPreciosTabla`): mismo alto de fila global **2rem** que **Lista Px Proveedores**; **ACCIONES** con patrón estándar **`TABLE_ROW_CELL_ICON_ACTIONS_FLEX_CLASS`** + **`TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS`**. Subfila expandida: **una `TableRow` por competidor** (sin `colSpan`) alineada al `<colgroup>` — nombre **a la derecha** en **DESCRIPCIÓN**, **px** en **PX PROMEDIO**, **dif.** en **DIF TIENDA** (`DetalleCompetidorFila`). `<colgroup>` **45% + 9%×5 + 10%**. Bloques secundarios `tabla-bloque-secundario-*`. Cálculos en `competenciaPreciosFilaResumen.ts`.
...
- **Vínculo manual:** botón **Asociar URLs** por fila → `AsociarUrlsCompetenciaModal` (todas las URLs del producto). Cada **URL del producto** lleva tacho a la derecha del input (borra el texto para reemplazar la URL). `EditarUrlVinculoModal` queda para uso puntual si se reutiliza. Filtros **COMPETIDOR** + **ESTADO VÍNCULO** (ej. **ERROR** para revisar URLs).
