...
- **Permisos en Px Listas:** acceso/edición lista → `PERMISOS.cxPxTienda.acceso`; botones **Asociar URL** y **Relevar URLs** (columna ACCIONES, `RefreshCw`) → `PERMISOS.competenciaPrecios.editar`; relevar por ítem usa `relevarUrlsProductoCompetenciaAction` (todos los vínculos del producto, sin sync masivo).
