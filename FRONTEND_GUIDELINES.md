...
8. **Tablas (encabezado fijo + paginación)**  
   ...
   - **Pie de totales fijo** (patrón `contenedor-tabla-gestion--pie-fijo`, dos `<table>`): `scrollbar-gutter: stable` en scroll y pie (`globals.css`). Para alineación exacta con la barra vertical u otros anchos útiles distintos, usar **`usePieFijoColumnWidthsSync`** (`src/lib/hooks/usePieFijoColumnWidthsSync.ts`): ref en `.contenedor-tabla-gestion--pie-fijo-scroll`, ref en la `<table>` del pie, mismo número de `<col>` que de `th`; pasar firma de columnas si cambia el layout (ej. **ACCIONES**). **`TablaGastos`** con **ACCIONES**: panel secundario separado + **`useMirrorScrollTop`** (`src/lib/hooks/useMirrorScrollTop.ts`). Referencia: **`TablaGastos`**, **`TablaTesoreriaCajas`**.
