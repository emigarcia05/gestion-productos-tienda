...
- **`/finanzas/analisis-mc/margen-contribucion`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Margen Contribución**. **`actions`**: **Gestion Cx. Y Formulas** (`Calculator`) → **`GestionCxYFormulasMargenContribucionModal`** (VARIABLES + VARIABLES CALCULADAS; valores PX LISTA / IVA / IIBB desde `fin_ana_mc_formulas`); **Gestionar Pagos** (`Settings2`) → **`GestionarPagosFinAnaCosFinaModal`**. **Configuración** (`FilterBar`, una fila): **TERMINAL**, **TIPO DE COMPROBANTE** (`FACTURA A` | `FACTURA C`; **C** no calcula IVA ni IIBB), **PORC. UTILIDAD** (`PorcentajeCentInput` placeholder **PORC. UTILIDAD** si vacío/`0`; → CX MERCADERÍA), **`LimpiarFiltrosButton`**. Tabla **`TablaFinAnaMargenContribucion`**: columnas = formas de pago con `en_margen_contribucion`; secciones **INGRESO** (**solo DESCUENTO** editable por columna; celda `.tabla-mc-celda-descuento` al mismo alto de fila que IVA) → **COSTOS** (IVA · IIBB · CX MERCADERÍA · CX FINANCIERO) → **MARGEN** (M.C · M.C PONDERADO). Montos en **`N%`**. Debajo de la tabla: gráfico SVG **`GraficoMcVsPorcUtilidad`** — eje X **PORC. UTILIDAD** 20 %…200 %; eje Y **M.C** de la forma de pago **3 CUOTAS** (`idFormaPagoTresCuotasMargenContribucion`); si el usuario carga un PORC. UTILIDAD en ese rango, línea vertical discontinua desde el eje X hasta la curva + punto. Ver BACKEND §2.5i.
...
*Última actualización (2026-07-23): **Margen Contribución · gráfico M.C** — debajo de la tabla, SVG **`GraficoMcVsPorcUtilidad`**: X = PORC. UTILIDAD 20–200 %; Y = M.C de **3 CUOTAS**; marca vertical al PORC. UTILIDAD del filtro.*
