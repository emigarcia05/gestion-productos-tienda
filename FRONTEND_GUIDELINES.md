...
- **`/finanzas/analisis-mc/costos-financieros`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Costos Financieros** (módulo sidebar **ANALISIS M.C.**). **Filtros** (`FilterBar` + `FilaFiltrosDesplegables`): **TERMINAL**, **PAGO**, **HABILITADO** (cada uno en `FiltroIndividualContainer`; opciones en MAYÚSCULAS; habilitado = **HABILITADO** / **NO HABILITADO**); slot **`col-span-2`** con contador **X COMBINACIÓN(ES)** + **`LimpiarFiltrosButton`**. Tabla **`TablaFinAnaCosFina`**: matriz **terminal × pago** (21 filas semilla). Columnas **HABILITADO** (`tabla-check-toggle tabla-check-toggle--alto-fila` en celda **`celda-datos--accion-relleno-fila`**: fondo **`bg-background`**, borde **`#0072BB`**, tilde **`Check`** azul al activar; solo `editor` con `Button` clickeable), **TERMINAL** y **PAGO** (solo lectura), **DÍAS ACRED.** (`Input` entero, guarda en `onBlur`), **ARANCEL** y **CX FINANCIERO** (`PorcentajeCentInput` con **`showPctSuffix={false}`** — máscara POS 2 decimales y coma es-AR, sin `%`; guarda en `onCommit`). **CX TERMINAL S/ IVA** y **CX TERMINAL C/ IVA**: columnas **calculadas** (solo lectura, `Input` `readOnly` con fondo `bg-muted/30`); fórmulas en `@/lib/finAnaCosFina.ts` — S/ IVA = arancel + cx financiero; C/ IVA = suma × **1,21**. Las columnas editables numéricas son **siempre** `<Input>` / `PorcentajeCentInput` (en `simple`, `readOnly`). Alto acotado a **`--tabla-body-row-min-height`**. Mutación: **`actualizarFinAnaCosFinaAction`**. Sin paginación. Ver `BACKEND_GUIDELINES` §2.5h.
