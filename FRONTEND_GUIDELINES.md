...
- **Columnas** (sin `tabla-bloque-*`; `<colgroup>` 5/15/45/15/10/10% con editor, 5/15/55/15/10% sin editor):
  1. **BASE** (`5%`, header con ícono `Check`): `<input type="checkbox">` por fila; solo una marcada a la vez. Refleja **`cod_ext_costo_compra`** (la fila tildada es la **base de comparación** para la columna VARIACION y a la vez el costo elegido en Cx & Px Tienda). Click persiste vía `establecerCostoListaTiendaAction(codTienda, codExt | null)` (`vinculos.ts`): cliquear una fila no marcada → setea esa como base; cliquear la ya marcada → destilda (FK a `null` = Cx. Prom.). Deshabilitado para no editores y mientras hay `isPending`.
