...
3. **Input de búsqueda en filtros (reutilización)**
  ...
  - **Px. Vta. Sugeridos** (`/gestion-productos/proveedores/sugeridos`): la grilla lista ítems que coinciden con filtros y con **`habilitado = true`** (no exige `px_vta_sugerido` no nulo). La columna de encabezado es **`DESCRIPCIÓN`** y muestra descripción efectiva por `cod_ext`: primero **`descripcion_tienda`** (si existe en `precios_tienda`), y como fallback **`descripcion_proveedor`** (`precios_proveedores`). Si **`px_vta_sugerido`** viene nulo, la celda se muestra vacía (sin `$0`). En el payload de lectura (`getListaPreciosConOpcionesAction`), el campo **`pxVtaSugerido`** debe enviarse siempre para evitar celdas vacías por ausencia de mapeo.
...
- **Tabla `/pedidos/enviar` (columnas, orden):** **TIPO PEDIDO** (**15%**, valor de `pedidos_mercaderia.tipo_de_pedido`), **SUCURSAL** (**15%**, valor visible siempre en MAYÚSCULA), **DESCRIPCIÓN** (**60%**), **CANT. PEDIR** (**10%**).
...
| `.tabla-gestion-compacta.tabla-deuda-proveedores` | Solo **Finanzas / Deuda Proveedores**: habilita alto automático de fila + wrap en `.celda-proveedor-deuda` para textos largos. |
| `.tabla-gestion-compacta` | **Diseño único** de tablas (referencia: Comp. Proveedores). Usar siempre `<Table>` de `@/components/ui/table`; no usar otra clase. **Encabezado fijo obligatorio**: al hacer scroll los encabezados no desaparecen (`position: sticky` en `globals.css`). **`thead th`**: altura global **`--tabla-thead-height`** calculada como **(2 líneas de texto + padding vertical corto)**; títulos centrados horizontal y verticalmente, hasta **2 líneas** con recorte del excedente; sin `nowrap` en encabezados. `TableHead` mantiene `text-xs` y `font-normal`. **Inputs y Select** en celdas: fondo transparente, recuadro #0072bb. **Select en tablas**: texto en negro, sin bold (`globals.css`). **Hover global de filas (páginas y modales):** el resaltado es uniforme para toda la fila, sin distinción por columnas ni overlays por celda (`td:hover::before` desactivado). Los colores se definen por estado de fila (cebra `odd`/`even`, `hover`, `data-state="selected"`) y, en `hover`, el color de fuente de todas las celdas se fuerza a **`foreground`** para mantener contraste homogéneo. |
...
| `--tabla-thead-height`, `--tabla-thead-lines`, `--tabla-thead-line-height`, `--tabla-thead-padding-y`, `--tabla-body-row-min-height`, `--tabla-body-cell-padding-y`, `--tabla-body-cell-padding-x` | **`--tabla-thead-height`** = `calc((0.75rem * --tabla-thead-line-height * --tabla-thead-lines) + (--tabla-thead-padding-y * 2))` con defaults `--tabla-thead-lines: 2`, `--tabla-thead-line-height: 1.1`, `--tabla-thead-padding-y: 0.2rem`; **`--tabla-body-row-min-height`** = **2rem**; padding vertical celdas ~**`py-0.5`** (**0.125rem**); inputs/botones en celdas ~**1.75rem**. Sin cambio de **`font-size`** en **`tbody tr:hover td`** (evita saltos). |
...
| `.tabla-check-toggle` | Checkbox/toggle compacto para columnas de **tilde** en tablas (`.tabla-gestion-compacta`): cuadrado (mismo alto/ancho) con tamaño derivado de `--tabla-body-row-min-height`, sin superar el alto de fila; borde **`#0072BB`**, fondo transparente y solo ícono **Check** en **`#0072BB`** al seleccionar. Reutilizar en tablas actuales y futuras para mantener consistencia visual. |
...
- **Validación de sobrestock (otra sucursal):** al confirmar **Generar Pedido**, la primera llamada es `generarPdfEnviarPedidoAction` **sin** `confirmarSobreStock`. El servidor valida antes del snapshot: para cada línea del pedido con **`cod_tienda`**, si en la **otra** sucursal hay sobrestock (stock en `precios_tienda` vs tope resuelto con filas REPOSICIÓN en `pedidos_mercaderia`), responde `SOBRESTOCK_REQUIERE_CONFIRMACION:…` **sin persistir** (aplica con cualquier combinación de tipos URGENTE / TINTOMÉTRICO / REPOSICIÓN). El cliente llama `getSobreStockReposicionParaModalAction` y abre **`SobreStockReposicionAdvertenciaModal`**. **Confirmar Cant. Pedida** reintenta con `confirmarSobreStock: true`.
...
- **Uso**: `SyncStatusIndicator` (lista precios en áreas **Gestión Productos** / **Estadísticas Productos**; compras en **Finanzas**); `SincronizarComprobantesProveedorDuxButton` (Finanzas en contenido de página, misma piel `surface="sidebar"`).
...
- **`/finanzas/deuda-proveedores`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Saldo Proveedores**. Tabla **`TablaDeudaProveedores`**: columnas **PROVEEDOR**, **DEUDA TOTAL**, **VENCIDA**, **5 DÍAS**, **30 DÍAS**, **45 DÍAS**, **60 DÍAS** (importes `$` + `es-AR` 2 dec.); reparto del saldo por vencimiento según servicio (ver `BACKEND_GUIDELINES` §2.5a). Contador **X PROVEEDOR(ES) CON DEUDA**. Sin paginación. `PERMISOS.finanzas.acceso`.
- **`/finanzas/venc-por-fecha`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Flujo De Fondo**. Datos solo desde **hoy** (Argentina) hasta **hoy + 150 días** inclusive (`listarVencimientosEnRango` en servidor); sin query **`?mes=`** ni flechas de mes. En lugar de bloque de rango, la vista usa un **FilterBar de una sola fila** sin búsqueda por descripción: un único Select **PROVEEDOR** con opciones de proveedores que tienen cuentas a vencer en la ventana. Tabla: **`contenedor-tabla-gestion no-scroll-x`**, **`Table variant="compact"`**. La grilla lista **todos los días calendario** de la ventana (incluye días con **VENCIMIENTO DEL DÍA** en `$0,00` cuando no hay comprobantes vencidos ese día). Columnas (cinco, **20%** c/u): **FECHA** (`formatMesDiaMayusculasDesdeIsoYmd`, ej. **`ABRIL 1`**) + **`uppercase`** en celda; **VENCIMIENTO DEL DÍA** total del día (si hay proveedor seleccionado, esta columna muestra solo los vencimientos de ese proveedor por día); **VTOS ACUMULADOS** = total de saldos con vencimiento **anterior a hoy** (`sumarSaldoVencimientosConFechaVencAnteriorA`) + suma corrida del vencimiento de cada día en la ventana; **CAJA DISPONIBLE**: primera fila usa la suma de montos de `cajas_tesoreria`, y desde la segunda fila usa `max(SALDO fila anterior, 0)`; **SALDO** = **`CAJA DISPONIBLE - VTOS ACUMULADOS`** en cada fila. Estado visual de advertencia: cuando existe al menos una fila con **SALDO negativo**, se mantiene el encabezado azul original y solo las filas negativas usan fondo rojo suave (`.venc-saldo-negativo`) para distinguir deuda por encima de caja disponible. Paginación estándar por URL con **`PaginacionTabla`**: **100 filas por página** (`PAGE_SIZE`) usando query **`pagina`**. **Doble clic** → **`AppModal`** solo lectura (detalle por proveedor); tabla del modal con **`no-scrollbar`**. `PERMISOS.finanzas.acceso`.
...
- **`/finanzas/balance/gastos/catalogo`**: **`ClassicFilteredTableLayout`** título **FINANZAS**, subtítulo **Balance · Catálogo Gastos** (submódulo **Catálogo Gastos** dentro del módulo **BALANCE**, ícono `FolderTree`). Página dedicada al mantenimiento del catálogo jerárquico `fin_bal_gasto_tipo → fin_bal_gasto_rubro → fin_bal_gasto` (ver `BACKEND_GUIDELINES` §2.5e) **+ catálogo maestro de proveedores "no-mercadería"** (4ª columna autónoma). **Layout tipo Finder de 4 columnas** con `grid md:grid-cols-2 xl:grid-cols-4 gap-3` dentro del contenido del layout: **TIPOS** · **RUBROS** · **GASTOS** · **PROVEEDORES** (las 3 primeras son la cascada jerárquica; la 4ª es independiente de la selección). En viewports medianos se apilan en 2×2, en `xl` se expanden a 4 columnas. Cada columna es una `<section>` con borde y `rounded-lg`, header en `bg-muted/60` que muestra el **título** (uppercase tracking), un **subtítulo contextual** (contador de hijos o prompt `Seleccioná un tipo/rubro`) y un botón **`+ Nuevo`** (solo `editor`, `size="sm"`, `h-8`, ícono `Plus`). El body de la columna es `flex-1 overflow-y-auto` y renderiza filas `FilaCatalogo`. **Selección en cascada:** click en un tipo selecciona y puebla la columna de rubros (reset del rubro seleccionado); click en un rubro selecciona y puebla la columna de gastos. Estado seleccionado: `bg-primary/10` + `ChevronRight` en color `primary`. Filas: `truncate` para el nombre + meta auxiliar (`N rubros` / `N gastos`) en `text-[11px]`, con acciones **editar** (ícono `Pencil`) y **eliminar** (ícono `Trash2`, color destructive) ocultas por defecto y visibles en `group-hover`/`group-focus-within` (solo `editor`). Click/Enter/Space en la fila dispara la selección; los `<Button>` de acción usan `e.stopPropagation()` para no activarla. Estados vacíos centrados con `EmptyState` (columna vacía, padre no seleccionado, padre sin hijos). Todas las mutaciones se resuelven con **dos modales reutilizables**: `CrearEditarFinBalCatalogoItemModal` y `EliminarFinBalCatalogoItemModal` (ambos con prop `nivel: "tipo" | "rubro" | "gasto"`). El modal de alta/edición tiene un único campo **NOMBRE** (`Input`, `maxLength=120`, `uppercase` en `onChange`, `autoFocus`) y — para rubro y para **edición** de gasto — muestra el **nombre del padre** como contexto en solo lectura (bloque `border + bg-muted`). **Caso especial — alta de gasto (`modo === "crear" && nivel === "gasto"`, título "Crear Gasto"):** el modal **solo** expone el campo **NOMBRE**. Se ocultan el bloque contextual del RUBRO (el `rubroId` ya queda implícito en la columna seleccionada, no requiere input) y el Select **PROVEEDOR** (el `proveedorId` se persiste como `null` por defecto y se asigna después desde "Editar Gasto"). Se controla con el flag local `esAltaGastoSoloNombre = modo === "crear" && nivel === "gasto"`. **Edición de gasto (`modo === "editar" && nivel === "gasto"`, título "Editar Gasto"):** además del NOMBRE y el contexto del RUBRO, se expone un **Select "PROVEEDOR"** (shadcn `Select`, trigger con `SELECT_TRIGGER_FILTER_CLASS`, content `select-content-filtro`) con la opción explícita **SIN PROVEEDOR** al inicio + la lista de proveedores (la página del catálogo pre-carga `getProveedoresNoMercaderia()` en paralelo con la jerarquía y la propaga vía prop `proveedores`; el client deriva `{id,nombre}[]` con `useMemo`). Internamente usa un sentinel `"__sin_proveedor__"` mapeado a `null` al enviar; preselecciona `proveedorIdInicial`. **También** se expone un Select **GASTO MENSUAL** (`SI`/`NO`, controlado vía `useState<boolean>`, default `NO`, precarga `gastoMensualInicial`) que persiste en `fin_bal_gasto.gasto_mensual`. El `hasChanges` contempla cambios de proveedor **y** de `gastoMensual` para habilitar **Guardar**. Llama a las 6 actions correspondientes y al tener éxito hace `router.refresh()` vía callback `onSuccess` del client. **Columna GASTOS:** cada fila muestra como meta bajo el nombre una línea compacta unida por `·` con `[proveedor | "Sin proveedor"]` y — solo si `gastoMensual = true` — el badge textual `Mensual` (ej. `ANDRES MERINO · Mensual` / `Sin proveedor · Mensual` / `COLORIN S.A.`). El modal de eliminación es un `AppModal size="sm"` con botón destructivo **Sí, Eliminar**; el backend fuerza `onDelete: Restrict` por lo que si el nodo tiene hijos el toast devuelve el mensaje mapeado ("No se puede eliminar el tipo porque tiene rubros asociados.", etc.). **Columna PROVEEDORES** (4ª): autónoma — **no** depende de la selección de tipo/rubro/gasto. **Solo lista proveedores con `proveedor_mercaderia = false`** (catálogo maestro de proveedores "no mercadería" — gastos operativos, servicios, impuestos, etc.); los proveedores de mercadería viven exclusivamente en `/gestion-productos/proveedores/lista`. Header con botón `+ Nuevo` (solo `editor`); filas ordenadas alfabéticamente (provee el servicio `getProveedoresNoMercaderia()`) con `meta` = `PREFIJO` (como la lista nunca contiene mercadería, no se imprime el sufijo `· Mercadería`). **Click en una fila** (o `+ Nuevo`) abre el mismo **`ProveedorModal`** usado en `/gestion-productos/proveedores/lista` (alta / edición, incluye botón Eliminar interno); por eso la fila **no** renderiza las micro-acciones de editar/eliminar al hover (a diferencia de las 3 primeras columnas), para evitar redundancia. El Server Component pasa `proveedores: ProveedorListItem[]` ya filtrado desde `getProveedoresNoMercaderia()` — ese payload alimenta **ambos** usos: la columna PROVEEDORES y el Select "PROVEEDOR" del modal de gasto (por lo que el select de gasto solo ofrece proveedores no-mercadería, que es el dominio del módulo). Al guardar/eliminar el modal llama `router.refresh()` para repintar tanto la columna de proveedores como la meta "proveedor" de cada gasto (que lee `fin_bal_gasto.proveedor_id`). **Importante**: si en alta/edición el usuario marca `PROVEEDOR MERCADERÍA = SI`, ese proveedor saldrá del listado (quedará visible en `/gestion-productos/proveedores/lista`). **Permiso:** `PERMISOS.finanzas.acceso` (rol `simple` ve la jerarquía + lista de proveedores en modo lectura, sin botones de mutación).
- **`/finanzas/balance/gastos`**: **`ClassicFilteredTableLayout`** título **Balance**, subtítulo **Gastos** (submódulo **Gastos** dentro del módulo **BALANCE**). En **`actions`** del **`ClassicPageHeader`** (solo `editor`, `h-10 px-4 gap-2`, agrupados en `flex flex-wrap items-center gap-2`) hay un botón **Nuevo Gasto** (primary, ícono `Plus`) que registra un **movimiento** en `movimientos_finanzas`. Abre **`NuevoGastoModal`** (`AppModal`, título **Crear Gasto**, `sm:max-w-xl`) con **TIPO GASTO** (`EFECTIVO | BANCO | CHEQUE`), **NOMBRE** (persistido en MAYÚSCULAS), **SUCURSAL** (FK a `Sucursal`; incluye la opción maestra **CORPORATIVO** para gastos sin sucursal física; ver `BACKEND_GUIDELINES` §2.5b) y **MONTO** (`Input type="number" step="0.01"`, dos decimales); guarda con `crearMovimientoFinanzasAction` y `router.refresh()`.
...
*Última actualización (2026-04-18): **baja** del Select **REPITE MONTO** en el modal de **Editar Gasto** (`CrearEditarFinBalCatalogoItemModal`) por baja de la columna `fin_bal_gasto.repite_monto` en backend (ver `BACKEND_GUIDELINES` §2.5e y footer). Se elimina el estado `repiteMonto`, la prop `repiteMontoInicial` y el badge `Repite monto` de la meta en la columna **GASTOS**; **se conserva** el Select `GASTO MENSUAL` y su badge `Mensual`. La meta queda: `[proveedor|Sin proveedor]` + (opcional) `· Mensual`. Histórico: el modal **Crear Gasto** (`CrearEditarFinBalCatalogoItemModal` con `modo === "crear" && nivel === "gasto"`) pasa a mostrar **solo el campo NOMBRE**. Se ocultan el bloque contextual del RUBRO (ya implícito por la columna seleccionada) y el Select PROVEEDOR (el `proveedorId` se persiste como `null` y se asigna desde "Editar Gasto"). Se introduce el flag local `esAltaGastoSoloNombre` para gobernar esta variante. El modo **Editar Gasto** conserva la UI completa (NOMBRE + contexto del RUBRO + Select PROVEEDOR con opción "SIN PROVEEDOR"); el submit del alta llama `crearFinBalGastoAction({ nombre, rubroId, proveedorId: null })`. Histórico: se **restaura la columna GASTOS** en `/finanzas/balance/gastos/catalogo` tras una iteración previa que la había removido. El layout vuelve a ser el Finder de **4 columnas** `TIPOS → RUBROS → GASTOS · PROVEEDORES` (`grid md:grid-cols-2 xl:grid-cols-4`); rubros vuelven a ser seleccionables (meta `N gastos`) y el modal reutilizable abre con `nivel === "gasto"` cuando se opera sobre esa columna, habilitando nuevamente el Select "PROVEEDOR" con la opción "SIN PROVEEDOR". **Se conserva** el filtro `proveedor_mercaderia = false` aplicado en la iteración anterior: la página sigue consumiendo `getProveedoresNoMercaderia()` (no `getProveedores()`), por lo que tanto la columna PROVEEDORES como el Select "PROVEEDOR" del modal de gasto operan únicamente sobre proveedores no-mercadería, que es el dominio del módulo. El `meta` de la columna PROVEEDORES permanece simplificado a `PREFIJO` (sin sufijo `· Mercadería`, que por filtro nunca aplicaría). El grid pasa a `md:grid-cols-2 xl:grid-cols-3`, los rubros dejan de ser seleccionables (no hay columna siguiente que poblar) y la meta `N gastos` ya no se muestra. El cliente deja de derivar `proveedoresParaSelect` y no instancia el modal con `nivel === "gasto"`; el modelo `fin_bal_gasto` y las 3 actions/services asociadas permanecen intactas en backend (compatibles con futuros consumidores). Histórico 2026-04-18: la columna PROVEEDORES se añadió como catálogo maestro autónomo (no depende de selección) reutilizando **`ProveedorModal`** con Select SI/NO de `proveedor_mercaderia`; alta del campo **PROVEEDOR MERCADERÍA** (Select SI/NO) en el modal "Nuevo/Editar Proveedor" (`ProveedorForm.tsx`), default `SI` en alta y precarga del valor persistido en edición con hint bajo el Select. Alta original de la página `/finanzas/balance/gastos/catalogo` (submódulo **Catálogo Gastos** bajo **BALANCE**) con layout tipo Finder sobre `ClassicFilteredTableLayout` + `contentWidth="full"`. Dos modales reutilizables por `nivel`: `CrearEditarFinBalCatalogoItemModal` (alta/edición con campo `NOMBRE` normalizado a MAYÚSCULAS + contexto del padre en solo lectura) y `EliminarFinBalCatalogoItemModal` (confirmación destructiva que respeta `onDelete: Restrict` del backend).*
