...
7. **Nuevo modal con tabla**  
   ...
   - **`PedidoHistoriaDetalleModal` — cabecera del cuerpo:** grid `grid-cols-1 sm:grid-cols-[9fr_62.5fr_10.5fr_10.5fr_15fr]` (mismas proporciones que las columnas de la tabla); filas 1–4 unificadas (`sm:col-span-4`) con proveedor/sucursal y fecha generación + estado (centrado); quinta columna: **FECHA RECEPCIÓN** + `<Input type="date">` a **ancho completo** de esa columna, `px-[var(--tabla-body-cell-padding-x)]` como las celdas, `sm:shadow-[inset_1px_0_0_#0072bb]` alineado con `tabla-bloque-secundario-cell-divider` de **ACCIONES**. Estado local `fechaRecepcion` (`YYYY-MM-DD`) reseteado al abrir; deshabilitado si `locked` o `loading`. Calendario nativo del navegador. La fecha **no** se persiste en backend hasta que exista campo y acción acordados.
