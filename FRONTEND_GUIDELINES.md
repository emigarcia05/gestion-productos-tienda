...
| `.lista-precios-sublinea-grid`, `.lista-precios-sublinea-celda`, `.lista-precios-sublinea-divisor` | Modo **Desc. en fila** (`ListaPreciosTablaConFiltros`): grilla de **7 columnas fijas** (marca + 5 descuentos + transp.) con divisores verticales; celdas centradas y vacías si no hay marca; misma plantilla en todas las filas para alinear columnas. |
...
- **Props**
  ...
  - **`progreso`**: `{ mensaje, detalle? }` — reemplaza el botón por indicador en el **mismo slot** (`min-h-[3.5rem]`, `bg-accent2`); línea 2 = **X de Y** o texto. Usado para sync lista y compras (`SyncStatusIndicator`). **`onProgresoDoubleClick`** para cancelar.
...
- **Jerarquía canónica de URLs (2026-03):** para el área **Gestión Productos** usar siempre prefijo **`/gestion-productos`** con estructura **área/módulo/submódulo**:
  - Proveedores: `/gestion-productos/proveedores`, `/gestion-productos/proveedores/lista-precios`, `/gestion-productos/proveedores/lista-precios/reglas-descuentos`, `/gestion-productos/proveedores/sugeridos`, `/gestion-productos/proveedores/comparacion-categorias` (**Comparacion**), `/gestion-productos/proveedores/comparacion-categorias/categorias` (**Categorias**), `/gestion-productos/proveedores/lista`.
...
- **Estados**
  ...
  - **Sync lista precios en curso** (polling): **`SINCRONIZANDO PROD.`** / **`GUARDANDO PROD.`** (limpieza final). El cliente encadena **`POST /api/sync-lista-precios-tienda`** mientras **`continuing: true`**. Reanuda solo al recargar si quedó `running`. Toast al finalizar; doble clic → cancelar.
...
Submódulo **Comparacion** (`ComparacionCategoriasClient.tsx`): sin `FilterBar`. Arriba, selector en cascada **solo lectura** (`ComparacionCategoriaSelector`, **4 columnas** `grid-cols-[1fr_1fr_1fr_2fr]` → **20 % / 20 % / 20 % / 40 %**: CATEGORÍA · SUBCATEGORÍA · PRESENTACIÓN · REFERENCIA COMPETENCIA; filas del Finder **sin** línea `meta` de conteos). El mantenimiento del catálogo está en **Categorias**. Al elegir una **presentación**, se cargan los productos asignados (`getProductosPorPresentacionAction` → filas de `prod_precios_provee` vía `id_presentacion`).
...
Columnas bloque principal: **TILDE 3 %** · **PROVEEDOR 7 %** · **DESCRIPCIÓN 40 %** (32 % si hay columna acciones) · **DTO. EXTRA 6 %** · **COSTO 8 %** · **VAR 7 %** (`scrollX` en tabla). **DTO. EXTRA** — input entero 0–99 con máscara `%` (`PorcentajeEnteroMaskInput`, `formatDtoExtraComparacionMask` / `parseDtoExtraComparacionMask` en `comparacionCategoriasFormat.ts`; default **0%**); componente **`CeldaDtoExtraComparacion`**; recálculo instantáneo de **COSTO**, **VAR** y márgenes vía `dtoExtraDraft`; persistencia al blur (`actualizarDtoExtraComparacionAction`, `prod_comp_dto_extra`). Bloque secundario (`tabla-bloque-secundario-*`): **MARGEN (SEGÚN PX REFERENCIA) 7 %** — `calcMargenSegunPxReferencia`; **PX MANUAL 8 %** — calculado (`calcPxManualDesdeDifPctReferencia`, máscara **`formatPxManualEnteroMask`**); **DIF PX REF MANUAL 7 %** — **input** (`CeldaDifPxRefManualComparacion`, `PorcentajeEnteroMaskInput` con `signed`; default **0%**); **MARGEN MANUAL 7 %** — calculado (`calcMargenManualDesdeDifPctReferencia`, `fmtMargenComparacionPct`); recálculo en cliente vía `difPxRefManualDraft`; persistencia al blur del input → **`margen_manual`** (`actualizarMargenManualComparacionAction`, `prod_comp_margen_manual`). Acciones **8 %**. Modales **Asignar productos** y **Elegir referencia**: **`MODAL_COMP_CATEGORIAS_BUSQUEDA_MAX_WIDTH_CLASS`** (`!max-w-[151.2rem]`); stack de filtros **`MODAL_COMP_CATEGORIAS_FILTROS_STACK_CLASS`** (select entidad + input búsqueda); tabla **`MODAL_COMP_CATEGORIAS_TABLA_COLUMN_WIDTHS_PCT`** `[5,15,65,15]`; búsqueda multi-término (AND).
...
**Prioridad en `SyncStatusIndicator` (un mensaje visible):** sync lista precios → sync compras (Finanzas). Solo uno se renderiza.

| Flujo | API | Progreso UI | Mutex |
|-------|-----|-------------|-------|
| Sync lista precios tienda | `GET`/`POST` `/api/sync-lista-precios-tienda` | Sidebar ✅ | — |
| Act. Cx. costos | Excel (import manual DUX) | Botón **Act. Cx.** en Cx Compra | independiente |
...
**Checklist PR (flujos DUX nuevos):** ¿progreso en `SyncStatusIndicator`? ¿sin banner duplicado en la página? ¿lotes de **50** y pausa **5 s** vía `duxApiBatchPolicy.ts`?
...
**LISTA PROVEEDORES** (rol **editor**): `Lista Precios`, **`Reglas Descuentos`** (`/gestion-productos/proveedores/lista-precios/reglas-descuentos`, permiso `listaPrecios.acciones.gestionarReglasDescuentos`), `Lista Proveedores`.

**ANALISIS DE PRECIOS** (rol **editor**, orden): agrupador desplegable **`Cx y Px Tienda`** (sin ruta; ícono `Layers`) → **Cx Compra** + **Px Listas**; luego **Px Competencia** (`/gestion-productos/tienda/cx-px-tienda`); agrupador **`Comp. Categorias`** (sin ruta; ícono `FolderTree`) → **Comparacion** (`/gestion-productos/proveedores/comparacion-categorias`) + **Categorias** (`/gestion-productos/proveedores/comparacion-categorias/categorias`, permiso `comparacionCategorias.editar`). Los agrupadores se abren al click y auto-expanden si la ruta activa es un hijo. **Categorias** (`ComparacionCategoriasCatalogoPageClient`): layout **Finder** de **3 columnas** (`grid-cols-3`) — **CATEGORÍA** → **SUBCATEGORÍA** → **PRESENTACIÓN** — con componentes compartidos `@/components/shared/catalogo-finder/*` (misma estética que **Balance · Catálogo Gastos**). Cascada por click; `+ Nuevo` por columna; filas con editar/eliminar al hover (`CatalogoFinderRow`). Modales `CrearEditarComparacionCategoriaModal` / `EliminarComparacionCategoriaModal`. La asignación de productos sigue en **Comparacion**. **`Cx Compra`**: grilla con **CX PROD.** + **`ActCxButton`** (Excel costos + modal opcional PDF informe aumentos). **`Px Competencia`** (`/gestion-productos/tienda/cx-px-tienda`, rewrite → `/tienda/cx-px`): listado paginado **solo** de productos con **`comparar_competencia = true`**; header **`Prod. Comparar`** + **`Gestionar Competidores`** + **`Comparar Precios Competencia`** (`SincronizarCompetenciaModal`; permiso `competenciaPrecios.editar`). **`CompetenciaSyncProgresoBanner`** sobre filtros. Filtros **`FiltrosPxListas`**: **MARCA**, **RUBRO**, **PX PROMEDIO** (DIF TIENDA vs promedio; query `filtroPxPromedio`) + búsqueda. Grilla **`TablaPxListas`**: **DESCRIPCIÓN**, **PX PROMEDIO**, **DIF TIENDA**, **ACCIONES** (detalle expandido por competidor; asociar/relevar URLs; quitar de comparación). DIF TIENDA compara `px_lista_tienda` (DUX) vs promedio competidores. **Eliminado (2026-05-28, reimplementar):** columna **DET PRECIO**, **PX LISTA**, **MARCACION**, filtros `detPrecio` / `ordenMarcacion`, **`ExportarPxButton`**, `guardarPxListaTiendaAction`. Componentes conservados para la próxima versión (no usados en grilla): `PxListaCxPxCelda`, `MarcacionPxListaCelda`. Permiso `PERMISOS.cxPxTienda.acceso`. `@/components/px-listas/`.
...
- **Exportación DUX:** botón **Act. Px.** → un Excel **por** `nombre_lista` del catálogo (`prod_tienda_listas_precios`); columnas **CODIGO** (`cod_tienda`) + **PORC UTILIDAD** (margen desde precio efectivo: edición ?? DUX); **solo ítems con diff** de PORC UTILIDAD respecto al margen calculado desde precio DUX (`margenesPorcUtilidadDifieren`, 4 decimales); si no hay filas en ninguna lista → **`ModalSinProductosExportar`**: **No hay actualizaciones para mostrar** (sin descarga); archivo **`Act. {nombre_lista} dd-mm hh-mm.xls`**; `exportarPxListasMargenAction` + `exportPxListasMargenExcelClient.ts`. Sin filtros de página (catálogo completo por lista).
...
- **Sidebar:** **ANALISIS DE PRECIOS** agrupa **`Cx y Px Tienda`** (desplegable: **Cx Compra**, **Px Listas**), **Px Competencia** y **`Comp. Categorias`** (desplegable: **Comparacion**, **Categorias**).
...
*Última actualización (2026-06-19): **Reglas descuentos lista precios (Fase 2 UI)** — ruta `/gestion-productos/proveedores/lista-precios/reglas-descuentos` (`ReglasDescuentosListaPrecioPageClient`): CRUD con `listarReglasDescuentosListaPrecioAction` / `crearReglaDescuentosListaPrecioAction` / `actualizarReglaDescuentosListaPrecioAction` / `eliminarReglaDescuentosListaPrecioAction`; catálogos vía `listarCatalogosReglasDescuentosAction`; formulario `CrearEditarReglaDescuentoListaPrecioModal` (`PorcentajeCentInput`, selects proveedor/marca/rubro con comodín **TODOS**); permiso `listaPrecios.acciones.gestionarReglasDescuentos` + `esEditor()`. Enlace en toolbar de **Lista Precios** y submódulo sidebar **Reglas Descuentos**. Descuentos en grilla (`ListaPreciosTablaConFiltros`, `TablaProductosFiltrada`) **solo lectura** (motor § BACKEND_GUIDELINES 1.8d); `EdicionMasivaListaPreciosModal` sin campos `dto_*` / `cx_transporte`. Labels UI: `@/lib/descuentosListaPrecioReglasUi`.*
