...
- **`/finanzas/balance/gastos`**: **`ClassicFilteredTableLayout`** `contentWidth="full"`, título **Balance**, subtítulo **Gastos**. **Filtros** (`FilterBar` + `FilterRowSelection`): dos Select obligatorios **Año** (2000–2100) y **Mes** (ENERO–DICIEMBRE), siempre con valor (default: calendario Argentina vía servidor; navega `?mes=&anio=` + `router.replace`/`refresh`); Selects **RUBRO**, **GASTO** (opciones acotadas si hay rubro), **SUCURSAL**, **PROVEEDOR**; **Pagado** con opción **NO** = solo filas con `montoDevengadoPendiente > 0`; contador `FILTER_COUNT_CLASS`; `LimpiarFiltrosButton`. Solo `editor`: **Cargar Datos Mes.** llama `cargarFinBalGastoMensualMesAction({ mes, anio })` del periodo mostrado. Tabla **`TablaGastos`**: mismas columnas de datos + columna final **Acc.** (`bg-muted/25`, botones `Pencil` / `Trash2`): **Editar monto** abre `EditarMontoFinBalGastoMensualModal` (Input MONTO + **Repetir Ult. Monto** vía `obtenerMontoMesAnteriorFinBalGastoMensualAction` = `monto` del mes calendario inmediato anterior para el mismo `gastoFinalId`); **Eliminar** → `EliminarFinBalGastoMensualModal` → `eliminarFinBalGastoMensualAction`. Pie **TOTAL**: `colspan` 7 con columna de acciones vacía en footer. `PERMISOS.finanzas.acceso`.
...
*Última actualización (2026-04-21): **`/finanzas/balance/gastos`** — filtros (periodo **Año** + **Mes** en Selects, rubro/gasto/sucursal/proveedor, pagado **NO** = devengado pendiente mayor a 0), columna acciones, modales editar monto / eliminar imputación, `gastoFinalId` en fila para repetir último monto; ver `BACKEND_GUIDELINES` §2.5e.*
