...
- **Área Marketing** (`/marketing/*`): SSOT rutas **`MARKETING_ROUTES`** (`src/lib/marketingRoutes.ts`). Entrada modal / `/marketing` → redirect a **`/marketing/publicaciones/calendario`**. **Calendario De Publicaciones** (`MarketingCalendarioPageClient`): header (solo `editor`, `Settings2`) — **Gestionar Redes**, **Gestionar Tipo Publicaciones**, **Gestionar Tipo Contenido**. Los tres usan **`GestionarMktCatalogoNombreModal`** (`kind: "red" | "tipo" | "contenido"`): buscador + **`+`**, lista con **Pencil** / **Trash2**, alta/edición en submodal (solo nombre). Catálogos independientes (sin relación UI entre ellos). Cuerpo: **`MktCalendarioPublicacionesGrid`** — grilla **5 semanas × 7 días** (columnas **LUN.–DOM.**); al cargar 1.ª fila = **semana actual** AR; barra: izquierda **Ir A Esta Semana** + **Este Mes**; centro **nombre del mes** (`JULIO 2026`); derecha **← →** mensuales (`variant="default"`); días del **mes en vista** con tinte `bg-primary/8`; **hoy** con anillo + badge primary. Calendario **compacto** (filas `min-h-[3.25rem]`). Arriba **Cuadro De Mando** (`MktPublicacionesCuadroMando`, sin título propio): selector de periodo (`Button` `size="sm"`, activo `default` / inactivo `outline`) **Este Mes** | **Esta Semana** | **Siguiente Semana** (calendario AR; semana lun–dom; independiente del mes navegado en la grilla) + 3 columnas **Redes** (conteo + ícono) · **Tipo** · **Contenido** (**Planificado** / **Terminado** según `contenido_creado`); filtro SSOT `filtrarPublicacionesPorPeriodoCuadroMando`. Ícono en celda: **Terminado** (`contenido_creado` SI) = relleno `bg-primary` + ícono blanco; **Planificado** (NO) = borde `primary` + fondo `card` + ícono primary. Click en un día: **sin** publicaciones → **`CrearEditarMktPublicacionModal`** (crear); **con** publicaciones → **`MktPublicacionesDiaModal`** (botón **Crear Nueva Publicación** + listado: ícono red · tipo publicación · texto · Pencil/Trash2; editar abre el mismo form en modo editar; eliminar confirma en **`EliminarMktPublicacionModal`**); form de alta/edición (`CrearEditarMktPublicacionModal`): 3 bloques con `divide-y divide-primary/25` — **Publicación** (fecha + Red / Tipo publicación / Tipo contenido) · **Contenido** (**SECCION** + **IDEA** de `mkt_publi_ideas_detalle` con `usada = false` o la vinculada en edición + **Detalle** solo lectura) · **Contenido Creado** (`ModalSiNoChoice` SI/NO); al guardar se lista en `mkt_publi` (FK `idea_detalle_id`) y la idea queda **USADA: SI**; en la celda aparecen **íconos de red** (`MktRedSocialIcon`: Instagram, Facebook, TikTok, YouTube, LinkedIn, X/Twitter, WhatsApp; fallback `Share2`). Helpers SSOT: `@/lib/mktCalendarioPublicaciones.ts`. **Ideas** (`MarketingIdeasPageClient`): Finder **2 columnas** (**SECCIONES** | **DETALLE**), botón **`+`** a la **derecha** del encabezado; cascada sección → detalles; create/edit sección: **Nombre** + **Resumen** (`idea_resumen`); create/edit detalle: **Red** y **Tipo de publicación** multi-select (checkboxes, mín. 1 cada uno; N:M en backend); **Tipo de contenido** select simple; **Usada** solo en edición (alta siempre NO); meta del listado une nombres con ` · `; acciones fila (**Eye** → `VerMktIdeaSeccionModal` / `VerMktIdeaDetalleModal` solo lectura con `divide-y divide-primary/25` entre campos; editar/eliminar) con `CrearEditarMktIdeaSeccionModal` / `CrearEditarMktIdeaDetalleModal` / `EliminarMktIdeaModal` (solo `editor`). `PERMISOS.marketing.acceso` (solo `editor`).
