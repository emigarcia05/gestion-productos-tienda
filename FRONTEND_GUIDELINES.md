...
3. **Texto en mayúscula inicial (title case)**  
   ...
   - **Sidebar**: nombre del **módulo** en MAYÚSCULAS (ej. "LISTA PROVEEDORES", "PEDIDO MERCADERÍA"). Nombre del **submódulo**: primera letra de cada palabra en mayúscula (title case) (ej. "Lista Precios", "Cx Compra", "Pedido Urgente", "Px. Vta. Sugeridos").  
...
- En la barra de filtros de **Generar Pedido** (`FiltrosEnviarPedido` y modal `GenerarPedidoToolbarButton`), el orden es `SUCURSAL` → `TIPO DE PEDIDO` → `PROVEEDOR`, con habilitación en cascada (ver ítem anterior).
...
- **COMPROBANTE en Excel (recepción):** correlativo en `prod_ped_ult_comp` **por tipo** (FACTURA vs Comprobante_Compra; ver `BACKEND_GUIDELINES` §2.8).
...
- **Jerarquía canónica de URLs (2026-03):** para el área **Gestión Productos** usar siempre prefijo **`/gestion-productos`** con estructura **área/módulo/submódulo**:
  ...
  - Análisis de Precios: `/gestion-productos/tienda/comp-proveedores` (**Cx Compra**), `/gestion-productos/tienda/cx-px-tienda` (**Px Listas**, incluye comparación competencia), `/gestion-productos/proveedores/comparacion-categorias`. `/gestion-productos/precios-competencia` redirige a **Px Listas**.
  - Procesos: `/gestion-productos/procesos` (guías de importación DUX tras exportar Excel).
  - **Cx Compra** (antes *Vinculacion Con Prov.*; `src/app/tienda/page.tsx`, URL canónica **`/gestion-productos/tienda/comp-proveedores`**): al abrir sin query de filtros carga **todos** los ítems de **`prod_precios_tienda`** paginados (`getTiendaPageData`); los filtros en URL **reducen** el listado. Cabecera **`ClassicFilteredTableLayout`** título **Cx Compra**; acciones **`ExportarCxButton`** (solo Excel **CODIGO** + **COSTO**, diferencias DUX vs **CX PROD.**) y **`ExportarResumenAumentosButton`** (solo PDF `Resumen Aumentos {dd-mm hh-mm}.pdf`, aumentos promedio por marca/rubro desde Px Listas; `exportarResumenAumentosPxAction` + `exportPxPdfClient.ts`); permiso **`PERMISOS.cxPxTienda.acceso`**; sin datos → **`ModalSinProductosExportar`**. **`FiltrosTienda`** (`modoFiltroTercero="cxCompra"`): **MARCA** → **RUBRO** → **CX COMPRA** (proveedores con ítems en **CX PROD.**; query `cxCompra`) → **PROV. VINC.** → **VINCULADO**; paginación **`PAGE_SIZE`**. Grilla **`TablaTienda`**: **COD. TIENDA**, **DESCRIPCIÓN**, **VINCULACIÓN**, **CX PROD.** (`CeldaCxProdTienda`), **ACCIONES** (expandir vínculos + vincular proveedor). Subfilas **`CxCompraVinculosDetalle`**: **VINCULACIÓN** = prefijo proveedor (solo texto); **CX PROD.** = grilla 2 col (casilla base + % variación | precio vínculo debajo del costo principal). Botón **Link** → **`SeleccionarProductoModal`** (“Vincular Nuevo Producto”). Sin **`VincularModal`**. Edición CX PROD.: `guardarCostoCxProdTiendaAction`.
...
**ANALISIS DE PRECIOS** (rol **editor**, orden): `Cx Compra` → `Px Listas` (incluye comparación competencia; ex **Px Competencia**) → `Comp. Por Cat.`. **`Cx Compra`**: grilla con **CX PROD.** + **`ExportarCxButton`**. **`Px Listas`** (`/gestion-productos/tienda/cx-px-tienda`, rewrite → `/tienda/cx-px`): listado paginado; header **`Gestionar Competidores`** + **`Comparar Precios Competencia`** (`SincronizarCompetenciaModal`, permiso `competenciaPrecios.editar`: tabla competidores + **DÍAS SIN ACT.** + **Actualizar** por fila; API `/api/sync-competencia-precios`) + **`ExportarPxButton`** (`cxPxTienda.acceso`). **`CompetenciaSyncProgresoBanner`** sobre filtros durante el scraping. Filtros **`FiltrosPxListas`** (fila de 5 desplegables + búsqueda): **MARCA**, **RUBRO**, **DET PRECIO** (**PX MANUAL** + competidores con vínculo PPC; query `detPrecio=manual` o CUID), **PX PROMEDIO** (`mayor-promedio` = DIF TIENDA &gt; 0; `menor-promedio` = DIF TIENDA &lt; 0; query `filtroPxPromedio`), **MARCACION** (orden; query `ordenMarcacion`). Filtros PX MANUAL / PX PROMEDIO se aplican en memoria tras armar filas. Grilla **`TablaPxListas`**: 7 columnas — **DESCRIPCIÓN**, **DET PRECIO**, **PX LISTA**, **MARCACION** | separador | **PX PROMEDIO** (media de todos los competidores con precio, scraping o **Px Sugerido**), **DIF TIENDA** (`%` de **PX LISTA** vs promedio; `calcularResumenPreciosPxListas`) | separador | **ACCIONES** (detalle expandido: **competidor** entre **PX LISTA** y **MARCACION** (alineado a la derecha), **precio** bajo **PX PROMEDIO**, **% vs tienda** bajo **DIF TIENDA**; fallos solo con URL y sin sugerido). Datos de competencia precalculados en servidor (`competidoresPrecioDetalle`, `vinculosCompetencia[]` serializable; no depender de `Record` en cliente). **Asociar URLs**: competidor con Px Sugerido muestra texto **Px Sugerido** (sin input URL). `/gestion-productos/precios-competencia` redirige aquí. **DESCRIPCIÓN**, **DET PRECIO** (`Select`: siempre **PX MANUAL** + competidores del ítem con **precio registrado** (`px` &gt; 0 tras sugerido/scraping); sin config: prioridad al competidor guardado, al del `px_vta_sugerido` o al primero; si el usuario eligió **PX MANUAL** en BD, se respeta), **PX LISTA** y **MARCACION** en modo **PX MANUAL**: inputs enlazados (`PxListaCxPxCelda` + **`MarcacionPxListaCelda`**; editar uno recalcula el otero en vivo; persistencia al blur). Competencia → solo lectura en ambas columnas. **MARCACION** (`calcMarcacionPxLista`: `((px/costo)/1,21 − 1)×100`; inversa `calcPxListaDesdeMarcacionPxLista`; 5 decimales; persistida en BD al guardar). **Exportar Px**: solo Excel **CODIGO** + **Importe** (marcación) si **PX LISTA** ≠ DUX. El PDF de aumentos está en **Cx Compra** (`ExportarResumenAumentosButton`). Permiso `PERMISOS.cxPxTienda.acceso`. Componentes `@/components/px-listas/` + `@/components/tienda/PxListaCxPxCelda.tsx`.
...
- Sidebar por rol: **simple** → **PEDIDO DE MERCADERÍA** + **AYUDA VENDEDOR**; **editor** → los cuatro módulos. **Cx Compra** solo **editor** (`PERMISOS.tienda.acceso`).
...
- **TablaTienda / Cx Compra** (`TablaTienda.tsx`, clase `tabla-tienda-listado`): cinco columnas **12/38/10/28/12 %** — **COD. TIENDA**, **DESCRIPCIÓN**, **VINCULACIÓN** (`-` o contador), **CX PROD.** (`CeldaCxProdTienda`), **ACCIONES** (▼/▲ detalle + **Link2** vincular). Subfilas con estilos `tabla-fila-detalle-competencia-*` (`CxCompraVinculosDetalle.tsx`). **`SeleccionarProductoModal`** solo desde ACCIONES. Utilidades en `@/lib/vinculosTiendaUi.ts`. **`ItemTiendaParaTabla.cxProd`**: `CxProdDatosFila`.
...
- **Vínculos en grilla:** **`VincularCxCompraModal`** (ícono **Link2**): **Vincular Nuevo Producto** → **`SeleccionarProductoModal`**; en el panel sin filtros, centrado bajo el aviso de filtros, botón **Producto Propio** (`Tag`). Lista de vínculos en modal padre; subfilas **`CxCompraVinculosDetalle`**. Badge **PROPIO** en **VINCULACIÓN**; filtro **VINCULADO=NO** excluye propios.
...
- **Componentes con `cn()`**: SyncButton, SyncDuxHeaderButton, UploadZone, ProveedorAlternativoRow, ImportarModal, ImportarListaPreciosModal (botones SÍ/NO y zona drag), FiltrosComparacionCategorias, SugeridosTablaConFiltros, ListaPreciosTablaConFiltros — todas las combinaciones de clase pasan por `cn()`.
...
*Última actualización (2026-05-28): **Marcación Px Listas — escala porcentual** — `prod_precios_tienda_marcacion.marcacion` pasa de factor `px/costo/1,21` a % utilidad `((px/costo)/1,21 − 1)×100` (migración `20260528260000_marcacion_pct_utilidad`; SSOT `calcMarcacionPxLista`).*
...
*Última actualización (2026-06-02): **PDF aumentos** (`generarPdfAumentosPx.ts`): segmentos **Resumen** (tabla MARCA/RUBRO/AUM. PROM., zebra por marca, línea #0072BB entre marcas, línea azul suave entre rubros) y **Detalle Por Producto** (jerarquía marca → rubro → `descripción - %`). **`ExportarResumenAumentosButton`** en Cx Compra; **`ExportarCxButton`** / **`ExportarPxButton`**: solo Excel.*
...
## 5.1 Módulo Px Competencia (UI unificada en Px Listas)

- **Pantalla canónica:** comparación en **`TablaPxListas`** (`/gestion-productos/tienda/cx-px-tienda`). **`/gestion-productos/precios-competencia`** (y legacy `/proveedores/competencia-precios`) **redirigen** a Px Listas. La grilla standalone **`CompetenciaPreciosTabla`** queda como referencia; componentes compartidos en `src/components/precios-competencia/*` (`AsociarUrlsCompetenciaModal`, `RelevamientoUltimoMensaje`, etc.).
- **Permisos en Px Listas:** acceso/edición lista → `PERMISOS.cxPxTienda.acceso`; botón **Asociar URL** → `PERMISOS.competenciaPrecios.editar`.
...
- **Tabla** (`CompetenciaPreciosTabla`): columnas **MENOR PRECIO** y **MAYOR PRECIO** muestran `global_proveedores.prefijo` del proveedor asociado al competidor (`prefijoProveedor` en `CompetenciaParaCliente`; `title` con nombre completo del competidor). Subfila **`tabla-fila-detalle-competencia`**: fondo **`var(--gris-inset)`** en todas las celdas; **alto fijo** = **`--tabla-thead-height`**; **sin** franja izquierda ni bordes en celdas huecas. Bloque de datos solo en cols 2–4 (**PRECIO TIENDA**, **PX PROMEDIO**, **DIF TIENDA**): divisor **#0072bb** **solo entre** subfilas consecutivas (`tr + tr` → `border-top` en `*-celda`). **`--cierre`**: última subfila del bloque con **`padding-bottom`** extra antes de la fila clásica siguiente. `<colgroup>` **45% + 9%×5 + 10%**.
- **Filtros** (`FiltrosCompetenciaPrecios`): fila 1 — **DIF. PROMEDIO** (`MAS CARO` = precio tienda > promedio de competidores con relevamiento **OK**; `MAS BARATO` = precio tienda < promedio); **PROV. CARO** (lista de competidores: precio tienda < precio del competidor elegido, **OK**); **PROV. BARATO** (lista de competidores: precio tienda > precio del competidor elegido, **OK**); **CONFIGURADO** (`SI` = al menos una URL de competidor cargada; `NO` = ninguna); **COMPETIDOR** (todos los competidores registrados; al elegir uno solo ítems con px relevado para ese competidor: scraping `estado = OK` con `px_competencia`, o `px_vta_sugerido` del proveedor asociado). Fila 2 — `FiltroBusquedaInput` + limpiar + contador **X PRODUCTO(S)**. Búsqueda por términos con `filtroTexto` en `competenciaPreciosList.service.ts` (cada palabra por separado, orden libre, parcial en descripción/código — igual que lista precios). Constantes UI: `@/lib/competenciaPreciosFiltros`.
- **Vínculo manual:** botón **Asociar URLs** por fila → `AsociarUrlsCompetenciaModal` (`max-w-[54rem]`, ~50 % más ancho que `lg`): solo competidores con URL (o **Px Sugerido**); buscador filtra la lista cargada; botón **+** abre `AgregarProveedorUrlCompetenciaModal` (proveedores sin URL). Grilla: basura (quitar fila) | **Proveedor** | **Ficha de Producto** | **URL**; **Guardar** persiste cambios y bajas. `RelevamientoUltimoMensaje` bajo filas editables.
...
- **Acciones (editor):** **Gestionar Competidores** + **Comparar Precios Competencia** (`SincronizarCompetenciaModal` `size="lg"`: filtro por nombre; lista con **DÍAS SIN ACT.** y **Actualizar** (`RefreshCw`) por competidor; en curso **Detener Comparación** + **`{processed} de {total}`**; `CompetenciaSyncProgresoBanner` sobre la grilla Px Listas).
...
- **Sidebar:** **ANALISIS DE PRECIOS** agrupa **Cx Compra**, **Px Listas** y **Comp. Por Cat.** (sin ítem **Px Competencia** separado).
