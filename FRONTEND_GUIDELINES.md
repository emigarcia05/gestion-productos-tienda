...
- **`/finanzas/balance/gastos`**: **`ClassicFilteredTableLayout`** título **Balance**, subtítulo **Gastos** (submódulo **Gastos** dentro del módulo **BALANCE**). Solo `editor`: botón **Cargar Datos Mes.** (`h-10 px-4 gap-2`, ícono `CalendarDays`, loading con `Loader2` + `aria-busy`) que ejecuta `cargarFinBalGastoMensualMesAction` y `router.refresh()`; toasts según filas creadas / ya cargadas / catálogo vacío.

  Tabla **`TablaGastos`**: columnas **FECHA** (devengo `dd/mm/yyyy`), **SUCURSAL**, **TIPO GASTO**, **RUBRO**, **GASTO** (`celda-destacado` + `truncate`), **PROVEEDOR** (`truncate`), **MONTO** (si es 0 → `—`; si no `$` + `fmtPrecio`), **PAGADO.** y **MTDO. DEVENG. PEND.** (importes alineados a derecha). Pie **TOTAL** con `colspan` 6 y totales de **MONTO** (o `—` si suma 0), **PAGADO.** y **MTDO. DEVENG. PEND.**. Datos: `listarImputacionesMensualesBalance` del mes calendario Argentina (`fin_bal_gasto_mensual` + joins). Sin paginación. `PERMISOS.finanzas.acceso`.
...
*Última actualización (2026-04-21): **`/finanzas/balance/gastos`** — botón **Cargar Datos Mes.** + tabla ampliada (`fin_bal_gasto_mensual`); ver `BACKEND_GUIDELINES` §2.5e.*
