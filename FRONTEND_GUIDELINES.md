...
8. **Tablas (encabezado fijo + paginación)**  
   - **Un solo diseño** para toda la app (referencia: Comp. Proveedores). Siempre usar `Table` de `@/components/ui/table`; aplica la clase `.tabla-gestion-compacta`. No usar `<table>` en crudo ni otras clases de tabla. Encabezados (`TableHead`) en **MAYÚSCULAS y negrita**. No sobrescribir padding ni altura en celdas (el diseño global manda).
  ...
  - **Botones de solo ícono en celdas de tabla (`tbody`) — prohibición y formato obligatorio:** está **prohibido** usar `<Button variant="outline" size="icon-xs">` (o `icon-sm` / `icon` con `outline`) para acciones por fila que generen apariencia **neutra tipo documento** (`border`, `bg-background`, `shadow-xs`, hover `accent`), como el antiguo **Registrar pago** (`Banknote`) en `TablaGastos`. **Obligatorio:** fondo **`#0072BB`**, **ícono** (o texto mínimo **+** / **−**) en **blanco** (`text-white`), hover **`bg-[#0072BB]/90`**. Patrón: `variant="ghost"` + `size="icon"` + **`TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS`** en `@/lib/ui-classes.ts` (**`aspect-square`**, **`!h-full`**, **`max-h-full`**, **`self-center`**, **`tabla-row-btn-filled-brand`**; **`globals.css`**: **`padding: 0.5rem`**). El grupo va en **`TABLE_ROW_CELL_ICON_ACTIONS_FLEX_CLASS`** (**`p-1.5`**, **`items-center`**, **`justify-center`**) para aire respecto del borde de la celda; el **ancho** del botón sigue al **alto** (cuadrado). En celdas **`.celda-datos`**, **`.celda-datos--accion-relleno-fila`** anula padding de celda cuando el aire lo aporta solo el wrapper (ej. **Pedido Urgente**). Si en la misma fila hay un **`Input`** entre **+**/**−**, dar al input **`self-center`**. **No** extrapolar a botones con **texto visible** de CTA en cabeceras, pies de modal o barras de herramientas: esos siguen `default` / `outline` / `secondary` del tema cuando corresponda.
...
- **Columnas** (misma jerarquía visual **unificada**, sin `tabla-bloque-*` en este modal): `OFICIAL`, `PREFIJO`, `PX. FINAL COMPRA`, `VARIAC.`, `MARGEN S/ IVA`, `DESVINC.` (ícono). Celdas con `celda-datos` / `celda-mono` / `celda-numero` / `celda-destacado` como en la grilla tienda donde aplique. Márgenes con `calcMargenSinIvaPct` en el propio modal. **Columna `OFICIAL` (solo lectura)**: indica qué fila es el proveedor oficial (coincidencia de prefijo con el ítem tienda); **no** incluye control para cambiar el oficial desde este modal — las filas no oficiales muestran `—` (`text-muted-foreground`); la fila oficial conserva `sr-only` ("Proveedor oficial actual"). **Regla de seguridad de desvinculación**: si hay al menos dos vínculos y una fila es oficial, el botón de cesto de esa fila queda deshabilitado y muestra tooltip de bloqueo; primero debe cambiarse el proveedor oficial fuera de este modal y recién luego se puede desvincular ese proveedor.
...
| `.tabla-gestion-compacta.tabla-deuda-proveedores` | Solo **Finanzas / Venc. Provee. Merc.** (`/finanzas/deuda-proveedores`): alto automático de fila + wrap en `.celda-proveedor-deuda`; primera columna **PROVEEDOR** (tbody y tfoot) **centrada** en horizontal y vertical (`text-align: center`, `vertical-align: middle`). |
...
| `.tabla-gestion-compacta.tabla-balance-gastos` | Solo **`TablaGastos`** (**Balance · Gastos**): `--tabla-body-row-min-height` **2.875rem**, `--tabla-body-cell-padding-y` **0.375rem**; **alto de fila fijo** (variable heredada del `<table>`). Botones de acción usan el patrón global cuadrado + **`TABLE_ROW_CELL_*`** con **`p-1.5`**. |
...
- **Confirmación de comprobante fiscal previo a la exportación:** antes de invocar `exportarExcelRecepcionPedidoAction` en cualquiera de los 3 disparadores del modal (**Registrar En Dux**, **Descargar Recepcion**, **Guardar Corrección**), `PedidoHistoriaDetalleModal` consulta `detalle.proveedorIva` (campo expuesto por `getPedidoHistoriaDetalle`):
  - Si `proveedorIva ∈ {SIEMPRE, NUNCA}` → la regla del enum prevalece y la Action se invoca **sin abrir modal** (la columna **TIPO COMPROBANTE** del Excel se resuelve en backend: `SIEMPRE → FACTURA`, `NUNCA → Comprobante_Compra`).
  - Si `proveedorIva === PREGUNTA` → se abre `ConfirmarComprobanteFiscalModal` (`src/components/pedidos/ConfirmarComprobanteFiscalModal.tsx`, **AppModal**, `size="sm"`, `max-w-md`, título **Confirmar Comprobante Fiscal**) con la pregunta **¿La compra genera comprobante fiscal?** y dos botones: **Si** (decisión `true` → **FACTURA** en Excel) y **No** (decisión `false` → Comprobante_Compra). El padre orquesta la promesa con un `useRef<((value: boolean | "cancelado") => void) | null>` (`decisionFiscalResolverRef`) y un `useState<boolean>` (`confirmarFiscalOpen`).
  - **Cancelar** (cierre por overlay/ESC) resuelve la promesa como `"cancelado"` y aborta el flujo entero: no se persiste la recepción, no se descarga el Excel y no se llama a `marcarPedidoHistoriaRegistradoAction`. El padre se asegura de que cualquier promesa pendiente quede limpia al reabrir el modal de detalle (`useEffect` que resuelve el resolver con `"cancelado"` y resetea `confirmarFiscalOpen = false`).
...
- **COMPROBANTE en Excel vs DUX:** una vez definido **TIPO COMPROBANTE** (**`FACTURA`** en mayúsculas cuando aplica fiscal; **`Comprobante_Compra`** con guión bajo en el caso opuesto), el servidor toma el **último número** en la API DUX sólo entre ítems cuyo `tipo_comp` coincida (**`FACTURA`** o **`COMPROBANTE_COMPRA`**). La regla del **incremento** por ordinal de recepción y el armado del Excel siguen igual; cambia sólo de qué subconjunto de compras se lee el máximo.
- **Manejo defensivo de Server Actions en recepción:** en `PedidoHistoriaDetalleModal`, cualquier llamada async crítica (`guardarRecepcionPedidoHistoriaAction`, `exportarExcelRecepcionPedidoAction`, `marcarPedidoHistoriaRegistradoAction`) debe ir envuelta en `try/catch` además del chequeo `res.ok`, para evitar que excepciones transport/runtime muestren el error genérico de Server Components en producción.
- **Cartel de diagnóstico copiable:** si ocurre error en cargar detalle, guardar recepción, exportar Excel o flujo **Registrar En Dux**, además del `toast` y del mensaje en la tabla vacía cuando aplica, se muestra debajo del resumen (**FECHA FACTURA**) un **`role="alert"`** con título **Error técnico (copiar y enviar a soporte)**, texto en `<pre>` monoespaciado (`select-all`, scroll vertical acotado), botones **Ocultar cartel** y **Copiar detalle** (`navigator.clipboard` + toast). El contenido incluye prefijo `[Recepcion Pedido · <contexto> · ISO-8601]` y el mensaje bruto para que el usuario lo pegue a soporte/desarrollo. Si el mensaje sugiere *action not found* o *digest* de Server Components, se muestra una **pista operativa** en `text-muted-foreground`.
...
- **Modales de drill-down** (detalle por rubro, gastos por rubro, líneas de gasto): última columna **Hist.** a la derecha de toda la tabla: divisor **`border-l-2 border-[#0072BB]`** (mismo azul de informe que la grilla de balance mensual), fondo **`bg-muted/35`**, cabecera **`text-muted-foreground`**; **`MONTO`** solo muestra el importe alineado a la derecha. Botón de evolución: **`ChartNoAxesColumn`**, `variant="ghost"` + `size="icon"` + **`TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS`** con overrides compactos **`!h-7 !w-7`** (fondo **#0072BB**, ícono blanco).
...
- **`/finanzas/tesoreria`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Tesorería**. Incluye bloque de filtros con tres desplegables (**CAJA**, **TITULAR**, **TIPO CAJA**), cada uno en `FiltroIndividualContainer`, y `LimpiarFiltrosButton` inline en la misma fila (`FilterBar` + `FilaFiltrosDesplegables` + `FILTER_INLINE_ACTION_SLOT_CLASS`) cuando el contenido entra en una línea. **`actions`** (solo `editor`): botón **Nueva Caja** (`h-10 px-4`, `Plus`) que abre **`NuevaCajaTesoreriaModal`** (título visible: **Crear Caja**). Modal (`AppModal`) con alta por tres campos: **NOMBRE CAJA**, **TITULAR** y **TIPO CAJA** (`DIGITAL | EFECTIVO | CHEQUE`); guarda con `crearCajaTesoreriaAction` y `router.refresh()`. **NOMBRE CAJA** y **TITULAR** se muestran en MAYÚSCULAS y se persisten en MAYÚSCULAS. **TITULAR** se elige por desplegable fijo (sin texto libre) con estos valores: `SUC. GUAYMALLEN`, `SUC. MAIPU`, `WALTER GARCIA`, `FERNANDO PANAIA`, `EMILIANO GARCIA`, `VANESA GARCIA`. Tabla **`TablaTesoreriaCajas`**: **`contenedor-tabla-gestion--pie-fijo`** (scroll solo el cuerpo; bajo el scroll, **TOTAL** y suma de **MONTO** en **`.finanzas-resumen-tarjeta`**, alineado a **Balance · Gastos**; `scrollbar-gutter: stable` en **`contenedor-tabla-gestion--pie-fijo-scroll`** vía `globals.css`). Columnas en orden: **ÚLT. ACTUALIZACIÓN** (primera; celda con **flex**: franja fija `size-9` alineada para ícono o hueco, fecha `flex-1 text-right tabular-nums`; sin divisor visible entre ambas; si &gt; **5 días** sin actualizar monto: **`TriangleAlert`** en caja **`border-2`/`bg-accent2`** con trazo blanco reforzado y `text-white`), **CAJA**, **TITULAR**, **TIPO CAJA** (`TipoCajaTesoreria`), **MONTO** (`$` + `fmtPrecio`, entero) y **ACCIONES** (solo `editor`, bloque `tabla-bloque-secundario-*`: editar/eliminar). Doble clic sobre fila (solo `editor`) abre **`ActualizarMontoCajaTesoreriaModal`** para modificar solo **MONTO** y guardar con `editarCajaTesoreriaAction`; para filas **TIPO CAJA = CHEQUE**, el doble clic abre **`ChequesCajaTesoreriaModal`** (título visible: **Detalles De Cheques**): encima de la tabla, **`ModalMicroLabel` “Vista”** + **`Select`** (`input-filtro-unificado`, `select-content-filtro`) **ACTUALES** (sin `fecha_transferencia`) / **ENTREGADOS** (historial transferidos); segundo filtro: **`ModalMicroLabel` “Tenencia”** + **`Select`** mismo estilo, **ACTUALES** (custodia `TIENDA` en servidor) / **DEPOSITADOS** (`tenencia` **DEPOSITADO** o **PROVEEDOR**); al cambiar **Vista** se resetea **Tenencia** a **ACTUALES**. Columnas de datos: **TIPO**, **TENENCIA**, **TENEDOR**; en **ACTUALES**, además **PROV. MERC.** (proveedor de mercadería opcional vía `entrega_proveedor`), **DÍAS** y **ACCIONES** (editor; ícono **Acreditar** solo si `tenencia === TIENDA`); en **ENTREGADOS**, **ENTREGA.** (fecha/hora Argentina con `formatFechaHoraCompletaArgentina`), **DESTINO.** (caja destino); modales **Registrar cheque** / **Editar cheque** / **Acreditar Cheque** con **`ModalMicroLabel` “Entrega Proveedor”** + **`Select`** **SIN PROVEEDOR** + listado desde `listarProveedoresMercaderiaParaChequeTesoreriaAction` (gate finanzas). Los cheques transferidos se conservan en BD **500 días** y luego se purgan al listar o al transferir (ver `BACKEND_GUIDELINES` §2.5c, cheques). Filas con altura fija y botones ajustados al alto de fila mediante `TABLE_ROW_CELL_ICON_ACTIONS_FLEX_CLASS` donde aplique. El ícono lápiz abre **`EditarCajaTesoreriaModal`** (edita nombre, titular y tipo; conserva monto) y el cesto abre **`EliminarCajaTesoreriaModal`** con confirmación. Sin paginación. Datos desde `listarCajasTesoreria` (servidor). `PERMISOS.finanzas.acceso`. La consulta de compras DUX queda solo en **`SyncStatusIndicator`** (slidenav, área Finanzas).
...
- **Jerarquía canónica de URLs (2026-03):** para el área **Gestión Productos** usar siempre prefijo **`/gestion-productos`** con estructura **área/módulo/submódulo**:
  ...
  - **Vinculacion Con Prov.** (`src/app/tienda/page.tsx`, URL canónica **`/gestion-productos/tienda/comp-proveedores`**): cabecera **`ClassicFilteredTableLayout`** título **Vinculacion Con Prov.**; **`FiltrosTienda`**: desplegable **VINCULADO** con **`?vinculado=no`** (ningún `prod_precios_provee` vinculado) y **`?vinculado=si`** (al menos uno); paginación conserva el parámetro. Acción **Exportar Prov. Menor Costo** (solo `editor`); grilla **`TablaTienda`** con **TILDE** (checkbox + seleccionar todos visibles), **COD. TIENDA**, **DESCRIPCIÓN**, **VINCULACIÓN** (`-` sin vínculos, número de filas vinculadas si hay). Al ejecutar export, **no modifica base de datos** y solo exporta dos Excel: **Act. Proveedor** (`CODIGO`, `CODIGO EXTERNO`, `PROVEEDOR`, `COSTO`) y **Act. Margen** (`CODIGO`, `IMPORTE`).
...
- **Instructivos post-export** (`ExportarStockInstructorModal`, `ExportarCsvInstructorModal`, `ExportarRecepcionInstructorModal`): **`gap-4`** único; sin `flex` duplicado en el contenedor del carrusel.
...
- **TablaTienda / Vinculacion Con Prov.** (`TablaTienda.tsx`, clase `tabla-tienda-listado` en `globals.css`): cuatro columnas fijas — **TILDE** (primera, **5%**): con `editor` y modo selección masiva, encabezado botón **`Check`** (`tabla-head-toggle`) + checkbox por fila; sin selección masiva, celda vacía y `sr-only` "TILDE" en cabecera. **COD. TIENDA** **12%**, **DESCRIPCIÓN** **58%**, **VINCULACIÓN** **15%**: muestra **`-`** si `_count.productos === 0`, si no el **número** de vínculos (misma cuenta que alimenta el modal de vínculos). Doble clic en fila (si permiso `vinculos`) abre **`VincularModal`**. El payload **`ItemTiendaParaTabla`** sigue incluyendo costos/prefijos/mejor proveedor para export y modal aunque no se listen en esta grilla.
- **Botón tilde en encabezado (`TablaTienda`)**: el encabezado de selección es botón (`tabla-head-toggle`) con **fondo blanco**, tilde **`#0072BB`** y trazo reforzado (`strokeWidth` alto) para lectura rápida. No debe modificar la altura fija del `th`; se ajusta con reglas en `globals.css` sobre `.tabla-gestion-compacta thead th .tabla-head-toggle`.
...
- **Sidebar — Sincronización DUX (persistente y accionable)**: `SyncStatusIndicator` permanece siempre visible en la slidenav. En reposo usa **`DuxSyncStyleButton`** con textos según área (**PROD.** vs **COMPRAS**). Fuera de **Finanzas**, el click ejecuta **`POST /api/sync-lista-precios-tienda`**. En **Finanzas**, el mismo botón ejecuta **`sincronizarComprobantesProveedorDesdeDuxAction`** (**sonner** en el cliente). Durante ejecución: **`SINCRONIZANDO PROD.`** + X de Y (lista precios) o **`SINCRONIZANDO COMPRAS`** + X de Y (compras; sucursales / total).
- **Pedido Reposición — Configuración (2026-03)**: `PUNTO REPOSIC.` admite valor `0` en el flujo completo (modal + action + servicio). La validación de `cant` configurada se mantiene en entero mínimo `1`. Desde 2026-04, el modal/Action ya no envía `idProveedor` para guardar: la configuración se persiste por `cod_tienda` y el backend resuelve automáticamente el proveedor vigente desde `prod_precios_tienda` (`cod_ext` + `proveedor`) para evitar desalineaciones cuando cambia el proveedor asociado al producto.
...
*Última actualización (2026-05-11): **`ChequesCajaTesoreriaModal`** — filtro **Tenencia** + columna **TENENCIA**; ícono **Acreditar** solo con `tenencia === TIENDA` en vista **ACTUALES**; modal **Destino Cheque** (`DestinoChequeTesoreriaModal`): ambos botones CTA **`Button`** **default**; si el cheque está **al día** (`chequePuedeAcreditarsePorFechaArgentina`) se ofrecen **Acreditar En Cuenta Propia** y **Pago Proveedor**; si está **diferido**, aviso **`CALLOUT_WARNING_CLASS`** y solo **Pago Proveedor**. **Acreditar En Cuenta Propia** → **`AcreditarChequeTesoreriaModal`** (`transferirFinTesoreriaChequeAction`). **Pago Proveedor** → **`PagoProveedorChequeTesoreriaModal`** + `marcarEntregaProveedorFinTesoreriaChequeAction`. **Detalles De Cheques**: `AppModal` `xl` + **`max-w-[calc(48rem*1.3)]`**; `colgroup` con nueve u ocho columnas según vista/rol (incluye **TENENCIA**).*
...
*Última actualización (2026-05-13): **`ChequesCajaTesoreriaModal`** — filtro **Vista** (**ACTUALES** / **ENTREGADOS**), columnas **ENTREGA.** / **DESTINO.** en entregados; ver §1 **`/finanzas/tesoreria`**.*
...
*Última actualización (2026-05-05): **`tabla-balance-gastos`** en `TablaGastos` — fila **alto fijo** **2.875rem**; mismos botones **cuadrados** (`aspect-square`) que el resto de tablas.*
