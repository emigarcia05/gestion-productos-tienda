...
- **Carrusel de procesos** (`ProcesoInstructivoCarrusel`): layout en columna flex (título, carrete, texto `h-28`, imagen `flex-1 basis-0`); carrete de pasos: reglas en § **Guías post-exportación (PROCESOS)** (3 slots fijos; flechas y pasos laterales `invisible` si no aplican); texto con scroll en altura fija; imagen centrada con `max-h-full` / `max-w-full` (sin `fill`); zonas sin imagen = contenedor `bg-transparent`; lightbox pantalla completa en `Dialog`.
