...
- **Guías post-exportación (PROCESOS):** las guías paso a paso para importar en DUX **no** se abren automáticamente al exportar Excel. Están centralizadas en **`/gestion-productos/procesos`** (`ProcesosPageClient` + `PROCESOS_INSTRUCTIVOS` en `src/lib/procesos-instructivos.ts`). Layout: grid **`25% | 75%`** (columnas). Columna izquierda: bloque superior **15%** alto con título **Módulo** y `Select` (`MODULOS_PROCESOS`; por ahora solo **Importacion Datos en Dux**); bloque inferior **85%** con listado de procesos (`grid-rows-[15%_85%]`) (`labelCorto`: **Imp. Stock**, **Imp. Compra**). Columna derecha (`ProcesoInstructivoCarrusel`, `grid-rows`): (1) **título fijo** (`tituloGuia`); (2) **carrete de pasos** — máximo 3 números (anterior · actual · siguiente) + flechas `<` `>`: flecha activa = botón relleno **#0072BB** (ícono blanco); flecha no disponible = recuadro vacío (`border` **#0072BB**, fondo transparente); paso **actual** = círculo relleno **#0072BB** (texto blanco); pasos adyacentes = fondo transparente, texto negro, borde **#0072BB**; (3) **texto del paso** en contenedor fijo `h-28` con scroll si desborda; (4) **imagen** opcional (`img` en el paso), sin fondo, `object-contain` al máximo dentro del contenedor; clic abre `Dialog` a pantalla completa. Imágenes: `importar_stock_{1..5}.png`, `importar_compra_{1..5}.png`. Sidebar: enlace directo **PROCESOS** (sin submódulos). Cada proceso se lista solo si el rol tiene permiso del módulo de origen (stock / pedidos).
