...
7. **Nuevo modal con tabla**  
   ...
   - **`PedidoHistoriaDetalleModal`:** `AppModal` `sm:max-w-[62.4rem]` (≈ +30% sobre 48rem). Tabla **5% + 55% + 10% + 15% + 15%**: columna **lista de verificación** (icono **Check** en cabecera; por fila input + OK o **Enter** → ícono check; estado solo en cliente hasta definir reglas/persistencia), **DESCRIPCIÓN**, **CANT. PEDIDA**, **CANT. RECIBIDA**, **ACCIONES**; sin **COD. TIENDA** (`title` en descripción con código si existe). (1) Resumen: hueco **5%** alineado al check; bloque proveedor **65%** (55+10); **CANT. RECIBIDA** **15%**; fecha **15%** (`GRID_CAPAS_SUP_PEDIDO_HISTORIA` = `5fr_65fr_15fr_15fr`). (2) Fila alta: hueco **5%**, **Agregar un producto** **55%**, vacío **10%** (CANT. PEDIDA), **Cant.** **15%**, **Agregar** **15%** (`GRID_TABLA_PEDIDO_HISTORIA_5COL` = `5fr_55fr_10fr_15fr_15fr`). (3) **Ítems del pedido** + tabla: `MODAL_RESUMEN_PANEL_CLASS` / `MODAL_SECTION_CARD_CLASS` = `bg-transparent`. `fechaRecepcion` sin persistencia backend hasta definir campo.
