...
*Última actualización (2026-05-05): **Gasto eventual** (`GASTO MENSUAL` = NO) — en `CrearEditarFinBalGastoFinalModal` los campos **DÍA DEVENGADO** y **PLAZO DE PAGO** quedan habilitados. Si `TIPO = MENSUAL`, ambos quedan bloqueados/visualmente vacíos; al guardar se normaliza persistencia a `diaDevengado=1` y `vencimiento=0`.*
...
*Última actualización (2026-05-05): **`CrearEditarFinBalGastoFinalModal`** (Nuevo/Editar Gasto Final) reordena campos a: **TIPO DE GASTO (MENSUAL/EVENTUAL)**, **SUCURSAL**, **PROVEEDOR**, **DÍA DEVENGADO**, **PLAZO DE PAGO**, **GENERA IVA CRÉDITO** (`Select` **SIEMPRE** / **NUNCA** / **PREGUNTA**, debajo de **PLAZO DE PAGO**; persiste en `fin_bal_gasto_final.iva`), **COMENTARIOS**. **DÍA DEVENGADO** usa `Select` 1..28 y **PLAZO DE PAGO** usa `Select` 0..30. Si **TIPO = MENSUAL**, ambos quedan deshabilitados y visualmente vacíos (persistencia técnica `1/0`). Si **TIPO = EVENTUAL**, ambos se editan normalmente.*
