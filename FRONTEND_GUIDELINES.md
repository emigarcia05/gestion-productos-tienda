...
- **Exportación DUX:** botón **Act. Px.** → un Excel **por** `nombre_lista`; columnas **CODIGO** + **PORC UTILIDAD** (margen % desde staging, texto es-AR con coma: `106,00`); al exportar se **eliminan** las filas exportadas (cierra la act. pendiente); si no hay filas → **`ModalSinProductosExportar`**; archivo **`Act. {nombre_lista} dd-mm hh-mm.xls`**; `exportarPxListasMargenAction` + `router.refresh()` en cliente.
