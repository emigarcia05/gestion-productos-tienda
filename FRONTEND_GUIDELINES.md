...
- **Columnas** (misma jerarquía visual **unificada**, sin `tabla-bloque-*` en este modal): `OFICIAL`, **`COSTO CX`**, `PREFIJO`, `PX. FINAL COMPRA`, `VARIAC.`, `MARGEN S/ IVA`, `DESVINC.` (ícono). Celdas con `celda-datos` / `celda-mono` / `celda-numero` / `celda-destacado` como en la grilla tienda donde aplique. Márgenes con `calcMargenSinIvaPct` en el propio modal (sigue usando `costo_compra` DUX del ítem, no el costo Cx/Px). **Columna `OFICIAL` (solo lectura)**: proveedor oficial DUX (prefijo vs texto tienda); sin control para cambiarlo aquí. **Columna `COSTO CX`**: fila activa para **Cx y Px Tienda** (`cod_ext_costo_lista`); tilde si coincide con `codExtCostoLista` de `getVinculos`; en rol editor, botón **Usar** en filas no activas → `establecerCostoListaTiendaAction`. Distinto de OFICIAL: un ítem puede tener oficial DUX y otro vínculo como costo Cx/Px. **Regla de seguridad de desvinculación**: si hay al menos dos vínculos y una fila es oficial, el botón de cesto de esa fila queda deshabilitado y muestra tooltip de bloqueo; primero debe cambiarse el proveedor oficial fuera de este modal y recién luego se puede desvincular ese proveedor.
...
- **Navegación lateral por área activa** (`Sidebar.tsx`): en **Gestión Productos** el orden de módulos es **PEDIDO DE MERCADERÍA** (simple) → **AYUDA VENDEDOR** (simple) → **LISTA PROVEEDORES** (editor) → **ANALISIS DE PRECIOS** (editor). Ya no existe el módulo **LISTA TIENDA** ni **Control Aumentos**. Con área **Finanzas** se muestran, en este orden, dos bloques: primero **`BALANCE`** (`Scale`) con submódulos **Gastos** → `/finanzas/balance/gastos` (`Receipt`) y **Catálogo Gastos** → `/finanzas/balance/gastos/catalogo` (`FolderTree`); luego **`FINANZAS`** (`Landmark`) con **Tesorería** → `/finanzas/tesoreria` (`Banknote`), **Flujo De Fondo** → `/finanzas/venc-por-fecha` (`CalendarDays`), **Venc. Provee. Merc.** → `/finanzas/deuda-proveedores` (`Wallet`), **Venc. Provee. Gastos** → `/finanzas/vencimientos-gastos` (`CalendarClock`) y **Control Comprobantes** → `/finanzas/control-comprobantes` (`FileSearch`) (todos `PERMISOS.finanzas.acceso`). `getOpenModule` abre **BALANCE** para rutas `/finanzas/balance/*` y **FINANZAS** para el resto de `/finanzas/*`. En **Estadísticas Productos** no hay módulos en sidebar todavía (mensaje vacío).
...
- **Jerarquía canónica de URLs (2026-03):** para el área **Gestión Productos** usar siempre prefijo **`/gestion-productos`** con estructura **área/módulo/submódulo**:
  ...
  - Análisis de Precios: `/gestion-productos/tienda/comp-proveedores`, `/gestion-productos/precios-competencia`, `/gestion-productos/proveedores/comparacion-categorias`.
  - Cx y Px Tienda: `/gestion-productos/tienda/cx-px-tienda` (rewrite → `/tienda/cx-px`).
...
**ANALISIS DE PRECIOS** (rol **editor**): `Vinc. Con Prov.`, `Precios Competencia`, `Comp. Por Cat.`.

**CX Y PX TIENDA** (rol **editor**, módulo propio en sidebar): `/gestion-productos/tienda/cx-px-tienda`. **`FiltrosCxPxTienda`**: **MARCA** → **RUBRO** → **SUB-RUBRO** + búsqueda **DESCRIPCIÓN** (`q` en URL); sin filtros carga todo `prod_precios_tienda` paginado. Grilla **`TablaCxPxTienda`**: **COD. TIENDA**, **DESCRIPCIÓN**, **MARCA**, **PROVEEDOR**, **CX. COMPRA**, **PX. LISTA**. **CX. COMPRA** = `px_compra_final_sin_iva` de la fila lista elegida (`cod_ext_costo_lista`); **PROVEEDOR** = prefijo (o nombre) de esa fila — no el texto DUX de tienda. **PX. LISTA** = `px_lista_tienda` (DUX). Sin FK: fallback en servidor (único vínculo o match DUX); elección explícita en **Vinc. Con Prov.** → modal vínculos, columna **COSTO CX** (ver § modal vínculos y `BACKEND_GUIDELINES` §1.10b).
...
- **Tabla** (`CompetenciaPreciosTabla`): subfila **`tabla-fila-detalle-competencia`**: fondo **`var(--gris-inset)`** en todas las celdas; **alto fijo** = **`--tabla-thead-height`**; **sin** franja izquierda ni bordes en celdas huecas. Bloque de datos solo en cols 2–4 (**PRECIO TIENDA**, **PX PROMEDIO**, **DIF TIENDA**): divisor **#0072bb** **solo entre** subfilas consecutivas (`tr + tr` → `border-top` en `*-celda`). **`--cierre`**: última subfila del bloque con **`padding-bottom`** extra antes de la fila clásica siguiente. `<colgroup>` **45% + 9%×5 + 10%**.
- **Filtros** (`FiltrosCompetenciaPrecios`): fila 1 — **DIF. PROMEDIO** (`MAS CARO` = precio tienda > promedio de competidores con relevamiento **OK**; `MAS BARATO` = precio tienda < promedio); **PROV. CARO** (lista de competidores: precio tienda < precio del competidor elegido, **OK**); **PROV. BARATO** (lista de competidores: precio tienda > precio del competidor elegido, **OK**); **CONFIGURADO** (`SI` = al menos una URL de competidor cargada; `NO` = ninguna). Fila 2 — `FiltroBusquedaInput` + limpiar + contador **X PRODUCTO(S)**. Búsqueda por términos con `filtroTexto` en `competenciaPreciosList.service.ts` (cada palabra por separado, orden libre, parcial en descripción/código — igual que lista precios). Constantes UI: `@/lib/competenciaPreciosFiltros`.
- **Vínculo manual:** botón **Asociar URLs** por fila → `AsociarUrlsCompetenciaModal` (todas las URLs del producto; **Guardar** solo persiste competidores con cambios; `RelevamientoUltimoMensaje` bajo cada competidor: último intento, fecha y mensaje copiable desde `error_mensaje` o guía `SIN_PRECIO`). Guía en modal: fallo en un producto → URL; fallos masivos del mismo competidor → **Configuracion Competidor**. Detalle expandido: precios OK + filas `ERROR`/`SIN_PRECIO` con el mismo componente.
- **Acciones (editor):** **Gestionar Competidores** (`GestionCompetidoresModal`: buscador por nombre + botón **+** → `AltaCompetidorModal`; lista con editar/eliminar; editar abre `ConfiguracionCompetidorModal` — título **Configuracion Competidor - {nombre}**, ahí nombre, sitio; en `CompetenciaExtraccionReglasEditor`: **Selector del precio** (placeholder con instrucción Copiar selector) y **Regex personalizado** siempre visible (placeholder **Configurar con IA**)) y **Comparar Precios Competencia** (`SincronizarCompetenciaModal`: **Select COMPETIDOR** con opción **TODOS** (compara todos los competidores con URL cargada, en secuencia, progreso global) + pie **Cancelar** / **Comparar Precios**; en curso **Detener Comparación** y cartel en el cuerpo del modal **`{processed} de {total}`** (`useCompetenciaSyncStatusPoll`); progreso detallado también en `CompetenciaSyncProgresoBanner` sobre la grilla; sync todas las URL cargadas del competidor elegido o de todos).
...
- **Sidebar:** **Precios Competencia** es submódulo de **ANALISIS DE PRECIOS** (junto con **Vinc. Con Prov.**).
