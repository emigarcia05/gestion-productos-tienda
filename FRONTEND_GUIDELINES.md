...
8. **Tablas (encabezado fijo + paginación)**  
   ...
   - **Pie de totales fijo** (patrón `contenedor-tabla-gestion--pie-fijo`, dos `<table>`): `scrollbar-gutter: stable` en scroll y pie (`globals.css`). Para alineación exacta con la barra vertical u otros anchos útiles distintos, usar **`usePieFijoColumnWidthsSync`** (`src/lib/hooks/usePieFijoColumnWidthsSync.ts`): ref en `.contenedor-tabla-gestion--pie-fijo-scroll`, ref en la `<table>` del pie, mismo número de `<col>` que de `th`; pasar firma de columnas si cambia el layout (ej. **ACCIONES**). Referencia: **`TablaTesoreriaCajas`**. **Balance · Gastos (`TablaGastos`):** sin tabla de pie; totales **MONTO / PAGADO / DEVENGADO** en tarjetas compactas bajo el scroll; con **ACCIONES**, panel secundario + **`useMirrorScrollTop`** (`src/lib/hooks/useMirrorScrollTop.ts`).
...
| `.tabla-gestion-compacta.tabla-deuda-proveedores` | Solo **Finanzas / Vencimientos Proveedores** (`/finanzas/deuda-proveedores`): habilita alto automático de fila + wrap en `.celda-proveedor-deuda` para textos largos. |
| `.tabla-gestion-compacta.tabla-flujo-de-fondo` | Solo **Finanzas / Flujo De Fondo** (`/finanzas/venc-por-fecha`): primera columna (**FECHA** / **PROVEEDOR** en modal) con cabecera y celdas alineadas a la izquierda vía `thead th:first-child` + `tbody td:first-child.celda-datos`. |
...
- **`/finanzas/deuda-proveedores`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Vencimientos Proveedores** (nombre visible; la URL se mantiene por compatibilidad). Tabla **`TablaDeudaProveedores`**: columnas **PROVEEDOR**, **DEUDA TOTAL**, **VENCIDA**, **5 DÍAS**, **30 DÍAS**, **45 DÍAS**, **60 DÍAS** (importes `$` + `es-AR` 2 dec.); reparto del saldo por vencimiento según servicio (ver `BACKEND_GUIDELINES` §2.5a). Contador **X PROVEEDOR(ES) CON DEUDA**. Sin paginación. `PERMISOS.finanzas.acceso`.
- **`/finanzas/vencimientos-gastos`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Vencimientos Gastos**; contenido placeholder hasta definir la tabla o integración con balance de gastos. `PERMISOS.finanzas.acceso`.
- **`/finanzas/venc-por-fecha`**: página servidor + cliente **`FinanzasVencPorFechaPageClient`** (`src/components/finanzas/FinanzasVencPorFechaPageClient.tsx`). **`ClassicFilteredTableLayout`** `contentWidth="full"` (ancho completo como **Balance · Gastos**), título **Finanzas**, subtítulo **Flujo De Fondo**; los filtros van en la prop **`filters`** (mismo patrón que **Tesorería** / **Control Comprobantes**): **FilterBar** una fila con Select **PROVEEDOR** + **`LimpiarFiltrosButton`** en **`FILTER_INLINE_ACTION_SLOT_CLASS`**. Sobre la tarjeta de tabla: línea meta como **Vencimientos Proveedores** — contador **`FILTER_COUNT_CLASS`** (**X DÍA(S) EN ESTA PÁGINA**), si hay paginación **`· Pág. n / m`**, y texto secundario **Ventana: Y días (hoy + 150)**. Datos solo desde **hoy** (Argentina) hasta **hoy + 150 días** inclusive (`listarVencimientosEnRango` en servidor); sin query **`?mes=`** ni flechas de mes. **Tabla principal** (mismo cascarón que **`TablaDeudaProveedores`** / **`TablaControlComprobantes`**): tarjeta **`contenedor-tabla-gestion flex min-h-0 flex-1 flex-col overflow-hidden rounded-md border border-border bg-card`** → hijo scroll **`flex-1 min-h-0 min-w-0 overflow-x-auto overflow-y-auto`** → **`Table variant="compact"`** **`scrollX={false}`** **`table-fixed`** + clase **`tabla-flujo-de-fondo`** (`globals.css`: primera columna cabecera/celdas **text-align left**) + **`<colgroup>`** en **20%** × cinco columnas; celdas numéricas **`TH_NUM` / `TD_NUM`**. **FECHA** con **`celda-destacado`** (columna guía); **SALDO** con **`celda-destacado`** (métrica principal). La grilla lista **todos los días calendario** de la ventana (incluye días con **VENCIMIENTO DEL DÍA** en `$0,00` cuando no hay comprobantes vencidos ese día). Columnas: **FECHA** (`formatMesDiaMayusculasDesdeIsoYmd`, **`uppercase`** en celda); **VENCIMIENTO DEL DÍA** (si hay proveedor seleccionado, solo ese proveedor por día); **VTOS ACUMULADOS** = total de saldos con vencimiento **anterior a hoy** (`sumarSaldoVencimientosConFechaVencAnteriorA`) + suma corrida del día; **CAJA DISPONIBLE**: primera fila suma `fin_tesoreria_cajas`, desde la segunda `max(SALDO fila anterior, 0)`; **SALDO** = **`CAJA DISPONIBLE - VTOS ACUMULADOS`**. Montos formateados con **`fmtMontoAr`** (locale **es-AR**, 2 dec.). Advertencia: si hay **SALDO** negativo en alguna fila, **`tabla-venc-por-fecha-alerta`** en **`<Table>`**; filas negativas **`venc-saldo-negativo`** + cebra (`globals.css`). Paginación **`PaginacionTabla`** (**100** filas, query **`pagina`**). **Doble clic** en fila → **`AppModal`** detalle por proveedor; tabla del modal con **`tabla-flujo-de-fondo`**, **PROVEEDOR** con **`celda-destacado`**, mismo patrón tarjeta + scroll y **`no-scrollbar`** en el scrollport. `PERMISOS.finanzas.acceso`.
...
- **`/finanzas/balance/gastos`**: **`ClassicFilteredTableLayout`** `contentWidth="full"`, título **Balance**, subtítulo **Gastos**. **Filtros** (`FilterBar` `filtros-contenedor-tienda bg-card`): **fila 1** `FilterRowSelection` + `FilaFiltrosDesplegables` — **RUBRO**, **GASTO** (opciones acotadas si hay rubro), **SUCURSAL**, **PROVEEDOR**, **PAGADO** (opción **PENDIENTE** = solo filas con `montoDevengadoPendiente > 0`); patrón global de Select (`input-filtro-unificado`, `SelectContent` `position="popper"` `side="bottom"` `align="start"` `select-content-filtro`, primera opción con valor `none` muestra el **nombre de la dimensión** en el trigger, ej. **RUBRO**, **GASTO**, …, no la palabra «TODOS»). **Fila 2** — misma retícula de cinco columnas: **Año**, **Mes** y, en `col-span-3` (`FILTER_INLINE_ACTION_SLOT_CLASS`), contador **`FILTER_COUNT_CLASS`** (**X GASTO(S)**) + **`LimpiarFiltrosButton`**. Opciones de periodo **solo** desde `listarPeriodosConImputacionesEnDb` (prop `periodosOpciones`); el mes listado es el subconjunto de meses que existen en BD para el año elegido. Si la URL pide un periodo sin filas y hay otros periodos en BD, el servidor **redirige** al más reciente. Solo `editor`: **Cargar Datos Mes.** llama `cargarFinBalGastoMensualMesAction({ mes, anio })`. Tabla **`TablaGastos`**: **FECHA**, **SUCURSAL**, **TIPO GASTO**, **RUBRO** (8% c/u), **GASTO** y **PROVEEDOR** (15% c/u), **MONTO**, **PAGADO**, **DEVENGADO** (8% c/u; muestra **pendiente de pago sobre el devengado**: acumulado proporcional hasta hoy − **PAGADO**, mínimo 0); con `editor`, **ACCIONES** (14%, `bg-muted/25`, `Pencil` / Pagar / `Trash2`). **Columna ACCIONES** en **`editor`**: tabla aparte al 14% con borde **`#0072bb`**; **`useMirrorScrollTop`** sincroniza el scroll vertical del panel **ACCIONES** con el bloque **FECHA…DEVENGADO**, de modo que solo este último muestra barras vertical/horizontal y la columna de acciones permanece estable. Sin columna **MONTO VENCIDO**. Sin **ACCIONES** (rol `simple`), el `colgroup` aplica la misma proporción de las 9 columnas de datos escalada a 100%. **Editar monto** → `EditarMontoFinBalGastoMensualModal` + **Repetir Ult. Monto**; **Eliminar** → `EliminarFinBalGastoMensualModal`. Debajo de la tabla, **tarjetas** compactas (`border-border`, `bg-card`, tipografía reducida) con totales **MONTO** / **PAGADO** / **DEVENGADO** para el listado visible (`aria-live="polite"`); sin fila de pie en la tabla. `PERMISOS.finanzas.acceso`.
...
- **Altura de filas en tablas**: **`thead th`**: **`min-height`** vía **`--tabla-thead-height`** (referencia ~2 líneas; el encabezado puede crecer si el texto lo requiere); **`--tabla-body-row-min-height: 2rem`**; **`TableCell`**: **`text-xs` `leading-tight` `align-middle`**; **`TableRow`**: **`transition-[background-color]`** (sin afectar layout). **`globals.css`**: **`tbody td`** con **`line-height: 1.25`**; **sin** zoom de texto en hover de fila. Inputs en celdas forzados a **~1.75rem** vía **`globals.css`** (las utilidades `h-6`/`h-7` en JSX quedan alineadas a ese valor).
