...
- **`/finanzas/venc-por-fecha`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Venc. por fecha**. Vista en **tabla** (no calendario), con columnas y anchos: **DÍA (10%)**, **A PAGAR (30%)**, **CAJA DISPONIBLE (30%)**, **SALDO (30%)**. `A PAGAR` se consolida por día desde `listarVencimientosEnRango` (sin discriminar proveedor). En la implementación actual, `CAJA DISPONIBLE` se muestra en `0` y `SALDO` = `CAJA DISPONIBLE - A PAGAR`. Las filas con fecha anterior a hoy (Argentina) se muestran con efecto visual de bloqueado (`bg-muted/60` + `text-muted-foreground` + `opacity-70`). **Doble click** en una fila abre modal con tabla de detalle del día (**PROVEEDOR** + **VENCIMIENTO**), agrupado por proveedor. Navegación mes anterior/siguiente por `?mes=YYYY-MM`. `PERMISOS.finanzas.acceso`.
