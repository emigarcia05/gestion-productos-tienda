...
*Última actualización (2026-05-11): **`ChequesCajaTesoreriaModal`** — solo filtro **Tenencia** (sin **Vista**); ícono **Acreditar** solo con `tenencia === TIENDA` y cheque no transferido; modal **Destino Cheque** (`DestinoChequeTesoreriaModal`): ambos botones CTA **`Button`** **default**; si el cheque está **al día** (`chequePuedeAcreditarsePorFechaArgentina`) se ofrecen **Acreditar En Cuenta Propia** y **Pago Proveedor**; si está **diferido**, aviso **`CALLOUT_WARNING_CLASS`** y solo **Pago Proveedor**. **Acreditar En Cuenta Propia** → **`AcreditarChequeTesoreriaModal`** (columna **ACCIONES**: `Button` **`size="icon"`** con **`BadgeCheck`**, **`Loader2`** al enviar; `transferirFinTesoreriaChequeAction`). **Pago Proveedor** → **`PagoProveedorChequeTesoreriaModal`** + `marcarEntregaProveedorFinTesoreriaChequeAction`. **Detalles De Cheques**: `AppModal` `xl` + **`max-w-[calc(48rem*1.3)]`**; `colgroup` siete u ocho columnas según rol.*

*Última actualización (2026-05-16): **`ChequesCajaTesoreriaModal`** — tabla **RECIBIDO**, **TIPO**, **TENEDOR**, **EMISOR**, **MONTO**, **ACREDITACION**, **DÍAS** (`diasTextoAcreditacionMenosHoyArgentina`), **ACCIONES** (**Editar** / **Acreditar** / **Borrar**); **`AltaChequeTesoreriaModal`**: título **Registrar Cheque**; **FECHA RECIBIDO** (default día calendario Argentina) inmediatamente antes de **FECHA ACREDITACIÓN**; doble clic en fechas → calendario nativo (`showPicker`); sin **Entrega Proveedor**.*
