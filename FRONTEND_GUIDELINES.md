...
- **`/finanzas/analisis-mc/margen-contribucion`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Margen Contribución**. **`actions`**: **Gestionar Pagos** (`Settings2`, botón primario `h-10 px-4`) → **`GestionarPagosFinAnaCosFinaModal`** (CRUD catálogo `fin_ana_cos_fina_pagos`; **reordenar** filas con arrastre desde ícono **`GripVertical`** — persiste `orden` y actualiza columnas de la tabla; mutaciones solo `editor`; `router.refresh()` + `key` por `id:orden` en página servidor). **Configuración** (`FilterBar`, dos filas): **fila 1** — **TERMINAL**, **TIPO DE COMPROBANTE** (`FACTURA A` | `FACTURA C` | `FACTURA X`; **C** no calcula IVA ni IIBB), **`LimpiarFiltrosButton`**; **fila 2** — **TIPO DE ANÁLISIS** (`PRODUCTO` | `PORC. UTILIDAD`) + input según modo (**ELEGIR PRODUCTO** → modal `prod_tienda` | **`PorcentajeCentInput`** margen). Si **PRODUCTO**: PX LISTA solo lectura en tabla; si **PORC. UTILIDAD**: PX LISTA fijo **$100** (solo lectura) y margen desde el input de filtros. Tabla **`TablaFinAnaMargenContribucion`**: columnas = formas de pago del catálogo (`en_margen_contribucion`). **Formato de celdas** (`fmtCeldaMontoMargenContribucion`): **PORC. UTILIDAD** → montos como `%` (ej. `10%`; base PX LISTA = 100); **PRODUCTO** → `$` + monto + `%` sobre PX LISTA entre paréntesis (ej. `$1.426 (54%)`). **DESCUENTO** siempre % firmado (− / +). **CX FINANCIERO** muestra el costo en pesos (misma regla de formato), no la tasa cruda. **Secciones de filas** (`FIN_ANA_MC_LAYOUT`): bloque precio (**PX LISTA**, **DESCUENTO** con input entero −100…100 **por forma de pago**, **PX VENTA** en negrita por columna), fila espacio, bloque costos, subtotal costos, **M.C**. Fórmulas en `@/lib/finAnaMargenContribucion.ts`.
