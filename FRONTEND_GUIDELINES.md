...
3. **Input de búsqueda en filtros (reutilización)**
  ...
  - **Tabla Pedido Urgente (columnas y anchos)**: `TILDE` **5%** (checkbox visual compacto, alineado al alto de fila: borde **`#0072BB`**, fondo transparente y check **`#0072BB`**), `OPC. COMPRA` **7%**, `PROVEEDOR` **10%**, `DESCRIPCIÓN` **50%**, `CANT. PED.` **7%**, columna cesto **7%**, `CONF. REPO.` **7%**, `CANT. REPO.` **7%** (suma 100%; columna `REG. DUX` removida). **`OPC. COMPRA`**: solo para filas tildadas; ordenar por `px_compra_final` ascendente y asignar ranking secuencial iniciando en 1 (menor costo = 1).
...
| `.tabla-check-toggle` | Checkbox/toggle compacto para columnas de **tilde** en tablas (`.tabla-gestion-compacta`): tamaño reducido (≈ `14px`) alineado a fila, borde **`#0072BB`**, fondo transparente y tilde **`#0072BB`**. Reutilizar en tablas actuales y futuras para mantener consistencia visual. |
