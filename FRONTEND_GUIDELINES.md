...
- **Lista de verificación / acciones:** el campo de la primera columna no admite tipeo; al abrir en estado **SIN RECEPCION**, la columna **CANT. RECIBIDA** inicia vacía para todas las filas y se completa de forma secuencial por acción del usuario. **OK** copia **cant. pedida** en **cant. recibida** y confirma checklist; **Editar** copia **cant. pedida** en **cant. recibida**, limpia confirmación y abre controles de edición (`-`, input, `+`); **cesto** coloca **0** y confirma checklist en UI. **Registrar En Dux** al completar cierra el modal. **Metodo Post** (`variant="outline"`, a la izquierda de **Registrar En Dux**): mismo gate (`puedeRegistrarEnDux`). Secuencia: fiscal (`PREGUNTA` si aplica) → **`ElegirPersonalRecepcionModal`** → guardar recepción local → `registrarRecepcionCompraDuxAction` con `idPersonal` (POST DUX v2). Toast con comprobante/`id_compra`; **no** descarga Excel ni marca `RECEPCIONADO` ni cierra el modal padre. Cancelar en fiscal o personal aborta sin guardar ni POST.
...
- **Confirmación de comprobante fiscal previo a la exportación:** antes de invocar `exportarExcelRecepcionPedidoAction` o `registrarRecepcionCompraDuxAction` en los disparadores del modal (**Metodo Post**, **Registrar En Dux**, **Descargar Recepcion**, **Guardar Corrección**), `PedidoHistoriaDetalleModal` consulta `detalle.proveedorIva` (campo expuesto por `getPedidoHistoriaDetalle`):
...
- **Selector de personal DUX (`ElegirPersonalRecepcionModal`):** el POST v2/compras exige `id_personal`. Catálogo en `global_personal` vía `listGlobalPersonalAction`. Componente: `src/components/pedidos/ElegirPersonalRecepcionModal.tsx` — **AppModal** `size="md"`, título **Elegir Personal**; al abrir carga la lista; tabla compacta **NOMBRE** + **ACCIONES** (botón **Check** por fila); búsqueda por nombre (`ModalMicroLabel` + `Input` MAYÚSCULAS); **Cancelar** / overlay / ESC resuelven `"cancelado"` en el padre. El modal **no** se cierra al elegir fila: el padre (`PedidoHistoriaDetalleModal`) mantiene `elegirPersonalOpen` hasta fin de `persistirRecepcionActual` + `registrarRecepcionCompraDuxAction` (`pending={guardando === "post"}`). Orquestación con `personalRecepcionResolverRef` + `pedirPersonalRecepcionSiAplica()` (mismo patrón que `ConfirmarComprobanteFiscalModal`). Hoy solo enlazado a **Metodo Post**; el hook queda listo para **Registrar En Dux** cuando se retire el Excel.
...
**ANALISIS DE PRECIOS** (rol **editor**, orden): `Cx Compra` → **Px Listas** (`/gestion-productos/tienda/px-listas`) → **Px Competencia** (`/gestion-productos/tienda/cx-px-tienda`) → `Comp. Por Cat.`. **`Cx Compra`**: grilla con **CX PROD.** + **`ExportarCxButton`** + PDF aumentos (`ExportarResumenAumentosButton`). **`Px Competencia`** (`/gestion-productos/tienda/cx-px-tienda`, rewrite → `/tienda/cx-px`): listado paginado; header **`Gestionar Competidores`** + **`Comparar Precios Competencia`** (`SincronizarCompetenciaModal`; permiso `competenciaPrecios.editar`). **`CompetenciaSyncProgresoBanner`** sobre filtros. Filtros **`FiltrosPxListas`**: **MARCA**, **RUBRO**, **PX PROMEDIO** (DIF TIENDA vs promedio; query `filtroPxPromedio`) + búsqueda. Grilla **`TablaPxListas`**: **DESCRIPCIÓN**, **PX PROMEDIO**, **DIF TIENDA**, **ACCIONES** (detalle expandido por competidor; asociar/relevar URLs). DIF TIENDA compara `px_lista_tienda` (DUX) vs promedio competidores. **Eliminado (2026-05-28, reimplementar):** columna **DET PRECIO**, **PX LISTA**, **MARCACION**, filtros `detPrecio` / `ordenMarcacion`, **`ExportarPxButton`**, `guardarPxListaTiendaAction`. Componentes conservados para la próxima versión (no usados en grilla): `PxListaCxPxCelda`, `MarcacionPxListaCelda`. Permiso `PERMISOS.cxPxTienda.acceso`. `@/components/px-listas/`.
...
- **Componentes con `cn()`**: UploadZone, ProveedorAlternativoRow, ImportarModal, ImportarListaPreciosModal (botones SÍ/NO y zona drag), FiltrosComparacionCategorias, SugeridosTablaConFiltros, ListaPreciosTablaConFiltros — todas las combinaciones de clase pasan por `cn()`.
...
- **Layout:** `area-page-shell` + `ClassicFilteredTableLayout`. **FilterBar** `filtros-contenedor-tienda bg-card` (mismo patrón que **Px Competencia** / **Cx Compra**): fila 1 `FilaFiltrosDesplegables` — **MARCA**, **RUBRO**, **SUB-RUBRO** en `FiltroIndividualContainer` + `FILTER_SELECT_WRAPPER_CLASS`, `SelectTrigger` **`input-filtro-unificado`**, `SelectContent` `select-content-filtro`; sin ítems «TODAS/TODOS» (estado vacío = `value` `undefined` + placeholder en trigger); cascada marca → limpia rubro/sub-rubro, rubro → limpia sub-rubro. Fila 2: `FilterRowSearch` `flex-1` + `FiltroBusquedaInput` + `LimpiarFiltrosButton` + `FILTER_COUNT_CLASS` (`X PRODUCTO(S)`).
- **Componentes:** `src/components/px-listas-precios/*` — `FiltrosPxListasPrecios`, `TablaPxListasPrecios`, `PxListasPreciosPageClient`.
- **Tabla:** clase `tabla-px-listas-listado tabla-px-listas-precios` (scroll horizontal si hay muchas listas). **DESCRIPCIÓN** + por cada fila de `prod_tienda_listas_precios`: subcabecera con `nombre_lista` (MAYÚSCULAS), columnas **PX.** (entero, sin decimales; `@/lib/pxListasPreciosFormat`) y **MARGEN** (4 decimales + `%`; `formatMargenPxListaInput` / `parseMargenPxListaInput`). Filas **`--tabla-body-row-min-height` (2rem)** — refuerzo en `globals.css` sobre `.tabla-px-listas-precios.tabla-gestion-compacta tbody tr/td`. Celdas numéricas: **`celda-datos celda-numero`** + `celda-px-lista-col` / `celda-marcacion-col`. **Sin** envoltorio `button`/`div` extra: **`<input>` directo en la celda** (mismo patrón global que inputs en `.tabla-gestion-compacta`: fondo transparente, borde **#0072BB**, alto **1.75rem**, texto **negro**); lectura con `readOnly` hasta foco. Clase **`px-lista-input--edicion`** = negrita si hay override en `prod_tienda_precios_edicion`. Guardado en blur → `guardarPxListaPrecioEdicionAction` (PX redondeado a entero en servicio).
- **Clases globales:** `.tabla-px-listas-precios` — `table-layout: auto`, `width: max-content`; inputs PX/MARGEN en `globals.css` bajo `.tabla-px-listas-precios tbody td.celda-px-lista-col` / `celda-marcacion-col`.
...
*Última actualización (2026-06-04): **`ElegirPersonalRecepcionModal`** — selector de `global_personal` antes de **Metodo Post**; `pedirPersonalRecepcionSiAplica()` + `idPersonal` en `registrarRecepcionCompraDuxAction`; ver § `PedidoHistoriaDetalleModal`.*
