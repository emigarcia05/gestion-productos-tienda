...
| `.tabla-fin-ana-margen-contribucion`, `.tabla-mc-col-forma`, `.tabla-mc-celda-dual`, `.tabla-mc-dual-grid`, `.tabla-mc-dual-pesos`, `.tabla-mc-dual-pct` | **Margen Contribución**: tabla **`w-max` / `max-content`**. Columna forma **12rem**. Par $/%: grid **65%\|35%** + pads compensados; bloque dual ancho fijo centrado en la celda. |
