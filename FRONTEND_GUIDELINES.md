...
7. **Nuevo modal con tabla**  
   ...
   - **`PedidoHistoriaDetalleModal`:** tabla **8% + 50% + 10% + 10% + 22%** (**COD. TIENDA** / **DESCRIPCIÓN** / **CANT. PEDIDA** / **CANT. RECIBIDA** / **ACCIONES**). **Cabecera** (proveedor + fecha): `GRID_CAPAS_SUP_PEDIDO_HISTORIA` = `sm:grid-cols-[68fr_10fr_22fr]` (68 = 8+50+10), celda vacía `hidden sm:block` bajo **CANT. RECIBIDA**, **FECHA RECEPCIÓN** con `COLUMNA_ACCIONES_EXTERNA_CLASS`. **Fila herramientas** (encima de la tabla): `GRID_TABLA_PEDIDO_HISTORIA_5COL` = `sm:grid-cols-[8fr_50fr_10fr_10fr_22fr]` (misma proporción que la tabla): primer bloque `sm:col-span-3` (celdas 11–13) = **DESCRIPCIÓN** + input buscar / producto elegido; columna 4 = **CANT RECIBIDA** + input; columna 5 = botón **Agregar** `w-full`. Controles `h-9`, `p-1.5` en bloques. Estado `fechaRecepcion` sin persistencia backend hasta definir campo.
