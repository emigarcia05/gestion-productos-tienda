...
- **Jerarquía canónica de URLs (2026-03):** para el área **Gestión Productos** usar siempre prefijo **`/gestion-productos`** con estructura **área/módulo/submódulo**:
  ...
  - **Cx Compra** (antes *Vinculacion Con Prov.*; `src/app/tienda/page.tsx`, URL canónica **`/gestion-productos/tienda/comp-proveedores`**): al abrir sin query de filtros carga **todos** los ítems de **`prod_precios_tienda`** paginados (`getTiendaPageData`); los filtros en URL **reducen** el listado. Cabecera **`ClassicFilteredTableLayout`** título **Cx Compra**; acción **`ActCxButton`** (**Act. Cx.**): `enviarCostoCxDuxAction` + polling en cliente (`consultarEstadoCostoCxDuxAction`, ≥ 5 s) donde `costo_compra` ≠ `px_compra_final_sin_iva` vía **`costo_compra_cod_ext`**; el botón muestra progreso mientras espera DUX; tras **FINALIZADO**, toast + modal **Costos Actualizados** pregunta **¿Desea descargar El Resumen de Aumentos?** — **Si** → `exportarResumenAumentosPxAction` + PDF; **No** cierra. Permiso **`PERMISOS.cxPxTienda.acceso`**; sin diff → **`ModalSinProductosExportar`**. **`FiltrosTienda`** (`modoFiltroTercero="cxCompra"`): **MARCA** → **RUBRO** → **CX COMPRA** → **PROV. VINC.** → **VINCULADO**; paginación **`PAGE_SIZE`**. Grilla **`TablaTienda`**: **COD. TIENDA**, **DESCRIPCIÓN**, **VINCULACIÓN**, **CX PROD.** (`CeldaCxProdTienda`), **ACCIONES**. Subfilas **`CxCompraVinculosDetalle`**. Edición CX PROD.: `guardarCostoCxProdTiendaAction`.
