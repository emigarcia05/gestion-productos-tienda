...
- **Acciones (editor):** **Gestionar Competidores** (nombre, sitio y lectura del precio: **Elemento precio** + **Contenedor del precio** opcional, con guía paso a paso en `GuiaCampoExtraccionCompetencia`; avanzado colapsado en `CompetenciaExtraccionReglasEditor`) y **Comparar Precios Competencia** (un competidor; solo filas con URL; **Detener Comparación** → `/api/sync-competencia-precios/cancel`).
