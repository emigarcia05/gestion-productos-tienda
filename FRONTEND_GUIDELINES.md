...
*Última actualización (2026-04-18): alta de la **4ª columna PROVEEDORES** en `/finanzas/balance/gastos/catalogo` como catálogo maestro autónomo (no depende de selección de tipo/rubro/gasto). Reutiliza **`ProveedorModal`** (el mismo de `/gestion-productos/proveedores/lista`, con Select SI/NO de `proveedor_mercaderia` incluido). El Server Component pasa `proveedores: ProveedorListItem[]` completo (vía `getProveedores()`) y el client deriva `{id,nombre}[]` para el Select del modal de gasto con `useMemo`. Grid ahora es `md:grid-cols-2 xl:grid-cols-4` (2×2 en medium, 4 columnas en xl). Click en fila de proveedor abre el modal en edición (el propio modal incluye botón Eliminar). — alta del campo **PROVEEDOR MERCADERÍA** (Select SI/NO) en el modal "Nuevo/Editar Proveedor" (`ProveedorForm.tsx`), abierto desde `/gestion-productos/proveedores/lista` y `/gestion-productos/proveedores/gestion`. Select shadcn controlado + hidden `<input name="proveedorMercaderia">` para que el valor viaje por `FormData` al action. Default en alta: **SI** (UX consistente con abrir el modal desde la pantalla de gestión de mercadería). En edición precarga el valor persistido. El helper hint debajo del Select avisa que SI implica aparecer en `/gestion-productos/proveedores/lista`. — alta del campo **PROVEEDOR** (FK opcional `fin_bal_gasto.proveedor_id` con `onDelete: SET NULL`, ver `BACKEND_GUIDELINES` §2.5e) en el modal **`CrearEditarFinBalCatalogoItemModal`** (solo visible con `nivel === "gasto"`; Select shadcn con opción inicial **SIN PROVEEDOR** + lista pre-cargada desde `getProveedores()` por el Server Component del catálogo; sentinel `"__sin_proveedor__"` → `null`). La columna **GASTOS** de `/finanzas/balance/gastos/catalogo` ahora muestra el nombre del proveedor (o "Sin proveedor") como meta bajo el nombre del gasto. — alta de la página **`/finanzas/balance/gastos/catalogo`** (submódulo **Catálogo Gastos** bajo **BALANCE**) con layout tipo Finder de 3 columnas (TIPOS → RUBROS → GASTOS) sobre `ClassicFilteredTableLayout` + `contentWidth="full"`. Dos modales reutilizables por `nivel`: `CrearEditarFinBalCatalogoItemModal` (alta/edición con campo `NOMBRE` normalizado a MAYÚSCULAS + contexto del padre en solo lectura) y `EliminarFinBalCatalogoItemModal` (confirmación destructiva que respeta `onDelete: Restrict` del backend).*
