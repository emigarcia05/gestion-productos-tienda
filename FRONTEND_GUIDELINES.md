...
- **Resumen superior (simetría total):** el panel usa compensación horizontal completa (`-mx-3 sm:-mx-4`) para anular el `px` del cuerpo del modal en ambos lados. Mantener grid `w-full px-0`, columna proveedor con `pl-0 pr-0` y `label`/`input` de **FECHA FACTURA** sin margen/padding derecho (`pr-0`, `mr-0`) para que el bloque quede borde a borde.
