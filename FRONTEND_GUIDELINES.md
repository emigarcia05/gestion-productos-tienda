...
- **Carrusel de procesos** (`ProcesoInstructivoCarrusel`): layout en columna flex (título, carrete, texto `h-28`, imagen `flex-1 basis-0`); carrete: clases `proceso-paso-nav-flecha` / `proceso-paso-nav-numero` en `globals.css` (excepción a `main button`; círculos `border-radius: 9999px`); texto con scroll en altura fija; imagen en `proceso-paso-imagen-btn` (borde **#0072BB**, imagen **90%** centrada); lightbox pantalla completa en `Dialog`.
