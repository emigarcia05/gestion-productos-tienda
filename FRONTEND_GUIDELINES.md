...
- **`/tienda/litros` — Cálculo de Lts**: selector **FORMA DE CÁLCULO** (`POR PAREDES`, `POR MÓDULO`, `PILETA`) + selector **TIPO DE PINTURA** (fuente `tipos_pintura_rendimientos`).  
  ...
  - **POR MÓDULO**: arriba, tabla de una fila con `LARGO`, `ANCHO`, `ALTO`, `INCLUYE TECHO` (referencia del módulo). Debajo, misma grilla que **POR PAREDES** (ocho columnas, mismos anchos y reglas), con **cinco filas fijas** etiquetadas `Pared 1` … `Pared 4` y `Techo`, sin botón `+` ni eliminar; columna `ACCIONES` vacía. Totales con la misma fila `TOTAL` y línea azul sobre `MTS2` / `1 MANO` / `2 MANOS`.
