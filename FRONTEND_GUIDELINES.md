...
**ANALISIS DE PRECIOS** (rol **editor**, orden): agrupador desplegable **`LISTA PROVEEDORES`** (sin ruta; ícono `Handshake`) → **Lista Precios** (toolbar: **`CotizacionUsdListaPreciosControl`** — cotización USD única; edición `listaPrecios.acciones.gestionarCotizacionUsd`), **Reglas Descuentos** (`/gestion-productos/proveedores/lista-precios/reglas-descuentos`, permiso `listaPrecios.acciones.gestionarReglasDescuentos`), **Lista Proveedores**; agrupador **`Cx y Px Tienda`** (sin ruta; ícono `Layers`) → **Cx Compra** + **Px Listas**; luego **Px Competencia** (`/gestion-productos/tienda/cx-px-tienda`); agrupador **`Comp. Categorias`** (sin ruta; ícono `FolderTree`) → **Comparacion** (`/gestion-productos/proveedores/comparacion-categorias`) + **Categorias** (`/gestion-productos/proveedores/comparacion-categorias/categorias`, permiso `comparacionCategorias.editar`). Los agrupadores se abren al click y auto-expanden si la ruta activa es un hijo. **Categorias** (`ComparacionCategoriasCatalogoPageClient`): layout **Finder** de **3 columnas** (`grid-cols-3`) — **CATEGORÍA** → **SUBCATEGORÍA** → **PRESENTACIÓN** — con componentes compartidos `@/components/shared/catalogo-finder/*` (misma estética que **Balance · Catálogo Gastos**). Cascada por click; `+ Nuevo` por columna; filas con editar/eliminar al hover (`CatalogoFinderRow`). Modales `CrearEditarComparacionCategoriaModal` / `EliminarComparacionCategoriaModal`. La asignación de productos sigue en **Comparacion**. **`Cx Compra`**: grilla con **CX PROD.** + **`ActCxButton`** (Excel costos + modal opcional PDF informe aumentos). **`Px Competencia`** (`/gestion-productos/tienda/cx-px-tienda`, rewrite → `/tienda/cx-px`): listado paginado **solo** de productos con **`comparar_competencia = true`**; header **`Prod. Comparar`** + **`Gestionar Competidores`** + **`Comparar Precios Competencia`** (`SincronizarCompetenciaModal`; permiso `competenciaPrecios.editar`). **`CompetenciaSyncProgresoBanner`** sobre filtros. Filtros **`FiltrosPxListas`**: **MARCA**, **RUBRO**, **PX PROMEDIO** (DIF TIENDA vs promedio; query `filtroPxPromedio`) + búsqueda. Grilla **`TablaPxListas`**: **DESCRIPCIÓN**, **PX PROMEDIO**, **DIF TIENDA**, **ACCIONES** (detalle expandido por competidor; asociar/relevar URLs; quitar de comparación). DIF TIENDA compara `px_lista_tienda` (DUX) vs promedio competidores. **Eliminado (2026-05-28, reimplementar):** columna **DET PRECIO**, **PX LISTA**, **MARCACION**, filtros `detPrecio` / `ordenMarcacion`, **`ExportarPxButton`**, `guardarPxListaTiendaAction`. Componentes conservados para la próxima versión (no usados en grilla): `PxListaCxPxCelda`, `MarcacionPxListaCelda`. Permiso `PERMISOS.cxPxTienda.acceso`. `@/components/px-listas/`.
...
- **Sidebar:** **ANALISIS DE PRECIOS** agrupa **`LISTA PROVEEDORES`** (desplegable: **Lista Precios**, **Reglas Descuentos**, **Lista Proveedores**), **`Cx y Px Tienda`** (desplegable: **Cx Compra**, **Px Listas**), **Px Competencia** y **`Comp. Categorias`** (desplegable: **Comparacion**, **Categorias**).
