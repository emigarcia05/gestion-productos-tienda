...
- **`/finanzas/analisis-mc/costos-financieros`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Costos Financieros** (módulo sidebar **ANALISIS M.C.**). Sin filtros desplegables: resumen **`X DE Y COMBINACIÓN(ES) HABILITADA(S)`** en el slot de filtros. Tabla **`TablaFinAnaCosFina`**: matriz **terminal × pago** (21 filas semilla: MercadoPago / Payway / Nave × débito y cuotas 1/3/6/9/12/18). Columnas **HABILITADO** (`Switch` inline, solo `editor`), **TERMINAL** y **PAGO** (solo lectura; etiquetas en `@/lib/finAnaCosFina.ts`), **DÍAS ACRED.** (`Input` entero 0–999 o vacío = `null`, guarda en `onBlur`), **ARANCEL** (solo lectura, `fmtPorcentajeTabla`), **COSTO FIN.** (`PorcentajeCentInput`, guarda en `onBlur`). Mutación única: **`actualizarFinAnaCosFinaAction`**. Rol **`simple`**: lectura sin controles editables. Sin paginación (catálogo fijo). Ver `BACKEND_GUIDELINES` §2.5h.
