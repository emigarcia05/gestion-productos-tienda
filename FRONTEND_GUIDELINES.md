...
- **Un solo diseño** para toda la app (referencia: Comp. Proveedores). Siempre usar `Table` de `@/components/ui/table`; aplica la clase `.tabla-gestion-compacta`. No usar `<table>` en crudo ni otras clases de tabla. Encabezados (`TableHead`) en MAYÚSCULAS. No sobrescribir padding ni altura en celdas (el diseño global manda).  
   - **Encabezado fijo (obligatorio)**: el encabezado de la tabla debe estar fijo y **no moverse con el scroll**. `TableHeader` (`<thead>`) usa **`sticky top-0 z-20`** y **`bg-primary`** (fondo opaco); `TableHead` (`<th>`) añade **`sticky top-0 z-20`**; el wrapper **`data-slot="table-container"`** lleva **`relative`**, **`min-h-0`**, **`overflow-y-auto`** y **`overflow-x-auto` / `overflow-x-hidden`**. **`globals.css`** (`.tabla-gestion-compacta thead th`) refuerza **`position: sticky`**, **`top: 0`** y **`z-index: 20`**. La tabla debe ir dentro de `.contenedor-tabla-gestion` con un ancestro que tenga **`overflow-y: auto`** (y **`relative`** si aplica) para que el sticky funcione respecto al scroll interno. Iconos en cabecera sobre **`bg-primary`**: **`text-primary-foreground`** (no **`text-foreground`**).  
...
- **Tabla ítems + totales:** pie **TOTAL PEDIDO** fuera del `<table>` (**`<section aria-label="Totales del pedido">`** + grid alineado a columnas); ver punto 7.
