...
- **Modales de drill-down** (detalle por rubro, gastos por rubro, líneas de gasto): última columna **Hist.** a la derecha de toda la tabla, estética secundaria (`border-l border-border`, `bg-muted/35`, cabecera `text-muted-foreground`); **`MONTO`** solo muestra el importe alineado a la derecha. Botón de evolución: **`ChartNoAxesColumn`**, `variant="outline"`, `size="icon"` (no **`TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS`** en esa columna).
