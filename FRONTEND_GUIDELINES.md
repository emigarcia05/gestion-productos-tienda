...
- **Props**
  ...
  - **`progreso`**: `{ mensaje, detalle? }` — reemplaza el botón por indicador en el **mismo slot** (`min-h-[3.5rem]`, `bg-accent2`); línea 2 = **X de Y · ~N min restantes** (string o `{ procesados, total }`). Usado en **`SyncStatusIndicator`** durante sync productos/compras. **`onProgresoDoubleClick`** para cancelar.
...
- **Progreso en curso:** el mismo slot pasa a modo **`progreso`** en **`DuxSyncStyleButton`** (`SINCRONIZANDO…` + línea 2 con **X de Y · ~N min restantes**). ETA aproximada: productos vía `remainingMinutes` del `GET /api/sync-lista-precios-tienda/status` (lotes de 50 + pausa DUX); compras vía `remainingMinutes` del status de compras (sucursales pendientes). Helpers: `formatSyncEta.ts`. Toasts al completar.
...
Columnas bloque principal: **TILDE 3 %** · **PROVEEDOR 7 %** · **DESCRIPCIÓN 40 %** (32 % si hay columna acciones) · **DTO. EXTRA 6 %** · **COSTO 8 %** · **VAR 7 %** (`scrollX` en tabla). **DTO. EXTRA** — input entero 0–99 con máscara `%` (`PorcentajeEnteroMaskInput`, `formatDtoExtraComparacionMask` / `parseDtoExtraComparacionMask` en `comparacionCategoriasFormat.ts`; default **0%**); componente **`CeldaDtoExtraComparacion`**; recálculo instantáneo de **COSTO**, **VAR** y márgenes vía `dtoExtraDraft`; persistencia al blur (`actualizarDtoExtraComparacionAction`, `prod_comp_dto_extra`). Bloque secundario (`tabla-bloque-secundario-*`): **MARG. SEG. REF. 7 %** — `calcMargenSegunPxReferencia` (2 dec. + `%`); **PX. CALC. 8 %** — calculado (`calcPxManualDesdeDifPctReferencia`, máscara **`formatPxManualEnteroMask`**); **DIF % REF. MAN. 7 %** — **input** (`CeldaDifPxRefManualComparacion`, `PorcentajeEnteroMaskInput` con `signed`; default **0%**); **MARG. CALC. 7 %** — calculado (`calcMargenManualDesdeDifPctReferencia`, `fmtMargenComparacionPct`, 2 dec.); recálculo en cliente vía `difPxRefManualDraft`; persistencia al blur del input → **`dif_px_ref_manual`** (`actualizarDifPxRefManualComparacionAction`, `prod_comp_dif_px_ref_manual`). Acciones **8 %**. Modales **Asignar productos** y **Elegir referencia**: **`MODAL_COMP_CATEGORIAS_BUSQUEDA_MAX_WIDTH_CLASS`** (`!max-w-[151.2rem]`); stack de filtros **`MODAL_COMP_CATEGORIAS_FILTROS_STACK_CLASS`** (select entidad + input búsqueda); tabla **`MODAL_COMP_CATEGORIAS_TABLA_COLUMN_WIDTHS_PCT`** `[5,15,65,15]`; búsqueda multi-término (AND).
...
- **Exportación DUX:** botón **Act. Px.** → un Excel **por** `nombre_lista` del catálogo (`prod_tienda_listas_precios`); columnas **CODIGO** (`cod_tienda`) + **PORC UTILIDAD** (margen manual guardado); **solo ítems con diff** de PORC UTILIDAD respecto al margen calculado desde precio DUX (`margenesPorcUtilidadDifieren`, 2 decimales); si no hay filas en ninguna lista → **`ModalSinProductosExportar`**: **No hay actualizaciones para mostrar** (sin descarga); archivo **`Act. {nombre_lista} dd-mm hh-mm.xls`**; `exportarPxListasMargenAction` + `exportPxListasMargenExcelClient.ts`. Sin filtros de página (catálogo completo por lista).
- **Tabla:** clase `tabla-px-listas-listado tabla-px-listas-precios` (scroll horizontal si hay muchas listas). **DESCRIPCIÓN** + por cada fila de `prod_tienda_listas_precios`: subcabecera con `nombre_lista` (MAYÚSCULAS), columnas **PX. CALC.** (entero, solo lectura) y **MARG. MAN.** (2 decimales + `%`; editable y persistido). **PX** = `Cx × (1 + Margen/100) × 1.21` (`calcPxListaDesdeMargenSinIvaPct`); recálculo instantáneo en cliente al tipear margen. Filas **`--tabla-body-row-min-height` (2rem)**. Celdas numéricas: **`celda-datos celda-numero`** + `celda-px-lista-col` / `celda-marcacion-col`. **MARG. MAN.:** `<input>` directo en celda (`readOnly` hasta foco). Clase **`px-lista-input--edicion`** = negrita si hay margen manual en `prod_tienda_margen_edicion`. Guardado en blur → `guardarPxListaMargenEdicionAction`. **Resaltado** (`.celda-px-listas-actualizar`, tinte `accent2`): **DESCRIPCIÓN** + columnas PX. CALC./MARG. MAN. de cada lista donde margen manual ≠ margen DUX.
