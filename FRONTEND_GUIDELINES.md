...
- **Carrusel de procesos** (`ProcesoInstructivoCarrusel`): layout en columna flex (título, carrete, texto `h-28`, imagen `flex-1 basis-0`); carrete de pasos con máximo 3 números visibles (números: borde **#0072BB**, fondo gris `bg-muted`; flechas con `Button` + `TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS` (#0072BB, ícono blanco); texto con scroll en altura fija; imagen sin fondo (`object-contain` máximo) y lightbox pantalla completa en `Dialog`.
