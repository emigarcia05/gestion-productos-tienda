...
- **`/finanzas/tesoreria`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Tesorería**. Encabezado del contenido con botón **Nueva Caja** (solo `editor`) que abre `NuevaCajaTesoreriaModal`. Modal (AppModal) con alta por tres campos: **NOMBRE CAJA**, **TIPO CAJA** (`Digital | Efectivo | Cheque`) y **SUCURSAL**; guarda con `crearCajaTesoreriaAction` y refresca la vista. Tabla **`TablaTesoreriaCajas`**: columnas **NOMBRE CAJA**, **SUCURSAL** (nombre de `sucursales`), **TIPO CAJA** (`TipoCajaTesoreria`), **MONTO** (`$` + `fmtPrecio`, entero), **ÚLT. ACTUALIZACIÓN** (`ult_actualizacion` con `formatFechaHoraCompletaArgentina`). Contador **X CAJA(S)**. Sin paginación. Datos desde `listarCajasTesoreria` (servidor). `PERMISOS.finanzas.acceso`.
