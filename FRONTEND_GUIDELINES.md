...
- **Guías post-exportación (PROCESOS):** las guías paso a paso para importar en DUX **no** se abren automáticamente al exportar Excel. Están centralizadas en **`/gestion-productos/procesos`** (`ProcesosPageClient` + `PROCESOS_INSTRUCTIVOS` en `src/lib/procesos-instructivos.ts`). Carrusel reutilizable: `ProcesoInstructivoCarrusel`. Tres procesos: stock (`importar_stock_{1..5}.png`), aumentos (`importar_precios_{1..5}.png`), recepción (`importar_compra_{1..5}.png`). Sidebar: módulo **PROCESOS** → **Guías De Importación**. Cada guía se muestra solo si el rol tiene permiso del módulo de origen (stock / control aumentos / pedidos).
