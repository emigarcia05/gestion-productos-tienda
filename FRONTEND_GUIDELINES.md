...
- **TablaTienda / Cx Compra** (`TablaTienda.tsx`, clase `tabla-tienda-listado` en `globals.css`): cuatro columnas — **COD. TIENDA** **12%**, **DESCRIPCIÓN** **46%**, **VINCULACIÓN** **12%**, **CX PROD.** **30%** (bloque secundario con divisor; **`CeldaCxProdTienda`** en `@/components/shared/CeldaCxProdTienda.tsx`). **VINCULACIÓN**: **`-`** o contador de vínculos. Doble clic en fila abre **`VincularModal`** (`stopPropagation` en celda CX PROD.). **`ItemTiendaParaTabla.cxProd`**: datos de costo producto (`CxProdDatosFila`).
