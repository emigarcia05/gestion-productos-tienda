...
- **Vínculo manual:** botón **Asociar URLs** por fila → `AsociarUrlsCompetenciaModal`: solo competidores con URL cargada (o **Px Sugerido**); buscador de proveedor + botón **+** (lista de competidores **sin** URL para el ítem); grilla 3 columnas — **Proveedor**, **Ficha de Producto** (regla de extracción), **URL**; **Guardar** solo persiste cambios y bajas de URL. Si `urlBloqueadaPorPxSugerido`, sin inputs URL/tipo. `RelevamientoUltimoMensaje` bajo cada fila con URL editable. Detalle expandido: precios OK + filas `ERROR`/`SIN_PRECIO` con el mismo componente.
