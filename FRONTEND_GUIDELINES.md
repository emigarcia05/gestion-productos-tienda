...
- **Altura de fila (obligatoria en este grid)**: **todas** las filas (incluida la cabecera de columnas) usan la misma altura fija **`h-10` (2,5 rem)**, tomada como referencia de la fila **Resultado operativo** (una línea `text-sm` sin botones). Celdas: `flex items-center`, `py-0`, `px-3`. En columnas de **sucursal**, el **10%** izquierdo es una **rejilla de dos ranuras fijas** (`grid-cols-2`): columna 1 solo **histórico** (placeholder), columna 2 solo **desglose** (costos) o **editar ventas** o vacío — así los históricos y los desgloses quedan alineados en vertical entre filas. Botones: `TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS` + `TABLE_ROW_ACTION_ICON_CLASS` (`@/lib/ui-classes`) con override compacto **`!h-7 !w-7 !p-1`**. Constantes `CLASE_FILA_BALANCE_MENSUAL_GRID` / `CLASE_CELDA_BALANCE_MENSUAL` / `CLASE_BOTON_ACCION_BALANCE_MENSUAL` en el componente.
...
- **Fila Ventas — edición**: si `puedeEditarVentas` y la columna tiene `sucursalId`, el **lápiz** va en la **ranura 2** del subgrid (junto al histórico en ranura 1); el monto sigue en el **90%** derecho.
...
*Última actualización (2026-05-06): `/finanzas/balance/mensual` — cada celda de sucursal usa sublayout `10%` + `90%`; el 10% es `grid-cols-2` (histórico | desglose/edición) con botones relleno marca (`TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS`).*
