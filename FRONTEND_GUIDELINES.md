...
| `.tabla-gestion-compacta.tabla-balance-gastos` | Solo **`TablaGastos`** (**Balance · Gastos**): `--tabla-body-row-min-height` **2.875rem**, `--tabla-body-cell-padding-y` **0.375rem**; filas **`height: auto`** con **`min-height`** (texto **GASTO** en varias líneas); anula **`max-height: 1.75rem`** de botones en **`td`** para que **ACCIONES** estiren con la fila. |
...
| `TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS` (`@/lib/ui-classes`) | Botones **solo ícono** en **filas de tabla** (y equivalentes en modales de grilla): incluye **`tabla-row-btn-filled-brand`**; fondo **`#0072BB`**, ícono blanco (**`TABLE_ROW_ACTION_ICON_CLASS`**), `size="icon"` + `variant="ghost"`, alto acoplado a la fila (`!h-full`, `min-h-[var(--tabla-body-row-min-height)]`, `self-stretch`). **Padding simétrico** **`0.5rem`** en los cuatro lados vía **`globals.css`** (`button.tabla-row-btn-filled-brand`), para anular **`main button { padding: 0 1rem }`**. Ancho **`min-w-10`**, `!w-auto` salvo **`!w-full`** en columna única. |
...
| `TABLE_ROW_CELL_ICON_ACTIONS_FLEX_CLASS` (`@/lib/ui-classes`) | `flex` envoltorio (`items-stretch`, `h-full`, `min-h-[var(--tabla-body-row-min-height)]`) para uno o varios botones **`TABLE_ROW_*`** en la misma celda. |
...
*Última actualización (2026-05-05): **`tabla-balance-gastos`** en `TablaGastos` — más padding vertical y altura mínima de fila; filas con `height: auto` para **GASTO** multilínea; anula tope **1.75rem** de botones en `td` para **ACCIONES**; `gap-1.5` entre botones y `gap-1` en bloque título/comentario del gasto.*
