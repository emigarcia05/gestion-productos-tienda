...
- **Área Marketing** (`/marketing/*`): SSOT rutas **`MARKETING_ROUTES`** (`src/lib/marketingRoutes.ts`). Entrada modal / `/marketing` → redirect a **`/marketing/publicaciones/calendario`**. **Calendario De Publicaciones** (`MarketingCalendarioPageClient`): header (solo `editor`, `Settings2`) — **Gestionar Redes**, **Gestionar Tipo Publicaciones**, **Gestionar Tipo Contenido**. Modales de catálogo alineados a **Gestionar Competidores**: buscador + botón **`+`** a la derecha; lista con **Pencil** / **Trash2** (`TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS`); alta/edición en submodal. Redes y tipo contenido: **`GestionarMktCatalogoNombreModal`** (`kind: "red" | "contenido"`). Tipos: **`GestionarMktTipoPublicacionesModal`** — en el submodal: **Nombre** + **Contenidos Permitidos** (lista desplegable multi-opción con checkboxes; obligatorio ≥1 para guardar). Cuerpo: **`MktCalendarioPublicacionesGrid`** — grilla **5 semanas × 7 días** (columnas **LUN.–DOM.**); al cargar 1.ª fila = **semana actual** AR; barra: izquierda **Ir A Esta Semana** + **Este Mes** (1.ª fila = lunes de la semana del día 1); centro **nombre del mes** (`JULIO 2026`); derecha **← →** juntos (±1 **mes**, ancla al inicio del mes; `variant="default"`); días del **mes en vista** con tinte `bg-primary/8`; **hoy** con anillo + badge primary. Helpers SSOT: `@/lib/mktCalendarioPublicaciones.ts`. **Ideas**: placeholder. `PERMISOS.marketing.acceso` (solo `editor`).
...
*Última actualización (2026-07-14): **Marketing · modales Gestionar** — patrón buscador + `+` + lista (Pencil/Trash2), como **Gestionar Competidores**; alta/edición en submodal.*
