...
- **Guías post-exportación (PROCESOS):** las guías paso a paso para importar en DUX **no** se abren automáticamente al exportar Excel. Están centralizadas en **`/gestion-productos/procesos`** (`ProcesosPageClient` + `PROCESOS_INSTRUCTIVOS` en `src/lib/procesos-instructivos.ts`). Layout: grid **`25% | 75%`** (columnas). Columna izquierda: bloque superior **15%** alto con título **Módulo** (`ModalMicroLabel` sin `uppercase`) y `Select` (`MODULOS_PROCESOS`; por ahora solo **Importación Datos En Dux**); bloque inferior **85%** con listado de procesos (`labelCorto`: **Imp. Stock**, **Imp. Compra**). **Títulos del módulo:** primera letra de cada palabra en mayúscula vía `formatoTituloProcesos` (no `uppercase` en UI). Columna derecha (`ProcesoInstructivoCarrusel`, `grid-rows`): (1) **título fijo** (`tituloGuia`); (2) **carrete de pasos** — **3 slots fijos** (anterior · actual · siguiente) + flechas `<` `>`: flecha activa = botón relleno **#0072BB**; flecha sin destino = **`invisible`**; slot anterior/siguiente sin paso = **`invisible`** (reserva espacio); paso **actual** = relleno **#0072BB**, texto blanco, tamaño **+50%** (`3.375rem` vs `2.25rem` adyacentes); adyacentes visibles = borde **#0072BB**, fondo transparente, texto negro; (3) **texto del paso** en contenedor fijo `h-28` con scroll si desborda; (4) **imagen** opcional (`img` en el paso), sin fondo, `object-contain` al máximo dentro del contenedor; clic abre `Dialog` a pantalla completa. Imágenes: `importar_stock_{1..5}.png`, `importar_compra_{1..5}.png` (sin recuadro cyan de guía; fondo transparente en esos píxeles). Sidebar: enlace directo **PROCESOS** (sin submódulos). Cada proceso se lista solo si el rol tiene permiso del módulo de origen (stock / pedidos).
...
- **Carrusel de procesos** (`ProcesoInstructivoCarrusel`): layout en columna flex (título, carrete, texto `h-28`, imagen `flex-1 basis-0`); carrete: clases `proceso-paso-nav-flecha` / `proceso-paso-nav-numero` en `globals.css` (excepción a `main button`; círculos `border-radius: 9999px`); texto con scroll en altura fija; imagen centrada con `max-h-full` / `max-w-full` (sin `fill`); zonas sin imagen = contenedor `bg-transparent`; lightbox pantalla completa en `Dialog`.
