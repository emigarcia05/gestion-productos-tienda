...
- **`/finanzas/analisis-mc/margen-contribucion`**: **`ClassicFilteredTableLayout`** título **Finanzas**, subtítulo **Margen Contribución**. **Configuración** (`FilterBar`, dos filas): **fila 1** — **TERMINAL**, **TIPO DE COMPROBANTE** (`FACTURA A` | `FACTURA C` | `FACTURA X`; **C** no calcula IVA ni IIBB), **`LimpiarFiltrosButton`**; **fila 2** — **TIPO DE ANÁLISIS** (`PRODUCTO` | `PORC. UTILIDAD`) + input según modo (**ELEGIR PRODUCTO** → modal `prod_tienda` | **`PorcentajeCentInput`** margen). Si **PRODUCTO**: PX LISTA solo lectura en tabla; si **PORC. UTILIDAD**: PX LISTA fijo **$100** (solo lectura) y margen desde el input de filtros. Tabla **`TablaFinAnaMargenContribucion`**: columnas = formas de pago. **Secciones de filas** (`FIN_ANA_MC_LAYOUT`): bloque precio (**PX LISTA**, **DESCUENTO** con input entero 0–100 + `%` fijo **por forma de pago**, **PX VENTA** en negrita por columna), fila espacio, bloque costos, subtotal costos, **M.C**. Fórmulas en `@/lib/finAnaMargenContribucion.ts`.
