...
Columnas bloque principal: **TILDE 3 %** · **PROVEEDOR 7 %** · **DESCRIPCIÓN 12 %** · **DTO. EXTRA 6 %** · **COSTO 8 %** · **VAR 7 %** (`scrollX` en tabla). **DTO. EXTRA** — input entero 0–99 con máscara `%` (`formatDtoExtraComparacionMask` / `parseDtoExtraComparacionMask` en `comparacionCategoriasFormat.ts`); componente **`CeldaDtoExtraComparacion`**; recálculo instantáneo de **COSTO**, **VAR** y márgenes vía `dtoExtraDraft`; persistencia al blur (`actualizarDtoExtraComparacionAction`, `prod_comp_dto_extra`). Bloque secundario (`tabla-bloque-secundario-*`): **MARGEN (SEGÚN PX REFERENCIA) 9 %** — `calcMargenSegunPxReferencia`; **PX MANUAL 8 %** — calculado (`calcPxManualDesdeDifPctReferencia`, máscara **`formatPxManualEnteroMask`**); **DIF PX REF MANUAL 8 %** — **input** (`CeldaDifPxRefManualComparacion`, `formatDifPxRefManualMask` / `parseDifPxRefManualMask`); **MARGEN MANUAL 9 %** — calculado (`calcMargenManualDesdeDifPctReferencia`, `fmtMargenComparacionPct`); recálculo en cliente vía `difPxRefManualDraft`; persistencia al blur del input → **`margen_manual`** (`actualizarMargenManualComparacionAction`, `prod_comp_margen_manual`). Acciones **8 %**. Modal **Asignar productos** y modal **Elegir referencia**: `max-w-[126rem]`.
