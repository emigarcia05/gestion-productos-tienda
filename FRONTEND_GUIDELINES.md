...
- **Tabla** (`CompetenciaPreciosTabla`): mismo alto de fila global **2rem**; subfila detalle **`tabla-fila-detalle-competencia`** (fondo uniforme sin cebra ni divisores verticales; borde superior + acento izquierdo). Una **`TableRow`** por competidor alineada al `<colgroup>`: nombre en **PRECIO TIENDA** (`text-right`), **px** en **PX PROMEDIO**, **dif.** en **DIF TIENDA**. `<colgroup>` **45% + 9%×5 + 10%**. Cálculos en `competenciaPreciosFilaResumen.ts`.
