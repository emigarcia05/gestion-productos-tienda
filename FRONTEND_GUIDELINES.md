...
| `.tabla-gestion-compacta.tabla-flujo-de-fondo` | Solo **Finanzas / Flujo De Fondo** (`/finanzas/venc-por-fecha`, `TablaFlujoDeFondo` / `TablaFlujoDeFondoDetalleDia`): **primera** columna (FECHA / PROVEEDOR) cabecera y celdas a la **izquierda**; **columnas de importe** (resto) — cabeceras a la **derecha** (`thead th:not(:first-child)` + `.table-head-inner` con `flex-end`) para alinear con celdas **`TD_NUM`**. |
...
- **`/finanzas/venc-por-fecha`**: **`src/app/finanzas/venc-por-fecha/page.tsx`** (servidor) + **`FinanzasVencPorFechaPageClient`**. **`ClassicFilteredTableLayout`** `contentWidth="full"`, título **Finanzas**, subtítulo **Flujo De Fondo**; **`filters`**: **FilterBar** con Select **PROVEEDOR** + **`LimpiarFiltrosButton`**. Sobre el bloque de grilla: contador **`FILTER_COUNT_CLASS`** (**X DÍA(S) EN ESTA PÁGINA**), si hay paginación **`· Pág. n / m`**, texto **Ventana: Y días (hoy + 150)**. Rango de datos: **hoy** (AR) a **hoy+150** (`listarVencimientosEnRango`). **Vista de tabla:** componentes **`TablaFlujoDeFondo`** y **`TablaFlujoDeFondoDetalleDia`** (`src/components/finanzas/TablaFlujoDeFondo.tsx`) con el **mismo cascarón** que **`TablaDeudaProveedores` / `TablaControlComprobantes`**: **`contenedor-tabla-gestion`**, scroll interno con **`[scrollbar-gutter:stable]`**, **`Table variant="compact"`** `scrollX={false}` `table-fixed`, clases **`tabla-flujo-de-fondo`**, y si aplica **`tabla-venc-por-fecha-alerta`**. En **`globals.css`**: primera columna (FECHA / **PROVEEDOR** en modal) a la **izquierda**; el resto de **cabeceras** alineadas a la **derecha** para emparejar **`TH_NUM` / `TD_NUM`**; celdas **`celda-datos`**, **FECHA** y **SALDO** (y **PROVEEDOR** en modal) con **`celda-destacado`**; **FECHA** formateada con `formatMesDiaMayusculasDesdeIsoYmd` (mes en mayúsculas en el string). Cálculo **VTOS ACUMULADOS / CAJA DISPONIBLE / SALDO** en el cliente. Si hay al menos un **SALDO** negativo, clase alerta y filas con **`venc-saldo-negativo`** (tinte sobre cebra, ver reglas en `globals.css`). **`<colgroup>`** 20% × 5. **Paginación** `PaginacionTabla` (100, `pagina`). **Doble clic** → `AppModal` con **`TablaFlujoDeFondoDetalleDia`**, scroll con **`no-scrollbar`**. `PERMISOS.finanzas.acceso`.
