...
*Última actualización (2026-04-21): catálogo de gastos de balance — migración `20260421140000_fin_bal_cat_gasto_sin_proveedor_mensual`: tabla hoja renombrada a `fin_bal_cat_gasto`, baja de `proveedor_id` y `gasto_mensual`, UNIQUE `(rubro_id, nombre)`. UI: se quitan Select **PROVEEDOR**, **GASTO MENSUAL** y la meta bajo cada gasto; `crearFinBalGastoAction` / `editarFinBalGastoAction` solo envían `nombre` + `rubroId`. La columna **PROVEEDORES** sigue igual (`ProveedorModal`, `getProveedoresNoMercaderia`). Ver `BACKEND_GUIDELINES` §2.5e.*
