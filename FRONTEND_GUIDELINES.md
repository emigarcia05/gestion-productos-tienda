...
- **Estados**
  ...
  - **Sync lista precios en curso** (polling): **`MensajeProceso`** con dos fases según `phase` del status API — **`SINCRONIZANDO PROD.`** (consulta paginada DUX) o **`GUARDANDO PROD.`** (persistencia en Neon; el contador **reinicia en 0** porque es otra fase, no un nuevo sync). Al terminar (`running: false` y `lastCompletedAt` nuevo): **toast** de éxito con totales; error en `error` → toast destructivo. **Doble clic** en ese bloque abre **`AppModal`** **Cancelar Sincronización** con confirmación *«¿Está seguro que desea cancelar la sincronización?»*; **Sí, Cancelar** llama **`POST /api/sync-lista-precios-tienda/cancel`**. Una cancelación **no** actualiza **`lastCompletedAt`** (no cuenta para **Últ. Act.**).
