...
8. **Tablas (encabezado fijo + paginación)**  
  ...
  - **Botones de solo ícono en celdas de tabla (`tbody`) — prohibición y formato obligatorio:** está **prohibido** usar `<Button variant="outline" size="icon-xs">` (o `icon-sm` / `icon` con `outline`) para acciones por fila que generen apariencia **neutra tipo documento** (`border`, `bg-background`, `shadow-xs`, hover `accent`), como el antiguo **Registrar pago** (`Banknote`) en `TablaGastos`. **Obligatorio:** fondo **`#0072BB`**, **ícono** (o texto mínimo **+** / **−**) en **blanco** (`text-white`), hover **`bg-[#0072BB]/90`**. Patrón: `variant="ghost"` + `size="icon"` + **`TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS`** en `@/lib/ui-classes.ts` (incluye **`tabla-row-btn-filled-brand`**; **`globals.css`** aplica **`padding: 0.5rem`** simétrico y anula **`main button { padding: 0 1rem }`**). El botón **estira al alto útil del contenedor** (`!h-full`, `min-h-0`, `max-h-full`, `self-stretch`); si hace falta aire respecto del borde de la celda, **`py-*`** en el **`div`** de **`TABLE_ROW_CELL_ICON_ACTIONS_FLEX_CLASS`** (ej. **Balance · Gastos**). Envolver el grupo en **`TABLE_ROW_CELL_ICON_ACTIONS_FLEX_CLASS`** (`items-stretch`) y, en celdas **`.celda-datos`**, añadir **`.celda-datos--accion-relleno-fila`** cuando haga falta anular el padding para que el relleno azul llegue al borde vertical de la celda (ej. columna eliminar en **Pedido Urgente**). Si en la misma fila hay un **`Input`** entre **+**/**−**, dar al input **`self-center`** para no estirarlo. **No** extrapolar a botones con **texto visible** de CTA en cabeceras, pies de modal o barras de herramientas: esos siguen `default` / `outline` / `secondary` del tema cuando corresponda.
...
| `.tabla-gestion-compacta.tabla-balance-gastos` | Solo **`TablaGastos`** (**Balance · Gastos**): `--tabla-body-row-min-height` **2.875rem**, `--tabla-body-cell-padding-y` **0.375rem**; **alto de fila fijo** (misma regla que **`.tabla-gestion-compacta tbody tr`**, variable heredada del `<table>`). **ACCIONES:** wrapper con **`py-1.5`** para separar el recuadro azul del borde de la fila; anula tope **1.75rem** de botones en **`td`**. |
...
| `.celda-datos.celda-datos--accion-relleno-fila` | Anula **`padding`** con **`!important`** en celdas **`.celda-datos`** usadas solo para botones de acción rellenos **`#0072BB`**, para que el botón ocupe todo el alto fijo de la fila (ver **`TABLE_ROW_CELL_ICON_ACTIONS_FLEX_CLASS`**). |
...
| `TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS` (`@/lib/ui-classes`) | Botones **solo ícono** en **filas de tabla** (y equivalentes en modales de grilla): incluye **`tabla-row-btn-filled-brand`**; fondo **`#0072BB`**, ícono blanco (**`TABLE_ROW_ACTION_ICON_CLASS`**), `size="icon"` + `variant="ghost"`, **`!h-full`**, **`min-h-0`**, **`max-h-full`**, **`self-stretch`** dentro del flex de **`TABLE_ROW_CELL_*`** (sin **`min-h`** = alto de fila: evita invadir el **`py-*`** del wrapper). **Padding simétrico** **`0.5rem`** en **`globals.css`**. Ancho **`min-w-10`**, `!w-auto` salvo **`!w-full`**. |
| `tabla-row-btn-filled-brand` | Clase en **`TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS`**; regla en **`globals.css`** con **`padding: 0.5rem !important`** simétrico. |
| `TABLE_ROW_CELL_ICON_ACTIONS_FLEX_CLASS` (`@/lib/ui-classes`) | `flex` envoltorio (`items-stretch`, `h-full`, `min-h-0`, `box-border`) para uno o varios botones **`TABLE_ROW_*`**; en **Balance · Gastos** añadir **`py-1.5`** al contenedor de **ACCIONES**. |
...
*Última actualización (2026-05-05): **`CrearEditarFinBalGastoFinalModal`** (Nuevo/Editar Gasto Final) reordena campos a: **TIPO DE GASTO (MENSUAL/EVENTUAL)**, **SUCURSAL**, **PROVEEDOR**, **DÍA DEVENGADO**, **PLAZO DE PAGO**. **DÍA DEVENGADO** usa `Select` 1..28 y **PLAZO DE PAGO** usa `Select` 0..30. Si **TIPO = MENSUAL**, ambos quedan deshabilitados y visualmente vacíos (persistencia técnica `1/0`). Si **TIPO = EVENTUAL**, ambos se editan normalmente.*
...
*Última actualización (2026-05-05): **Alto del botón = alto de fila** — `TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS` estira el control a la fila; `TABLE_ROW_CELL_ICON_ACTIONS_FLEX_CLASS` en el contenedor; `.celda-datos--accion-relleno-fila` en `globals.css` para celdas de solo acción; `Button` (`button.tsx`) fusiona `className` con `cn(buttonVariants({ variant, size }), className)`.*
...
*Última actualización (2026-05-05): **`tabla-balance-gastos`** en `TablaGastos` — fila **alto fijo** **2.875rem**; **ACCIONES** con **`py-1.5`** en el flex wrapper; botones **`min-h-0` / `max-h-full`** respecto al contenedor (sin pegar al borde de fila).*
