...
- **`pxListas.ts` (actions)**: `getPxListasPageData`, `guardarPxListaTiendaAction` — módulo **Px Listas** (`/gestion-productos/tienda/cx-px-tienda`). Tabla **`prod_precios_tienda_marcacion`** (Prisma `ProdPrecioTiendaMarcacion`): `det_precio_manual`, `competencia_id`, `px_lista_manual`. **DET PRECIO**: PX MANUAL o competidor asociado (`prod_precios_competencia`). **PX LISTA**: manual editable o precio competencia (misma prioridad que Px Competencia: sugerido proveedor → scraping). **MARCACION** en lectura: `(px_lista / costo_compra) / 1,21` (2 decimales; `calcMarcacionPxLista` en `pxListas.ts`). Servicios: `pxListasConfig.service.ts`, `pxListasRows.service.ts`.
