...
- **Convivencia con DUX**: `costo_compra` y `px_lista_tienda` siguen siendo **espejo DUX** (`syncListaPrecioTienda.service.ts`). Desde **2026-05-28** **`proveedor` y `cod_ext` quedaron congelados** (ya no se actualizan por DUX; solo conservan valores legacy — ver §1.4.2). El sync **no** escribe ni borra `costo_compra_cod_ext` ni **`es_producto_propio`**. Migración **`20260528210000`** retiró `px_lista_cx_px` y `cx_px_px_comp_ref`; **`es_producto_propio`** se **restauró** en **`20260530120000`** para **Cx Compra** (ver § Producto propio). **`prod_precios_competencia`** no se modifica.
...
- **`exportarCostoCxDiffAction`** (`exportCostoCxDiff.service.ts`): compara `costo_compra` (DUX) con `costoCxProdMostrado` (**CX PROD.**). Comparación en **pesos enteros** (`Math.round`). Solo diferencias en Excel (**CODIGO**, **COSTO** entero).
...
| `@/lib/validations/pedidosMutaciones.ts` | Mutaciones/envío: `proveedorIdPedidoSchema` (`prismaCuidSchema`), `listarProveedoresConPedidoActivoSchema`, `comprobarItemsParaGenerarPedidoSchema`, `generarPdfEnviarPedidoSchema` (`idItemPedidoEnvio` → `uuidSchema` / `prod_ped_merc`), `getSobreStockReposicionParaModalSchema`, `syncPedidoUrgenteEnvioSchema`, `upsertPedidoUrgenteItemSchema`, `upsertPedidoTintometricoItemsSchema`, `deleteTintometricoItemSchema`. |
...
- **`pxListas.ts` (actions)**: `getPxListasPageData`, `guardarPxListaTiendaAction` — módulo **Px Competencia** (UI en `/gestion-productos/tienda/cx-px-tienda`; código interno `px-listas/*`). `getPxListasPageData` devuelve también **`competencias`** (`listCompetencias`) para PX PROMEDIO, DIF TIENDA, detalle expandido y modal de URLs. **`/gestion-productos/precios-competencia`** redirige aquí. Tabla **`prod_precios_tienda_marcacion`** (Prisma `ProdPrecioTiendaMarcacion`): `det_precio_manual`, `competencia_id`, `px_lista_manual`. **DET PRECIO**: **PX MANUAL** | competidores del ítem (`prod_precios_competencia`). El precio por competidor usa `aplicarPrioridadPrecioMostrar` (sugerido proveedor → scraping). Si hay `px_vta_sugerido` sin fila PPC, se agrega el competidor de `prod_competencia` ligado al proveedor del vínculo (`buildMapPxSugeridoCompetenciaPorCodTienda`) con su **nombre** en el desplegable (no texto genérico «PX SUGERIDO»). En el desplegable DET PRECIO: siempre **PX MANUAL** + competidores con precio registrado (`px` &gt; 0). Sin config guardada: default competidor (guardado → sugerido → primero por nombre); con `det_precio_manual=true` se respeta **PX MANUAL**. **PX LISTA**: según selección. **MARCACION**: `((px_lista / costo_compra) / 1,21 − 1) × 100` (% utilidad sin IVA; 5 decimales; SSOT `calcMarcacionPxLista` en `@/lib/pxListas`). Antes de `20260528260000_marcacion_pct_utilidad` se guardaba el factor multiplicador `px/costo/1,21`. Columna **`marcacion`** en `prod_precios_tienda_marcacion` (se actualiza en `guardarPxListaConfig` vía `persistirMarcacionPxLista`). **Backfill único** (todos los `cod_tienda`): `npm run db:backfill-marcacion-px-listas` → `backfillMarcacionPxListas.service.ts` (misma lógica que la grilla; `--dry-run` en el script). **Exportar Px** (`exportarPxDiffAction`, `exportPxDiff.service.ts` → `obtenerExportPxDiffPayload`): solo filas con `marcacion` persistida; Excel **CODIGO** + **IMPORTE** (marcación % de la grilla). **Criterio de exportación:** solo si `round(px_lista_efectivo) !== round(px_lista_tienda)` (no basta comparar `marcacion` en BD: puede verse igual en pantalla y coincidir el importe en DUX). **`exportarPxDiffAction`**: solo `filas` (Excel en **Px Listas**). **`exportarResumenAumentosPxAction`** (`exportResumenAumentos.service.ts`): recorre **`prod_precios_tienda`** con **`costo_compra_cod_ext`** → **`prod_precios_provee`**. **Costo viejo** = `costo_compra` (tienda/DUX); **costo nuevo** = `px_compra_final_sin_iva` del proveedor vinculado. **Aumento %** = `((costoNuevo / costoViejo) − 1) × 100` (`calcAumentoPctCostoCompra` en `@/lib/aumentoCostoCompra`). Solo entran ítems con costos distintos (pesos enteros) y marca/rubro informados. PDF en cliente (`generarPdfAumentosPx.ts`); archivo `Resumen Aumentos {dd-mm hh-mm}.pdf`. **`exportarPxDiffAction`** (Excel Px) sigue comparando PX LISTA vs DUX — independiente del informe. Listado: **`pxListasPage.service.ts`** (`getPxListasPageDataFromDb`) — filtros URL `marca`, `rubro`, `detPrecio` (CUID competidor con `preciosCompetencia.some`), `ordenMarcacion` (`marcacion-desc` | `marcacion-asc`: ordena en memoria tras `buildPxListasItemsDesdeFilas`; sin orden, paginación en BD). Opciones DET PRECIO: `prod_competencia` con al menos un `prod_precios_competencia`. Servicios fila: `pxListasConfig.service.ts`, `pxListasRows.service.ts`.
...
| `src/actions/pedidos.ts` | **`comprobarItemsParaGenerarPedidoAction`**: delega en `getItemsTablaEnviarPedido` (§1.2.5.E). Esquemas Zod centralizados en `@/lib/validations/pedidosMutaciones` + lectura en `pedidosLectura`; `proveedorId` con `prismaCuidSchema`, `idItemPedidoEnvio` / borrado tintométrico por `id` con `uuidSchema` (`prod_ped_merc`). Payloads de mutación con firma `raw: unknown` (`generarPdfEnviarPedidoAction`, `syncPedidoUrgenteEnvioAction`, `upsertPedidoTintometricoItemsAction`, etc.). `sucursalPedidoHabilitada` movido a `pedidosEnvio.service.ts`. |
