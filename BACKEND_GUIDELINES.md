...
- **Gráfico M.C vs PORC. UTILIDAD**: helpers en `src/lib/finAnaMargenContribucion.ts` — `idFormaPagoTresCuotasMargenContribucion` (default **3 CUOTAS**), `serieMcVsPorcUtilidadMargenContribucion` / `mcPctEnPorcUtilidadMargenContribucion` con métrica **`MC` | `MC_PONDERADO`** (eje X **20…200** step 5). UI `GraficoMcVsPorcUtilidad`: select métrica + tabla checklist de formas (checkbox → graficar; columna valor = M.C o M.C PONDERADO al PORC. UTILIDAD del filtro); series con `COLORES_SERIE_GRAFICO_MC`. Reacciona a **TERMINAL**, **TIPO COMPROBANTE**, **PORC. UTILIDAD** y descuentos. Sin dependencia de charts externas.
