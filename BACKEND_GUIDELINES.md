...
- **`pxListas.ts` (actions)**: `getPxListasPageData` — listado paginado para **Px Listas** (`/gestion-productos/tienda/cx-px-tienda`); reutiliza filtros y query de `getTiendaPageData` sin enriquecer **CX PROD.**; expone `precioLista` (= `px_lista_tienda` DUX).
