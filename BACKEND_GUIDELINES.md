...
- **Columnas calculadas en UI** (no persistidas en BD): **CX TERMINAL S/ IVA** = `arancel + costo_financiero`; **CX TERMINAL C/ IVA** = `(arancel + costo_financiero) × 1,21` (`FIN_ANA_COS_FINA_IVA_FACTOR`). Funciones: `cxTerminalSinIvaFinAnaCosFina`, `cxTerminalConIvaFinAnaCosFina`, display `fmtPorcentajeDosDecimalesFinAnaCosFina`.
...
- **Validación** (`@/lib/validations/finAnaCosFina.ts`): `actualizarFinAnaCosFinaSchema` — `id` con `prismaCuidSchema`; `campos` estricto con al menos uno de: `habilitado`, `diasAcreditacion` (0–999 o `null`), `arancel` y `costoFinanciero` (0–100, 2 decimales).
