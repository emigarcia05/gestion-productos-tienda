...
- **Servicio**: `src/services/cajasTesoreria.service.ts`
  - `listarCajasTesoreria()`: lectura ordenada por `nombre_caja`; devuelve **`montoDisponible`**: si `tipo_caja = CHEQUE`, suma en `fin_tesoreria_cheques` con `fecha_acreditacion` ≤ **hoy calendario Argentina** (`dateToIsoYmdArgentina` + SQL `<= ::date`); si no, `montoDisponible = monto`.
...
- **Servicio**: `src/services/finTesoreriaCheques.service.ts`
  ...
  - `actualizarFinTesoreriaCheque(input)`: edita `tenedor`, `emisor`, `monto`, `fecha_acreditacion`; exige caja asociada `CHEQUE`.
...
- **Actions**: `src/actions/finTesoreriaCheques.ts`
  ...
  - `listarFinTesoreriaChequesPorCajaAction`: lectura con permiso finanzas; Zod `listarFinTesoreriaChequesPorCajaSchema`.
  - `actualizarFinTesoreriaChequeAction` / `eliminarFinTesoreriaChequeAction`: `esEditor()`; Zod `actualizarFinTesoreriaChequeSchema` / `eliminarFinTesoreriaChequeSchema`; revalida `/finanzas` y `/finanzas/tesoreria`.
...
| `@/lib/validations/finTesoreriaCheques.ts` | `crearFinTesoreriaChequeSchema`, `actualizarFinTesoreriaChequeSchema`, `eliminarFinTesoreriaChequeSchema`, `listarFinTesoreriaChequesPorCajaSchema`, `montoChequeTesoreriaSchema`, `tenedorChequeTesoreriaSchema` (mismo conjunto que titular de caja). |
