...
- **Filas / fórmulas** (`src/lib/finAnaMargenContribucion.ts`): **PRECIO VENTA** = `PX LISTA × (1 + descuento % / 100)` (descuento por forma de pago, entero **−100…100**, persistido en `fin_ana_mc_descuento_fp`; **negativo = descuento** / baja el precio, **positivo = recargo**; UI tipea negativo por defecto salvo `+` explícito; migración de signo **`20260721140000_fin_ana_mc_descuento_sign_convention`**); **IVA** = `(PRECIO VENTA / 1,21) × 0,21` (si comprobante aplica); **IIBB** = `(PRECIO VENTA / 1,21) × 0,04` (si comprobante aplica); **CX MERCADERÍA** = `(PRECIO VENTA / 1,21) / (1 + porc. utilidad % / 100)` (factor utilidad sobre costo, alineado a Px Listas); **CX FINANCIERO** = **`cxTotalConIvaFinAnaCosFina`** por forma de pago desde `fin_ana_cos_fina` (solo filas `habilitado = true`). Filtro **TERMINAL** opcional en UI: si se elige terminal, una fila por `pago_id`; si no, **promedio** entre terminales habilitadas del mismo pago.
...
- **UI**: `FinAnaMargenContribucionPageClient`, `TablaFinAnaMargenContribucion`, `ElegirProductoMargenContribucionModal`, **`GestionarPagosFinAnaCosFinaModal`** (header **Gestionar Pagos**). Layout de filas (`FIN_ANA_MC_LAYOUT`): **PX LISTA** → **DESCUENTO** → **PX VENTA** → espacio → **IVA** / **IIBB** / **CX MERCADERÍA** / **CX FINANCIERO** → subtotal costos → **M.C** (= PX VENTA − subtotal costos por forma de pago). **Formato de montos** (`fmtCeldaMontoMargenContribucion`): modo **`porc_utilidad`** → `N%` (PX LISTA = 100 ⇒ el $ coincide con el %); modo **`producto`** → `$N (P%)` con `P` = redondeo de monto / PX LISTA × 100. **DESCUENTO** permanece % firmado. **CX FINANCIERO** en grilla = pesos (`cxFinancieroPesosMargenContribucion`), no la tasa %.
