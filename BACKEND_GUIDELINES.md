...
- **`pxListas.ts` (actions)**: `getPxListasPageData`, `guardarPxListaTiendaAction` — módulo **Px Listas** (`/gestion-productos/tienda/cx-px-tienda`). Tabla **`prod_precios_tienda_marcacion`** (Prisma `ProdPrecioTiendaMarcacion`): `det_precio_manual`, `competencia_id`, `px_lista_manual`. **DET PRECIO**: **PX SUGERIDO** (`det_precio_manual=false`, `competencia_id=null`) | **PX MANUAL** | competidor (`prod_precios_competencia`). **Px sugerido por ítem**: `buildMapPxVtaSugeridoPorCodTienda` en `competenciaPxSugerido.service.ts` — `px_vta_sugerido` del vínculo habilitado más reciente en `prod_precios_provee` por `cod_tienda`. Sin fila en marcación: default **PX SUGERIDO** si existe precio; si no, **PX MANUAL**. Competidores siguen usando sugerido del proveedor asociado → scraping (`aplicarPrioridadPrecioMostrar`). **PX LISTA**: según selección DET PRECIO. **MARCACION**: `(px_lista / costo_compra) / 1,21` (5 decimales; `calcMarcacionPxLista`). Servicios: `pxListasConfig.service.ts`, `pxListasRows.service.ts`.
