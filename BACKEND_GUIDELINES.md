...
- **`pxListasPrecios.ts` (actions)**: `getPxListasPreciosPageData`, `guardarPxListaMargenEdicionAction`, **`exportarPxListasMargenAction`** — módulo **Px Listas** (`/gestion-productos/tienda/px-listas`; `px-listas-precios/*`). Listado: **`pxListasPreciosPage.service.ts`** — filtros `marca`, `rubro`, `subRubro`, `q`, **`actualizar`** (`si`/`no`: margen manual guardado distinto/igual al margen DUX; post-proceso en memoria); columnas dinámicas por `prod_tienda_listas_precios` (PX calculado + MARGEN editable por lista). **Export Excel:** **`exportPxListasMargen.service.ts`** — un `.xls` por `nombre_lista` (**CODIGO**, **PORC UTILIDAD**); solo filas con margen manual ≠ margen DUX (`margenesPorcUtilidadDifieren`). Persistencia edición: **`prod_tienda_margen_edicion`** (migración `20260624160000`). Helpers: **`pxListasPreciosCelda.ts`**, **`pxListasPreciosFiltros.ts`**. Tabla legacy **`prod_precios_tienda_marcacion`** eliminada (`20260528270000`).
