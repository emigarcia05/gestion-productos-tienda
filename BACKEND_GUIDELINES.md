...
- **UI**: `FinAnaMargenContribucionPageClient`, `TablaFinAnaMargenContribucion`, `ElegirProductoMargenContribucionModal`, **`GestionarPagosFinAnaCosFinaModal`** (header **Gestionar Pagos**). Layout de filas (`FIN_ANA_MC_LAYOUT`): **PX LISTA** → **DESCUENTO** → **PX VENTA** → espacio → **IVA** / **IIBB** / **CX MERCADERÍA** / **CX FINANCIERO** → subtotal costos → **M.C** (= PX VENTA − subtotal costos por forma de pago). **Formato de montos**: modo **`porc_utilidad`** → celda única `N%` (`fmtCeldaMontoMargenContribucion`; PX LISTA = 100). Modo **`producto`** → **2 columnas por forma de pago** (encabezado `colSpan={2}`): pesos (`fmtPesosMargenContribucion`, 75% ancho, alineados a la derecha) + % sobre PX LISTA (`fmtPctSobrePxListaMargenContribucion`, 25%, **negrita**, alineados a la derecha). **DESCUENTO** permanece % firmado. **CX FINANCIERO** en grilla = pesos (`cxFinancieroPesosMargenContribucion`), no la tasa %.
