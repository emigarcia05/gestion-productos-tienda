...
- **Venc. por fecha (Finanzas)**: `listarVencimientosEnRango(fechaDesde, fechaHasta)` en `src/services/vencimientosPorFecha.service.ts` — mismas líneas y misma `fecha_venc` que arriba; filtra `fecha_venc` en el rango **inclusive**. La página `src/app/finanzas/venc-por-fecha/page.tsx` pide **`fechaDesde` = hoy Argentina** y **`fechaHasta` = hoy + 120 días** (`addDaysToIsoYmdArgentina`); agrupa por día. La columna **VTOS ACUMULADOS** en UI es la **suma corrida** del vencimiento del día en ese listado; **SALDO** es acumulativo en la misma ventana (primera fila: caja − vencimiento del día; siguientes: saldo anterior − vencimiento del día). **CAJA DISPONIBLE** puede seguir en `0` hasta definir origen de datos.
