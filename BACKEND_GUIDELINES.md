...
- **Cx & Px Tienda** (`src/actions/cxPxTienda.ts`): `guardarCostoCxProdTiendaAction` / `guardarPxListaTiendaAction` (FK `cod_ext_costo_lista` y `competencia_id_px_lista`). Filtros URL: `q`, `marca`, `vincCosto`, `costoProv` (`prom` = Cx. Prom.), `pxLista` (`prom` = Px. Prom. / FK nula; si no `competenciaId`), `marcacionOrden` (`menor_mayor` \| `mayor_menor` — ordena en servidor tras calcular marcación de cada fila). Sin filtro **rubro**. **`exportarCostoCxDiffAction`**: Excel Cx diff.
