...
- **`getEnviarPedidoData`** / **`getProveedoresConPedidoActivo`**: el desplegable **PROVEEDOR** en Generar Pedido y filtros de **`/pedidos/enviar`** solo incluye proveedores con al menos un ítem con **cantidad a pedir > 0** (misma resolución que la tabla). Opcionalmente acotado por **sucursal** y **tipos** de pedido. **`listarProveedoresConPedidoActivoAction`** alimenta el modal cuando cambian sucursal/tipos.
