...
- **Servicio**: `src/services/movimientosFinanzas.service.ts`
  ...
  - `listarSucursalesParaGastos()`: `prisma.sucursal.findMany({ where: { centroCosto: true }, select: { id, nombre }, orderBy: { nombre: "asc" } })` — usa solo `global_sucursales.centro_costo`. Alta/edición de **gasto final** en `finBalGastosCatalogo.service.ts` valida `centroCosto = true` (sin exigir `genera_balance`).
