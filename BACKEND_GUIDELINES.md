...
- **Edición manual Px Listas**: **`prod_tienda_precios_edicion`** — misma PK `(cod_tienda, id_lista)`, `precio DECIMAL(14,4)`, `updated_at`. Modelo **`ProdTiendaPrecioEdicion`**. No la toca el sync; baseline DUX queda en `prod_tienda_precios` para diff/export futuro. Mutación: `guardarPxListaPrecioEdicionAction` (`src/actions/pxListasPrecios.ts`) → `guardarPrecioListaEdicion` (`pxListasPreciosEdicion.service.ts`); `precio: null` elimina el override. Margen en UI: `calcMargenSinIvaPct` / inversa `calcPxListaDesdeMargenSinIvaPct` (`src/lib/calculos.ts`); solo se persiste **precio**.
...
- **Sync** (`syncListaPrecioTienda.service.ts`): Fase 2 — `prod_tienda` + `prod_tienda_listas_precios` + `prod_tienda_precios` por chunk. Fase 3 — `cod_tienda` ausentes. Fase 4 — borrar precios y filas de catálogo con `id_lista` fuera del set visto en la corrida.
...
- **`exportarCostoCxDiffAction`** (`exportCostoCxDiff.service.ts`): compara **`prod_tienda.costo_compra`** (DUX) con **`prod_precios_provee.px_compra_final_sin_iva`** del vínculo **`costo_compra_cod_ext` → `cod_ext`** (4 decimales en comparación). Solo filas con diff en Excel/POST (**CODIGO**, **COSTO** = `px_compra_final_sin_iva` redondeado al peso entero). Requiere FK `costo_compra_cod_ext` y vínculo **habilitado**.
- **POST DUX costos** (`actualizarCostoCxDux.service.ts` + `duxItemModificarApi.ts` + `actCxDuxStatusDb.ts`): lotes de **`DUX_API_BATCH_SIZE` (50)** ítems; pausa **`DUX_API_BATCH_INTERVAL_MS` (5 s)** entre POST y entre GET de estado (§1.10c). **UI:** motor en **`SyncStatusIndicator`** encadena **`avanzarActCxDuxAction`** (POST lote → poll hasta `FINALIZADO` → siguiente lote; reanuda si `running` en BD tras recarga). **`iniciarActCxDuxAction`** reserva mutex + meta (`lotesConfirmados`, `idProcesoPendiente`, `pollIntento`, `estadoDux`). Progreso **`processed`** solo sube al confirmar lote en DUX. Poll hasta **120** intentos/lote (`actCxDuxPollPolicy.ts`). Mutex TTL **2 h**. Actions: `iniciarActCxDuxAction`, **`avanzarActCxDuxAction`**, `finalizarActCxDuxExitoAction` (legacy), `abortarActCxDuxAction`, `liberarActCxDuxTrabadoAction`. Deprecados flujo dos fases: `enviarLoteCostoCxDuxAction`, `comenzarConfirmacionActCxDuxAction`.
...
- **Act. Cx. costos** — POST `item/nuevoItem`: hasta **50** productos por body; **`throttleDuxItemModificarRequest()`** antes de cada POST/GET (`duxItemModificarApi.ts`). **Cliente intercalado:** `avanzarActCxDuxAction` hace POST de un lote, guarda `idProcesoPendiente` en `sync_dux_status.meta`, poll GET hasta `FINALIZADO`, incrementa `lotesConfirmados` y repite. Motor en layout (`SyncStatusIndicator` + `actCxDuxRunner.ts`); **`ActCxButton`** solo dispara `iniciarActCxDuxAction`. Caché filas (`actCxFilasCache.ts`) durante la corrida.
...
- **`cxPxTienda.ts` (actions)**: `guardarCostoCxProdTiendaAction`, `exportarCostoCxDiffAction`, **`enviarCostoCxDuxAction`**, **`consultarEstadoCostoCxDuxAction`**. Permiso `PERMISOS.cxPxTienda.acceso` (solo **editor**). FK `costo_compra_cod_ext` (ver §1.10b). **`costoListaTienda.service.ts`**, **`cxPxTiendaRows.service.ts`**, **`actualizarCostoCxDux.service.ts`**, **`duxItemModificarApi.ts`**.
...
- **`pxListasPrecios.ts` (actions)**: `getPxListasPreciosPageData`, `guardarPxListaPrecioEdicionAction` — módulo **Px Listas** (`/gestion-productos/tienda/px-listas`; `px-listas-precios/*`). Listado: **`pxListasPreciosPage.service.ts`** — filtros `marca`, `rubro`, `subRubro`, `q`; columnas dinámicas por `prod_tienda_listas_precios` (PX + MARGEN por lista); PX efectivo = `prod_tienda_precios_edicion` ?? `prod_tienda_precios`. Persistencia edición: **`prod_tienda_precios_edicion`** (migración `20260604190000`). Margen solo calculado (`calcMargenSinIvaPct` / `calcPxListaDesdeMargenSinIvaPct`). Tabla legacy **`prod_precios_tienda_marcacion`** eliminada (`20260528270000`).
...
*Última actualización (2026-06-04): **Act. Cx. POST+poll intercalado** — `avanzarActCxDuxAction` + motor en `SyncStatusIndicator`; meta persistida (`idProcesoPendiente`, `lotesConfirmados`); reanuda tras recarga; progreso solo al confirmar FINALIZADO.*

*Última actualización (2026-06-04): **Política lotes API DUX** — §1.10c: **50 ítems/lote**, **≥5 s** entre lotes (`duxApiBatchPolicy.ts`); Act. Cx. alineado (antes 100). SSOT progreso sidebar en `FRONTEND_GUIDELINES`.*
