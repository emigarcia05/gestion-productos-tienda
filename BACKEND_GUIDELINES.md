...
- **UI**: `FinAnaMargenContribucionPageClient`, `TablaFinAnaMargenContribucion`, `ElegirProductoMargenContribucionModal`, **`GestionarPagosFinAnaCosFinaModal`** (header **Gestionar Pagos**). Layout en **3 secciones** (`FIN_ANA_MC_SECCIONES`): **INGRESO** (PX LISTA · DESCUENTO · PX VENTA) → **COSTOS** (IVA · IIBB · CX MERCADERÍA · CX FINANCIERO) → **MARGEN** (M.C · **M.C PONDERADO**). Columna izquierda de sección con etiqueta **vertical** (`.tabla-mc-col-seccion` + `.tabla-mc-seccion-label`); sticky seccion + concepto. Líneas primary entre secciones (`.tabla-fila-mc-sep-linea`). **M.C PONDERADO**: fila reservada (fórmula pendiente; celdas `—`). **Formato de montos**: modo **`porc_utilidad`** → celda única `N%`; modo **`producto`** → 2 columnas $ / % (PX LISTA solo `$`). **DESCUENTO** % firmado. **CX FINANCIERO** = pesos.
