...
- **Tabla hoja** `fin_bal_gasto` (Prisma: `FinBalGasto`):
  ...
  - `repite_monto` (`BOOLEAN NOT NULL DEFAULT FALSE`, Prisma: `repiteMonto`). Flag "Repite monto": indica que el gasto mantiene el mismo importe entre períodos (ej. alquiler, cuota fija). Habilita precarga del último monto al registrar el movimiento. Sin índice, mismo criterio.
  - `created_at`, `updated_at` (`TIMESTAMP(3)`).
...
- **Migraciones**:
  ...
  - `prisma/migrations/20260418220000_add_fin_bal_gasto_flags_mensual_repite/migration.sql` (columnas `gasto_mensual` + `repite_monto`, `BOOLEAN NOT NULL DEFAULT FALSE`; los registros preexistentes quedan en `false`).
...
- **Servicio** (`src/services/finBalGastosCatalogo.service.ts`)
  ...
  - **Tipos expuestos** — `FinBalGastoItem` incluye `proveedorId: string | null` + `proveedor: FinBalGastoProveedorRef | null` + `gastoMensual: boolean` + `repiteMonto: boolean`. La presencia del objeto expandido es garantizada (nunca `undefined`) y los flags siempre llegan como `boolean` (nunca `null`) gracias al `DEFAULT FALSE` de la DB.
