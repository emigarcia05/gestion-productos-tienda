...
- **Servicio hechos** (`src/services/mktPublicaciones.service.ts`): `listarMktPublicacionesCalendario`, `crearMktPublicacion`, `editarMktPublicacion`, `eliminarMktPublicacion`. Al **crear/editar** con `ideaDetalleId`: exige idea existente y no usada (salvo la ya vinculada en edición); en transacción marca `mkt_publi_ideas_detalle.usada = true`. Al **cambiar** de idea o **eliminar** la publicación: libera la idea anterior (`usada = false`). Actions: `src/actions/mktPublicaciones.ts` (revalidan calendario + ideas). Validación: `@/lib/validations/mktPublicaciones.ts` (`ideaDetalleId` obligatorio en create/edit). Estadísticas UI: `@/lib/mktPublicacionesEstadisticas.ts` — `filtrarPublicacionesPorPeriodoCuadroMando` (`este_mes` | `esta_semana` | `siguiente_semana`, calendario AR lun–dom) + `calcularCuadroMandoPublicaciones` (redes, tipos, Planificado/Terminado por `contenido_creado`).
