...
- **`exportarCostoCxDiffAction`** (`exportCostoCxDiff.service.ts` + `exportCostoCxExcelClient.ts`): compara **`prod_tienda.costo_compra`** (DUX) con **`prod_precios_provee.px_compra_final_sin_iva`** del vínculo **`costo_compra_cod_ext` → `cod_ext`** (4 decimales en comparación). Solo filas con diff en Excel (**CODIGO**, **COSTO** = `px_compra_final_sin_iva` redondeado al peso entero). Requiere FK `costo_compra_cod_ext` y vínculo **habilitado**. **UI:** botón **Act. Cx.** en Cx Compra descarga `.xls` para import manual en DUX (sin POST API).
...
- **`pxListasPrecios.ts` (actions)**: `getPxListasPreciosPageData`, `guardarPxListaPrecioEdicionAction`, **`exportarPxListasMargenAction`** — módulo **Px Listas** (`/gestion-productos/tienda/px-listas`; `px-listas-precios/*`). Listado: **`pxListasPreciosPage.service.ts`** — filtros `marca`, `rubro`, `subRubro`, `q`; columnas dinámicas por `prod_tienda_listas_precios` (PX + MARGEN por lista); PX efectivo = `prod_tienda_precios_edicion` ?? `prod_tienda_precios`. **Export Excel:** **`exportPxListasMargen.service.ts`** + **`exportPxListasMargenExcelClient.ts`** — un `.xls` por `nombre_lista` (**CODIGO**, **PORC UTILIDAD**); solo filas donde el margen efectivo difiere del margen DUX (`margenesPorcUtilidadDifieren`). Persistencia edición: **`prod_tienda_precios_edicion`** (migración `20260604190000`). Margen solo calculado (`calcMargenSinIvaPct` / `calcPxListaDesdeMargenSinIvaPct`). Tabla legacy **`prod_precios_tienda_marcacion`** eliminada (`20260528270000`).
