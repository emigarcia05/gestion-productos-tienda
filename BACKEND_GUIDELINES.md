...
*Última actualización (2026-05-28): **Retiro comparación tienda↔competencia en `prod_precios_tienda`** — migración **`20260528210000_drop_px_lista_cx_px_es_producto_propio`**: DROP `px_lista_cx_px`, `es_producto_propio`, `cx_px_px_comp_ref`. Eliminados servicios/actions del módulo **Cx & Px Tienda** (`pxListaCxPxTienda`, `exportPxListaCxDiff`, `productoPropioTienda`, `getCxPxTiendaPageData`, backfill). **`costo_compra_cod_ext`** y **CX PROD.** permanecen en **Cx Compra** (`cxPxTienda.ts`, `cxPxTiendaRows.service.ts`). **`prod_precios_competencia`** (Px Competencia) sin cambios.*
...
*Última actualización (2026-05-30): **Unificación Px Competencia → Px Listas** — grilla con PX PROMEDIO, DIF TIENDA, detalle y URLs; `getPxListasPageData` incluye `competencias`; `/gestion-productos/precios-competencia` redirige a Px Listas.*
