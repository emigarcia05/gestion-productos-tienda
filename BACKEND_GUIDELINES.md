...
- **POST DUX costos** (`actualizarCostoCxDux.service.ts` + `duxItemModificarApi.ts` + `actCxDuxStatusDb.ts`): mutex en `sync_dux_status` id **`act-cx-costos-dux`** con `startedAt` (bloquea otra Act. Cx. y sync lista mientras corre; sync lista rechaza 409 si Act. Cx. activa). **Recuperación de bloqueo:** `reconcileStaleActCxDuxLockInDb()` libera automáticamente mutex huérfano (sin `startedAt`) o con TTL **15 min** (`ACT_CX_DUX_MAX_RUNTIME_MS`); action manual `liberarActCxDuxTrabadoAction`. UI progreso: **`MensajeProceso` variant `sidebar`** en **`SyncStatusIndicator`** (mismo slot que sync productos; prioridad sobre botón sync); doble clic (editor) libera bloqueo; polling cliente consulta estado ≥ 5 s. Lectura estado: `getActCxDuxStatusAction` con permiso **`PERMISOS.tienda.acciones.sincronizar`** (sidebar bloquea sync para simple y editor). Al finalizar: modal **«Se actualizaron X ítems»** + botón **Descargar PDF Resumen de Aumentos** (`ActCxButton`). Actions: `getActCxDuxStatusAction`, `enviarCostoCxDuxAction`, `consultarEstadoCostoCxDuxAction`, `finalizarActCxDuxExitoAction`, `abortarActCxDuxAction`, `liberarActCxDuxTrabadoAction`.
