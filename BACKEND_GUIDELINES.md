...
- **Cheques** (`fin_tesoreria_cheques`, Prisma `FinTesoreriaCheque`; migración **`20260515190000_fin_tesoreria_cheques_transferencia_historial`** — `fecha_transferencia`, `caja_destino_id` → `fin_tesoreria`, `onDelete: SetNull`; migración **`20260516120000_fin_tesoreria_cheques_entrega_proveedor`** — `entrega_proveedor` → `global_proveedores.id`, `onDelete: SetNull`, solo referencia a proveedores con `proveedor_mercaderia = true` validada en servicio):
  ...
  - **Transferencia** a caja **DIGITAL**: incrementa saldo destino y **marca** `fecha_transferencia` + `caja_destino_id` (no borra la fila). Retención **500 días** (`CHEQUE_TESORERIA_DIAS_RETENCION_TRAS_TRANSFERENCIA` en `src/lib/finTesoreriaChequesRetencion.ts`); **`eliminarChequesTransferidosVencidos()`** purga filas más viejas (se llama al listar y tras transferir).
  - **Listado** `listarChequesPorCajaId(cajaId, vista)` con `vista` `actuales` | `entregados` (`listarFinTesoreriaChequesPorCajaSchema`). Editar/eliminar rechazan cheques ya transferidos.
  ...
  - **Solo entrega proveedor (sin transferir)**: `marcarEntregaProveedorFinTesoreriaCheque` + `marcarEntregaProveedorFinTesoreriaChequeAction` + `marcarEntregaProveedorChequeSchema` — cheque con `fecha_transferencia` null; persiste `entrega_proveedor` (validación mercadería como en alta/transferencia).
