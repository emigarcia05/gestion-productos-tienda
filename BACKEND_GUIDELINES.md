...
- **POST DUX costos** (`actualizarCostoCxDux.service.ts` + `duxItemModificarApi.ts` + `actCxDuxStatusDb.ts`): lotes de **`DUX_API_BATCH_SIZE` (50)** ítems por POST; pausa **`DUX_API_BATCH_INTERVAL_MS` (5 s)** entre POST y entre GET de estado (§1.10c). **UI:** el cliente encadena **`iniciarActCxDuxAction`** → **`enviarLoteCostoCxDuxAction`** (un POST por invocación, evita timeout serverless con ~1800+ ítems) → **`consultarEstadoCostoCxDuxAction`** por lote (polling ≥5 s, hasta **120** intentos ≈10 min/lote). Progreso sidebar actualizado tras cada POST y al finalizar cada lote en DUX. Mutex TTL **2 h** (`ACT_CX_DUX_MAX_RUNTIME_MS`). Actions: `iniciarActCxDuxAction`, `enviarLoteCostoCxDuxAction`, `consultarEstadoCostoCxDuxAction`, `finalizarActCxDuxExitoAction`, `abortarActCxDuxAction`, `liberarActCxDuxTrabadoAction`. Legacy bloqueante: `enviarCostoCxDuxAction` / `enviarCostosCxADux` (no usar con muchos ítems).
...
*Última actualización (2026-06-04): **Act. Cx. POST por lotes** — cliente encadena `iniciarActCxDuxAction` + `enviarLoteCostoCxDuxAction` (evita timeout con ~1800 ítems); progreso sidebar por lote; mutex TTL 2 h; poll 120×5 s por lote.*
