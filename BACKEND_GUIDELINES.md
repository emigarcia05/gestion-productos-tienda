...
| Marketing | `MktPublicacionRed`, `MktPublicacionTipo`, `MktPublicacionContenidoTipo`, `MktPublicacion`, `MktPublicacionIdeaSeccion`, `MktPublicacionIdeaDetalle` | `mkt_publi_tipo_redes`, `mkt_publi_tipo_publicacion`, `mkt_publi_tipo_contenido`, `mkt_publi`, `mkt_publi_ideas_secciones`, `mkt_publi_ideas_detalle` |
...
Dominio **Marketing · Publicaciones**. Tres catálogos **independientes** (sin relación entre sí) + hechos. CRUD desde **Calendario De Publicaciones**. Renombre tablas: migración **`20260714240000_mkt_publi_rename_tables`**.
...
- **`mkt_publi_tipo_publicacion`** (Prisma `MktPublicacionTipo`): `id` (`cuid`), `tipo_publicacion_nombre` (`TEXT` **único**, MAYÚSCULAS), `created_at`, `updated_at`.
...
- **N:M redes / tipos** (mín. 1 cada uno al guardar): `mkt_publicaciones_ideas_detalle_redes` (`MktPublicacionIdeaDetalleRed`) y `mkt_publi_puente_ideas_detalle_tipos` (`MktPublicacionIdeaDetalleTipo`; renombre **`20260714250000`**); FKs detalle **Cascade**, catálogo **Restrict**. Migración N:M original **`20260714220000_mkt_ideas_detalle_red_tipo_nm`**.
