...
- **Vínculos** (`src/actions/vinculos.ts`): `getVinculos` devuelve `{ productos, codExtCostoLista, esProductoPropio }`. Tras `vincularProducto`: `autoAsignarCodExtCostoListaTrasVincular` (solo si FK vacía: un candidato o match DUX). Tras `desvincularProducto`: `limpiarCodExtCostoListaSiCoincide` si la FK apuntaba a ese `cod_ext`. Elección explícita de costo en UI:
  ...
  - **`establecerCostoListaTiendaAction`** (`vinculos.ts`, gate `PERMISOS.tienda.acceso` + editor) usado desde la columna **BASE** del modal **Vínculos Con Proveedores**; firma `(itemTiendaCod, productoListaCodExt: string | null)`: si `codExt` es `string` valida y persiste `cod_ext_costo_lista`; si es `null` llama `limpiarCodExtCostoLista` (destildar = Cx. Prom.). Revalida `/tienda`, `/gestion-productos/tienda/comp-proveedores` y `/gestion-productos/tienda/cx-px-tienda`.
- **Cx & Px Tienda** (`src/actions/cxPxTienda.ts`): `guardarCostoCxProdTiendaAction` / `guardarPxListaTiendaAction` (FK `cod_ext_costo_lista`; px lista → `competencia_id_px_lista` + `px_lista_cx_px`). Filtros URL: `q`, `marca`, `vincCosto`, `costoProv`, `pxLista`, `marcacionOrden`. Semántica: **`marca`** filtra por `prod_precios_tienda.marca`; **`vincCosto`** cuenta vínculos en `prod_precios_provee` por `cod_tienda_vinculo` (**sin** filtrar `habilitado`) y considera `es_producto_propio=true` como vinculado de empresa (`sin` = no propio + 0 vínculos, `uno` = 1 vínculo o propio, `mas` = 2+ vínculos); **`costoProv`** aplica por `id_proveedor` de la fila configurada en `cod_ext_costo_lista`; **`pxLista`** aplica por `id_proveedor` de la competencia configurada en `competencia_id_px_lista`. Para **`costoProv=prom`** y **`pxLista=prom`** el filtrado se hace con la **selección calculada** de la grilla (`ItemCxPxTiendaParaTabla.seleccion` / `seleccionPxLista`), no solo por FK nula. Sin filtro **rubro**. **`exportarCostoCxDiffAction`** (`exportCostoCxDiff.service.ts`): control de costos — compara `costo_compra` (DUX) con `costoCxProdMostrado` (mismo valor que **CX PROD.** en grilla): proveedor elegido → `px_compra_final_sin_iva`; **CX. PROM.** → promedio de vínculos habilitados (`calcularCostoPromedioVinculos`). Solo diferencias en Excel (**CODIGO**, **COSTO** redondeado). **`exportarPxListaCxDiffAction`**: Excel Px diff (**CODIGO**, **PORC UTILIDAD** = marcación); solo filas con `px_lista_tienda` DUX ≠ PX LISTA de la grilla (`exportPxListaCxDiff.service.ts` + `cxPxTiendaRows.service.ts`).
...
| `src/actions/vinculos.ts` | `vincularProducto` / `desvincularProducto`: `listaPreciosCodTiendaSchema` + `listaPreciosCodExtSchema` (`@/lib/validations/common`). `desvincularProducto` limpia `cod_ext_costo_lista` si apuntaba al `cod_ext` desvinculado; no bloquea por proveedor oficial DUX. |
