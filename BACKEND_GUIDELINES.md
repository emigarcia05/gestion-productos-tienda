...
- **Precios por lista** (origen de verdad): **`prod_listas_precios_tienda`** — PK `(cod_tienda, id_lista)`, `precio DECIMAL(14,4)`, FK → `prod_tienda` ON DELETE CASCADE. Modelo **`ProdListaPrecioTienda`**.
...
- **Convivencia con DUX**: `costo_compra` sigue siendo **espejo DUX** en `prod_tienda` (`syncListaPrecioTienda.service.ts`). Precios de venta por lista → **`prod_listas_precios_tienda`** (§1.4.3). Desde **2026-05-28** **`proveedor` y `cod_ext` quedaron congelados** (ya no se actualizan por DUX; solo conservan valores legacy — ver §1.4.2). El sync **no** escribe ni borra `costo_compra_cod_ext` ni **`es_producto_propio`**. Migración **`20260528210000`** retiró `px_lista_cx_px` y `cx_px_px_comp_ref`; **`es_producto_propio`** se **restauró** en **`20260530120000`** para **Cx Compra** (ver § Producto propio). **`prod_precios_competencia`** no se modifica.
