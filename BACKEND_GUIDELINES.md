...
- **Google Sheets (service account)**: `GOOGLE_SERVICE_ACCOUNT_EMAIL`, `GOOGLE_PRIVATE_KEY` (normalizar `\n` literales vía `normalizeGooglePrivateKey` en `@/lib/googleSheets.ts`), `GOOGLE_SHEETS_SPREADSHEET_ID`. Auth JWT scope `spreadsheets`. Probe de conexión: `probarConexionGoogleSheets` (`src/services/googleSheetsProbe.service.ts`) escribe marca en `A1`; Action `probarConexionGoogleSheetsAction` solo `esEditor()`. Script local: `npm run test:google-sheets`. El Sheet debe estar compartido como **Editor** con el `client_email` de la service account.
