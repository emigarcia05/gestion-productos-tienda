...
- **UI**: `FinAnaMargenContribucionPageClient`, `TablaFinAnaMargenContribucion`, `ElegirProductoMargenContribucionModal`. Layout de filas (`FIN_ANA_MC_LAYOUT`): **PX LISTA** → **DESCUENTO** → **PX VENTA** → espacio → **IVA** / **IIBB** / **CX MERCADERÍA** / **CX FINANCIERO** → subtotal costos → **M.C** (= PX VENTA − subtotal costos por forma de pago).
