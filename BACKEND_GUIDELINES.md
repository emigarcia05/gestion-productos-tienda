...
- **Google Sheets (service account)**: `GOOGLE_SERVICE_ACCOUNT_EMAIL`, `GOOGLE_PRIVATE_KEY` (normalizar `\n` literales vía `normalizeGooglePrivateKey` en `@/lib/googleSheets.ts`), `GOOGLE_SHEETS_SPREADSHEET_ID`. Auth JWT scope `spreadsheets`. Probe: `probarConexionGoogleSheets` escribe marca en `A1`. Helpers de escritura: `@/lib/googleSheetsWrite.ts` (`ensureGoogleSheetTab`, `replaceGoogleSheetTabValues`). Export **Secciones**: `exportarMktSeccionesAGoogleSheets` (`mkt_publi_ideas_secciones` → pestaña `Secciones`, columnas `id` / `idea_nombre` / `idea_resumen`; sobrescribe la pestaña). Actions solo `esEditor()`: `probarConexionGoogleSheetsAction`, `exportarMktSeccionesGoogleSheetsAction`. Script: `npm run test:google-sheets`. El Sheet debe estar compartido como **Editor** con el `client_email` de la service account.
...
| Marketing | `MktPublicacionRed`, `MktPublicacionContenidoTipo`, `MktPublicacion`, `MktPublicacionIdeaSeccion`, `MktPublicacionIdeaDetalle`, `MktPublicacionObj` | `mkt_publi_tipo_redes`, `mkt_publi_tipo_contenido`, `mkt_publi`, `mkt_publi_ideas_secciones`, `mkt_publi_ideas_detalle`, `mkt_publi_obj` |
...
- **`mkt_publi`** (Prisma `MktPublicacion`): hecho de publicación. Campos: `fecha` (`DATE`, día del calendario; se define al crear desde el Calendario), `publicacion` (`TEXT`, contenido; default `''`), `contenido_creado` (`BOOLEAN`, SI/NO; default **NO**; migración **`20260714270000_mkt_publi_contenido_creado`**), FKs independientes a **red** y **tipo contenido** (`onDelete: Restrict`), **`idea_detalle_id`** opcional **único** → `mkt_publi_ideas_detalle` (`onDelete: Restrict`; migración **`20260714280000_mkt_publi_idea_detalle`**): 1 publicación por idea. Índice `mkt_publi_fecha_idx`. Migración columnas fecha/texto **`20260714260000_mkt_publi_fecha_publicacion`**. Históricas: **`20260713180000`**, **`20260714120000`**, **`20260714140000`**, baja puente N:M contenido **`20260714180000`**, renombre **`20260714240000`**, drop tipo publicación **`20260715120000`**.
...
*Última actualización (2026-07-15): **Google Sheets — export Secciones** — pestaña `Secciones` desde `mkt_publi_ideas_secciones` (`id`, `idea_nombre`, `idea_resumen`); clear+write; botón **Exportar Secciones** en Calendario.*

*Última actualización (2026-07-15): **Google Sheets — probe de conexión** — ENV service account + `probarConexionGoogleSheets` / Action (solo `esEditor`); escribe `A1`; script `npm run test:google-sheets`. Export de datos de negocio pendiente.*
