...
| `src/app/api/sync-lista-precios-tienda/route.ts` | `GET` y `POST` validan `puede(rol, PERMISOS.tienda.acciones.sincronizar)` (simple y editor); evitan doble ejecución y persisten progreso/resultado vía helper. **`export const maxDuration = 300`** (sync larga en Vercel). Progreso en dos fases: `sincronizando` (paginado DUX) y `guardando` (persistencia Neon; `processed` vuelve a 0 al cambiar fase — el cliente debe mostrar **`GUARDANDO PROD.`**, no interpretarlo como reinicio). Ante `SyncListaPrecioTiendaCancelledError` limpia estado con `clearListaPrecioTiendaSyncRunningStateInDb` y responde `200` con `cancelled: true` **sin** tocar `lastCompletedAt`. |
