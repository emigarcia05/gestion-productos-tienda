...
| Finanzas análisis M.C. | `FinAnaCosFinaTerminal`, `FinAnaCosFinaPagoCat`, `FinAnaCosFina`, `FinAnaMcDescuentoFp`, `FinAnaMcFormula`, `FinAnaMcCategoria` | `fin_ana_cos_fina_terminales`, `fin_ana_cos_fina_pagos`, `fin_ana_cos_fina`, `fin_ana_mc_descuento_fp`, `fin_ana_mc_formulas`, `fin_ana_mc_categorias` |
...
- **Gráfico M.C vs PORC. UTILIDAD**: helpers en `src/lib/finAnaMargenContribucion.ts` — `idFormaPagoTresCuotasMargenContribucion` (default **3 CUOTAS**), `serieMcVsPorcUtilidadMargenContribucion` / `mcPctEnPorcUtilidadMargenContribucion` con métrica **`CX_MERCADERIA` | `CX_FINANCIERO` | `MC` | `MC_PONDERADO`** (eje X **20…200** step 5; etiquetas `ETIQUETA_METRICA_GRAFICO_MC`). UI `GraficoMcVsPorcUtilidad`: select métrica + tabla checklist de formas; series con `COLORES_SERIE_GRAFICO_MC`. Reacciona a **TERMINAL**, **TIPO COMPROBANTE**, **PORC. UTILIDAD** y descuentos. Sin dependencia de charts externas.
...
*Última actualización (2026-07-24): **Margen Contribución · categorías** — tabla `fin_ana_mc_categorias` (`desde_pct`/`hasta_pct`); rangos `[desde, hasta)` con continuidad obligatoria (sin huecos/solapes; cobertura 0…100); semilla MUY BAJO…MUY ALTO.*
