...
- **`mkt_publi_ideas_detalle`** (Prisma `MktPublicacionIdeaDetalle`): `id` (`cuid`); `seccion_id` FK → secciones (**`onDelete: Cascade`**); `titulo_idea` (`TEXT` corto); `detalle` (`TEXT` largo); `tipo_contenido_id` → contenido (**`onDelete: Restrict`**); `usada` (default **NO** al crear); `created_at`, `updated_at`.
...
*Última actualización (2026-07-14): **Marketing · Ideas detalle** — FKs a red / tipo / contenido; `usada` siempre NO al crear (`20260714200000_mkt_ideas_detalle_catalogos`).*
