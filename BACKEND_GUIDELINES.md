...
### 1.4.3 Listas de precio DUX (`prod_tienda` + `prod_listas_*`)

- **Tabla producto tienda**: SQL **`prod_tienda`** (rename `20260604120000`; antes `prod_precios_tienda`). Prisma **`ProdTienda`** → `prisma.prodTienda`. PK **`cod_tienda`**. Sin columna **`px_lista_tienda`** (DROP `20260604130000`).
- **Precios por lista** (origen de verdad): **`prod_tienda_listas_precios`** — PK `(cod_tienda, id_lista)`, `precio DECIMAL(14,4)`, FK → `prod_tienda` ON DELETE CASCADE. Modelo **`ProdListaPrecioTienda`**.
- **Catálogo listas DUX**: **`prod_listas_dux`** — PK `id_lista` (entero API), `nombre`, `activa`, `ultima_sync`. Modelo **`ProdListaDux`**. Se alimenta en sync desde cada `precios[]` del ítem; al cerrar la corrida, listas no vistas → `activa = false`.
- **API / parseo** (`duxApi.ts`): misma URL de ítems; `mapItem` arma `ItemDux.precios[]` (`idLista`, `nombre`, `precio`) desde el JSON. Lista **principal** para lecturas legacy en UI: `getIdPrecioListaPrincipal()` (`DUX_ID_PRECIO_LISTA`, default **56994**); en sync cada entrada de `precios[]` se upsertea en `prod_tienda_listas_precios`; por ítem se borran filas cuyo `id_lista` ya no vino en el array.
- **Sync** (`syncListaPrecioTienda.service.ts`): Fase 2 — upsert `prod_tienda` + `prod_listas_dux` + `prod_tienda_listas_precios` en la misma transacción por chunk. Fase 3 — `deleteMany` de `cod_tienda` ausentes (CASCADE en precios por lista). Fase 4 — marcar `prod_listas_dux.activa = false` fuera del set visto.
...
- **SQL raw** (`competenciaPreciosFiltrosQuery.ts`): JOIN `prod_tienda_listas_precios pl` con `pl.id_lista = getIdPrecioListaPrincipal()` en lugar de `t.px_lista_tienda`.
- **Migraciones**: `20260604120000_prod_tienda_listas_dux` (rename + tablas nuevas + backfill inicial lista 56994); `20260604130000_drop_prod_tienda_px_lista_tienda`; `20260604140000_reconcile_purge_legacy_db_objects`; `20260604160000_rename_prod_tienda_listas_precios` (tabla `prod_listas_precios_tienda` → **`prod_tienda_listas_precios`**).
...
| Productos / precios | `ListaPrecioProveedor`, `ComparacionDtoExtraItem`, `CategoriaComparacion`, `SubcategoriaComparacion`, `PresentacionComparacion`, `Marca`, `ProdListaDux`, `ProdListaPrecioTienda`, `ProdDepositoDux`, `ProdTiendaStock`, `ProdTienda` | `prod_precios_provee`, `prod_comp_dto_extra`, `prod_comp_cat`, `prod_comp_sub_cat`, `prod_comp_presentaciones`, `prod_marcas`, `prod_listas_dux`, `prod_tienda_listas_precios`, `prod_depositos_dux`, `prod_tienda_stock`, `prod_tienda` |
...
- **Convivencia con DUX**: `costo_compra` sigue siendo **espejo DUX** en `prod_tienda` (`syncListaPrecioTienda.service.ts`). Precios de venta por lista → **`prod_tienda_listas_precios`** (§1.4.3). Desde **2026-05-28** **`proveedor` y `cod_ext` quedaron congelados** (ya no se actualizan por DUX; solo conservan valores legacy — ver §1.4.2). El sync **no** escribe ni borra `costo_compra_cod_ext` ni **`es_producto_propio`**. Migración **`20260528210000`** retiró `px_lista_cx_px` y `cx_px_px_comp_ref`; **`es_producto_propio`** se **restauró** en **`20260530120000`** para **Cx Compra** (ver § Producto propio). **`prod_precios_competencia`** no se modifica.
