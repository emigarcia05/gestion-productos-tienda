...
- **`getPedidoUrgenteData`**: debe cargar **`getProveedoresParaPedidoUrgente`** y luego **`getListaPreciosParaPedidoUrgente`** sin acoplarse en un único **`Promise.all`**: si el listado falla, igual se devuelve el catálogo de proveedores (antes un rechazo combinado podía vaciar ambos payloads y la pantalla perdía el desplegable **PROVEEDOR**). Con **sucursal** válida se llama a **`getListaPreciosParaPedidoUrgente`**; proveedor y `q` (≥ 3 caracteres) son opcionales para filtrar. El parámetro `pedido` soporta `cualquier`, `urgente` y `reposicion`: **`urgente`** y **`cualquier`** usan **`getListaPedidoUrgenteDesdeListaPrecios`**: listado desde **`prod_precios_provee`** (**`habilitado = true`**) con **agrupación por `codTiendaVinculo`**: varias filas con el mismo vínculo a **`prod_precios_tienda`** se devuelven como **un** ítem (`miembrosAgrupacion` con cada `cod_ext`; **`prefijo`** vacío y **`descripcion`** = **`descripcion_tienda`** unificada entre miembros); **`total`** / **`totalPaginas`** cuentan **filas de vista** (grupos), no filas crudas. Orden estable por menor `cod_ext` del grupo. Cantidades y reposición por sucursal vía **`mercaderiaMapsDesdeMerc2`**. **`reposicion`** acota solo por **`cod_tienda`** donde haya reposición configurada/elegible en **`prod_ped_merc`**, igual que antes.
- **Comparación «menor costo» / proveedor recomendado (Pedido Urgente, filtros `urgente` / `cualquier`):** ante doble clic, si hay más de una fila habilitada con el mismo **`cod_tienda`** (**`codTiendaVinculo`**, mismo vínculo a **`prod_precios_tienda`**, p. ej. REX-20 y Mer-50), el ranking usa **`pxComparablePedidoUrgenteReposicion`** (misma regla que el filtro Reposición en esa pantalla): balance acumulado IVA Posición IVA (`sumarIvaSaldoAcumuladoParaComparacionProveedoresPedido`); si **IVA SALDO ≥ 0**, comparable = **`px_compra_final_sin_iva`**; si **IVA SALDO < 0**, comparable = precio final **con** IVA según `Proveedor.iva` (**×1,21** si `SIEMPRE`, **×1** si **NUNCA** o **PREGUNTA**). La respuesta **`sugerenciaProveedorMenorCosto.costo`** sigue siendo valor **sin IVA** para el modal de cantidad. Si la fila está **agrupada** (`miembrosAgrupacion`), el usuario elige primero proveedor en la UI y la sugerencia se calcula para el **`cod_ext`** de ese miembro.
- **Comparación «menor costo» (filtro Reposición en la misma pantalla):** regla **`pxComparablePedidoUrgenteReposicion`** según **`sumarIvaSaldoAcumuladoParaComparacionProveedoresPedido`** (abr. 2026 → mes actual AR; error BD → **`0`** cuenta como **≥ 0**): si **IVA SALDO ≥ 0**, comparable sin IVA; si **< 0**, comparable con IVA como arriba. Implementación en **`getListaPreciosParaPedidoUrgente`** (rama reposición) con `proveedor.iva` en `select`/`include`.
