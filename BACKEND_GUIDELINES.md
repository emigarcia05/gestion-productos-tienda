...
- **Modo producto**: búsqueda `prod_tienda` vía `buscarProductosMargenContribucionAction`; al elegir, `getProductoDatosMargenContribucion` devuelve `codTienda`, `descripcionTienda`, `costoCompra`, `pxLista` (lista principal DUX, `getPrecioListaPrincipal`) y `porcUtilidadPct` (`margenDesdePrecioDux`). PX LISTA en tabla solo lectura. **CX MERCADERÍA** = `costoCompra` de BD (fijo; no se recalcula por forma de pago).
- **Modo porc. utilidad**: usuario ingresa % en filtros; **PX LISTA** fijo en **$100** (`FIN_ANA_MC_PX_LISTA_ESTIMADO_PORC_UTILIDAD`); **CX MERCADERÍA** = `(PX LISTA / 1,21) / (1 + porc. utilidad % / 100)` (`cxMercaderiaDesdePorcUtilidadMargenContribucion`; mismo valor en todas las columnas).
...
- **Filas / fórmulas** (`src/lib/finAnaMargenContribucion.ts`): **PRECIO VENTA** = `PX LISTA × (1 + descuento % / 100)` (descuento por forma de pago, entero **−100…100**, persistido en `fin_ana_mc_descuento_fp`; **negativo = descuento** / baja el precio, **positivo = recargo**; UI tipea negativo por defecto salvo `+` explícito; migración de signo **`20260721140000_fin_ana_mc_descuento_sign_convention`**); **IVA** = `(PRECIO VENTA / 1,21) × 0,21` (si comprobante aplica); **IIBB** = `(PRECIO VENTA / 1,21) × 0,04` (si comprobante aplica); **CX MERCADERÍA** según modo (arriba); **CX FINANCIERO** = **`cxTotalConIvaFinAnaCosFina`** por forma de pago desde `fin_ana_cos_fina` (solo filas `habilitado = true`). Filtro **TERMINAL** opcional en UI: si se elige terminal, una fila por `pago_id`; si no, **promedio** entre terminales habilitadas del mismo pago.
...
- **UI**: `FinAnaMargenContribucionPageClient`, `TablaFinAnaMargenContribucion`, `ElegirProductoMargenContribucionModal`, **`GestionarPagosFinAnaCosFinaModal`** (header **Gestionar Pagos**). Layout de filas (`FIN_ANA_MC_LAYOUT`): **PX LISTA** → **DESCUENTO** → **PX VENTA** → **línea divisora primary** (`.tabla-fila-mc-sep-linea`, misma marca que el borde del subtotal; reemplaza la fila hueca) → **IVA** / **IIBB** / **CX MERCADERÍA** / **CX FINANCIERO** → subtotal costos (`.tabla-fila-mc-subtotal`) → **M.C** (= PX VENTA − subtotal costos por forma de pago). **Formato de montos**: modo **`porc_utilidad`** → celda única `N%` (`fmtCeldaMontoMargenContribucion`; PX LISTA = 100). Modo **`producto`** → **2 columnas por forma de pago** (encabezado `colSpan={2}`): pesos (`fmtPesosMargenContribucion`, 75% ancho, alineados a la derecha) + % sobre PX LISTA (`fmtPctSobrePxListaMargenContribucion`, 25%, **negrita**, alineados a la derecha). **DESCUENTO** permanece % firmado. **CX FINANCIERO** en grilla = pesos (`cxFinancieroPesosMargenContribucion`), no la tasa %.
