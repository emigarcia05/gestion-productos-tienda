...
- **Cx & Px Tienda** (`src/actions/cxPxTienda.ts`): `guardarCostoCxProdTiendaAction` / `guardarPxListaTiendaAction` (FK `cod_ext_costo_lista`; px lista → `competencia_id_px_lista` + `px_lista_cx_px`). Filtros URL: `q`, `marca`, `vincCosto`, `costoProv` (`prom` = Cx. Prom.), `pxLista` (`prom` = Px. Prom. / FK nula; si no `competenciaId`), `marcacionOrden` (`menor_mayor` \| `mayor_menor` — ordena en servidor tras calcular marcación de cada fila). Sin filtro **rubro**. **`exportarCostoCxDiffAction`**: Excel Cx diff (`costo_compra` ≠ costo de `cod_ext_costo_lista`). **`exportarPxListaCxDiffAction`**: Excel Px diff (**CODIGO**, **PORC UTILIDAD** = marcación); solo filas con `px_lista_tienda` DUX ≠ PX LISTA de la grilla (`exportPxListaCxDiff.service.ts` + `cxPxTiendaRows.service.ts`).
