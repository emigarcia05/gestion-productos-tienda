...
- **Servicio objetivos** (`src/services/mktPublicacionesObj.service.ts`): `listarMktPublicacionObjs`, `crearMktPublicacionObj`, `editarMktPublicacionObj` (periodo + cantidad; el eje/destino no cambia), `eliminarMktPublicacionObj`, `evaluarMktPublicacionObjs` (ventana semana/mes; cuenta hechos programados por eje). Evaluación en UI del Calendario: helpers cliente `@/lib/mktPublicacionesObj.ts` (`evaluarMktPublicacionObjsCliente`, `periodoObjParaSemanaFiltro`, `textoIncumplimientoObjetivo`). Validación: `@/lib/validations/mktPublicacionesObj.ts`. Actions: `src/actions/mktPublicacionesObj.ts` (lectura marketing; mutaciones + `esEditor`; `revalidatePath` calendario + **objetivos**). UI de gestión: página **`/marketing/publicaciones/objetivos`** (`MarketingObjetivosPageClient`), no modal.
