...
- **Convivencia con DUX**: `costo_compra` sigue siendo **espejo DUX** en `prod_tienda` (`syncListaPrecioTienda.service.ts`). Precios de venta por lista → **`prod_tienda_precios`** (§1.4.3). Desde **2026-05-28** **`proveedor` y `cod_ext` quedaron congelados** (ya no se actualizan por DUX; solo conservan valores legacy — ver §1.4.2). El sync **no** escribe ni borra `costo_compra_cod_ext` ni **`es_producto_propio`**. Migración **`20260528210000`** retiró `px_lista_cx_px` y `cx_px_px_comp_ref`; **`es_producto_propio`** se **restauró** en **`20260530120000`** para **Cx Compra** (ver § Producto propio). **`prod_precios_competencia`** no se modifica.
...
- **POST DUX costos** (`actualizarCostoCxDux.service.ts` + `duxItemModificarApi.ts`): reemplazo programático del Excel de control de costos. Endpoint v1 **POST** `/item/nuevoItem` ([Modificar Precios/Productos](https://duxsoftware.readme.io/reference/modificar-preciosproductos-1)); por ítem **`cod_item`**, **`costo`** (entero CX PROD.) y **`precios: []`** (array vacío, contrato probado). Misma selección que export (`listarFilasExportCostoCxDiff`). Respuesta asíncrona → polling **GET** `/obtenerEstadoItem?idProceso=`. Lotes de **100** ítems con pausa **`DUX_ITEM_MODIFICAR_MIN_INTERVAL_MS`** (≥ 5 s). Action **`actualizarCostoCxDuxAction`**; UI botón **Metodo Post** convive con **Exportar Cx** (Excel).
...
- **`cxPxTienda.ts` (actions)**: `guardarCostoCxProdTiendaAction`, `exportarCostoCxDiffAction`, **`actualizarCostoCxDuxAction`**. Permiso `PERMISOS.cxPxTienda.acceso` (solo **editor**). FK `costo_compra_cod_ext` (ver §1.10b). **`costoListaTienda.service.ts`**, **`cxPxTiendaRows.service.ts`**, **`actualizarCostoCxDux.service.ts`**, **`duxItemModificarApi.ts`**.
