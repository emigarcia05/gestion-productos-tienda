...
- **Lecturas con datos sensibles** (precios, vínculos, catálogos):
  ...
  - **Vínculos tienda** (`getVinculos`, `listarProductosParaVincular` en `vinculos.ts`): `getRol()` + `puede(rol, PERMISOS.tienda.acceso)`; ítem tienda por `cod_tienda` (`listaPreciosCodTiendaSchema`); línea lista proveedor por `cod_ext` (`listaPreciosCodExtSchema`). Filtros de búsqueda acotados con Zod en la Action cuando aplique.
...
- **Cheques** (`fin_tesoreria_cheques`, Prisma `FinTesoreriaCheque`; migración **`20260511143000_fin_tesoreria_cheques_tenencia`** — enum Postgres `TenenciaChequeTesoreria` (`TIENDA` | `DEPOSITADO` | `PROVEEDOR`), columna `tenencia` NOT NULL default `TIENDA` (custodia del cheque; **no** confundir con el campo texto **tenedor** = titular de caja); migración **`20260515190000_fin_tesoreria_cheques_transferencia_historial`** — `fecha_transferencia`, `caja_destino_id` → `fin_tesoreria`, `onDelete: SetNull`; migración **`20260516120000_fin_tesoreria_cheques_entrega_proveedor`** — `entrega_proveedor` → `global_proveedores.id`, `onDelete: SetNull`, solo referencia a proveedores con `proveedor_mercaderia = true` validada en servicio):
  ...
  - **Transferencia** a caja **DIGITAL**: incrementa saldo destino y **marca** `fecha_transferencia` + `caja_destino_id` y **`tenencia = DEPOSITADO`** (no borra la fila). Retención **500 días** (`CHEQUE_TESORERIA_DIAS_RETENCION_TRAS_TRANSFERENCIA` en `src/lib/finTesoreriaChequesRetencion.ts`); **`eliminarChequesTransferidosVencidos()`** purga filas más viejas (se llama al listar y tras transferir).
  - **Listado** `listarChequesPorCajaId(cajaId, vista, tenenciaFiltro)` con `vista` `actuales` | `entregados` y `tenenciaFiltro` `actuales` (solo `tenencia = TIENDA`) | `depositados` (`tenencia IN (DEPOSITADO, PROVEEDOR)`), default `actuales` (`listarFinTesoreriaChequesPorCajaSchema` + `finTesoreriaChequesTenenciaFiltroSchema`). Editar/eliminar rechazan cheques ya transferidos.
  - **Proveedores mercadería para UI de cheques**: `listarProveedoresMercaderiaParaChequeTesoreriaAction` (`finTesoreriaCheques.ts`) — gate `PERMISOS.finanzas.acceso` (no exige permisos del catálogo Lista Proveedores).
  - **Solo entrega proveedor (sin transferir)**: `marcarEntregaProveedorFinTesoreriaCheque` + `marcarEntregaProveedorFinTesoreriaChequeAction` + `marcarEntregaProveedorChequeSchema` — cheque con `fecha_transferencia` null; persiste `entrega_proveedor` y **`tenencia = PROVEEDOR`** (validación mercadería como en alta/transferencia).
...
*Última actualización (2026-05-11): **Cheques tesorería — destino cheque** — `marcarEntregaProveedorFinTesoreriaCheque` + Action + Zod `marcarEntregaProveedorChequeSchema` (solo `entrega_proveedor`, cheque no transferido). Ver §2.5c (cheques).*

*Última actualización (2026-05-16): **Cheques tesorería — entrega proveedor** — columna **`entrega_proveedor`** (FK `global_proveedores`, mercadería); `listarProveedoresMercaderiaParaChequeTesoreriaAction`; alta/edición/acreditación persisten el registro sin otras acciones contables. Migración **`20260516120000_fin_tesoreria_cheques_entrega_proveedor`**. Ver §2.5c (cheques).*
