...
- **`syncListaPrecioTienda.service.ts`**: deduplica por `cod_tienda` dentro de cada chunk y hace `upsert` con `where: { codTienda }`; si DUX cambia `cod_ext`/`proveedor` para el mismo `cod_tienda`, la misma fila se actualiza (no se crea otra por `cod_ext`). Además, al finalizar la sync elimina de `prod_precios_tienda` los `cod_tienda` que ya no llegaron en la corrida actual desde DUX.
