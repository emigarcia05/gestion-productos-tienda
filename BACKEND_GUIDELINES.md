...
- **POST DUX costos** (`actualizarCostoCxDux.service.ts` + `duxItemModificarApi.ts`): misma selección que export (`listarFilasExportCostoCxDiff`). **Envío:** `enviarCostosCxADux` → POST `/item/nuevoItem` (lotes 100). **Estado:** polling desde UI con `consultarEstadoCostoCxDuxAction` cada ≥ **5 s** (evita timeout de Server Action). `FINALIZADO` abre modal PDF aunque DUX devuelva `errores[]` parciales (se muestran como advertencia). Actions: `enviarCostoCxDuxAction`, `consultarEstadoCostoCxDuxAction`. UI **Act. Cx.**
