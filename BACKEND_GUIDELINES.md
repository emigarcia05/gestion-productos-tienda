...
- **Venc. por fecha (Finanzas)**: `listarVencimientosEnRango(fechaDesde, fechaHasta)` en `src/services/vencimientosPorFecha.service.ts` — mismas líneas y misma `fecha_venc` que arriba; filtra `fecha_venc` en el rango **inclusive** (un mes calendario). La UI agrupa por día (`calendarioMesFinanzas.ts`). La columna **SALDO** en pantalla es **acumulativa** en el mes (primera fila: caja − a pagar; siguientes: saldo anterior − a pagar); **CAJA DISPONIBLE** puede seguir en `0` hasta definir origen de datos.
