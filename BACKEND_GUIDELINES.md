...
- **Edición manual Px Listas (margen %)**: **`prod_tienda_margen_edicion`** — PK `(cod_tienda, id_lista)`, `margen_manual DECIMAL(8,4)`, `updated_at`. Modelo **`ProdTiendaMargenEdicion`**. No la toca el sync; baseline DUX queda en `prod_tienda_precios`. Mutación: `guardarPxListaMargenEdicionAction` (`src/actions/pxListasPrecios.ts`) → `guardarMargenListaEdicion` (`pxListasMargenEdicion.service.ts`); `margenManual: null` elimina el override. **PX en UI** = `calcPxListaDesdeMargenSinIvaPct` si hay margen manual; si no, precio DUX. **Pendiente DUX / filtro Actualizar:** `celdaRequiereActualizar` (`pxListasPreciosCelda.ts`) — compara **precio entero** DUX (`prod_tienda_precios`) vs PX desde margen manual (`preciosPxListaSincronizados`); fallback a `margenesPorcUtilidadDifieren` solo sin PX calculable. Tabla legacy **`prod_tienda_precios_edicion`** (precio override) permanece en esquema pero **no** la usa este módulo desde 2026-06-24.
...
- **`pxListasPrecios.ts` (actions)**: `getPxListasPreciosPageData`, `guardarPxListaMargenEdicionAction`, **`exportarPxListasMargenAction`** — módulo **Px Listas** (`/gestion-productos/tienda/px-listas`; `px-listas-precios/*`). Listado: **`pxListasPreciosPage.service.ts`** — filtros `marca`, `rubro`, `subRubro`, `q`, **`actualizar`** (`si`/`no`: margen manual con precio DUX distinto del PX calculado; `celdaRequiereActualizar`); columnas dinámicas por `prod_tienda_listas_precios` (PX calculado + MARGEN editable por lista). **Export Excel:** **`exportPxListasMargen.service.ts`** — un `.xls` por `nombre_lista` (**CODIGO**, **PORC UTILIDAD**); solo filas pendientes según `celdaRequiereActualizar`. Persistencia edición: **`prod_tienda_margen_edicion`** (migración `20260624160000`). Helpers: **`pxListasPreciosCelda.ts`**, **`pxListasPreciosFiltros.ts`**. Tabla legacy **`prod_precios_tienda_marcacion`** eliminada (`20260528270000`).
