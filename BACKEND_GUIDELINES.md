...
| `src/lib/validations/listaPrecios.ts` | Reexport `listaPreciosCodExtListSchema`; `actualizacionMasivaListaPreciosSchema` con `porcentajeListaPreciosSchema` (0–100, máx. 2 decimales) en `dto_*` y `cxTransporte`; `cotizacionDolar` y `pxListaProveedor` con **`.min(0)`** (0 permitido). **`listaPreciosFiltrosExportSchema`** (lectura/export sin `pagina`). **`crearProductoListaPrecioSchema`** (alta manual: proveedor CUID, código, descripción, px lista, marca opcional). |
