...
- **`mkt_publi_ideas_detalle`** (Prisma `MktPublicacionIdeaDetalle`): `id` (`cuid`); `seccion_id` FK → secciones (**`onDelete: Cascade`**); `titulo_idea` (`TEXT` corto); `detalle` (`TEXT` largo); `tipo_contenido_id` → contenido (**`onDelete: Restrict`**); `usada` (default **NO** al crear; pasa a **SI** al programar en calendario vía `mkt_publi.idea_detalle_id`); relación opcional 1:1 `publicacion` → `MktPublicacion`; `created_at`, `updated_at`.
