...
- **Pantalla** `/finanzas/balance/gastos` (no confundir con el catálogo):
  ...
  - **Página** `src/app/finanzas/balance/gastos/page.tsx`: `searchParams` opcionales `mes` / `anio` (coerción + clamp Zod); default calendario Argentina; lista con `listarImputacionesMensualesBalance`.
