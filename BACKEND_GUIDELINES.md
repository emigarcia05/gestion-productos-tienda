...
- **Tabla** `fin_bal_gasto_final` (Prisma: `FinBalGastoFinal`):
  ...
  - `dia_devengado` (`INTEGER`, CHECK 1–28): día del mes en que se devenga el gasto. En UI (`CrearEditarFinBalGastoFinalModal`), para **TIPO = EVENTUAL** se elige en lista 1..28; para **TIPO = MENSUAL** se bloquea y queda visualmente vacío. Persistencia de seguridad en servicio: si `gasto_mensual = true`, se guarda `dia_devengado = 1`.
  - `plazo_pago_dias` (`INTEGER`): plazo de pago en días entre la fecha del gasto (devengo) y la fecha de pago/vencimiento. En UI, para **TIPO = EVENTUAL** se elige en lista 0..30; para **TIPO = MENSUAL** se bloquea y queda visualmente vacío. Persistencia de seguridad en servicio: si `gasto_mensual = true`, se guarda `plazo_pago_dias = 0`.
- **Tabla** `fin_bal_gasto_mensual` (Prisma: `FinBalGastoMensual`): imputación por mes/año ligada a `fin_bal_gasto_final` (`gasto_final_id`, `mes` 1–12, `anio`, `monto` y `pagado` enteros ≥ 0, `pagado ≤ monto`). UNIQUE `(gasto_final_id, mes, anio)`. Migración `20260421200000_add_fin_bal_gasto_mensual`.
  ...
  - Al guardar una carga eventual, se sincroniza el `fin_bal_gasto_final` asociado con `dia_devengado = min(28, día(fechaGasto))` y `plazo_pago_dias = plazoPago|0` para mantener coherencia del cálculo de vencimientos.
