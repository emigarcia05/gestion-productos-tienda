...
- **POST DUX costos** (`actualizarCostoCxDux.service.ts` + `duxItemModificarApi.ts`): misma selección que export (`listarFilasExportCostoCxDiff`). Endpoint v1 **POST** `/item/nuevoItem`; por ítem **`cod_item`**, **`costo`** y **`precios: []`**. Polling **GET** `/obtenerEstadoItem?idProceso=` con **`throttleDuxItemModificarRequest()`** (≥ **5 s** entre POST y cada consulta de estado; reintento ante **429**). Lotes de **100**. Action **`actualizarCostoCxDuxAction`**; UI **Act. Cx.**
