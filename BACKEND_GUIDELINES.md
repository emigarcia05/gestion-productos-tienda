...
- **Sync**: `syncListaPrecioTienda.service.ts` incluye `stockeable` en create/update del upsert. Desde 2026-05, el upsert de `prod_precios_tienda` se resuelve por **`cod_tienda`** (clave de negocio estable). A partir de **2026-05-28** el sync **deja de escribir `cod_ext` y `proveedor`** en `prod_precios_tienda` (ver §1.4.2): el `create` y `update` del upsert solo escriben `rubro`, `subRubro`, `marca`, `idMarca`, `descripcionTienda`, `costoCompra`, `pxListaTienda`, `stockMaipu`, `stockGuaymallen`, `stockeable` (+ `lastSync` en update).
...
- **Pedido Reposición** (`pedidosReposicionProveedor.service.ts`): se eliminó el fallback legacy por `cod_ext` y el helper `cargarListaPrecioReposicionFallbackPorCodExt`. `elegirListaPrecioProveedorReposicion` solo opera sobre vínculos manuales (`lpPorCodTienda`); sin vínculos → `null`. `existeListaPrecioParaReposicionCodTienda(codTienda)` ya no recibe `{ codExt }`. Se eliminó `pickListaPrecioProveedorPorCodExtYTienda` en `pedidosEnvio.service.ts` (no quedan call sites). `getSobreStockOtraSucursalParaPedidoEnviar` y `getPedidosReposicion` también dejaron de leer `tienda.codExt` / `tienda.proveedor` como fallback.
- **Cx/Px Tienda** (`costoListaTienda.service.ts → resolverCostoCxPxParaFila`): mantiene el orden FK persistida → único candidato habilitado → match DUX (`proveedorTextoCoincideConDux`) → espejo DUX (`costo_compra`, `proveedor`). El match DUX y el espejo siguen apuntando a los **valores históricos** preservados en `cod_ext` / `proveedor`; para ítems creados luego del 2026-05-28 (sin esos valores) la resolución queda reducida a las dos primeras ramas.
- **Producto TiendaColor** (`prod_precios_tienda.es_producto_propio`): independiente del cambio anterior; sigue siendo el flag que excluye ítems del filtro `vinculado=no` y bloquea la apertura de **Vincular Nuevo Producto**.
...
- **Convivencia con DUX**: el sync `syncListaPrecioTienda.service.ts` **no** incluye `esProductoPropio` en el `update`/`create` del upsert (igual patrón que `codExtCostoCompra`, `competenciaIdPxLista`, `pxListaCxPx`); cualquier corrida DUX preserva el valor existente.
...
### 1.10b Costo lista para Cx/Px Tienda (`cod_ext_costo_compra`)

- **Columna** `prod_precios_tienda.cod_ext_costo_compra` (`TEXT NULL`, FK → `prod_precios_provee.cod_ext`). Migración **`20260520120000_add_cod_ext_costo_compra_prod_precios_tienda`**: índice, FK y backfill (único vínculo habilitado por ítem, o match texto DUX ↔ nombre/prefijo proveedor).
- **Convivencia con DUX**: `costo_compra` y `px_lista_tienda` siguen siendo **espejo DUX** (`syncListaPrecioTienda.service.ts`). Desde **2026-05-28** **`proveedor` y `cod_ext` quedaron congelados** (ya no se actualizan por DUX; solo conservan valores legacy — ver §1.4.2). El sync **no** escribe ni borra `cod_ext_costo_compra`, `competencia_id_px_lista` ni `px_lista_cx_px`. La columna **PX LISTA** en Cx/Px persiste **`competencia_id_px_lista`** (FK competidor; `NULL` = Px. Prom., migración `20260524120000`) y el **precio entero** elegido en **`px_lista_cx_px`** (migración `20260525120000`); al guardar, `guardarPxListaTiendaAction` resuelve el valor con la misma lógica que la UI (`pxListaCxPxTienda.service.ts`) y lo escribe en BD. La grilla muestra `px_lista_cx_px` si existe; si no, calcula en lectura.
- **CX. COMPRA** y **PROVEEDOR** en la grilla Cx/Px (`getCxPxTiendaPageData` → `resolverCostoCxPxParaFila` en `src/services/costoListaTienda.service.ts`):
  ...
  2. Si FK null: fallback solo lectura — único candidato habilitado (`cod_tienda_vinculo` + `habilitado`) o fila cuyo proveedor coincide con texto **histórico** DUX (`proveedorTextoCoincideConDux`; solo aplica a filas con `proveedor` legacy — ver §1.4.2).
  3. Si no aplica: espejo DUX (`costo_compra`, `proveedor`) — `proveedor` puede ser `null` para ítems creados luego del 2026-05-28.
- **Candidatos** para asignar o validar: `prod_precios_provee` con `cod_tienda_vinculo = ítem`, `habilitado = true`.
- **Vínculos** (`src/actions/vinculos.ts`): `getVinculos` devuelve `{ productos, codExtCostoCompra, esProductoPropio }`. Tras `vincularProducto`: `autoAsignarCodExtCostoListaTrasVincular` (solo si FK vacía: un candidato o match DUX). Tras `desvincularProducto`: `limpiarCodExtCostoListaSiCoincide` si la FK apuntaba a ese `cod_ext`. Elección explícita de costo en UI:
  - **`guardarCostoCxProdTiendaAction`** (`cxPxTienda.ts`, gate `PERMISOS.cxPxTienda.acceso` + editor) usado desde el dropdown CX PROD. de **Cx & Px Tienda**.
  - **`establecerCostoListaTiendaAction`** (`vinculos.ts`, gate `PERMISOS.tienda.acceso` + editor) usado desde la columna **BASE** del modal **Vínculos Con Proveedores**; firma `(itemTiendaCod, productoListaCodExt: string | null)`: si `codExt` es `string` valida y persiste `cod_ext_costo_compra`; si es `null` llama `limpiarCodExtCostoLista` (destildar = Cx. Prom.). Revalida `/tienda`, `/gestion-productos/tienda/comp-proveedores` y `/gestion-productos/tienda/cx-px-tienda`.
- **Cx & Px Tienda** (`src/actions/cxPxTienda.ts`): `guardarCostoCxProdTiendaAction` / `guardarPxListaTiendaAction` (FK `cod_ext_costo_compra`; px lista → `competencia_id_px_lista` + `px_lista_cx_px`). Filtros URL: `q`, `marca`, `vincCosto`, `costoProv`, `pxLista`, `marcacionOrden`. Semántica: **`marca`** filtra por `prod_precios_tienda.marca`; **`vincCosto`** cuenta vínculos en `prod_precios_provee` por `cod_tienda_vinculo` (**sin** filtrar `habilitado`) y considera `es_producto_propio=true` como vinculado de empresa (`sin` = no propio + 0 vínculos, `uno` = 1 vínculo o propio, `mas` = 2+ vínculos); **`costoProv`** aplica por `id_proveedor` de la fila configurada en `cod_ext_costo_compra`; **`pxLista`** aplica por `id_proveedor` de la competencia configurada en `competencia_id_px_lista`. Para **`costoProv=prom`** y **`pxLista=prom`** el filtrado se hace con la **selección calculada** de la grilla (`ItemCxPxTiendaParaTabla.seleccion` / `seleccionPxLista`), no solo por FK nula. Sin filtro **rubro**. **`exportarCostoCxDiffAction`** (`exportCostoCxDiff.service.ts`): control de costos — compara `costo_compra` (DUX) con `costoCxProdMostrado` (**CX PROD.**; **CX. PROM.** = promedio vínculos). Comparación en **pesos enteros** (`Math.round`, sin decimales). Solo diferencias en Excel (**CODIGO**, **COSTO** entero). **`exportarPxListaCxDiffAction`** (`exportPxListaCxDiff.service.ts`): control de precios de venta — compara `px_lista_tienda` (DUX) con `pxListaMostradoParaSeleccion` (**PX LISTA**; competidor vía `competencia_id_px_lista` → `prod_precios_competencia` / sugerido; **PX. PROM.** = promedio competidores). Comparación en **pesos enteros** con **margen relativo 0,02 %** (`CX_PX_DIFF_PRECIO_MARGEN_PCT` en `cxPxTienda.ts`: |DUX − PX LISTA| ≤ max(DUX, ref) × 0,02/100 no exporta). Solo diferencias en Excel (**CODIGO**, **PORC UTILIDAD** = marcación con ese px y **CX PROD.**). **Backfill** (misma lógica que la grilla): `npm run db:backfill-px-lista-cx-px` → `backfillPxListaCxPx.service.ts` + `scripts/backfill-px-lista-cx-px.ts` (por defecto solo filas con `competencia_id_px_lista` y `px_lista_cx_px` vacíos; `--dry-run`, `--todos`).
...
- **Tabla** `fin_bal_gasto_mensual` (Prisma: `FinBalGastoMensual`): imputación por mes/año ligada a `fin_bal_gasto_final` (`gasto_final_id`, `mes` 1–12, `anio`, `monto` y `pagado` enteros ≥ 0, `pagado ≤ monto`). Columna `gasto_mensual_en_alta`: `true` en filas creadas con **Cargar mes** (catálogo mensual); índice único parcial `(gasto_final_id, mes, anio) WHERE gasto_mensual_en_alta = true` (migración `20260520120000_fin_bal_gasto_mensual_varias_eventuales_mes`). **Eventuales** (`gasto_mensual_en_alta = false`): pueden repetirse en el mismo mes/año y sucursal de imputación.
  - **Carga eventual manual** (`crearImputacionGastoUnicoBalance` / `crearFinBalImputacionGastoUnicoAction`): además de `monto` y `pagado`, recibe `fechaGasto` (`YYYY-MM-DD`) y `plazoPago` (0..30, obligatorio salvo pago total). `fechaGasto` debe pertenecer al `mes/anio` de la carga. Si `pagado === monto`, el plazo deja de ser obligatorio y se persiste como `0`. No hay tope de filas por gasto final en el periodo.
...
3. `getPedidoHistoriaDetalle({ pedidoHistoriaId })`
   ...
   - **`serializarPedidoHistoriaDetalleParaCliente(d)`** (mismo archivo): antes de responder **`getPedidoHistoriaDetalleAction`** *o el route* **`GET /api/pedidos-historia/[pedidoHistoriaId]/detalle`** (`src/app/api/pedidos-historia/[pedidoHistoriaId]/detalle/route.ts`), las fechas `generadoAt` / `registradoAt` deben pasarse a **ISO string**. Devolver objetos Prisma `Date` en el retorno de una Server Action puede romper la serialización del runtime y el cliente recibe solo el error genérico *"An error occurred in the Server Components render"* (producción). El tipo **`PedidoHistoriaDetalle`** declara `generadoAt` y `registradoAt` como `Date | string` (o `null`) para cubrir servicio vs wire. El serializador **no usa spread `...d`**: reconstruye el objeto con primitivos para no arrastrar propiedades del runtime de Prisma. La Action aplica además **`JSON.parse(JSON.stringify(wired))`** como barrera final; si falla, devuelve `{ ok: false, error: ... }` y loggea en consola (evita que el cliente solo vea digest opaco).
   - **Route HTTP (`GET .../detalle`)**: mismo gate que la Action (`getRol` + **`PERMISOS.pedidos.acceso`**), mismo servicio + serializador. La UI de **Recepcion Pedido** y **Ver Pedido** cargan el detalle vía **`fetchPedidoHistoriaDetalle`** (`src/lib/fetchPedidoHistoriaDetalle.ts`) para obtener **JSON puro** (`NextResponse.json`) y evitar el transporte Flight de Server Actions cuando en producción fallara de forma opaca. En errores internos **500**, el servidor responde con **`supportId`** (UUID), cabecera **`x-support-id`** y log **`[api][pedidos-historia][detalle]`** grepeable; el usuario puede copiar la ref. desde el cartel de error y buscar en **Vercel → Logs** ese identificador.
...
- **`cxPxTienda.ts`**: `getCxPxTiendaPageData` — listado paginado con filtros `q`, `marca`, `vincCosto`, `costoProv`, `pxLista`, `marcacionOrden`; `getCxPxTiendaPageParamsSchema` (Zod). Permiso `PERMISOS.cxPxTienda.acceso` (solo **editor**). Regla de filtros: `vincCosto` cuenta vínculos por `cod_tienda_vinculo` en `prod_precios_provee` (no solo habilitados), `costoProv` filtra por `id_proveedor` de `cod_ext_costo_compra`, `pxLista` por `id_proveedor` de la competencia de `competencia_id_px_lista`. FK `cod_ext_costo_compra` y `competencia_id_px_lista` (ver §1.10b). **`costoListaTienda.service.ts`** / **`pxListaCxPxTienda.service.ts`**.
...
| `src/lib/validations/listaPrecios.ts` | Reexport `listaPreciosCodExtListSchema`; `actualizacionMasivaListaPreciosSchema` con `porcentajeListaPreciosSchema` (0–100, máx. 2 decimales) en `dto_*` y `cxTransporte`. |
...
| `src/actions/vinculos.ts` | `vincularProducto` / `desvincularProducto`: `listaPreciosCodTiendaSchema` + `listaPreciosCodExtSchema` (`@/lib/validations/common`). `desvincularProducto` limpia `cod_ext_costo_compra` si apuntaba al `cod_ext` desvinculado; no bloquea por proveedor oficial DUX. |
...
| `src/lib/validations/comparacionCategorias.ts` | CRUD + asignación por `listaPreciosCodExtSchema`; presentación → `productoReferenciaCodExt`. |
