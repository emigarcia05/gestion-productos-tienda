...
*Última actualización (2026-07-03): **Comp. Categorias — búsqueda referencia competencia (todos los competidores)** — con `q` no vacío, `buscarOpcionesReferenciaCompetencia` resuelve primero `cod_tienda` en `prod_tienda` (`comparar_competencia`) y expande **todos** los competidores de ese ítem (`prod_precios_competencia` + `px_vta_sugerido`); `buildMapPxVtaSugerido` siempre como fallback de `pxMostrar` (no solo filas sin vínculo DB).*
