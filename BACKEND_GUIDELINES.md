...
- **POST DUX costos** (`actualizarCostoCxDux.service.ts` + `duxItemModificarApi.ts` + `actCxDuxStatusDb.ts`): mutex en `sync_dux_status` id **`act-cx-costos-dux`** (bloquea otra Act. Cx. y sync lista mientras corre; sync lista rechaza 409 si Act. Cx. activa). UI: **`MensajeProceso`** (`ActCxDuxProgresoBanner`) con progreso `{ procesados, total }`; polling cliente ≥ 5 s. Al finalizar: modal **«Se actualizaron X ítems»** + botón **Descargar PDF Resumen de Aumentos**. Actions: `getActCxDuxStatusAction`, `enviarCostoCxDuxAction`, `consultarEstadoCostoCxDuxAction`, `finalizarActCxDuxExitoAction`, `abortarActCxDuxAction`.
