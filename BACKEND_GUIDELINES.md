...
- **Convivencia con DUX**: `costo_compra` y `proveedor` en tienda siguen siendo **espejo DUX** (`syncListaPrecioTienda.service.ts`); la sync **no** escribe ni borra `cod_ext_costo_lista` ni `competencia_id_px_lista`. **`px_lista_tienda`** sigue siendo espejo DUX; la columna **PX LISTA** en Cx/Px elige referencia de precio vía `competencia_id_px_lista` (migración `20260524120000_prod_precios_tienda_competencia_id_px_lista`).
...
- **Cx & Px Tienda** (`src/actions/cxPxTienda.ts`): `guardarCostoCxProdTiendaAction({ codTienda, seleccion })` — `seleccion = cod_ext` persiste FK; `seleccion = prom` limpia FK (`limpiarCodExtCostoLista`). **`guardarPxListaTiendaAction`**: `seleccion = prom` limpia `competencia_id_px_lista`; `seleccion = competenciaId` persiste FK. Opciones PX LISTA en `pxListaCxPxTienda.service.ts` (registro en `prod_precios_competencia` o `px_vta_sugerido`; precio con misma prioridad que Px Competencia). Filtros URL: `vincCosto` (cantidad de vínculos habilitados); `costoProv` — `prom` = `cod_ext_costo_lista` **NULL** (Cx. Prom.); si no, `costoListaProveedor.id_proveedor` = proveedor elegido (la fila referenciada por `cod_ext_costo_lista` debe ser de ese proveedor y `habilitado`). Desplegable **CX. PROV.**: proveedores mercadería con al menos un vínculo habilitado. **`exportarCostoCxDiffAction`** + `exportCostoCxDiff.service.ts`: devuelve filas con FK `cod_ext_costo_lista` donde `costo_compra` ≠ `px_compra_final_sin_iva` del proveedor; UI **Exportar Cx** genera `.xls` (columnas **CODIGO**, **COSTO** = costo lista redondeado).
...
- **`cxPxTienda.ts`**: `getCxPxTiendaPageData` — listado paginado de `prod_precios_tienda` con filtros URL `q`, `marca`, `rubro`, `vincCosto`, `costoProv` (sin `subRubro`); `getCxPxTiendaPageParamsSchema` (Zod). Permiso `PERMISOS.cxPxTienda.acceso` (solo **editor**). FK `cod_ext_costo_lista` (ver §1.10b). **`costoListaTienda.service.ts`**: validar, establecer, limpiar y auto-asignar costo lista.
