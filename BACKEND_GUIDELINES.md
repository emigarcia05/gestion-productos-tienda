...
- **Convivencia con DUX**: el sync `syncListaPrecioTienda.service.ts` **no** incluye `esProductoPropio` en el `update`/`create` del upsert (igual patrón que `cxPxCxCodExt`, `cxPxPxCompRef`, `pxListaCxPx`); cualquier corrida DUX preserva el valor existente.
...
### 1.10b Costo Cx & Px Tienda (`cx_px_cx_cod_ext`)

- **Columna** `prod_precios_tienda.cx_px_cx_cod_ext` (`TEXT NULL`, FK → `prod_precios_provee.cod_ext`). Alta original **`20260520120000_add_cod_ext_costo_lista_prod_precios_tienda`** (columna `cod_ext_costo_lista`, luego `cod_ext_costo_compra`); renombre definitivo **`20260528160000_prod_precios_tienda_cx_px_cx_cod_ext`**. Campo Prisma: `cxPxCxCodExt`.
- **Convivencia con DUX**: `costo_compra` y `px_lista_tienda` siguen siendo **espejo DUX** (`syncListaPrecioTienda.service.ts`). Desde **2026-05-28** **`proveedor` y `cod_ext` quedaron congelados** (ya no se actualizan por DUX; solo conservan valores legacy — ver §1.4.2). El sync **no** escribe ni borra `cx_px_cx_cod_ext`, `cx_px_px_comp_ref` ni `px_lista_cx_px`. **PX LISTA** persiste **`cx_px_px_comp_ref`** (FK `prod_competencia.id`; `NULL` = Px. Prom.; alta `20260524120000`, renombre `20260528170000`; Prisma `cxPxPxCompRef`) y **`px_lista_cx_px`** (precio entero; migración `20260525120000`); al guardar desplegable, `guardarPxListaTiendaAction` resuelve el valor con la misma lógica que la UI (`pxListaCxPxTienda.service.ts`) y lo escribe en BD; edición manual del importe en grilla → **`guardarPxListaCxPxValorAction`** (`guardarPxListaCxPxValorSchema` en `@/lib/validations/cxPxTienda`) persiste `px_lista_cx_px` sin cambiar `cx_px_px_comp_ref`. La grilla muestra `px_lista_cx_px` si existe; si no, calcula en lectura.
...
- **Vínculos** (`src/actions/vinculos.ts`): `getVinculos` devuelve `{ productos, cxPxCxCodExt, esProductoPropio }`. Tras `vincularProducto`: `autoAsignarCodExtCostoListaTrasVincular` (solo si FK vacía: un candidato o match DUX). Tras `desvincularProducto`: `limpiarCodExtCostoListaSiCoincide` si la FK apuntaba a ese `cod_ext`. Elección explícita de costo en UI:
...
| `src/actions/vinculos.ts` | `vincularProducto` / `desvincularProducto`: `listaPreciosCodTiendaSchema` + `listaPreciosCodExtSchema` (`@/lib/validations/common`). `desvincularProducto` limpia `cx_px_cx_cod_ext` si apuntaba al `cod_ext` desvinculado; no bloquea por proveedor oficial DUX. |
