...
- **Comparación «menor costo» (Pedido Urgente, filtros `urgente` / `cualquier`):** ante doble clic, si hay más de una fila habilitada con el mismo **`cod_tienda`** (**`codTiendaVinculo`**, mismo vínculo a **`prod_precios_tienda`**, p. ej. REX-20 y Mer-50), el ranking usa **siempre precio final con IVA** por proveedor:** `pxFinalCompraConIvaProveedor`** (`src/lib/precioComparacionPedidoUrgenteReposicion.ts`) — **×1,21** si `proveedor.iva === SIEMPRE`, **×1** si **NUNCA** o **PREGUNTA**. La respuesta **`sugerenciaProveedorMenorCosto.costo`** sigue exponiendo valor **sin IVA** para el modal.
