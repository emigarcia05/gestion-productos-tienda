...
- **Servicio**: `src/services/movimientosFinanzas.service.ts`
  ...
  - `listarSucursalesParaGastos()`: `prisma.sucursal.findMany({ where: { centroCosto: true }, select: { id, nombre }, orderBy: { nombre: "asc" } })` — solo **centros de costo** (`global_sucursales.centro_costo`), incluida la fila seed **CORPORATIVO** (`suc_corporativo`) si tiene el flag. Alta/edición de **gasto final** valida centro de costo en `finBalGastosCatalogo.service.ts`; el cliente del catálogo **fusiona** la sucursal actual al editar una fila cuya sucursal ya no cumpla el filtro, para que el `Select` siga mostrando el valor hasta migrar.
...
Modelo para persistir saldos de cajas con tipo cerrado y trazabilidad de última modificación del saldo. **Nombre físico actual de la tabla**: `fin_tesoreria` (antes `fin_tesoreria_cajas`; migración `20260422140000_fin_tesoreria_rename_y_cheques`).

- **Tabla**: `fin_tesoreria`
  - `id` (`TEXT`, PK; Prisma `cuid()`).
  - `nombre_caja` (`TEXT`; unicidad compuesta con `titular`).
  - `titular` (`TEXT`, obligatorio).
  - `tipo_caja` (enum PostgreSQL/Prisma `TipoCajaTesoreria`: `DIGITAL | EFECTIVO | CHEQUE`; acreditación de cheques hacia caja **DIGITAL** vía `transferirChequeFinTesoreria`).
  - `monto` (`INTEGER`, default `0`; saldo persistido sin decimales; en cajas `CHEQUE` **no** representa el disponible para totales — ver `fin_tesoreria_cheques` y `montoDisponible` en el servicio).
  - `ult_actualizacion` (`TIMESTAMP`): última vez que cambió el saldo (`monto`).
  - `created_at`, `updated_at` (`TIMESTAMP`).
- **Índices** (nombres físicos tras el rename):
  - `fin_tesoreria_nombre_titular_ux` — único (`nombre_caja`, `titular`);
  - `fin_tesoreria_tipo_caja_idx` — (`tipo_caja`).
- **Regla de negocio en BD**:
  - trigger `fin_tesoreria_set_timestamps` (histórico: `cajas_tesoreria_set_timestamps` → `fin_tesoreria_cajas_set_timestamps`) sobre `BEFORE UPDATE`, ejecutando la función **`set_cajas_tesoreria_timestamps()`** (nombre legado en PostgreSQL):
...
- **Migraciones históricas** (tabla intermedia `cajas_tesoreria` → `fin_tesoreria_cajas` → `fin_tesoreria`): `prisma/migrations/20260414130000_add_cajas_tesoreria/migration.sql`, `20260418290000_rename_7_tablas_prod_fin`, `20260422140000_fin_tesoreria_rename_y_cheques`.
- **Servicio**: `src/services/cajasTesoreria.service.ts`
  - `listarCajasTesoreria()` / **`listarCajasTesoreriaPorTipo(tipo)`**: lectura ordenada por `nombre_caja`; **`montoDisponible`**: si `tipo_caja = CHEQUE`, suma en `fin_tesoreria_cheques` con `fecha_acreditacion` ≤ **hoy calendario Argentina**; si no, `montoDisponible = monto`.
  - `crearCajaTesoreria` / `editarCajaTesoreria` / `eliminarCajaTesoreria`: CRUD sobre `fin_tesoreria`.
  - `editarCajaTesoreria`: si la caja era `CHEQUE` y el nuevo tipo **no** es `CHEQUE`, rechaza la operación si existen filas en `fin_tesoreria_cheques` para ese `id` (integridad de negocio).
- **Actions**: `src/actions/cajasTesoreria.ts`
  - Lectura `listarCajasTesoreriaAction`; **`listarCajasTesoreriaTipoDigitalAction`** (solo cajas `tipo_caja = DIGITAL`, UI de acreditación de cheques): mismos permisos.
...
- **UI tesorería** (`TablaTesoreriaCajas`, `src/app/finanzas/tesoreria/page.tsx`): columna **MONTO** y totales por tipo usan **`montoDisponible`**. Doble clic para “actualizar monto” **no** aplica a filas `CHEQUE` (el saldo disponible viene de cheques acreditados).

#### 2.5c.1 Cheques de tesorería (`fin_tesoreria_cheques`, Prisma: `FinTesoreriaCheque`)

Detalle de importes por caja **solo** cuando `fin_tesoreria.tipo_caja = CHEQUE`. Ortogonal a `movimientos_finanzas_cheques` (gastos).

- **Tabla**: `fin_tesoreria_cheques`
  - `id` (`TEXT`, PK; `cuid()`).
  - `caja_id` → `fin_tesoreria.id` (`ON DELETE CASCADE`).
  - `tipo` (enum PostgreSQL/Prisma `TipoChequeTesoreria`: **`FISICO` \| `ECHEQUE`**).
  - `emisor` (`TEXT`, obligatorio; texto libre; se persiste con `trim` en alta).
  - `tenedor` (`TEXT NOT NULL`): lista cerrada **igual** que titulares de caja (`TITULARES_CAJA_TESORERIA` / `src/lib/cajasTesoreriaTitulares.ts`). CHECK en BD `fin_tesoreria_cheques_tenedor_check`; validación Zod `tenedorChequeTesoreriaSchema`.
  - `monto` (`INTEGER NOT NULL`, `CHECK monto >= 0`).
  - `fecha_acreditacion` (`DATE`): desde esta fecha inclusive el importe **cuenta** en `montoDisponible` si ≤ hoy AR.
  - `created_at`, `updated_at`.
- **Índices**: `fin_tesoreria_cheques_caja_id_idx`; `fin_tesoreria_cheques_caja_id_fecha_acreditacion_idx`.
- **Trigger**: `fin_tesoreria_cheques_assert_caja_cheque_trg` (`BEFORE INSERT OR UPDATE`) ejecuta `fin_tesoreria_cheques_assert_caja_cheque()` y exige que la caja referenciada exista y tenga `tipo_caja = CHEQUE`.
- **Servicio**: `src/services/finTesoreriaCheques.service.ts`
  - `sumarMontosChequesAcreditadosHasta(hoyIso)`: agregación raw SQL por `caja_id` (consumo interno + coherencia con listados).
  - `listarChequesPorCajaId(cajaId)`: listado ordenado por `fecha_acreditacion` desc.
  - `crearFinTesoreriaCheque(input)`: valida en servicio que la caja sea `CHEQUE` antes de insertar (refuerzo además del trigger).
  - `actualizarFinTesoreriaCheque(input)`: edita `tipo`, `tenedor`, `emisor`, `monto`, `fecha_acreditacion`; exige caja asociada `CHEQUE`.
  - `eliminarFinTesoreriaCheque(id)`: elimina la fila; `P2025` → error amigable.
  - **`transferirChequeFinTesoreria({ chequeId, cajaDestinoId })`** (transacción): comprueba que `fecha_acreditacion` ≤ **hoy** (`dateToIsoYmdArgentina`); que `cajaDestinoId` ≠ caja del cheque; que exista la caja destino con **`tipo_caja = DIGITAL`**; que `destino.monto + cheque.monto` quede dentro del mismo rango que `montoCajaTesoreriaSchema` (±999.999.999); luego **`increment`** en `fin_tesoreria.monto` destino y **`delete`** del cheque. El disponible de la caja CHEQUE origen se reduce al eliminar el cheque (ya no suma en agregados).
- **Actions**: `src/actions/finTesoreriaCheques.ts`
  - `crearFinTesoreriaChequeAction`: `PERMISOS.finanzas.acceso` + `esEditor()`; Zod `crearFinTesoreriaChequeSchema` (`@/lib/validations/finTesoreriaCheques.ts`); revalida `/finanzas`, `/finanzas/tesoreria`, `/finanzas/venc-por-fecha`.
  - `listarChequesPorCajaAction`: lectura con permiso finanzas; Zod `listarFinTesoreriaChequesPorCajaSchema`.
  - `transferirFinTesoreriaChequeAction`: **acreditar** el cheque en una caja **DIGITAL** (`esEditor()`; Zod `transferirFinTesoreriaChequeSchema`; mismo bloque de revalidación que las demás mutaciones de cheques).
  - `actualizarFinTesoreriaChequeAction` / `eliminarFinTesoreriaChequeAction`: `esEditor()`; Zod `actualizarFinTesoreriaChequeSchema` / `eliminarFinTesoreriaChequeSchema`; revalida `/finanzas`, `/finanzas/tesoreria`, `/finanzas/venc-por-fecha`.
...
- **Pantalla** `/finanzas/balance/gastos` (no confundir con el catálogo):
  ...
  - **Página** `src/app/finanzas/balance/gastos/page.tsx`: `searchParams` opcionales `mes` / `anio` (Zod); si hay al menos un periodo en BD y el par pedido no existe, **`redirect`** al periodo más reciente `(mes,anio)` en datos; si la tabla mensual está vacía, mantiene calendario Argentina y pasa al client un único par sintético para los Select. Lista con `listarImputacionesMensualesBalance`.
...
- **Convención de normalización**: al persistir desde service/action, aplicar `trim + toUpperCase` sobre `nombre` (alineado a `fin_tesoreria`, `movimientos_finanzas.nombre`).
...
1. `getExportRecepcionPedidoExcelPayload({ pedidoHistoriaId, fechaFacturaIso, idEmpresaCompras? })`
   ...
   - Proceso:
     - Lee desde DB:
      ...
      - En el Excel, columnas `FECHA` y `FECHA IMPUTACION CONTABLE` se exportan en formato `DD-MM-AAAA` usando **la misma fecha ingresada** en recepción (sin sumar ni restar días; si el usuario carga `2026-04-14`, se exporta `14-04-2026`).
...
| `@/lib/validations/finTesoreriaCheques.ts` | `crearFinTesoreriaChequeSchema`, `actualizarFinTesoreriaChequeSchema`, `eliminarFinTesoreriaChequeSchema`, `transferirFinTesoreriaChequeSchema`, `listarFinTesoreriaChequesPorCajaSchema`, `montoChequeTesoreriaSchema`, `tipoChequeTesoreriaSchema` (`FISICO` \| `ECHEQUE`), `tenedorChequeTesoreriaSchema` (mismo conjunto que titular de caja). |
...
*Última actualización (2026-04-22): **`fin_tesoreria_cajas` → `fin_tesoreria`** (rename de tabla + PK/índices/trigger `fin_tesoreria_set_timestamps`); nueva tabla **`fin_tesoreria_cheques`** (`caja_id` → `fin_tesoreria`, `emisor` TEXT, `monto` INT ≥ 0, `fecha_acreditacion` DATE, trigger `fin_tesoreria_cheques_assert_caja_cheque_trg`). Listados y totales de caja `CHEQUE` usan **`montoDisponible`** = suma de cheques con `fecha_acreditacion` ≤ hoy calendario Argentina (`listarCajasTesoreria`, `sumarMontosChequesAcreditadosHasta`). Servicios `cajasTesoreria.service.ts`, `finTesoreriaCheques.service.ts`; actions `finTesoreriaCheques.ts`; validaciones `@/lib/validations/finTesoreriaCheques.ts`. Migración `20260422140000_fin_tesoreria_rename_y_cheques`. Ver §2.5c y §2.5c.1.*

*Última actualización (2026-04-22, bis): columna **`tenedor`** en `fin_tesoreria_cheques` (CHECK + Zod; mismos valores que titulares de caja). Migración `20260422160000_add_tenedor_fin_tesoreria_cheques` (backfill desde `fin_tesoreria.titular` para filas existentes).*

*Última actualización (2026-04-22, ter): **transferencia de cheque** — `transferirChequeFinTesoreria` + `transferirFinTesoreriaChequeAction`: solo si `fecha_acreditacion` ≤ hoy AR; suma el importe a `fin_tesoreria.monto` destino y elimina el cheque; destino ≠ origen; tope de saldo alineado a cajas. Ver §2.5c.1.*

*Última actualización (2026-04-22, quater): columna **`tipo`** en `fin_tesoreria_cheques` (`TipoChequeTesoreria`: `FISICO`, `ECHEQUE`). Migración `20260422170000_add_tipo_fin_tesoreria_cheques`. Ver §2.5c.1.*
