...
- **POST DUX costos** (`actualizarCostoCxDux.service.ts` + `duxItemModificarApi.ts` + `actCxDuxStatusDb.ts`): lotes de **`DUX_API_BATCH_SIZE` (50)** ítems por POST; pausa **`DUX_API_BATCH_INTERVAL_MS` (5 s)** entre POST y entre GET de estado (§1.10c). Mutex en `sync_dux_status` id **`act-cx-costos-dux`** con `startedAt` (bloquea otra Act. Cx. y sync lista mientras corre; sync lista rechaza 409 si Act. Cx. activa). **Recuperación de bloqueo:** `reconcileStaleActCxDuxLockInDb()` libera automáticamente mutex huérfano (sin `startedAt`) o con TTL **15 min** (`ACT_CX_DUX_MAX_RUNTIME_MS`); action manual `liberarActCxDuxTrabadoAction`. UI progreso: **`MensajeProceso` variant `sidebar`** en **`SyncStatusIndicator`** (mismo slot que sync productos; prioridad sobre botón sync); doble clic (editor) libera bloqueo; polling cliente consulta estado cada **5 s** (`ActCxButton` + throttle servidor). Lectura estado: `getActCxDuxStatusAction` con permiso **`PERMISOS.tienda.acciones.sincronizar`**. Al finalizar: modal **«Se actualizaron X ítems»** + PDF aumentos opcional. Actions: `getActCxDuxStatusAction`, `enviarCostoCxDuxAction`, `consultarEstadoCostoCxDuxAction`, `finalizarActCxDuxExitoAction`, `abortarActCxDuxAction`, `liberarActCxDuxTrabadoAction`.
...
- **Act. Cx. costos** — POST `item/nuevoItem`: hasta **50** productos por body; **`throttleDuxItemModificarRequest()`** antes de cada POST/GET (`duxItemModificarApi.ts`). Tras cada POST, polling GET `obtenerEstadoItem` con la misma pausa de **5 s** entre consultas.
