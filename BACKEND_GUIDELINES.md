...
- **Servicio**: `src/services/movimientosFinanzas.service.ts`
  ...
  - `listarSucursalesParaGastos()`: `prisma.sucursal.findMany({ where: { centroCosto: true, id: { not: "suc_corporativo" } }, select: { id, nombre }, orderBy: { nombre: "asc" } })` — solo **centros de costo** (`global_sucursales.centro_costo`), excluyendo el id sembrado `suc_corporativo` (ver **CORPORATIVO**). Alta/edición de **gasto final** valida centro de costo en `finBalGastosCatalogo.service.ts`; el cliente del catálogo **fusiona** la sucursal actual al editar una fila legacy que ya no cumpla el filtro, para que el `Select` siga mostrando el valor hasta migrar.
...
- **Tabla** `fin_bal_gasto_final` (Prisma: `FinBalGastoFinal`):
  ...
  - Índices en `gasto_id`, `proveedor_id`, `sucursal_id`. La columna **PROVEEDORES** en `/finanzas/balance/gastos/catalogo` sigue siendo CRUD autónomo de `global_proveedores` (`proveedor_mercaderia = false`); los gastos finales consumen ese listado y **`listarSucursalesParaGastos()`** (sucursales con `centro_costo`, sin id `suc_corporativo`) para el select de sucursal.
