...
- **Pantalla** `/finanzas/balance/gastos` (no confundir con el catálogo):
  ...
  - **Página** `src/app/finanzas/balance/gastos/page.tsx`: `searchParams` opcionales `mes` / `anio` (`mesAnioQuerySchema`: **mes** 1–12, **anio** 2026–2046). Si el parse falla, se usa el mes/año por defecto (calendario Argentina con **año** acotado al rango). Sin **redirect** por periodo sin filas en BD. Lista con `listarImputacionesMensualesBalance`.
