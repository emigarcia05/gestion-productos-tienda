...
function FormulaDestacada({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <p
      className={cn(
        "rounded-md border border-border bg-muted/30 px-3 py-2 font-mono text-xs leading-relaxed text-foreground",
        className
      )}
    >
      {children}
        ...
        <div className="flex flex-col gap-5">
          <div className="flex flex-col gap-2">
            ...
            <div className="ml-4 flex flex-col gap-2 border-l-2 border-border pl-3">
              ...
              <FormulaDestacada>
                (1 − (CX TERMINAL + IVA) / 100) ×{" "}
                {FIN_ANA_COS_FINA_IMP_CHEQUE_FACTOR.toLocaleString("es-AR")} × 100
              </FormulaDestacada>
            </div>
          </div>

          <div className="flex flex-col gap-2 border-t border-border pt-4">
            <ModalMicroLabel>CX TOTAL</ModalMicroLabel>
            <FormulaLinea>
