...
export function parsearCSVCrudo(raw: string, tieneEncabezados: boolean): {
  ...
  const todas = lines.map((l) => l.split(separador).map((c) => c.trim().replace(/^["']|["']$/g, "")));
...
function detectarSeparador(linea: string): RegExp {
  const conteos = {
  ...
  const mejor = Object.entries(conteos).sort((a, b) => b[1] - a[1])[0][0];
  return new RegExp(mejor === "\t" ? "\t" : `\\${mejor}`);
}
...
export function aplicarMapeo(filas: string[][], mapeo: MapeoColumnas): FilaProducto[] {
  return filas.map((cols, rowIdx) => {
    ...
    if (isNaN(precioVentaSugerido)) throw new Error(`Fila ${rowIdx + 1}: precio venta inválido ("${precioVentaRaw}").`);
