...
**AppModal (componente estándar):** Layout wrapper para modales (`@/components/shared/AppModal`). **Header:** fondo corporativo #0072BB (`bg-primary`), texto blanco Geist centrado. **Cuerpo y footer:** gris universal (`bg-gris`, slate-200); card interna blanca (`bg-white`) centrada con padding; contenido vía `children`. **Props:** `title`, `children`, `actions`. Uso: `<Dialog open={open} onOpenChange={setOpen}><AppModal title="..." actions={<>...</>}>{contenido}</AppModal></Dialog>`.

**Gris universal:** En esta app, **"gris"** = **slate-200** (#e2e8f0). Variable CSS **`--gris`** en `globals.css` (`:root`); en Tailwind: **`bg-gris`**. Es el fondo de **páginas** (body en `layout.tsx`, main en `AppShell.tsx`, `ClassicFilteredTableLayout`) y de **ventanas modales** (contenedor, cuerpo y footer de `AppModal`). Referencia única: cuando se pida "gris" o "fondo gris" = `--gris` / `bg-gris` (slate-200). `--surface` es alias de `--gris` por compatibilidad.
