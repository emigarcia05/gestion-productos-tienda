...
- **Variables de prompt:** sintaxis **`{{CLAVE}}`** en **MAYÚSCULA** (`ASISTENTE_IA_VARIABLES_PROMPT`). **`{{RGB}}`** (cuentagotas); Diseñar Colores: **`{{SUPERFICIES}}`**, **`{{OBJETIVOS}}`**, **`{{ESTILO}}`**, **`{{COMBINARCON}}`** (vacío = no especificada). Compat: `{{CombinarCon}}` / `{{COMBINAR}}`, `(R,G,B)`.
