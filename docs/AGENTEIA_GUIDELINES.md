...
- **Variables de prompt:** sintaxis **`{{CLAVE}}`** (catálogo `ASISTENTE_IA_VARIABLES_PROMPT`). **`{{RGB}}`** (cuentagotas); Diseñar Colores: **`{{CANTIDAD_COLORES}}`**, **`{{SUPERFICIES}}`**, **`{{OBJETIVOS}}`**, **`{{ESTILO}}`**, **`{{COMBINAR}}`**. En el editor, chips **Insertar Variable**. Compat: plantillas viejas con `(R,G,B)`.
- **Diseñar Colores:** layout 2 columnas — izquierda foto de referencia (cliente, no se persiste); derecha cuestionario ordenado: (1) cantidad 1–4; (2) superficies multi + Select **Color N** por ítem; (3) objetivos multi; (4) estilo único; (5) combinar multi (opcional). CTA copia prompt (`aplicarRespuestasAlPromptDisenarColores`) + abre `url_redireccion`.
