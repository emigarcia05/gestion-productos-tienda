...
- Hub de funciones: tiles **`AsistenteIaFuncionTile`** proporción **5:1** (alto:ancho = `aspect-[1/5]`, ancho fijo `w-36`), ícono arriba + label en **MAYÚSCULAS**. Funciones en hub: **BUSCAR CÓDIGO DESDE IMAGEN** + **DISEÑAR COLORES**. Header (solo editor/administrador): **GESTION PROMP & URL** + **GESTION DISEÑO** → modal hub con **Superficies A Pintar** / **Estilos De Diseño** / **Combinar** / **Objetivos De Diseño** → CRUD nombre (`prod_ia_diseno_sup_pintar`, `prod_ia_diseno_estilos`, `prod_ia_diseno_combinar`, `prod_ia_diseno_objetivo`; patrón buscador + `+` como Marketing).
...
- **Diseñar Colores:** layout 2 columnas a pantalla completa (`overflow-hidden`) — izquierda foto de referencia (cliente, no se persiste); derecha cuestionario en **acordeón** (solo una pregunta abierta; respuestas con `overflow-y-auto`):
  1. **Superficie A Pintar** — obligatorio, hasta **4** (multi + Select **Color A…D**)
