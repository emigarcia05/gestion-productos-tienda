...
- **Diseñar Colores:** layout 2 columnas a pantalla completa (`overflow-hidden`) — izquierda foto de referencia (cliente, no se persiste); derecha cuestionario en **acordeón** (solo una pregunta abierta; respuestas con `overflow-y-auto`): (1) superficies multi + Select fijo **Color 1…4**; (2) objetivos multi; (3) estilo único; (4) combinar multi (opcional); (5) luz natural única; (6) luz artificial única. Catálogos en **GESTION DISEÑO** (orden del hub = orden de preguntas): `sup_pintar`, `objetivo`, `estilos`, `combinar`, `luz_natural`, `luz_artificial`. Al copiar: `formatSuperficiesParaPrompt` → 1 ítem `Nombre, ColorN`; ≥2 → **tabla Markdown** Superficie|Color; listas con salto de línea; variables vacías si no hay selección. Plantilla seed = **Prompt Maestro** (`buildPromptDisenarColoresDefault`). CTA copia + abre `url_redireccion`.
