...
- **Variables de prompt:** fuentes en código (`ASISTENTE_IA_VARIABLES_PROMPT`, filtradas por módulo). En **Editar Prompt Y Url**: botón **Insertar Variable** (desplegable con token + etiqueta) e, al editar, **Gestionar Variables** (nombre MAYÚSCULA en **`prod_ia_diseno_promp_var`**). Runtime: `expandirValoresConAliasVariables`. Tokens canónicos: **`{{RGB}}`**, **`{{SUPERFICIES}}`**, **`{{OBJETIVOS}}`**, **`{{ESTILO}}`**, **`{{COMBINARCON}}`**.
- **Diseñar Colores:** layout 2 columnas a pantalla completa (`overflow-hidden`) — izquierda foto de referencia (cliente, no se persiste); derecha cuestionario en **acordeón** (solo una pregunta abierta; respuestas con `overflow-y-auto`): (1) superficies multi + Select fijo **Color 1…4**; (2) objetivos multi; (3) estilo único; (4) combinar multi (opcional). Al copiar: `formatSuperficiesParaPrompt` → 1 ítem `Nombre, ColorN`; ≥2 → **tabla Markdown** Superficie|Color; listas con salto de línea; variables vacías si no hay selección. Plantilla seed = **Prompt Maestro** (`buildPromptDisenarColoresDefault`). CTA copia + abre `url_redireccion`.
