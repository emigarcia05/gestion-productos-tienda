...
- **Diseñar Colores:** layout 2 columnas a pantalla completa (`overflow-hidden`) — izquierda foto de referencia (cliente, no se persiste); derecha cuestionario en **acordeón** (solo una pregunta abierta; respuestas con `overflow-y-auto`):
  ...
  Catálogos en **GESTION DISEÑO** (orden del hub = orden de preguntas): `sup_pintar`, `objetivo`, `estilos`, `luz_natural`, `luz_artificial`, `combinar`. Al copiar: `formatSuperficiesParaPrompt` → 1 ítem `Nombre, ColorN`; ≥2 → **tabla Markdown** Superficie|Color; listas/únicos con salto de línea o texto; variables vacías si no hay selección (p. ej. Combinar). Plantilla seed = **Prompt Maestro** (`buildPromptDisenarColoresDefault`). CTA copia + abre `url_redireccion`.
