...
- Prompt/URL en tabla **`prod_ia_diseno_promp`** (CRUD: **GESTION PROMP & URL**); la **URL** se usa al buscar. Lookup: **Buscar Código Desde Imagen** (+ legacy **Buscar Color Desde Imagen**) y **Diseñar Colores**.
- **Variables de prompt:** sintaxis **`{{CLAVE}}`** (catálogo `ASISTENTE_IA_VARIABLES_PROMPT`). **`{{RGB}}`** (cuentagotas); Diseñar Colores: **`{{Superficies}}`**, **`{{Objetivos}}`**, **`{{Estilo}}`**, **`{{CombinarCon}}`** (vacío = no especificada). Compat: `{{COMBINAR}}`, `(R,G,B)`.
- **Diseñar Colores:** layout 2 columnas a pantalla completa (`overflow-hidden`) — izquierda foto de referencia (cliente, no se persiste); derecha cuestionario en **acordeón** (solo una pregunta abierta; respuestas con `overflow-y-auto`): (1) superficies multi + Select fijo **Color 1…4**; (2) objetivos multi; (3) estilo único; (4) combinar multi (opcional). Al copiar: `formatSuperficiesParaPrompt` → líneas `Superficie, ColorN`; listas con salto de línea; variables vacías si no hay selección. Plantilla seed = **Prompt Maestro** (`buildPromptDisenarColoresDefault`). CTA copia + abre `url_redireccion`.
