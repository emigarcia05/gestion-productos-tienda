...
- **Variables de prompt:** fuentes en código (`ASISTENTE_IA_VARIABLES_PROMPT`, filtradas por módulo). En **Editar Prompt Y Url** → **Gestionar Variables**: listado fuente + nombre en MAYÚSCULA persistido en **`prod_ia_diseno_promp_var`**. Chips insertan el token configurado. Runtime: `expandirValoresConAliasVariables`. Tokens canónicos: **`{{RGB}}`**, **`{{SUPERFICIES}}`**, **`{{OBJETIVOS}}`**, **`{{ESTILO}}`**, **`{{COMBINARCON}}`**.
