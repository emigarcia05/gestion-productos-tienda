...
- Hub de funciones: tiles **`AsistenteIaFuncionTile`** proporción **5:1** (alto:ancho = `aspect-[1/5]`, ancho fijo `w-36`), ícono arriba + label en **MAYÚSCULAS**. Funciones: **BUSCAR CÓDIGO DESDE IMAGEN**, **GESTION PROMP & URL** (editor). Header: **GESTION DISEÑO** (editor) → modal hub con **Superficies A Pintar** / **Estilos De Diseño** / **Combinar** / **Objetivos De Diseño** → CRUD nombre (`prod_ia_diseno_sup_pintar`, `prod_ia_diseno_estilos`, `prod_ia_diseno_combinar`, `prod_ia_diseno_objetivo`; patrón buscador + `+` como Marketing).
