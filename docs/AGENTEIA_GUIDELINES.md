...
- **Diseñar Colores:** layout 2 columnas a pantalla completa (`overflow-hidden`) — izquierda foto de referencia (cliente, no se persiste); derecha cuestionario en **acordeón** (solo una pregunta abierta; respuestas con `overflow-y-auto`):
  ...
  Catálogos en **GESTION DISEÑO** (orden del hub = orden de preguntas): `sup_pintar`, `objetivo`, `estilos`, `luz_natural`, `luz_artificial`, `combinar`. Cada ítem tiene **`nombre`** (español, pantalla) y **`nombre_en`** (inglés, prompt). Al copiar: `nombreCatalogoParaPrompt` + `formatSuperficiesParaPrompt` → 1–4 filas `Paint the {{Surface Name}} with a color of your choice (Color A)`; variables vacías si no hay selección (p. ej. Combinar). Plantilla seed = **Prompt Maestro** (`buildPromptDisenarColoresDefault`). CTA copia + abre `url_redireccion`.
