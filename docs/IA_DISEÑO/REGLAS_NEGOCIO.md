...
| 7 | Superficie A Pintar | Obligatorio | Hasta 4 |
...
Los valores de catálogo viven en **`prod_ia_diseno_catalogo`** (discriminador `kind`). Se muestran con **`nombre`** en la UI (MAYÚSCULAS) y se inyectan con **`texto`** al armar el prompt (**sentence case**; **`combinar` → minúsculas**). Excepción **Superficies**: `texto` + Color A–D se combinan con una plantilla única (`plantilla_superficies`) repetida por cada superficie elegida.
