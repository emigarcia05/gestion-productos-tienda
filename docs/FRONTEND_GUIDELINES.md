...
- **`/pedido-a-fabrica`** (**Pedido A Fáb.**): pilar sidebar **PEDIDO A FÁB.**; `PedidoAFabricaPageClient`. **Dos** `FilterBar` (`filtros-contenedor-tienda bg-card`) en columna (`flex flex-col gap-2`): (1) **PROVEEDOR** (`FiltroIndividualContainer` + `input-filtro-unificado`; solo `es_fabrica = true` vía `getProveedoresFabrica`; opción incluye `tiempoEntregaEnDias`) + **TIEMPO STOCKEO** (`Input` numérico entero, solo dígitos, `INPUT_FILTER_CLASS`, cesto individual; alimenta el cálculo de **CANT. SUGERIDA**); (2) fila **MARCA** | **RUBRO** | **SUB-RUBRO** (`FilaFiltrosDesplegables` default 5; opciones dinámicas desde `prod_tienda` de ítems vinculados del proveedor; cascada marca→limpia rubro/sub; rubro→limpia sub) + fila **`FiltroBusquedaInput`** (`useFiltrosConBusqueda`) **Buscar por descripción…** + **`LimpiarFiltrosButton`** + contador **X PRODUCTO(S)**. Al elegir proveedor: **`TablaPedidoAFabrica`** — orden: **DESCRIPCIÓN** (**35%**; texto = `descripcion_tienda` vía vínculo `cod_tienda`, fallback `descripcion_proveedor`) · grupo **TOTAL** (**25%** = **CANT. SUGERIDA** **8%** | **CANT. A PEDIR** **8%** | botón **4%**) · luego cada sucursal `pedido = true` (**20%** = **STOCK ACTUAL** **10%** | **PROM. VTA.** **10%**; `rowSpan`/`colSpan` como **Px Listas**). Con **2** sucursales la suma es **100%**. **CANT. SUGERIDA** (solo lectura): `calcularCantSugeridaPedidoAFabrica` en `@/lib/pedidoAFabricaPromVta` con stock/prom totales (suma sucursales), `tiempo_entrega_en_dias` del proveedor y **TIEMPO STOCKEO** — ver fórmula en `docs/BACKEND_GUIDELINES.md` §1.11f. Sin **TIEMPO STOCKEO** la celda queda vacía y el botón deshabilitado. **CANT. A PEDIR**: `Input` numérico local por `codExt`. Botón: `TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS` + ícono **Check**; al clickear copia **CANT. SUGERIDA** → **CANT. A PEDIR**. Datos por sucursal: stock = `prod_tienda_stock.stock_real` del depósito; prom. vta. = `(ventas 2 meses previos) / 48`, `Math.round` (`@/lib/pedidoAFabricaPromVta`; requiere `cod_tienda`). Header **Info Sobre Fórmulas** → **`InfoPromedioPedidoAFabricaModal`**: secciones **PROM. VTA.** · **FECHAS** · **STOCK Y CANT. SUGERIDA** (lado izquierdo de cada fórmula en negrita; **Cant. Sugerida** en dos subfórmulas condicionales; periodo AR vigente en PROM. VTA.). Carga: `getProductosPedidoAFabricaAction` (filtros marca/rubro/subRubro/q + página) + `PaginacionClient` (100/página). Legacy **`/estadisticas-productos/est-para-compra`** → `permanentRedirect`.
...
*Última actualización (2026-08-11): **Pedido A Fáb.** — modal **Info Sobre Fórmulas** (secciones + formato negrita/normal).
