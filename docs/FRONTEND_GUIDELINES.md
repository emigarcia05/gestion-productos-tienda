...
- **Reposición:** desplegables SUCURSAL → PROVEEDOR → MARCA → RUBRO → CONFIGURADO. Bloque secundario STOCK / CANT. A PEDIR. En este módulo, PROVEEDOR solo muestra proveedores de mercadería con `global_proveedores.es_fabrica = false` (también en el modal **Generar Pedido** cuando el origen es Reposición), y la grilla solo muestra productos que tengan al menos un vínculo habilitado con proveedor no fábrica. `ConfigurarReposicionModal`: punto/cant vacíos (no `0`) si no hay regla. FORMA PEDIR (vendedor): **UN. MÁXIMAS** (`UNIDADES_MAX`) / **BULTO** (`POR_BULTO`; **siempre** en el select, aunque no haya fila en `prod_tienda_bultos`). Layout de campos: con **UN. MÁXIMAS** mostrar `FORMA PEDIR | PUNTO REPOSICIÓN | UN. MÁXIMAS`; con **BULTO** mostrar `FORMA PEDIR | PUNTO REPOSICIÓN (EN UN.)` y, debajo del grid, el input **Un. que viene en un bulto cerrado** (entero ≥ 1; al Guardar crea o actualiza `prod_tienda_bultos` del ítem principal). No hay input de cantidad de bultos ni `TOTAL EN UN.`: `reposicion_cant_conf` queda en 1 (un bulto cerrado). En ambos casos, las etiquetas del bloque usan altura uniforme para mantener una fila horizontal de labels y otra de inputs. En la grilla, **CANT. REPOSIC.** muestra `reposicion_cant_conf` (con **BULTO** = 1); **CANT. A PEDIR** y el pedido generado van en unidades (`cantConf × prod_tienda_bultos`). Encima de la tabla del modal va el botón ícono `+` centrado y con estilo `default` (fondo `primary`, ícono `primary-foreground`) para abrir el selector de productos adicionales (con **BULTO**, exige el input de unidades y filtra por ese valor). No CANT. FIJA POR UNID.
