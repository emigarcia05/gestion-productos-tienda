...
- **Reposición:** desplegables SUCURSAL → PROVEEDOR → MARCA → RUBRO → CONFIGURADO. Bloque secundario STOCK / CANT. A PEDIR. `ConfigurarReposicionModal`: punto/cant vacíos (no `0`) si no hay regla. FORMA PEDIR (vendedor): **UN. MÁXIMAS** (`CANT_MAX`) / **BULTO** (`CANT_FIJA_POR_BULTO`; solo si hay fila en `prod_tienda_bultos`). Grid dinámico de configuración: con **UN. MÁXIMAS** se muestran 3 columnas (`FORMA PEDIR | PUNTO REPOSIC. | UN. MÁXIMAS`); con **BULTO** se muestran 4 (`FORMA PEDIR | PUNTO REPOSIC. (EN UN.) | BULTO | TOTAL UNIDADES`), donde **TOTAL UNIDADES** es de solo lectura y calcula `bultos × unidades por bulto`. El botón `+` para abrir el selector de productos adicionales queda debajo de la tabla de productos incluidos. Si la forma seleccionada es **BULTO**, el selector del `+` solo lista productos con el mismo valor de bulto que el producto base. No CANT. FIJA POR UNID.
