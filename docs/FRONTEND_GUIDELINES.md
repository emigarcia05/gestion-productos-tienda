...
- **Ventas Mensuales:** una fila por mes/año; columnas **MES** + sucursales `genera_balance` + **ACCIONES** (Cargar / Editar / Eliminar; Cargar se deshabilita si todas las sucursales ya tienen monto). Filtros MES / AÑO / SUCURSAL + contador de periodos. Sin botón de header: Cargar/Editar abren `CrearFinBalVtasModal` con periodo fijo. Eliminar borra el periodo completo (`eliminarFinBalVtasPorPeriodoAction`).
