...
`@/components/shared/catalogo-finder/` + `CATALOGO_FINDER_*` en `ui-classes`. Columnas con header `bg-primary` (`headerVariant="finder"` default), `+` (`nuevoLado`: `end` default, `start` en Ideas), `className?` para altura en modal. Wizard Envios: `headerVariant="titulo"` (título de paso, sin barra primary) **en todos los pasos** (SUCURSAL / CLIENTES / DIRECCIONES / FECHA / MERCADERÍA). Paso CLIENTE / DIRECCIÓN: `mostrarNuevo={false}` y CTA **Nuevo Cliente** / **Nueva Dirección** (`Button` `w-full` en wrapper `p-4`, no `rounded-none`) debajo de la lista. Filas: hover editar/eliminar por default; `eliminarSiempreVisible` deja el `Trash2` fijo a la derecha (`CATALOGO_FINDER_COLUMN_NOVO_BUTTON_CLASS`; Envios · CLIENTE y DIRECCIÓN) y el lápiz sigue en hover. En DIRECCIÓN, `accionesSiempreVisibles`: **MapPin + lápiz + Trash2 siempre visibles** en la columna de acciones (sin hover). Selección `CATALOGO_FINDER_ROW_SELECTED_CLASS`. `nombreAccion` opcional a la derecha del nombre (Envios · DIRECCIÓN: `MapPin`). `iconoIzquierda` opcional a la izquierda del nombre (Envios · PINTOR: `Users`, filtra asociados). Envios · CLIENTE: `reservarEspacioIconoIzquierda` reserva el hueco `size-7` en CONSUMIDOR_FINAL para alinear nombres. `etiquetaIzquierda` opcional: columna fija de etiqueta a la izquierda. `nombreSufijo` opcional: ` - texto` en `font-normal text-[0.75em]` (Envios · CONSUMIDOR_FINAL con pintor asociado). Envios · DIRECCIÓN: una sola línea `DIR1: Dirección (Referencia).` (`etiquetaDireccionEnvioFilaListado`) con `nombreLineas={2}` (hasta 2 renglones). `nombreCentrado` centra el nombre (Envios · CLIENTE). Usos: Catálogo Gastos (5 cols), Comp. Categorías (4), Ideas (2), Envios · Crear Envío (finder 1 col por paso, dentro del wizard modal), Envios · Gestionar Direcciones (finder 2 cols CLIENTES | DIRECCIONES, dentro de `EnviosGestionarDireccionesModal`).
...
**Dock** (abajo, `mt-auto`): Sync DUX (`SyncStatusIndicator` / `DuxSyncStyleButton`) → superficie sesión (`Pendientes` + usuario). Click en el nombre de usuario abre `Elegir Usuario`; en ese modal, el flujo es en 2 pasos dentro del mismo contenido (`Sucursal` → `Usuario`, sin abrir segundo modal). Si el usuario puede cambiar módulo, el ícono de módulo mantiene la apertura de `Cambiar Módulo`. Sync **no** se duplica en headers de página. Editor: modal Productos / Compras. Simple: sync productos. Import Excel: `ImportStatusIndicator` (independiente).
...
- **Envios:** sidenav **Programados** / **Conductor**. Programados = CFTL `contentWidth="full"` con header **Gestionar Direcciones** (`MapPin` abre `EnviosGestionarDireccionesModal` `size="xl"` `h-[85vh]` `scrollBody={false}`: finder 2 columnas CLIENTES | DIRECCIONES, mismas acciones de alta/edición/baja que el wizard) y **Crear Envío** (`Plus` abre `CrearEnvioWizardModal` en la misma página) y tabla de `envios_final` (SUCURSAL; CLIENTE FINAL; PINTOR; FECHA `dd/mm/aaaa` AR; HORARIO `hora_desde – hora_hasta`; DIRECCIÓN / TELEFONO / UBI / PDF / ENTREGADO = tilde `tabla-check-toggle` si hay dato — dirección `direccionEnvioTieneDato`, teléfono `telefonoEnvio`, UBI `url_maps`, PDF `tienePdf`, ENTREGADO `entregado`; ACCIONES editar/borrar). Filtros SUCURSAL + FECHA (HOY / MAÑANA, calendario AR) + ENTREGADO (SÍ / NO). **Conductor:** única pantalla mobile (`w-[24rem]` centrado, sin breakpoints) **y sin slidenav**. Al abrir esa URL directa no se pide **Elegir Usuario**. Bajo el título, botón **DIRECCIONES** (`h-12 w-full`) abre `EnviosConductorDireccionesModal` (`AppModal` `size="sm"` `h-[85vh]` `scrollBody={false}`): **1** buscar cliente (`useFiltrosConBusqueda` + `FiltroBusquedaInput` + finder de `listarClientes`); **2** listado de `envios_direcciones` del cliente; **3** datos de la dirección (`etiquetaDireccionEnvio` + referencia) y **Maps** (deshabilitado si no hay `url_maps`). Pie Cancelar / Atrás; al cerrar se resetean los pasos. Debajo de DIRECCIONES, `grid grid-cols-2`: filtros **SUCURSAL** (`TODAS` + sucursales `pedido`) y **DÍA** (HOY / MAÑANA AR, default HOY). Tarjetas de envíos pendientes (`fecha_envio` ≥ hoy AR y `entregado = false`), filtradas por sucursal/día y ordenadas por fecha + `hora_desde` + `hora_hasta` (más cercanos a ahora primero): destinatario, fecha, horario y **Ver** (dirección + `observacion_envio` si hay + Maps / Llamar / Pdf + **ENTREGADO**). **ENTREGADO** abre `AppModal` `size="sm"` de confirmación (Cancelar / Entregar) y llama `marcarEnviosFinalEntregadoAction`; al confirmar, la tarjeta sale del listado. **Wizard Nuevo Envío / Editar Envío:** mismo `CrearEnvioWizardModal` `size="xl"` `scrollBody={false}` `h-[85vh]` `max-h-[85vh]` (desde Programados: **Crear Envío** o lápiz). Header **Nuevo Envío** o **Editar Envío**. En edición los pasos ya tienen sucursal / cliente / dirección / fecha-horario / forma-PDF cargados y el stepper permite ir a cualquier paso (`pasoMaximoAlcanzable` = 5). Pie: Cancelar / Atrás / Siguiente o **Crear Envío** / **Guardar**. Cuerpo en 2 filas: **15 %** stepper `EnviosWizardPasos` (SUCURSAL / CLIENTE / DIRECCIÓN / FECHA / MERCADERÍA; el paso actual va `bg-primary`; separador inferior `border-primary`); **resto** el formulario del paso (título `headerVariant="titulo"` sin padding superior, pegado al stepper). Pie: Cancelar / Atrás / Siguiente o **Crear Envío**. **1. SUCURSAL:** mismo header `headerVariant="titulo"` **SUCURSAL**. Botones `w-full` centrados en el área (`min-h-full items-center justify-center`) con sucursales `pedido=true` (`listarSucursalesParaEnvios`); click elige (sin preselección) y pasa a **CLIENTE**. **2. CLIENTE:** Finder 1 col `headerVariant="titulo"` **CLIENTES** `mostrarNuevo={false}` (CONSUMIDOR_FINAL y PINTOR) + `FiltroBusquedaInput`. Fila alineada a la izquierda: `iconoIzquierda` `Users` **solo si `tipo=PINTOR`** (click filtra el listado a ese pintor + `CONSUMIDOR_FINAL` con `pintor_asociado`; el pintor queda primero y debajo sus clientes en orden alfabético `es`; otro click o **Limpiar Filtro** quita el filtro); `reservarEspacioIconoIzquierda` deja el hueco vacío en CONSUMIDOR_FINAL para alinear nombres | **NOMBRE** | ` - NOMBRE PINTOR` (`nombreSufijo`, solo CONSUMIDOR_FINAL con pintor). Acciones a la derecha: lápiz (hover) + `Trash2` fijo (`eliminarSiempreVisible`); sin `Phone`. Click en la fila selecciona el cliente y pasa a **DIRECCIÓN**; debajo de la lista, wrapper `p-4` + `Button` `w-full` **Nuevo Cliente** (abre modal **Nuevo Cliente** (NOMBRE COMPLETO en mayúsculas, CEL opcional, TIPO default CONSUMIDOR FINAL | PINTOR, PINTOR ASOCIADO, DIRECCIONES). Pintor asociado: `+` abre listado; con pintor = nombre + editar / borrar. Un `PINTOR` no tiene pintor asociado. **3. DIRECCIÓN:** mismo header **DIRECCIONES** (subtítulo = nombre del cliente). Finder de `envios_direcciones.persona_id` del cliente; fila una sola línea: columna izquierda **DIRECCIÓN n:** + `calle_nombre numeracion, distrito - departamento.` (`etiquetaDireccionEnvioListado`) + referencia `nombreSufijo` + `MapPin` si hay `url_maps`; click en la fila selecciona la dirección y pasa a **FECHA**; debajo, wrapper `p-4` + `Button` `w-full` **Nueva Dirección** (abre el modal: CALLE NOMBRE y DISTRITO proper case; numeración/referencia primera mayúscula; DEPARTAMENTO `LAS HERAS` | `GODOY CRUZ` | `GUAYMALLEN` | `MAIPU` | `LUJAN`; al menos un dato). **4. FECHA:** mismo header **FECHA** (subtítulo = `dd/mm/aaaa`). Contenido centrado. Segmento **FECHA** (`ModalMicroLabel`): HOY / MAÑANA / `CalendarDays`. RANGO HORARIO: **DESDE** etiqueta + Select en la misma fila (09:00–18:30) ` - ` **HASTA** etiqueta + Select (hueco siempre reservado; `invisible` hasta elegir DESDE). HASTA = horas estrictamente posteriores, hasta 19:00. Ej. DESDE 15:00 → HASTA 15:30…19:00. Al completar fecha + rango se pasa a **MERCADERÍA**. **5. MERCADERÍA:** mismo header **MERCADERÍA**. Contenido centrado (`min-h-full items-center justify-center`). PDF opcional (máx. 5 MB) a `w-full`; títulos **PDF MERCADERÍA**, **FORMA DE PAGO** y **OBSERVACIÓN ENVÍO** centrados (`ModalMicroLabel` `align="center"`). **FORMA DE PAGO** Select `w-full`: `PAGADO` | `EFECTIVO` | `TRANSFERENCIA` | `POSNET` | `CUENTA CORRIENTE`. Si eligen **PAGADO**, el alta persiste `pagado = true`; si no, `pagado = false`. **OBSERVACIÓN ENVÍO**: textarea opcional (`envios_final.observacion_envio`, máx. 5000). Guardar = `crearEnviosFinalAction` o, en edición, `editarEnviosFinalAction` (pintor = `pintor_asociado` si hay). El lápiz de Programados abre el mismo wizard con datos cargados.
