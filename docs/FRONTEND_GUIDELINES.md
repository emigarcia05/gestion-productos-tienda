...
- **`SidebarMainAppArea`**: indicador compacto de pendientes (reemplaza el logo). Vista fija: dos filas **Pedidos →** / **Transferencias →** (sin título de sucursal). Hover (`Tooltip` `side="right"`) despliega al costado el detalle: **Pedido** (`Urgente - N` / `Tintométrico - N` / `Reposición - N` = ítems con cant. pedir &gt; 0 de **Generar Pedido**) y **Transferencia** (`Emisión - N` TRANSFERIR / `Recepción - N` RECIBIR = filas del modal **Transf. Pendiente Registro**). Click en **Transferencias** con emisión o recepción &gt; 0 abre el mismo aviso. Action `getIndicadorSlidenavAction`. Refresh al cambiar sucursal, al navegar, al enfocar, cada 30 s y vía `EVENTO_INDICADOR_SLIDENAV`. Tras **Elegir Usuario**, `EVENTO_ADVERTIR_TRANSF_PENDIENTES` recarga y, si hay transferencia pendiente, modal **Transferencias Pendientes** (`CALLOUT_WARNING_CLASS` + **Ir A Trans. Depósitos**). No se dispara al **Cambiar Módulo**.
