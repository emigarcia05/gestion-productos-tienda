...
- **`SidebarMainAppArea`**: fila **Pendientes** del dock de sesión. Vista fija: label + badge total (`urgente+tintométrico+reposición+emisión+recepción`, `tabular-nums`) + chevron. Badge con `bg-accent2` si Emisión o Recepción &gt; 0; si no, `bg-sidebar-accent`. Hover (`Tooltip` `side="right"`): grilla 3 columnas (**sección** | **tipo** | **cantidad**): **Pedido** / divisor `h-px bg-primary` / **Transferencia**. Click con transf. pendientes → modal aviso. Action `getIndicadorSlidenavAction`. Refresh al cambiar sucursal, navegar, enfocar, cada 30 s y `EVENTO_INDICADOR_SLIDENAV`. Tras **Elegir Usuario**, `EVENTO_ADVERTIR_TRANSF_PENDIENTES` abre el aviso si corresponde. No se dispara al **Cambiar Módulo**.
- **`SidebarAreaSwitcher`**: fila de usuario dentro del mismo dock (sin superficie propia). **Ícono módulo (15 %) | nombre (85 %)** solo si `modulos_permitidos.length > 1` (`Store` / `Landmark` / `Megaphone`); click → **Cambiar Módulo** (lista visible, un click aplica). Con un solo módulo: solo el nombre. Ambas filas del dock usan **`h-9`** + hover `sidebar-accent/80`. **Primera visita**: modal **Elegir Usuario** con lista clickeable. Si tiene **Administración**, pide clave. Sucursal = `sucursal_por_defecto`. Al aplicar usuario → `avisarAdvertirTransfPendientes()`.
...
- **Estructura**: fila **Pendientes** (`h-9`, badge total, chevron) → divisor `h-px bg-sidebar-foreground/40` → fila **usuario** (`h-9`).
- **Pendientes**: hover con detalle Pedido / Transferencia; badge `accent2` si hay Emisión o Recepción.
- **Usuario**: **ícono del módulo actual** (15 %) + **nombre** MAYÚSCULAS (85 %, `truncate`) si puede cambiar de módulo; click → modal **Cambiar Módulo** con **lista clickeable** de módulos permitidos (ícono + label MAYÚSCULAS; activo `variant="default"`; **un click** aplica, sin Select ni Aplicar). Con un solo módulo: solo el nombre centrado.
