...
| `.sidebar-nav-depth-1`, `.sidebar-nav-depth-2` | Fondos de jerarquía del acordeón sidebar: mezcla `color-mix` de `--sidebar-user-switcher-bg` (#021D36) sobre `--sidebar` (#0072BB) — **16 %** (primer desglose) y **30 %** (hojas anidadas). Aplicar al contenedor de `CollapsibleContent` al abrir. |
...
- **Navegación lateral por área activa** (`Sidebar.tsx`): árbol de decisiones en **acordeón vertical** en las tres áreas (**Vendedor**, **Administración**, **Marketing**), con separador suave **`.sidebar-nav-divider`** / `SidebarNavDivider` entre opciones hermanas (línea blanca al 22 % desde el margen del ícono hasta el borde derecho del hover). Al abrir cada nivel, el panel hijo usa **`.sidebar-nav-depth-1`** / **`.sidebar-nav-depth-2`** (azul más profundo por jerarquía). En **Vendedor** el orden de módulos es **PEDIDO MERCADERIA** → **AYUDA VENDEDOR** → **ASISTENTE IA** (todos accesibles al rol vendedor/`simple`; CRUD de prompts del Asistente IA solo `editor`). Dentro de **PEDIDO MERCADERIA**: **Generar Pedido** → agrupador **Conf. Pedido** (sin ruta; ícono `ListChecks`) con **Urgente**, **Tintométrico** y **Reposición** → **Recepcion Pedido** (`/gestion-productos/pedidos/historial`; subtítulo de página; antes **Historial Pedidos**). El agrupador **Conf. Pedido** auto-expande si la ruta activa es un hijo. Módulos retirados (no existen más): **LISTA TIENDA** y **Control Aumentos** (este último eliminado por completo el 2026-05-28; será reimplementado más adelante). Con área **Administración** el sidebar muestra solo **3 pilares** (`AdministracionAccordionNav`, SSOT `administracionNav.ts`): **FINANZAS**, **LISTAS PRECIOS**, **ESTADÍSTICAS**. Desglose en acordeón anidado: **FINANZAS** → **BALANCE** | **OPERACIONES** → pantallas; **LISTAS PRECIOS** → **Vta Por Prod.** | **ANÁLISIS M.C.** → pantallas; **ESTADÍSTICAS** → pantallas directas bajo el pilar. **ANÁLISIS M.C.** vive bajo **LISTAS PRECIOS** (no bajo FINANZAS). En **Marketing** (`MARKETING_MODULES`, SSOT rutas `src/lib/marketingRoutes.ts`): módulo **`PUBLICACIONES`** (`Megaphone`) con **Calendario** → `/marketing/publicaciones/calendario` (`CalendarRange`), **Ideas Contenido** → `/marketing/publicaciones/ideas` (`Lightbulb`) y **Objetivos**; módulo **`BASE MULTIMEDIA`**; permiso **`PERMISOS.marketing.acceso`** (solo `editor`).
...
**AYUDA VENDEDOR** (rol **vendedor**/`simple`): agrupador **Px Venta** (sin ruta; ícono `CircleDollarSign`) → **Px. Vta. Sugerido** + **Px Tintométrico**; luego **Calc. Litros**, **Procesos**, **Cargar Gasto**, **Control Stock**. El agrupador auto-expande si la ruta activa es un hijo. **Cargar Gasto** (`/gestion-productos/cargar-gasto`, `PERMISOS.ayudaVendedor.cargarGasto` para `simple` y `editor`): abre al entrar el modal **`GastoUnicoBalanceModal`** (**Nuevo Gasto Eventual**, mismo flujo que **GASTO EVENTUAL** en `/finanzas/balance/gastos`); periodo = mes/año calendario Argentina. Actions: `listarFinBalGastosFinalesNoMensualesAction` / `crearFinBalImputacionGastoUnicoAction` con gate `requireCargarGastoEventual` (permiso `cargarGasto`).
...
*Última actualización (2026-08-06): **Sidebar · profundidad azul** — `.sidebar-nav-depth-1` / `.sidebar-nav-depth-2` (mezcla #021D36 sobre #0072BB) al abrir el acordeón en las tres áreas.
