...
- **Envios:** sidenav **Programados** / **Crear Envío**. Programados = CFTL `contentWidth="full"` con tabla de `envios_final` (FECHA `dd/mm/aaaa` AR; HORARIO `hora_desde – hora_hasta`; PAGADO = ícono `Check`; PDF vía `GET /api/envios/[id]/comprobante`). **Crear Envío:** CFTL vacío (como Cargar Gasto); al entrar abre `CrearEnvioWizardModal` (`AppModal` `size="xl"` `scrollBody={false}`; panel `inset-4` = padding 1 rem en los 4 lados y el resto del viewport). Header **Nuevo Envío**. Cuerpo en 2 filas: **15 %** stepper `EnviosWizardPasos` (CLIENTE / DIRECCIÓN / FECHA / MERCADERÍA; el paso actual va `bg-primary`); **resto** el formulario del paso. Pie: Cancelar / Atrás / Siguiente o **Crear Envío**. **1. CLIENTE:** Finder 1 col `headerVariant="titulo"` `mostrarNuevo={false}` (CONSUMIDOR_FINAL y PINTOR) + `FiltroBusquedaInput`; fila = **nombre** (una línea) + si `CONSUMIDOR_FINAL` tiene pintor asociado: ` - NOMBRE PINTOR` (`nombreSufijo`, sin negrita, 50 % del nombre) + ícono `Users` si `tipo=PINTOR` (abre modal de CONSUMIDOR_FINAL con `pintor_asociado` = ese pintor; click elige al consumidor y pasa al paso 2) + ícono `Phone` (`EnviosTelLink`, `tel:`) si hay `cel` + `Trash2` fijo (`eliminarSiempreVisible`; editar sigue en hover); click en la fila selecciona el cliente y pasa a **DIRECCIÓN**; debajo de la lista, `Button` `w-full` **Nuevo Cliente** (abre modal **Nuevo Cliente** (NOMBRE COMPLETO en mayúsculas, CEL opcional, TIPO default CONSUMIDOR FINAL | PINTOR, PINTOR ASOCIADO, DIRECCIONES). Pintor asociado: `+` abre listado; con pintor = nombre + editar / borrar. Un `PINTOR` no tiene pintor asociado. **2. DIRECCIÓN:** Finder de `envios_direcciones.persona_id` del cliente; fila `calle_nombre numeracion, distrito. departamento` + `MapPin` si hay `url_maps`; `+` abre **Nueva Dirección** (CALLE NOMBRE y DISTRITO proper case; numeración/referencia primera mayúscula; DEPARTAMENTO `LAS HERAS` | `GODOY CRUZ` | `GUAYMALLEN` | `MAIPU` | `LUJAN`; al menos un dato). **3. FECHA:** HOY / MAÑANA + `CalendarDays`; HORA DESDE / HASTA 09:00–19:00 cada 30 min (desde < hasta). **4. MERCADERÍA:** PDF opcional (máx. 5 MB) + forma `EFECTIVO` | `TRANSFERENCIA` | `POSNET` | `CUENTA CORRIENTE`. Guardar = `crearEnviosFinalAction` (pintor = `pintor_asociado` si hay) y redirect a Programados. Edición fecha/horario/PDF/pago también desde el modal de Programados.
