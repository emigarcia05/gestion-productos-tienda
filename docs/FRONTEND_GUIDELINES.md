...
- **Texto**: **nombre de usuario** en MAYÚSCULAS (`global_personal.nombre_personal`). Sin sucursal ni módulo en este bloque (la sucursal vive en el indicador).
...
- **Onboarding (primera visita de la pestaña)**: modal **Elegir Usuario** (`listUsuariosParaInicioSesionAction`). Placeholder **USUARIO**. No se cierra hasta Aplicar (sin X). Persiste `sessionStorage` `main-app-usuario-sesion` y copia `sucursal_por_defecto` a `main-app-sucursal-preferida`.
