...
- **`SidebarMainAppArea`**: fila **Pendientes** del dock de sesión. Vista fija: label **PENDIENTES** (`uppercase`, `text-center`) + badge de **categorías** (Pedido si Urgente+Tintométrico+Reposición &gt; 0 → 1; Transferencia si Emisión+Recepción &gt; 0 → 1; **máx. 2**, `tabular-nums`) + chevron. Badge con `bg-accent2` si Emisión o Recepción &gt; 0; si no, `bg-sidebar-accent`. Hover (`Tooltip` `side="right"`, `disableHoverableContent={false}`): grilla 3 columnas (**sección** | **tipo** | **cantidad**): botón **Pedido** → `GP_ROUTES.pedidoMercaderia.generarPedido` / divisor `h-px bg-primary` / botón **Transferencia** → `GP_ROUTES.ayudaVendedor.transfDepositos?origen=` + sucursal preferida (`leerSucursalPreferida`). Click con transf. pendientes → modal aviso (**Ir A Trans. Depósitos** usa el mismo `origen`). Action `getIndicadorSlidenavAction`. Refresh al cambiar sucursal, navegar, enfocar, cada 30 s y `EVENTO_INDICADOR_SLIDENAV`. Tras **Elegir Usuario**, `EVENTO_ADVERTIR_TRANSF_PENDIENTES` abre el aviso si corresponde. No se dispara al **Cambiar Módulo**.
- **`SidebarAreaSwitcher`**: fila de usuario dentro del mismo dock (sin superficie propia). **Ícono módulo + nombre** centrados en la fila (`justify-center`; ícono ~15 % / nombre ~85 % del bloque) si `modulos_permitidos.length > 1` (`Store` / `Landmark` / `Megaphone`); click → **Cambiar Módulo** (lista visible, un click aplica). Con un solo módulo: solo el nombre, también centrado. Ambas filas del dock usan **`h-9`** + hover `sidebar-accent/80`. **Primera visita**: modal **Elegir Usuario** con lista clickeable. Si tiene **Administración**, pide clave. Sucursal = `sucursal_por_defecto`. Al aplicar usuario → `avisarAdvertirTransfPendientes()`.
...
- **Pendientes**: hover con detalle; **Pedido** y **Transferencia** son botones (Generar Pedido / Trans. Depósitos con `origen` = sucursal preferida); badge = categorías con pendientes (máx. 2); `accent2` si hay Emisión o Recepción.
