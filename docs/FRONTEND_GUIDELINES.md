...
- **Reposición:** desplegables SUCURSAL → PROVEEDOR → MARCA → RUBRO → CONFIGURADO. Bloque secundario STOCK / CANT. A PEDIR. `ConfigurarReposicionModal`: punto/cant vacíos (no `0`) si no hay regla. FORMA PEDIR (vendedor): **UN. MÁXIMAS** (`CANT_MAX`) / **BULTO** (`CANT_FIJA_POR_BULTO`; solo si hay fila en `prod_tienda_bultos`). Layout de campos: con **UN. MÁXIMAS** mostrar `FORMA PEDIR | PUNTO REPOSICIÓN | UN. MÁXIMAS`; con **BULTO** mostrar `FORMA PEDIR | PUNTO REPOSICIÓN (EN UN.) | BULTO | TOTAL EN UN.` donde `TOTAL EN UN.` es solo lectura (`bulto ingresado × prod_tienda_bultos`). En ambos casos, las etiquetas del bloque usan altura uniforme para mantener una fila horizontal de labels y otra de inputs. Encima de la tabla del modal va el botón ícono `+` para abrir el selector de productos adicionales. No CANT. FIJA POR UNID.
