...
2. **Clases de filtros (SSOT en FilterBar / globals.css)**
   ...
   - **Select shadcn (obligatorio):** no usar `<select>` nativo. Importar `Select`, `SelectContent`, `SelectItem`, `SelectTrigger`, `SelectValue` de `@/components/ui/select` (incluye input **BUSCAR…** en `SelectContent`). Triggers tipo filtro: `SELECT_TRIGGER_FILTER_CLASS` / `input-filtro-unificado`; en modales/tablas compactas, mantener `h-8`/`h-9`/`w-full` con `cn()`. **Valor vacío:** Radix no admite `SelectItem value=""` — usar sentinel (`"none"` / `"todos"`) y mapear en `onValueChange` a `""` / estado vacío. Placeholder del ítem sentinel = mismo texto que la opción vacía previa (ej. `SELECCIONAR PROVEEDOR...`).
