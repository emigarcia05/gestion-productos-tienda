...
| **Celdas vacías (tablas)** | En tablas, los valores vacíos/null **se renderizan en blanco** (string vacío). **No** usar `"-"` ni `"—"` como placeholder. Para valores opcionales, usar `fmtCelda()` / `fmtNumero()` de `@/lib/format` (devuelven `""` para vacío). **PROM. VTA. P/ DÍA** (Pedido A Fáb.): `fmtPromVtaDiaria()` (2 decimales; no `fmtNumero`). |
...
*Última actualización (2026-08-11): **Pedido A Fáb.** — PROM. VTA. P/ DÍA a **2 decimales** (`fmtPromVtaDiaria`; evita que 65/48 y 40/48 colapsen a 1).
