...
*Última actualización (2026-07-27): **Asistente IA · cuentagotas** — en **Buscar Color Desde Imagen**: botón **Abrir Imagen Muestra** (`CuentagotasImagenMuestra`) carga imagen solo en cliente (`ObjectURL` + canvas; no upload ni persistencia). El usuario selecciona zona (clic o arrastre); se promedia RGB (`colorMuestraImagen.ts`). El prompt es el de **`prod_ia_diseno_promp`**; `aplicarRgbAlPromptBuscarColor` sustituye **`(R,G,B)`** por el RGB del cuentagotas. CTA **Buscar Color Desde Imagen** copia ese prompt y abre `url_redireccion`. Excepción de estilo: `style` dinámico para tamaño del lienzo y `backgroundColor` del swatch.*
