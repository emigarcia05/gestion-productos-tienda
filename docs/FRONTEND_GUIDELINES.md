...
- **`LimpiarFiltrosButton`**: el tacho global **siempre se muestra**; no reintroducir una prop `visible` (eliminada 2026-08-13: el cuerpo ya no la leía y los call sites solo pasaban `hayFiltros` muerto).
...
- **Hooks huérfanos:** no conservar `useListaPreciosTiendaModalSync`, `useSyncListaPreciosStatusPoll`, `SyncModal` (sync → **`SyncStatusIndicator`** + **`DuxSyncStyleButton`**, 2026-06-30) ni **`useMirrorScrollTop`** / **`usePieFijoColumnWidthsSync`** (sin call sites; Tesorería / Balance · Gastos usan **`.finanzas-resumen-tarjeta`**, 2026-08-13).
- **Helpers UI huérfanos:** no reintroducir **`src/lib/pedidosTabs.tsx`** (tabs de pedidos; la navegación es el sidebar) ni **`src/lib/calendarioMesFinanzas.ts`** (grilla mensual AR sin imports; `FiltroRangoFechasCalendarioModal` arma su propio calendario en zona local — **no** unificar).
- **Exports UI huérfanos:** no reintroducir `FilterRowNoSearchActions`, `MODAL_FIELD_LABEL_CLASS`, `fmtPct` de `@/lib/format` (1 decimal + `≈0%`; el `fmtPct` **local** de `GraficoMcVsPorcUtilidad` es otro, entero `Math.round`), `parsePorcentaje0a100Input` / `formatPorcentaje0a100Input` (los inputs de % usan `PorcentajeCentInput` + `porcentajeCentMask.ts`).
- **Componentes huérfanos:** antes de crear un modal/página, verificar que un `*PageClient` o ruta App Router lo importe. Tras redirects (p. ej. `/precios-competencia` → Px Competencia), borrar la grilla standalone y filtros legacy del cluster, conservando modales compartidos (`AsociarUrlsCompetenciaModal`, `RelevamientoUltimoMensaje`, etc.). No reintroducir `SelectorProveedor`, `EditarCostoObjetivoModal`, `GestionCategoriasPageClient` / `ComparacionCategoriasCatalogoPageClient` (el Finder vivo es **`ComparacionCategoriaSelector`** dentro de **`ComparacionCategoriasClient`**), **`src/components/ui/tabs.tsx`** (shadcn Tabs sin call sites; 2026-08-13) ni **`VincularModal.tsx`** (vínculos = subfilas `CxCompraVinculosDetalle` + `SeleccionarProductoModal`).
...
2. **Clases de filtros (SSOT en FilterBar / globals.css)**
   ...
   - **Sin búsqueda por descripción**: cuando una pantalla no tiene input de búsqueda y los filtros entran en una sola línea, ubicar las acciones en la **misma fila** usando un slot inline dentro de `FilaFiltrosDesplegables` (`FILTER_INLINE_ACTION_SLOT_CLASS`, por ejemplo con `col-span-2`) para compactar altura. Si no entra en una sola línea, añadir otra fila dentro de `FilterBar` (p. ej. `flex items-center justify-end w-full`); **no** reintroducir `FilterRowNoSearchActions` (export huérfano, retirado 2026-08-13).
...
### Tienda — Vínculos con Proveedores (`TablaTienda` + `CxCompraVinculosDetalle`)

El modal **`VincularModal.tsx` no existe**. El flujo vivo está en la grilla de **Vinculación Con Prov.** (`TablaTienda.tsx`):

- **Vincular nuevo producto:** ícono **`Link2`** (columna **ACCIONES**) abre **`SeleccionarProductoModal`** (`modal-app`). Recibe `itemDescripcion`, `marca`, `rubro`, `subRubro` y `idsProveedoresYaVinculados` (exclusión por `proveedorId`). Toggle **Producto TiendaColor** (`Tag` → `setProductoPropioTiendaAction`) vive en ese modal: si `esPropio`, se oculta el alta de vínculo y el cuerpo muestra aviso de producto propio.
- **Ver / editar vínculos existentes:** chevron de la fila expande **`CxCompraVinculosDetalle`** (subfilas `tabla-fila-detalle-competencia`), no un modal de tabla.
- **Subfila** (columnas alineadas a la grilla padre: hueca, descripción, prefijo, **CX PROD.**, desvincular):
  1. Celda hueca (columna COD. TIENDA).
  2. **DESCRIPCIÓN** (`descripcion_proveedor`, `truncate` + `title`).
  3. **PROVEEDOR**: prefijo.
  4. **CX PROD.** (`.cx-compra-subfila-cx-prod`): checkbox **BASE** (`establecerCostoListaTiendaAction(codTienda, codExt | null)`) + variación vs. base (`variacion-costo--*` / `variacion-costo-icon--positiva|negativa`) + precio (`px_compra_final_sin_iva`). Destildar la base = Cx. Prom.
  5. **DESVINC.** (solo editor): `Trash2` + `TABLE_ROW_ICON_BUTTON_FILLED_BRAND_CLASS` → `desvincularProducto`.
- **Orden de filas:** `ordenarFilasVinculosTienda` — primero el proveedor oficial (`prefijoProveedor` / `proveedorDux`); el resto por px. final ascendente.
- **Producto propio:** `CxCompraDetallePropio` (aviso centrado, sin filas de vínculo). Columna **VINCULACIÓN** de `TablaTienda`: badge **PROPIO** si `esProductoPropio`.
- **`.tabla-vinculos-modal`:** ya no es el modal de vínculos; se reutiliza en modales de **reglas de descuento** de lista precios. **No** reintroducir `.modal-vinculos-*` ni `VincularModal.tsx`.
...
| `MODAL_MICRO_LABEL_CLASS` (`@/lib/ui-classes`) | Clases Tailwind para micro-etiquetas en modales (foreground). **`ModalMicroLabel`** usa esta constante vía CVA. La clase CSS **`.modal-field-label`** permanece (la incluye la micro constante). No reintroducir `MODAL_FIELD_LABEL_CLASS`. |
...
| `.tabla-gestion-compacta.tabla-vinculos-modal` | Variante de ancho `table-layout: fixed` para tablas densas en modales (p. ej. **reglas de descuento** de lista precios). Los encabezados usan la misma regla global de `tabla-gestion-compacta`. |
...
| `.tabla-bloque-secundario-cell`, `.tabla-bloque-secundario-cell-divider` | Celdas de **tbody** secundarias; fondo transparente (cebra). `*-divider`: línea vertical **#0072bb** con `box-shadow: inset 1px 0 0 #0072bb` (evita artefactos con `border-collapse: collapse` y scroll). Las subfilas de vínculos (`CxCompraVinculosDetalle`) usan `*-cell-divider` en **CX PROD.** |
...
| *(retiradas 2026-08-13)* `.celda-sublinea-tabla`, `.celda-sublinea-tabla-divisor`, `.lista-precios-sublinea-*` (incl. `.lista-precios-sublinea-descuentos`, `.lista-precios-sublinea-grid`) | Lista precios ya no usa sublínea «Desc. en fila»: **DESC.** es ícono `%` → `DescuentosAplicadosListaPreciosModal`. No reintroducir. |
| *(retirada 2026-08-13)* `.contenedor-tabla-gestion--pie-fijo-pie` | Segunda `<table>` de pie; Tesorería / Gastos usan **`.finanzas-resumen-tarjeta`**. |
| *(retiradas 2026-08-13, 2.ª pasada)* `.contenedor-tabla-paginada`, `.selector-cuadro--circle`, `.tabla-head-toggle`, `.cx-compra-subfila-cx-prod__meta-base`, `.card-contenedor-tabla`, `.contenedor-tabla-card`, `.panel-con-cabecera`, `.panel-cabecera-primary`, `.modal-app__body--centered`, `.modal-app__body-title`, `.modal-app__form-row`, `.modal-resumen-producto*`, `.modal-panel-scroll*`, `.modal-mensaje-carga`, `.modal-mensaje-vacio`, `.variacion-costo-icon--neutra`, `.modal-tabla-*`, `.contenedor-tabla-gestion--altura-contenido`, `.tabla-fila-mc-subtotal`, `.est-vtas-link-btn`, `.proceso-paso-imagen-btn`, alias `.input-filtro-custom` / `.btn-main` | Sin call sites en `src/`; el vivo es `.card-tabla-envoltorio`, `.modal-app__*`, `ModalTablaConFiltros`, `--mc-overlay` / `tabla-fila-mc-sep-linea`, `.input-filtro-unificado` y `main button`. No reintroducir. |
...
- **Implementación:** `globals.css` (`--modal-field-label-color` + selectores en `.app-modal__body`, `.modal-app__body`, `[data-slot="dialog-content"]`). Constante **`MODAL_MICRO_LABEL_CLASS`** en `@/lib/ui-classes` (incluye `.modal-field-label`). Preferir **`ModalMicroLabel`**.
...
- **Navegación lateral por área activa** (`Sidebar.tsx`): acordeón vertical en **Vendedor**, **Administración** y **Marketing**. Jerarquía por **indent + tipografía + chevron**; **sin** guías verticales de árbol. Separadores **`.sidebar-nav-divider`** **solo entre módulos de primer nivel**. **`data-active`** solo en la página actual; padres con **`data-ancestor`**. En **Vendedor** el orden de módulos es **MERCADERÍA** → **PRECIOS** → **CALCULAR LTS** → **CONTROL STOCK** → **CARGAR GASTOS** → **ASISTENTE IA** (accesibles al rol vendedor/`simple`; CRUD de prompts del Asistente IA solo `editor`). Dentro de **MERCADERÍA**: agrupador **Cant. Pedida** (Urgente / Tintométrico / Reposición) → **Generar Pedido** → **Recepción Pedido**. **PRECIOS**: **Px Sugeridos** + **Px Tintométricos**. **CONTROL STOCK**, **CALCULAR LTS** y **CARGAR GASTOS** son enlaces directos (sin hijos). **ASISTENTE IA**: **Buscar Cód. Imagen** + **Diseñar** (rutas `GP_ROUTES.asistenteIa.*`). Módulo **Procesos** eliminado. **Control Stock** (`GP_ROUTES.ayudaVendedor.controlStock`, `PERMISOS.stock.acceso`) es enlace directo en sidebar Vendedor (ruta canónica conservada). Con área **Administración** el sidebar muestra **4 pilares** (`AdministracionAccordionNav`, SSOT `administracionNav.ts`): **FINANZAS**, **LISTA PRECIOS**, **PEDIDO A FÁB.**, **ESTADÍSTICAS**. Desglose: **FINANZAS** → **BALANCE** | **OPERACIONES** → pantallas; **LISTA PRECIOS** → **PX TIENDA** | **PROVEEDORES** | **ANÁLISIS M.C.** → pantallas; **PEDIDO A FÁB.** → **Pedido A Fáb.** (`/pedido-a-fabrica`); **ESTADÍSTICAS** → **VENTAS** | grupo **CONFIGURACION** (**Carga De Datos** | **Configuracion**). **ANÁLISIS M.C.** vive bajo **LISTA PRECIOS** (no bajo FINANZAS). En **Marketing** (`MARKETING_MODULES`, SSOT rutas `src/lib/marketingRoutes.ts`): módulo **`PUBLICACIONES`** (`Megaphone`) con **Calendario** → `/marketing/publicaciones/calendario` (`CalendarRange`), **Ideas Contenido** → `/marketing/publicaciones/ideas` (`Lightbulb`) y **Objetivos**; módulo **`BASE MULTIMEDIA`**; permiso **`PERMISOS.marketing.acceso`** (**libre** para `simple` y `editor`; mutaciones CRUD solo `editor`).
- **`SidebarMainAppArea`**: solo **logo** decorativo (sin nombre de área, sin modal).
- **`SidebarAreaSwitcher`**: botón `sidebar-user-switcher-surface` debajo del logo; muestra el área actual (MAYÚSCULAS) y al hover **CAMBIAR MÓDULO**. Click → modal **Módulos De La Aplicación** con las tres opciones. **Primera apertura de sesión** (`sessionStorage` `main-app-area-elegida`): el modal se abre solo y **no se puede cerrar** hasta elegir un módulo (sin X ni Cerrar). **Vendedor** y **Marketing**: `router.push` libre. **Administración** (`requierePassword`): si la sesión no es `editor`, modal de clave → `activarModoEditor` (`EDITOR_PASSWORD`) y luego navega; si ya es `editor`, navega directo.
- **Acordeón sidebar (cerrado al inicio)**: en **Vendedor**, **Marketing** (`Sidebar.tsx`) y **Administración** (`AdministracionAccordionNav`) los módulos/pilares arrancan **cerrados** (no se auto-abren por la ruta). Al cambiar de área se cierran todos. **Módulo/pilar/grupo con un solo destino navegable visible**: el click **navega directo** (sin chevron/acordeón); ej. **PEDIDO A FÁB.** → `/pedido-a-fabrica`.
...
- **Administración · pilares** (SSOT `administracionNav.ts`): **FINANZAS** · **LISTA PRECIOS** · **PEDIDO A FÁB.** · **ESTADÍSTICAS**. **Estadísticas** (URLs `/estadisticas-productos/*`, SSOT `ESTADISTICAS_PRODUCTOS_ROUTES`): pantalla **VENTAS** (`estadisticas-vtas`); grupo **CONFIGURACION** → **Carga De Datos** + **Configuracion** (ruta `categorizacion`). **Pedido A Fáb.** (SSOT `pedidoAFabricaRoutes.ts`, `/pedido-a-fabrica`; legacy `/estadisticas-productos/est-para-compra` → redirect). `/estadisticas-productos` → redirect a Carga De Datos.
...
- **Jerarquía canónica de URLs (2026-06-19):** SSOT en **`src/lib/gestionProductosRoutes.ts`** (`GP_ROUTES`). Prefijo **`/gestion-productos/{módulo}/{agrupador}/{submódulo}`** alineado al sidebar. Rewrites en `next.config.ts` sirven páginas internas (`src/app/pedidos`, `proveedores`, `tienda`, …); redirects permanentes desde URLs legacy y canónicas anteriores.
  ...
  - **Ayuda Vendedor:** `/gestion-productos/ayuda-vendedor/px-venta/{px-vta-sugerido|px-tintometrico}`, `/gestion-productos/ayuda-vendedor/{calc-litros|cargar-gasto|control-stock}`.
...
- **Acceso**:
  - **Vendedor** / **Marketing**: libre (`requierePassword: false`).
  - **Administración**: pide clave si `rol !== "editor"` → `activarModoEditor` + navegación.
- **Logo** (`SidebarMainAppArea`): decorativo; ya no abre el selector ni muestra el nombre del área.
...
**PRECIOS** (rol **vendedor**/`simple`): **Px Sugeridos** + **Px Tintométricos**. **CONTROL STOCK**, **CALCULAR LTS** y **CARGAR GASTOS** son módulos de enlace directo. **Control Stock** → `GP_ROUTES.ayudaVendedor.controlStock` (`PERMISOS.stock.acceso`). **Cargar Gasto** (`/gestion-productos/cargar-gasto`, `PERMISOS.ayudaVendedor.cargarGasto` para `simple` y `editor`): abre al entrar el modal **`GastoUnicoBalanceModal`** (**Nuevo Gasto Eventual**, mismo flujo que **GASTO EVENTUAL** en `/finanzas/balance/gastos`); periodo = mes/año calendario Argentina. Actions: `listarFinBalGastosFinalesNoMensualesAction` / `crearFinBalImputacionGastoUnicoAction` con gate `requireCargarGastoEventual` (permiso `cargarGasto`).
...
- Sidebar por rol: **simple** → **MERCADERÍA** + **PRECIOS** + **CALCULAR LTS** + **CONTROL STOCK** + **CARGAR GASTOS** + **ASISTENTE IA**; **editor** → mismos módulos Vendedor + área Administración/Marketing. **Cx Compra** solo **editor** (`PERMISOS.tienda.acceso`).
...
- [ ] **Anchos de columna estáticos** en tablas con `<colgroup>`: usar `<col className="w-[x%]" />` (ver `ComparacionCategoriasClient`, `TablaFlujoDeFondo`, `TablaTienda`). `style={{ width }}` queda solo para anchos verdaderamente dinámicos (p. ej. `esEditor ? "24%" : "36%"`).
...
### Auditoría 2026-08-13 (limpieza frontend — código muerto)

Sin cambios funcionales ni estéticos intencionales. Solo dead code, props ignoradas, CSS huérfano y docs alineadas al código.

**1.ª pasada**

- **Componentes huérfanos eliminados:** `SelectorProveedor.tsx` (filtros de pedidos ya cubren PROVEEDOR); `EditarCostoObjetivoModal.tsx`; `GestionCategoriasPageClient.tsx` (wrapper) + `ComparacionCategoriasCatalogoPageClient.tsx` (Finder duplicado; el vivo es `ComparacionCategoriaSelector` en `ComparacionCategoriasClient`); `src/components/ui/tabs.tsx` (shadcn Tabs sin import).
- **Hooks / helpers UI huérfanos:** `useMirrorScrollTop.ts`, `usePieFijoColumnWidthsSync.ts`, `src/lib/pedidosTabs.tsx`.
- **`LimpiarFiltrosButton`:** se elimina la prop `visible` (ya no se leía) y los `hayFiltros` / `hayFiltrosOBusqueda` / `hayFiltrosCatalogo` que solo la alimentaban. El tacho global sigue **siempre visible**.
- **CSS muerto en `globals.css`:** `.contenedor-tabla-gestion--pie-fijo-pie`; `.celda-sublinea-tabla` / `-divisor`; bloque `.lista-precios-sublinea-*` (el toggle «Desc. en fila» ya no existe; **DESC.** = ícono `%`).
- **Exports muertos:** `FILTER_TEXT_COLOR_CLASS` (`FilterBar`); reexport de `toolbarActionButtonVariants` (CVA interno sin consumidores).
- **Docs:** catálogo §2 y Guía para IA §8 (pie fijo) alineados; `GestionCategoriasModal` no existe (referencia histórica en la auditoría 2026-05-07).
- **Lint (preexistente):** `GraficoMcVsPorcUtilidad` — `setMostrarCategoriasMc(false)` en `useEffect` envuelto en **`queueMicrotask`** (`react-hooks/set-state-in-effect`); mismo patrón de esta guía. Sin cambio observable.

**2.ª pasada**

- **Helpers / exports UI huérfanos:** `src/lib/calendarioMesFinanzas.ts` (sin imports; no unificar con `FiltroRangoFechasCalendarioModal`); `FilterRowNoSearchActions`; `MODAL_FIELD_LABEL_CLASS`; `fmtPct` / `parsePorcentaje0a100Input` / `formatPorcentaje0a100Input` de `@/lib/format`.
- **CSS huérfano:** alias `.input-filtro-custom` y `.btn-main` (el vivo es `.input-filtro-unificado` y `main button`); bloques `.contenedor-tabla-paginada`, `.selector-cuadro--circle`, `.tabla-head-toggle`, `.cx-compra-subfila-cx-prod__meta-base`, `.card-contenedor-tabla`, `.contenedor-tabla-card`, `.panel-con-cabecera` / `.panel-cabecera-primary`, `.modal-app__body--centered` / `__body-title` / `__form-row`, `.modal-resumen-producto*`, `.modal-panel-scroll*`, `.modal-mensaje-carga` / `-vacio`, `.variacion-costo-icon--neutra`, `.modal-tabla-*`, `.contenedor-tabla-gestion--altura-contenido`, `.tabla-fila-mc-subtotal`, `.est-vtas-link-btn`, `.proceso-paso-imagen-btn`.
- **Docs:** sección viva de vínculos Tienda reescrita (`VincularModal.tsx` no existe; flujo = `TablaTienda` + `CxCompraVinculosDetalle` + `SeleccionarProductoModal`); catálogo §2 y anti-código muerto alineados.
- **No aplicado (riesgo funcional/estético):** migrar `btn-primario-gestion` → `ToolbarActionButton`; extraer Finders Categorías/Gastos/Marketing; unificar `fmtPct` local de `GraficoMcVsPorcUtilidad` con `@/lib/format`; unificar calendarios (zona local vs AR); quitar CVA `density="tight"` de `ToolbarActionButton`; aliases `@deprecated` de dominio y stores no-UI.
...
- **Grillas:** una sola **`grid-cols-*`** por nodo en **`PedidoHistoriaDetalleModal`** (**`GRID_CAPAS_SUP_PEDIDO_HISTORIA`**), **`AgregarProductosModal`**, **`TiendaCalcLitrosPageClient`**, **`GastoUnicoBalanceModal`**, **`NuevoItemTintometricoModal`**.
...
*Última actualización (2026-08-13): **Auditoría limpieza frontend (2.ª pasada)** — `calendarioMesFinanzas`, `FilterRowNoSearchActions`, `MODAL_FIELD_LABEL_CLASS`, `fmtPct`/`parsePorcentaje*` de `@/lib/format`; CSS huérfano (`.modal-tabla-*`, `.panel-con-cabecera`, alias `.input-filtro-custom`/`.btn-main`, etc.); docs de vínculos Tienda sin `VincularModal`. Sin cambios funcionales ni estéticos.

*Última actualización (2026-08-13): **Auditoría limpieza frontend** — dead code (`SelectorProveedor`, catálogo Comp. Categorías huérfano, `ui/tabs`, hooks `useMirrorScrollTop` / `usePieFijoColumnWidthsSync`, `pedidosTabs`); CSS sublínea lista precios y `--pie-fijo-pie`; prop `visible` de `LimpiarFiltrosButton`; docs §2 / §5 / anti-código muerto. Sin cambios funcionales ni estéticos.
