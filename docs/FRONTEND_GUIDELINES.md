...
- **`/pedido-a-fabrica`** (**Pedido A Fáb.**): pilar sidebar **PEDIDO A FÁB.**; `PedidoAFabricaPageClient`. **FilterBar**: **PROVEEDOR** (`FiltroIndividualContainer` + `input-filtro-unificado`; solo `es_fabrica = true` vía `getProveedoresFabrica`) + **TIEMPO STOCK** (`Input` numérico entero, solo dígitos, `INPUT_FILTER_CLASS`, cesto individual). Al elegir proveedor: **`TablaPedidoAFabrica`** — **DESCRIPCIÓN** + una columna de grupo por cada sucursal con **`pedido = true`** (nombre en fila 1 del `thead`, `colSpan={2}`), subdividida en **STOCK ACTUAL** | **PROM. VTA.** (fila 2; patrón `rowSpan`/`colSpan` como **Px Listas**), y al final grupo **TOTAL** con las mismas subcolumnas = **suma** de stock y de prom. vta. de todas las sucursales. Datos: stock = `prod_tienda_stock.stock_real` del depósito de la sucursal; prom. vta. = promedio diario `(ventas de los 2 meses calendario previos) / 48` (24 días × 2), redondeado al entero (`@/lib/pedidoAFabricaPromVta`; requiere `cod_tienda`). Header **Info Promedio** (`ToolbarActionButton`) abre **`InfoPromedioPedidoAFabricaModal`** con la explicación del cálculo. Carga vía `getProductosPedidoAFabricaAction` + `PaginacionClient` (100/página). Legacy **`/estadisticas-productos/est-para-compra`** → `permanentRedirect` a canónica.
