...
- **`SidebarAreaSwitcher`**: fila de usuario dentro del mismo dock (sin superficie propia). **Ícono módulo (15 %) | nombre (85 %)** solo si `modulos_permitidos.length > 1` (`Store` / `Landmark` / `Megaphone`); click → **Cambiar Módulo**. Con un solo módulo: solo el nombre. Ambas filas del dock usan **`h-9`** + hover `sidebar-accent/80`. **Primera visita**: modal **Elegir Usuario** con lista clickeable. Si tiene **Administración**, pide clave. Sucursal = `sucursal_por_defecto`. Al aplicar usuario → `avisarAdvertirTransfPendientes()`.
...
- **Usuario**: **ícono del módulo actual** (15 %) + **nombre** MAYÚSCULAS (85 %, `truncate`) si puede cambiar de módulo; click → modal **Cambiar Módulo**. Con un solo módulo: solo el nombre centrado.
