...
| `.sidebar-nav-tree` | Guía vertical del acordeón: línea `::before` amarilla **tenue** (`color-mix` ~40 % de `--sidebar-indicator`) en el margen izquierdo del hover (`left` = `--sidebar-nav-tree-indent`). |
| `.sidebar-nav-item`, `.sidebar-nav-module` | **Hover**: solo `bg-sidebar-accent` (sin barra). **`data-active`**: página actual — accent + barra amarilla (`--sidebar-indicator`). **`data-ancestor`**: padre/agrupador con hijo activo — tipografía semibold, **sin** accent ni barra. Íconos siempre blancos. |
