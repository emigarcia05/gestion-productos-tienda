...
- **`SidebarAreaSwitcher`**: botón `sidebar-user-switcher-surface` debajo del logo; muestra **`SUC / MÓDULO`** (abrev. sucursal + área en MAYÚSCULAS, ej. **GUAY / VENDEDOR**) y al hover **CAMBIAR SUC / MÓDULO**. Click → modal **Sucursal Y Módulo**: primero **Sucursal** (GUAYMALLÉN / MAIPÚ) y luego **Módulo** (las tres áreas; deshabilitado hasta elegir sucursal). **Primera apertura de sesión** (`sessionStorage` `main-app-area-elegida` **y** `main-app-sucursal-preferida`): el modal se abre solo y **no se puede cerrar** hasta elegir sucursal y módulo (sin X ni Cerrar). La sucursal queda como default de filtros (STOCK / Trans. Depósitos); el usuario puede cambiarla después. **Vendedor** y **Marketing**: `router.push` libre. **Administración** (`requierePassword`): si la sesión no es `editor`, modal de clave → `activarModoEditor` (`EDITOR_PASSWORD`) y luego navega; si ya es `editor`, navega directo.
...
- **Click**: modal **Sucursal Y Módulo** — sección **Sucursal** (`@/lib/sucursalPreferida.ts`, `sessionStorage` `main-app-sucursal-preferida`) + sección **Módulo** (`MAIN_APP_AREAS`).
- **Onboarding**: exige sucursal **y** módulo (`main-app-area-elegida` + sucursal preferida); no se cierra hasta completar.
