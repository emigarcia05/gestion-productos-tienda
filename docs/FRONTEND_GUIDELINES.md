...
- **`SidebarMainAppArea`**: indicador compacto (reemplaza el logo). Vista fija: un botón **Pendientes →**. Hover (`Tooltip` `side="right"`) despliega al costado una grilla de 3 columnas alineadas (**sección** | **tipo** | **cantidad** `tabular-nums`): bloque **Pedido** (Urgente / Tintométrico / Reposición = ítems con cant. pedir &gt; 0 de **Generar Pedido**), divisor `h-px bg-primary`, bloque **Transferencia** (Emisión TRANSFERIR / Recepción RECIBIR = filas del modal **Transf. Pendiente Registro**). Click en **Pendientes** con emisión o recepción &gt; 0 abre el aviso. Action `getIndicadorSlidenavAction`. Refresh al cambiar sucursal, al navegar, al enfocar, cada 30 s y vía `EVENTO_INDICADOR_SLIDENAV`. Tras **Elegir Usuario**, `EVENTO_ADVERTIR_TRANSF_PENDIENTES` recarga y, si hay transferencia pendiente, modal **Transferencias Pendientes** (`CALLOUT_WARNING_CLASS` + **Ir A Trans. Depósitos**). No se dispara al **Cambiar Módulo**.
- **`SidebarAreaSwitcher`**: superficie `sidebar-user-switcher-surface` debajo del indicador de pendientes. Una fila **ícono módulo (15 %) | nombre usuario (85 %)**: el ícono del área actual (`Store` Vendedor / `Landmark` Administración / `Megaphone` Marketing) solo si `modulos_permitidos.length > 1`; click abre **Cambiar Módulo**. Con un solo módulo, solo el nombre centrado. **Primera visita**: modal **Elegir Usuario** (catálogo `global_personal` con sucursal y módulos cargados; no se cierra hasta Aplicar). Si el usuario tiene **Administración**, pide clave (`activarModoEditor`); después puede cambiar de módulo sin volver a pedirla. Sucursal = `sucursal_por_defecto` del usuario (`sessionStorage`). Al aplicar usuario dispara `avisarAdvertirTransfPendientes()`.
...
### Slidenav — Usuario y módulo (`src/components/shared/SidebarAreaSwitcher.tsx`)

Bloque en el pie de la slidenav (**debajo** del indicador de pendientes), superficie **`sidebar-user-switcher-surface`**.

- **Fila**: `grid-cols-[15%_85%]` — **ícono del módulo actual** (15 %) + **nombre de usuario** en MAYÚSCULAS (85 %, `truncate`). Íconos: Vendedor `Store`, Administración `Landmark`, Marketing `Megaphone`.
- **Cambiar Módulo**: el ícono solo aparece si `modulos_permitidos.length > 1`; toda la fila es el botón (`aria-label` **Cambiar Módulo**) → modal con `Select` de los módulos permitidos + **Aplicar**. Con un solo módulo: solo el nombre, centrado, sin ícono ni botón.
- **Onboarding (primera visita de la pestaña)**: modal **Elegir Usuario** (`listUsuariosParaInicioSesionAction`). Placeholder **USUARIO**. No se cierra hasta Aplicar (sin X). Persiste `sessionStorage` `main-app-usuario-sesion` y copia `sucursal_por_defecto` a `main-app-sucursal-preferida`. Si la sucursal tiene **Emisión** o **Recepción** &gt; 0, al cerrar el onboarding se abre el modal **Transferencias Pendientes**.
