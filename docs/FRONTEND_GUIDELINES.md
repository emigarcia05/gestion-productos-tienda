...
- **`/estadisticas-productos/estadisticas-vtas`** (**Estadísticas Vtas**): **`ClassicFilteredTableLayout`** título **ESTADÍSTICAS PRODUCTOS**, subtítulo **Estadísticas Vtas**. **Dos** `FilterBar` (`filtros-contenedor-tienda bg-card`): (1) misma fila de 6 desplegables + búsqueda que Categorizacion (Marca/Rubro/Sub Rubro/Color/Terminacion/Presentacion + `FiltroBusquedaInput`); (2) **UNIDADES** (`UNIDAD` \| `SUMA DE UNIDADES`) · **SUCURSAL** (`genera_est`) · **AÑO** multi · **MES** multi (default = periodo más reciente con ventas; al menos uno de cada; patrón checkbox como Balance · Gastos). Contador de productos filtrados. **Tres gráficos** en fila: (1) `EstVtasGraficoVarianteBarras` — título Select **píldora `primary`** (**Dimensión**); opciones compartidas `EST_VTAS_DIMENSION_OPTIONS` = VARIANTE · MARCA · RUBRO · SUB RUBRO · COLOR · TERMINACION · **SUCURSAL** (default **MARCA**); **Desglose** = `SIN DESGLOSE` + las mismas dimensiones **excepto la ya elegida en Dimensión** (`opcionesDesgloseEstVtas` / `opcionesDimensionEstVtas`, sin repetir); SIN DESGLOSE = barras planas (`agregarUnidadesPorEjeY`); con desglose = grupos categoría → hijos (`agregarUnidadesPorDobleDimension`, azules estables + leyenda); clic filtra G2/G3 (producto y/o sucursal según cruce); (2) `EstVtasGraficoTopProductos` — **tabla Top 10** (`agregarTopProductos`): columnas **DESCRIPCION** 70 % (botón `.est-vtas-desc-btn`, sin formato CTA) · **TO.** 15 % (total periodo) · **PM.** 15 % (promedio mensual); ranking por total del periodo FECHA; promedio = acumulado sin FECHA / periodos mes×año con venta; clic en descripción filtra G3; (3) `EstVtasGraficoBarrasMensual` — barras **verticales** ENE–DIC; **respeta AÑO** multi (ENE…DIC; ignora filtro MES de página); aplica filtros de página no temporales + G1/G2 si hay selección (`codTienda` si hay producto del Top 10). El Top 10 se muestra siempre con los filtros de página; si hay selección en G1, también la aplica. Sidebar: **Estadísticas Vtas**. Cliente **`EstVtasPageClient`**; ruta SSOT `ESTADISTICAS_PRODUCTOS_ROUTES.estadisticasVtas`.
