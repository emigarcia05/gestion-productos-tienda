...
- **`/pedido-a-fabrica`** (**Pedido A Fáb.**): pilar sidebar **PEDIDO A FÁB.**; `PedidoAFabricaPageClient`. **FilterBar**: **PROVEEDOR** (`FiltroIndividualContainer` + `input-filtro-unificado`; solo `es_fabrica = true` vía `getProveedoresFabrica`; opción incluye `tiempoEntregaEnDias`) + **TIEMPO STOCKEO** (`Input` numérico entero, solo dígitos, `INPUT_FILTER_CLASS`, cesto individual; alimenta el cálculo de **CANT. SUGERIDA**). Al elegir proveedor: **`TablaPedidoAFabrica`** — **DESCRIPCIÓN** + una columna de grupo por cada sucursal con **`pedido = true`** (nombre en fila 1 del `thead`, `colSpan={2}`), subdividida en **STOCK ACTUAL** | **PROM. VTA.** (fila 2; patrón `rowSpan`/`colSpan` como **Px Listas**), y al final grupo **TOTAL** (`colSpan={3}`): **CANT. SUGERIDA** | **CANT. A PEDIR** | tilde (`tabla-check-toggle`; cabecera con ícono **Check**). **CANT. SUGERIDA** (solo lectura): `calcularCantSugeridaPedidoAFabrica` en `@/lib/pedidoAFabricaPromVta` con stock/prom totales (suma sucursales), `tiempo_entrega_en_dias` del proveedor y **TIEMPO STOCKEO** — ver fórmula en `docs/BACKEND_GUIDELINES.md` §1.11f. Sin **TIEMPO STOCKEO** la celda queda vacía y el tilde deshabilitado. **CANT. A PEDIR**: `Input` numérico local por `codExt`. Tilde: al clickear copia **CANT. SUGERIDA** → **CANT. A PEDIR** y queda activo (`aria-pressed`) mientras coinciden. Datos por sucursal: stock = `prod_tienda_stock.stock_real` del depósito; prom. vta. = `(ventas 2 meses previos) / 48`, `Math.round` (`@/lib/pedidoAFabricaPromVta`; requiere `cod_tienda`). Header **Info Promedio** → **`InfoPromedioPedidoAFabricaModal`**. Carga: `getProductosPedidoAFabricaAction` + `PaginacionClient` (100/página). Legacy **`/estadisticas-productos/est-para-compra`** → `permanentRedirect`.
