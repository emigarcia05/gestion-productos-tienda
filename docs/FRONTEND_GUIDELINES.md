...
- **`/estadisticas-productos/ventas-por-producto`** (**Carga de Datos**): **`ClassicFilteredTableLayout`** `contentWidth="full"`, título **ESTADÍSTICAS PRODUCTOS**, subtítulo **Carga de Datos**. Cascarón **`area-page-shell`**. Grilla **periodo × sucursal**: filas = meses desde el **mes actual AR** hasta **Septiembre 2025** (primera fila = más actual); columnas = sucursales con `deposito` no nulo. Celda (solo `editor`): **Subir** (`FileUp` → **`ImportarEstPorProdModal`** con periodo/sucursal fijos desde la celda) y **Borrar** (`Trash2` → `eliminarEstPorProdPorPeriodoAction`) si hay registros. Sidebar: **Carga de Datos**. `PERMISOS.estadisticasProductos.acceso`.
