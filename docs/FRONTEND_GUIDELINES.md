...
- **`SidebarMainAppArea`**: fila **Pendientes** del dock de sesión. Vista fija: label **PENDIENTES** (`uppercase`, `text-center`) + badge de **categorías** (Pedido si Urgente+Tintométrico+Reposición &gt; 0 → 1; Transferencia si Emisión+Recepción &gt; 0 → 1; **máx. 2**, `tabular-nums`) + chevron. Badge con `bg-accent2` si Emisión o Recepción &gt; 0; si no, `bg-sidebar-accent`. Hover (`Tooltip` `side="right"`): grilla 3 columnas (**sección** | **tipo** | **cantidad**): **Pedido** / divisor `h-px bg-primary` / **Transferencia**. Click con transf. pendientes → modal aviso. Action `getIndicadorSlidenavAction`. Refresh al cambiar sucursal, navegar, enfocar, cada 30 s y `EVENTO_INDICADOR_SLIDENAV`. Tras **Elegir Usuario**, `EVENTO_ADVERTIR_TRANSF_PENDIENTES` abre el aviso si corresponde. No se dispara al **Cambiar Módulo**.
...
- **Pendientes**: hover con detalle Pedido / Transferencia; badge = cantidad de categorías con pendientes (máx. 2); `accent2` si hay Emisión o Recepción.
