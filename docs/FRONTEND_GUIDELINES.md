...
- **`/pedido-a-fabrica`** (**Pedido A Fáb.**): pilar sidebar **PEDIDO A FÁB.**; `PedidoAFabricaPageClient`. **FilterBar**: **PROVEEDOR** (`FiltroIndividualContainer` + `input-filtro-unificado`; solo `es_fabrica = true` vía `getProveedoresFabrica`) + **TIEMPO STOCK** (`Input` numérico entero, solo dígitos, `INPUT_FILTER_CLASS`, cesto individual). Al elegir proveedor: **`TablaPedidoAFabrica`** con primera columna **DESCRIPCIÓN** (`prod_precios_provee.descripcion_proveedor`, `habilitado = true`); carga cliente vía `getProductosPedidoAFabricaAction` + `PaginacionClient` (100/página). Legacy **`/estadisticas-productos/est-para-compra`** → `permanentRedirect` a canónica.
