...
- **`/estadisticas-productos/estadisticas-vtas`** (**Estadísticas Vtas**): **`ClassicFilteredTableLayout`** título **ESTADÍSTICAS PRODUCTOS**, subtítulo **Estadísticas Vtas**. **Dos** `FilterBar` (`filtros-contenedor-tienda bg-card`): (1) misma fila de 6 desplegables + búsqueda que Categorizacion (Marca/Rubro/Sub Rubro/Color/Terminacion/Presentacion + `FiltroBusquedaInput`); (2) **SUCURSALES** (`genera_est`), **FECHA** (periodos `EST_POR_PROD_CARGA_DESDE` → mes actual AR; default mes actual), **UNIDAD** (`UNIDAD` \| `SUMA DE UNIDADES`). Contador de productos filtrados. **Tres gráficos** en fila: (1) `EstVtasGraficoVarianteBarras` — título Select **píldora `primary`**; eje Y seleccionable (default **MARCA**); **Desglose** (`ninguno` \| `sucursal`): SIN DESGLOSE = barra por categoría; SUCURSAL = grupos categoría → sucursales (`agregarUnidadesPorEjeYDesgloseSucursal`, azules estables + leyenda); clic filtra G2/G3; (2) `EstVtasGraficoTopProductos` — **tabla Top 10** (`agregarTopProductos`): columnas **DESCRIPCION** (botón `.est-vtas-desc-btn`, sin formato CTA) · **TOTAL PERIODO** · **PROMEDIO MENSUAL**; ranking por total del periodo FECHA; promedio = acumulado sin FECHA / periodos mes×año con venta; clic en descripción filtra G3; (3) `EstVtasGraficoBarrasMensual` — barras **verticales** ENE–DIC; **ignora FECHA** (agrega todos los años); aplica filtros de página no temporales + G1/G2 si hay selección (`codTienda` si hay producto del Top 10). Sin selección en 1, el Top 10 muestra prompt. Sidebar: **Estadísticas Vtas**. Cliente **`EstVtasPageClient`**; ruta SSOT `ESTADISTICAS_PRODUCTOS_ROUTES.estadisticasVtas`.
...
*Última actualización (2026-08-07): **Est. Vtas · anchos 35/30/35** — G1 35 % · G2 30 % · G3 35 %; Top 10 columnas DESCRIPCION 80 % (2 líneas) · TOTAL/PROMEDIO 10 %.
