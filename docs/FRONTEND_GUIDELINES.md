...
- **`/estadisticas-productos/categorizacion`**: **`ClassicFilteredTableLayout`** título **ESTADÍSTICAS PRODUCTOS**, subtítulo **Categorizacion**. Listado `prod_tienda` (`listarProdTiendaCategorizacion`): **DESCRIPCIÓN**, **MARCA**, **RUBRO**, **SUB RUBRO**, **COLOR** (calculado vía catálogo), **LTS** (catálogo `est_por_prod_lts_conversion` + fallback «N LTS»). **Filtros** fila 1: Marca / Rubro / Sub Rubro / Color / Lts; fila 2: `FiltroBusquedaInput` por descripción + contador + limpiar. Paginación cliente. Solo `editor`: **`actions`** — **Gestion Colores** (`Palette` → **`GestionarEstPorProdColoresModal`**, catálogo `est_por_prod_colores` en MAYÚSCULAS; `onCatalogoChanged` → `router.refresh()`) · **Gestion Lts** (`Beaker` → **`GestionarEstPorProdLtsConversionModal`**: texto + conversión litros, p. ej. «440 CC» → 0,4; `onCatalogoChanged` → `router.refresh()`).
