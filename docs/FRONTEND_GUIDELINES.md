...
- **`SidebarAreaSwitcher`**: botón `sidebar-user-switcher-surface` debajo del logo; **dos filas**: sucursal · línea blanca `h-px w-[80%] bg-sidebar-foreground/85` · módulo (MAYÚSCULAS). Click → modal **Sucursal Y Módulo** con dos `Select` (`input-filtro-unificado`): **SUCURSAL** (placeholder, **sin default**) y **MÓDULO** (default **VENDEDOR**). Footer **Aplicar** (requiere sucursal). Si elige **ADMINISTRACIÓN** y `rol !== "editor"` → modal de clave (`activarModoEditor`). **Primera apertura** (`sessionStorage` `main-app-area-elegida` **y** `main-app-sucursal-preferida`): el modal se abre solo y **no se puede cerrar** hasta aplicar sucursal (sin X). Default de filtros STOCK / Trans. Depósitos. **Vendedor** / **Marketing**: `router.push` libre.
...
Botón de **dos filas** en el pie de la slidenav (**debajo** del logo), superficie **`sidebar-user-switcher-surface`**.
