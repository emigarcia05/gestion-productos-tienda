...
| `.sidebar-nav-divider` / `SidebarNavDivider` | Separador entre hermanas: `1px`, contraste por **`level`**. **1** (módulos): ~32 % + aire vertical. **2** (submódulos): ~16 %. **3** (hojas): ~10 % (en Vendedor las hojas suelen ir **sin** divisor). Márgenes al ícono de *ese* nivel (no atraviesan indent de padres). |
| `.sidebar-nav-tree` / `--nested` | Panel de hijos del acordeón: **solo indentación** (`padding-left`), **sin** guía vertical. `--nested` = segundo nivel de indent. |
