...
`@/components/shared/catalogo-finder/` + `CATALOGO_FINDER_*` en `ui-classes`. Columnas con header `bg-primary` (`headerVariant="finder"` default), `+` (`nuevoLado`: `end` default, `start` en Ideas), `className?` para altura en modal. Wizard Envios: `headerVariant="titulo"` (título de paso, sin barra primary) **en todos los pasos** (SUCURSAL / CLIENTES / DIRECCIONES / FECHA / MERCADERÍA). Paso CLIENTE: `mostrarNuevo={false}` y CTA **Nuevo Cliente** (`Button` `w-full` en wrapper `p-4`, no `rounded-none`) debajo de la lista. Filas: hover editar/eliminar por default; `eliminarSiempreVisible` deja el `Trash2` fijo a la derecha (`CATALOGO_FINDER_COLUMN_NOVO_BUTTON_CLASS`; Envios · CLIENTE) y el lápiz sigue en hover. Selección `CATALOGO_FINDER_ROW_SELECTED_CLASS`. `nombreAccion` opcional a la derecha del nombre (Envios: `MapPin` / `Phone` / `Users` en PINTOR). `nombreSufijo` opcional: ` - texto` en `font-normal text-[0.75em]` (Envios · CONSUMIDOR_FINAL con pintor asociado). `nombreCentrado` centra el nombre (Envios · CLIENTE). Usos: Catálogo Gastos (5 cols), Comp. Categorías (4), Ideas (2), Envios · Crear Envío (finder 1 col por paso, dentro del wizard modal).
