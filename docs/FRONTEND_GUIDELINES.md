...
- **`/estadisticas-productos/ventas-por-producto`** (**Carga de Datos**): **`ClassicFilteredTableLayout`** `contentWidth="full"`, título **ESTADÍSTICAS PRODUCTOS**, subtítulo **Carga de Datos**. Cascarón **`area-page-shell`**. Grilla **periodo × sucursal**: filas = meses desde el **mes actual AR** hasta **Septiembre 2025** (primera fila = más actual); columnas = sucursales con **`genera_est = true`**. **Unidad de trabajo = celda (mes/año × sucursal)**: no hay CRUD ítem a ítem. Celda (solo `editor`): **Subir** (`FileUp` → **`ImportarEstPorProdModal`** con periodo/sucursal fijos; si ya hay datos, confirma y **reemplaza todo el periodo** de esa sucursal) y **Borrar** (`Trash2` → `eliminarEstPorProdPorPeriodoAction`, borra **todos** los productos de esa celda). Sidebar: **Carga de Datos**. `PERMISOS.estadisticasProductos.acceso`.
