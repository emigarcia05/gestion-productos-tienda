...
- **`SidebarAreaSwitcher`**: botón `sidebar-user-switcher-surface` debajo del logo; muestra **`SUC / MÓDULO`** (abrev. sucursal + área en MAYÚSCULAS, ej. **GUAY / VENDEDOR**) y al hover **CAMBIAR SUC / MÓDULO**. Click → modal **Sucursal Y Módulo** con dos `Select` (`input-filtro-unificado`): **SUCURSAL** (placeholder, **sin default**) y **MÓDULO** (default **VENDEDOR**). Footer **Aplicar** (requiere sucursal). Si elige **ADMINISTRACIÓN** y `rol !== "editor"` → modal de clave (`activarModoEditor`). **Primera apertura** (`sessionStorage` `main-app-area-elegida` **y** `main-app-sucursal-preferida`): el modal se abre solo y **no se puede cerrar** hasta aplicar sucursal (sin X). Default de filtros STOCK / Trans. Depósitos. **Vendedor** / **Marketing**: `router.push` libre.
...
- **Texto**: **`{abrev sucursal} / {módulo}`** (MAYÚSCULAS; abrev. **GUAY** / **MAIPÚ**); hover → **CAMBIAR SUC / MÓDULO**. Primera visita: **SUC / MÓDULO**.
