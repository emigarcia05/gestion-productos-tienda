...
- **Envios:** sidenav **Programados** / **Crear Envío**. Programados = CFTL `contentWidth="full"` con tabla de `envios_final` (SUCURSAL; FECHA `dd/mm/aaaa` AR; HORARIO `hora_desde – hora_hasta`; PAGADO = ícono `Check`; PDF vía `GET /api/envios/[id]/comprobante`). **Crear Envío:** CFTL vacío (como Cargar Gasto); al entrar abre `CrearEnvioWizardModal` (`AppModal` `size="xl"` `scrollBody={false}` `h-[85vh]` `max-h-[85vh]`). Header **Nuevo Envío**. Cuerpo en 2 filas: **15 %** stepper `EnviosWizardPasos` (SUCURSAL / CLIENTE / DIRECCIÓN / FECHA / MERCADERÍA; el paso actual va `bg-primary`); **resto** el formulario del paso. Pie: Cancelar / Atrás / Siguiente o **Crear Envío**. **1. SUCURSAL:** contenido centrado. Label **SUCURSAL QUE ENVÍA**. Botones `w-full` con sucursales `pedido=true` (`listarSucursalesParaEnvios`); click elige (sin preselección) y pasa a **CLIENTE**. **2. CLIENTE:** Finder 1 col `headerVariant="titulo"` `mostrarNuevo={false}` (CONSUMIDOR_FINAL y PINTOR) + `FiltroBusquedaInput`; fila = **nombre centrado** (una línea) + si `CONSUMIDOR_FINAL` tiene pintor asociado: ` - NOMBRE PINTOR` (`nombreSufijo`, sin negrita, 75 % del nombre) + ícono `Users` si `tipo=PINTOR` (abre modal de CONSUMIDOR_FINAL con `pintor_asociado` = ese pintor; click elige al consumidor y pasa al paso 3) + ícono `Phone` (`EnviosTelLink`, `tel:`) si hay `cel` + `Trash2` fijo (`eliminarSiempreVisible`; editar sigue en hover); click en la fila selecciona el cliente y pasa a **DIRECCIÓN**; debajo de la lista, `Button` `w-full` **Nuevo Cliente** (abre modal **Nuevo Cliente** (NOMBRE COMPLETO en mayúsculas, CEL opcional, TIPO default CONSUMIDOR FINAL | PINTOR, PINTOR ASOCIADO, DIRECCIONES). Pintor asociado: `+` abre listado; con pintor = nombre + editar / borrar. Un `PINTOR` no tiene pintor asociado. **3. DIRECCIÓN:** Finder de `envios_direcciones.persona_id` del cliente; fila `calle_nombre numeracion, distrito. departamento` + `MapPin` si hay `url_maps`; click en la fila selecciona la dirección y pasa a **FECHA**; `+` abre **Nueva Dirección** (CALLE NOMBRE y DISTRITO proper case; numeración/referencia primera mayúscula; DEPARTAMENTO `LAS HERAS` | `GODOY CRUZ` | `GUAYMALLEN` | `MAIPU` | `LUJAN`; al menos un dato). **4. FECHA:** contenido centrado. FECHA: HOY / MAÑANA / `CalendarDays`. RANGO HORARIO: chips 09:00–19:00 cada 30 min; dos clics (se ordenan) = `hora_desde` / `hora_hasta`; el tramo intermedio queda marcado. Al completar fecha + rango se pasa a **MERCADERÍA**. **5. MERCADERÍA:** PDF opcional (máx. 5 MB) a `w-full`. **PAGADO** = `ModalSiNoChoice` (SÍ/NO, default NO) a ancho completo. **FORMA DE PAGO** Select `w-full`: `EFECTIVO` | `TRANSFERENCIA` | `POSNET` | `CUENTA CORRIENTE`. Guardar = `crearEnviosFinalAction` (pintor = `pintor_asociado` si hay) y redirect a Programados. Edición sucursal/fecha/horario/PDF/pago también desde el modal de Programados (campo **SUCURSAL QUE ENVÍA**).
