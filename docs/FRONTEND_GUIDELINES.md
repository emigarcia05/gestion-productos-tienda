...
`AppModal`: `size` `sm|md|lg|xl` (md = `max-w-lg`), `padding`, `scrollBody` (default true), `hideBodyScrollbars`, `bodyShellClassName`. Tabla + pie fijo: tabla `flex-1 min-h-0`; no `h-0`.

### 1.5 Finder

`@/components/shared/catalogo-finder/` + `CATALOGO_FINDER_*` en `ui-classes`. Columnas con header `bg-primary` (`headerVariant="finder"` default), `+` (`nuevoLado`: `end` default, `start` en Ideas), `className?` para altura en modal. Wizard Envios: `headerVariant="titulo"` (título de paso, sin barra primary). Paso CLIENTE: `mostrarNuevo={false}` y CTA **Nuevo Cliente** (`Button` `w-full`) debajo de la lista, ancho del modal. Filas: hover editar/eliminar; selección `CATALOGO_FINDER_ROW_SELECTED_CLASS`. `nombreAccion` opcional a la derecha del nombre (Envios: `MapPin` / `Phone` / `Users` en PINTOR). Usos: Catálogo Gastos (5 cols), Comp. Categorías (4), Ideas (2), Envios · Crear Envío (finder 1 col por paso, dentro del wizard modal).
...
- **Envios:** sidenav **Programados** / **Crear Envío**. Programados = CFTL `contentWidth="full"` con tabla de `envios_final` (FECHA `dd/mm/aaaa` AR; HORARIO `hora_desde – hora_hasta`; PAGADO = ícono `Check`; PDF vía `GET /api/envios/[id]/comprobante`). **Crear Envío:** CFTL vacío (como Cargar Gasto); al entrar abre `CrearEnvioWizardModal` (`AppModal` `size="xl"` `h-[90vh]` `scrollBody={false}`). Header **Nuevo Envío**. Cuerpo en 2 filas: **15 %** stepper `EnviosWizardPasos` (CLIENTE / DIRECCIÓN / FECHA / MERCADERÍA; el paso actual va `bg-primary`); **resto** el formulario del paso. Pie: Cancelar / Atrás / Siguiente o **Crear Envío**. **1. CLIENTE:** Finder 1 col `headerVariant="titulo"` `mostrarNuevo={false}` (CONSUMIDOR_FINAL y PINTOR) + `FiltroBusquedaInput`; fila = **nombre** (una línea) + ícono `Users` si `tipo=PINTOR` (abre modal de CONSUMIDOR_FINAL con `pintor_asociado` = ese pintor; click elige al consumidor) + ícono `Phone` (`EnviosTelLink`, `tel:`) si hay `cel`; debajo de la lista, `Button` `w-full` **Nuevo Cliente** (abre modal **Nuevo Cliente** (NOMBRE COMPLETO en mayúsculas, CEL, TIPO default CONSUMIDOR FINAL | PINTOR, PINTOR ASOCIADO, DIRECCIONES). Pintor asociado: `+` abre listado; con pintor = nombre + editar / borrar. Un `PINTOR` no tiene pintor asociado. **2. DIRECCIÓN:** Finder de `envios_direcciones.persona_id` del cliente; fila `calle_nombre numeracion, distrito. departamento` + `MapPin` si hay `url_maps`; `+` abre **Nueva Dirección** (CALLE NOMBRE y DISTRITO proper case; numeración/referencia primera mayúscula; DEPARTAMENTO `LAS HERAS` | `GODOY CRUZ` | `GUAYMALLEN` | `MAIPU` | `LUJAN`; al menos un dato). **3. FECHA:** HOY / MAÑANA + `CalendarDays`; HORA DESDE / HASTA 09:00–19:00 cada 30 min (desde < hasta). **4. MERCADERÍA:** PDF opcional (máx. 5 MB) + forma `EFECTIVO` | `TRANSFERENCIA` | `POSNET` | `CUENTA CORRIENTE`. Guardar = `crearEnviosFinalAction` (pintor = `pintor_asociado` si hay) y redirect a Programados. Edición fecha/horario/PDF/pago también desde el modal de Programados.
