...
- **Control de ítem:** primera columna de checklist local (`TablaControlItemHead` + `TablaControlItemCelda`). Encabezado = ícono `Check` (`sr-only` LISTA DE VERIFICACIÓN). Celda: badge circular `bg-primary/20` si está marcado; hueco `h-7` si no. Las acciones (OK, etc.) van en **ACCIONES**. Fila: `recepcion-fila-verificada` / `recepcion-fila-pendiente` (`.tabla-recepcion-pedido`). Usos: Recepción Pedido, Generar Transf., Generar Nota Crédito.
...
| `MontoArInput` / `MontoArSaldoEnteroInput` / `PorcentajeCentInput` / `PorcentajeEnteroMaskInput` / `PxListaEnteroInput` | Máscaras AR. `MontoArInput` `allowNegative` solo en TOTAL PEDIDO de recepción. |
...
- **Recepción** (`PedidoHistoriaDetalleModal`): `AppModal` `size="xl"` + `max-w-[66rem] h-[95vh]`. Scroll solo en tabla; pie TOTAL PEDIDO fuera. Checklist local (**Control de ítem** + OK) hasta **Registrar En Dux** (fiscal → personal → POST). **CANT. RECIBIDA** y **TOTAL PEDIDO** admiten negativos (devolución / NC): en cantidad, `-` en el input o el stepper; en total, `MontoArInput` `allowNegative` (tecla `-` o pegado con signo). Header **Generar Nota Crédito** (`ToolbarActionButton`): abre `GenerarNotaCreditoModal` con pedidos `RECEPCIONADO` (FECHA RECEPCIÓN / PROVEEDOR / TOTAL / ACCIONES: Ver + tilde Elegir). Encima de la tabla (`shrink-0`, `text-sm text-foreground`): texto de asistencia DUX + instrucción **Elegí el pedido del cual querés generar la Nota de Crédito**. Tilde **Elegir Pedido** cierra el picker y abre el mismo modal con `variante="nota-credito"` (título **Nota Crédito**): arranca editable (FECHA FACTURA vacía, CANT. REC. vacía, checklist vacío; CANT. PED. del snapshot); no persiste el pedido origen. Footer **Generar Nota Crédito** (mismo gating que Registrar En Dux): abre DUX (`DUX_NUEVA_NOTA_CREDITO_DEBITO_VENTA_URL`) en pestaña nueva y `GenerarNotaCreditoDuxModal` (`scrollBody={false}` `size="xl"` `max-w-[54rem]` `h-[85vh]`, mismo cascarón que Generar Transf.): Control de ítem / COD. TIENDA / DESCRIPCIÓN / CANT. / ACCIONES. **OK** copia `cod_tienda` (toggle); **Nota Generada** exige todos TRUE, reserva `prod_ped_ult_comp` id=3 (`c-00000-########`) y cierra el asistente. Encima de la tabla: **PROVEEDOR** + **Nº COMPROBANTE** (ícono Copiar). Pie **PRECIO UNITARIO A COLOCAR EN TODOS LOS ITEM** = TOTAL PEDIDO / suma CANT. (2 decimales, `montoArSignedCentsToDisplayWithCurrency`) con ícono Copiar. Sin POST DUX. FECHA RECEPCIÓN = `prod_ped_historial.fecha_recepcion` (FECHA FACTURA persistida al registrar). Corrección: **Corregir Recepcion** / **Guardar Corrección**. Tabla `tabla-recepcion-pedido`. Listado: `FiltrosHistorialPedidos` default PENDIENTE; acciones Recepción / Ver (`PedidoHistoriaLecturaModal`) / Borrar.

### Tienda / precios

- **Cx Compra:** 6 columnas (`tabla-tienda-listado`, colgroup 11/32/9/22/12/14 %). Link2 → `SeleccionarProductoModal`. Vínculos en subfilas `CxCompraVinculosDetalle` (no modal de lista; `colSpan` 6; hueca extra en BULTO). CX PROD. = BASE + variación. **BULTO** a la derecha de ACCIONES: `CeldaBultoTienda` + `EnteroStepperInput`; vacío = `prod_tienda.bulto` null. Header **Act. Cx.** (`ActCxButton`) / **Prueba PUT Cx** (`PruebaPutCxButton` + `ToolbarActionButton`): mismos ítems que el Excel (diff ≥ 0,01); exige usuario slidenav (`id_personal`); un ítem por Action (PUT snake_case `costo_compra` = CX PROD., **sin** `stock[]`; pausa 5 s y GET v1 del costo). El toast distingue PUT 200 **sin impacto** vs GET confirmado. No pisa el espejo local (sigue la sincro).
