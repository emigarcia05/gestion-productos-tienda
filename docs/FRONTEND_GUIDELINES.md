...
- **Formato**: ícono `User` (`aria-hidden`) + texto **`VENDEDOR` / `EDITOR`** (según `rolActual`; valor interno del rol vendedor sigue siendo `"simple"`).
- **Interacción**
  - En **VENDEDOR**: click abre modal de contraseña para pasar a **EDITOR**.
  - En **EDITOR**: click vuelve a **VENDEDOR** sin modal (`aria-label` **Volver A Modo Vendedor**).
- **Modo no compacto**: etiqueta **Modo Vendedor** / **Modo Editor**.
...
### Orden y labels — Sidebar Gestión Del Vendedor / Finanzas · Análisis (`Sidebar.tsx`)
