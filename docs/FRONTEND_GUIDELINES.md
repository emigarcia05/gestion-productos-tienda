...
- **Pedido A Fáb.:** dos FilterBar. Fila 1: PROVEEDOR / FECHA DE PEDIDO / TIEMPO STOCKEO (días calendario; 30 calendario = 24 hábiles de venta en CANT. SUG.) / **PROD. VINCULADO** (SI/NO en servidor: hay fila `prod_tienda` vía `cod_tienda`; sin elegir = todos) / STOCK QUEBRADO (solo filtra si SI/NO está elegido). Secciones: **STOCK** (`UN. ACT.` / `QUEBR.`) y **COMPRA** (FORMA **BULTO**/**UNIDAD** | BULTO si vínculo `cod_tienda` | `CANT. PED.` `EnteroStepperInput` en orden `- | input | + | check | borrar` (check copia CANT. SUGERIDA y borrar limpia el valor) | `CANT. SUG.`: UNIDAD techo entero; BULTO techo al múltiplo de `prod_tienda.bulto`). En **QUEBR.** mostrar tilde cuando `UN. ACT. ≤ 0` o `Stock Hasta Llegada De Pedido ≤ 0`; si no, vacío. Si **FECHA DE PEDIDO** está vacía, el cálculo inicial usa hoy (AR) como base y proyecta provisión hasta `tiempo_entrega_en_dias`. Modal de soporte: título `INFO FORMULAS` y fórmulas alineadas a la lógica vigente de provisión/quiebre/cant. sugerida. Última columna de acciones: Info de PROM. VTA. por sucursal (modal `SUCURSALES` + `PROM. VTA POR DÍA` 1 decimal = ventas 2 meses / 48). Info junto a **UN. ACT.**: grilla número a la derecha + Info en columna fija; modal solo sucursales con `id_deposito` (`prod_tienda_stock.stock_real` + TOTAL). Info final: PROM. VTA. de sucursales `genera_est`. DESCRIPCIÓN = `descripcion_tienda` si hay vínculo, si no `descripcion_proveedor`. Header `GenerarPedidoToolbarButton` `modulo="a-fabrica"`.
