...
- **Reposición:** desplegables SUCURSAL → PROVEEDOR → MARCA → RUBRO → CONFIGURADO. Bloque secundario STOCK / CANT. A PEDIR. En este módulo, PROVEEDOR solo muestra proveedores de mercadería con `global_proveedores.es_fabrica = false` (también en el modal **Generar Pedido** cuando el origen es Reposición), y la grilla solo muestra productos que tengan al menos un vínculo habilitado con proveedor no fábrica. `ConfigurarReposicionModal`: punto/cant vacíos (no `0`) si no hay regla. FORMA PEDIR (vendedor): **UN. MÁXIMAS** (`UNIDADES_MAX`) / **BULTO** (`POR_BULTO`; **siempre** en el select, aunque no haya fila en `prod_tienda_bultos`). Modal siempre `max-w-[56rem]`. Campos en `grid-cols-5` de anchos iguales (también con UN. MÁXIMAS: las dos columnas de más van `invisible`). Con **UN. MÁXIMAS**: `FORMA PEDIR | PUNTO REPOSICIÓN | UN. MÁXIMAS`; con **BULTO**: `FORMA PEDIR | UN. POR BULTO | PUNTO REPOSICIÓN (EN UN.) | BULTOS REPOSICIÓN | UN. TOTALES`. Columnas siguientes se ocultan con `invisible` (no se desmontan): si **UN. POR BULTO** está vacío, PUNTO / BULTOS / UN. TOTALES; si **PUNTO REPOSICIÓN** está vacío, BULTOS / UN. TOTALES. **UN. TOTALES** solo lectura = `BULTOS REPOSICIÓN × UN. POR BULTO`. **UN. POR BULTO** (entero ≥ 1) al Guardar crea o actualiza `prod_tienda_bultos` del ítem principal. Etiquetas `h-12` + `line-clamp-3` (hasta 3 renglones, alineadas). En la grilla, **CANT. REPOSIC.** muestra la cantidad de bultos (`reposicion_cant_conf`); **CANT. A PEDIR** y el pedido generado van en unidades (`cantConf × prod_tienda_bultos`). Encima de la tabla del modal va el botón ícono `+` centrado y con estilo `default` (fondo `primary`, ícono `primary-foreground`) para abrir el selector de productos adicionales (con **BULTO**, exige Un. por bulto y filtra por ese valor). No CANT. FIJA POR UNID.
