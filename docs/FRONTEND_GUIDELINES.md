...
- **Envios:** sidenav **Programados** / **Crear Envío**. Programados = CFTL `contentWidth="full"` con tabla de `envios_final` (PAGADO = ícono `Check`; PDF vía `GET /api/envios/[id]/comprobante`). **Crear Envío:** paso 1 en card `1. SELECCIONAR CLIENTE` con Finder 2 columnas (CLIENTES `tipo=FINAL` + DIRECCIONES). Click en cliente lista solo sus direcciones (`envios_direcciones.persona_id`). Cada columna tiene `FiltroBusquedaInput`. `+` abre modal **Nuevo Cliente** con Select **TIPO** (`FINAL` | `PINTOR`); si eligen `FINAL` aparece **PINTOR ASOCIADO** (opcional) + **Nuevo Pintor**. Un `PINTOR` no tiene pintor asociado. La columna CLIENTES lista solo `FINAL`.
