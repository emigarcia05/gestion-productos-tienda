...
10. **Al terminar un cambio**  
   ...
   - Si ajustas elementos de **slidenav/sidebar**, mantener componentes compactos y consistentes:
    - **Ritmo vertical** (`Sidebar.tsx`): **navegación** arriba (`sidebar-nav-scroll`, `pt-3 px-4 pb-2`). Abajo (`mt-auto`, `px-4 pb-3`, **`gap-2`**): **regla horizontal** → **sync/import** → **indicador de pendientes** (`SidebarMainAppArea` con **`my-2`**) → **`SidebarAreaSwitcher`**. Sin paddings apilados entre bloques salvo el margen del indicador.
...
- **`SidebarMainAppArea`**: indicador de pendientes (reemplaza el logo). Título = sucursal preferida en MAYÚSCULAS (`sessionStorage`; sin elegir: **SUCURSAL**). Bloque **Pedido**: filas `Urgente - N` / `Tintométrico - N` / `Reposición - N` = ítems con cant. pedir &gt; 0 que salen en **Generar Pedido** (`getItemsTablaEnviarPedido` filtrado por sucursal). Bloque **Transferencia**: `Emisión - N` (TRANSFERIR / EGRESO) y `Recepción - N` (RECIBIR / INGRESO) = filas pendientes del modal **Transf. Pendiente Registro**. Action `getIndicadorSlidenavAction`. Refresh al cambiar sucursal, al navegar, al enfocar, cada 30 s y vía `EVENTO_INDICADOR_SLIDENAV` (Generar Pedido / encolar o exportar Excel).
- **`SidebarAreaSwitcher`**: botón `sidebar-user-switcher-surface` debajo del indicador de pendientes; **dos filas**: sucursal · línea blanca `h-px w-[80%] bg-sidebar-foreground/85` · módulo (MAYÚSCULAS). Click → modal **Sucursal Y Módulo** con dos `Select` (`input-filtro-unificado`): **SUCURSAL** (placeholder, **sin default**) y **MÓDULO** (default **VENDEDOR**). Footer **Aplicar** (requiere sucursal). Si elige **ADMINISTRACIÓN** y `rol !== "editor"` → modal de clave (`activarModoEditor`). **Primera apertura** (`sessionStorage` `main-app-area-elegida` **y** `main-app-sucursal-preferida`): el modal se abre solo y **no se puede cerrar** hasta aplicar sucursal (sin X). Default de filtros STOCK / Trans. Depósitos. **Vendedor** / **Marketing**: `router.push` libre.
...
### Slidenav — Cambio de sucursal y módulo (`src/components/shared/SidebarAreaSwitcher.tsx`)

Botón de **dos filas** en el pie de la slidenav (**debajo** del indicador de pendientes), superficie **`sidebar-user-switcher-surface`**.

- **Texto**: fila 1 **sucursal** (MAYÚSCULAS: **GUAYMALLÉN** / **MAIPÚ**; sin elegir: **SUCURSAL**); **línea blanca** `h-px w-[80%] bg-sidebar-foreground/85` (misma regla del pie del sidebar); fila 2 **módulo** (MAYÚSCULAS: **VENDEDOR** / **ADMINISTRACIÓN** / **MARKETING**; onboarding: **MÓDULO**). Sin ícono, sin hover «CAMBIAR…».
- **Click**: modal **Sucursal Y Módulo** — dos desplegables (`FiltroIndividualContainer` + `Select`): **SUCURSAL** sin valor por defecto (placeholder **SUCURSAL**); **MÓDULO** default **VENDEDOR** (`gestion-productos`). Footer **Aplicar** (deshabilitado sin sucursal) + **Cerrar** (oculto en onboarding).
- **Administración**: al elegir **ADMINISTRACIÓN** en el Select (o al Aplicar) si `rol !== "editor"` → modal **Acceso A Administración** (`activarModoEditor`). Cancelar vuelve el módulo a **VENDEDOR**.
- **Onboarding**: exige sucursal (`main-app-sucursal-preferida`) + flag de área (`main-app-area-elegida`); no se cierra hasta Aplicar.
- **Default**: STOCK (filtro sucursal) y Trans. Depósitos (origen + modal pendientes) precargan esa sucursal; el usuario puede cambiarla.
- **Acceso**:
  - **Vendedor** / **Marketing**: libre (`requierePassword: false`).
  - **Administración**: pide clave si `rol !== "editor"` → `activarModoEditor` + navegación.
- **Logo**: retirado del pie de slidenav (queda el indicador `SidebarMainAppArea`).
