...
3. **Texto en mayúscula inicial (title case)**  
   ...
   - **Sidebar**: nombre del **módulo** en MAYÚSCULAS (ej. "PEDIR MERC.", "ANALISIS DE PRECIOS"). Nombre del **submódulo** o **agrupador desplegable** dentro de un módulo: title case (ej. "Lista Precios", "Lista Proveedores", "Cx Compra", "Pedido Urgente", "Px. Vta. Sugerido").  
...
| `.sidebar-nav-item`, `.sidebar-nav-module` | Hover / activo (`bg-sidebar-accent`) en **módulo / submódulo 1 / submódulo 2**. El módulo (`.sidebar-nav-module`) lleva `data-active` si la ruta actual es suya o de un descendiente (`isNavModuleActive` / `isAdmPillarActive`). Íconos **siempre blancos** (`sidebar-foreground`). |
...
- **`MAIN_APP_AREAS`**: cada ítem tiene `id`, `label` (title case en datos), `statusLabel` (ej. **Terminada** / **A construir**), `href` (entrada al elegir el área). En slidenav y modal, el nombre visible usa **`areaLabelMayusculas(label)`** (`toLocaleUpperCase("es")`) → **MAYÚSCULAS** (ej. **VENDEDOR**, **ADMINISTRACIÓN**, **MARKETING**).
...
- **Formato**: ícono `User` (`aria-hidden`) + texto **`VENDEDOR` / `ADMINISTRADOR`** (valores internos: `"simple"` / `"editor"`).
- **Interacción**
  - En **VENDEDOR**: click abre modal de contraseña para pasar a **ADMINISTRADOR**.
  - En **ADMINISTRADOR**: click vuelve a **VENDEDOR** sin modal (`aria-label` **Volver A Modo Vendedor**).
- **Modo no compacto**: etiqueta **Modo Vendedor** / **Modo Administrador**.
...
#### Modal “Acceso De Administrador” (mismo archivo)

- El modal se adapta al diseño estándar usando `AppModal` (header corporativo + footer con botonera).
- Botones y título respetan el Title Case (ej. `Acceso De Administrador`, `Activar Modo Administrador`).
...
- Sidebar por rol: **simple** → **PEDIR MERC.** + **ASISTENCIA PRECIOS** + **CALCULAR LTS** + **CARGAR GASTOS** + **ASISTENTE IA**; **editor** → mismos módulos Vendedor + área Administración/Marketing. **Cx Compra** solo **editor** (`PERMISOS.tienda.acceso`).
