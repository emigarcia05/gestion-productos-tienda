...
- **Control Stock:** header **Exportar Excel** / **Prueba API PUT** / **Imprimir**. Sin modal de sync al entrar. Excel y PUT usan el mismo conjunto: ítems con variación (`collectFilasVariacion` / `filasConVariacionStockParaExportar`). El Check confirma sin variación (solo Excel persiste ÚLT. CONTROL). **Prueba API PUT** (`PruebaPutStockDuxButton` + `ToolbarActionButton`): exige usuario del slidenav; un PUT por ítem con pausa `DUX_API_BATCH_INTERVAL_MS`; body `cod_tienda`, `stock` (cantidad contada), `deposito` (id DUX de la sucursal del filtro), `usuario` (`idPersonal`). No persiste ÚLT. CONTROL.
