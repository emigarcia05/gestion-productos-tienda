...
La app se divide en **cuatro áreas** de alto nivel. El id interno `gestion-productos` se muestra como **Gestión Del Vendedor** (pedidos, ayuda vendedor, asistente IA). El id `finanzas` se muestra como **Gestión Fin. & Adm.** (**Análisis de Precios** vive en ese sidebar; URLs canónicas siguen bajo `/gestion-productos/analisis-precios/...`).

- **`MAIN_APP_AREAS`**: cada ítem tiene `id`, `label` (title case en datos), `statusLabel` (ej. **Terminada** / **A construir**), `href` (entrada al elegir el área). En slidenav y modal, el nombre visible usa **`areaLabelMayusculas(label)`** (`toLocaleUpperCase("es")`) → **MAYÚSCULAS** (ej. **GESTIÓN DEL VENDEDOR**, **GESTIÓN FIN. & ADM.**).
- **`getMainAppAreaIdFromPathname(pathname)`**: rutas de Análisis de Precios (`isAnalisisPreciosPathname` / `getGpSidebarModule === "analisis-precios"`) → área **Gestión Fin. & Adm.** (`finanzas`); `/finanzas` y `/finanzas/*` → **Gestión Fin. & Adm.**; `/estadisticas-productos` y subrutas → **Estadísticas Productos**; `/marketing` y `/marketing/*` → **Marketing**; cualquier otra ruta → **Gestión Del Vendedor** (incluye `/gestion-productos/*` no-análisis, `/`, `/stock`, `/pedidos`, ayuda vendedor, etc.).
- **Navegación lateral por área activa** (`Sidebar.tsx`): en **Gestión Del Vendedor** el orden de módulos es **PEDIDO MERCADERIA** → **AYUDA VENDEDOR** → **ASISTENTE IA** (todos accesibles al rol vendedor/`simple`; CRUD de prompts del Asistente IA solo `editor`). Dentro de **PEDIDO MERCADERIA**: **Generar Pedido** → agrupador **Conf. Pedido** (sin ruta; ícono `ListChecks`) con **Urgente**, **Tintométrico** y **Reposición** → **Recepcion Pedido** (`/gestion-productos/pedidos/historial`; subtítulo de página; antes **Historial Pedidos**). El agrupador **Conf. Pedido** auto-expande si la ruta activa es un hijo. Módulos retirados (no existen más): **LISTA TIENDA** y **Control Aumentos** (este último eliminado por completo el 2026-05-28; será reimplementado más adelante). Con área **Gestión Fin. & Adm.** se muestran, en este orden: **`BALANCE`** (`Scale`) con submódulos **Balance Mensual**, **Gastos**, **Catálogo Gastos** y **Ventas Mensuales**; **`FINANZAS`** (`Landmark`) con **Tesorería**, **Posición de IVA**, **Flujo De Fondo**, **Venc. Provee. Merc.**, **Venc. Provee. Gastos** y **Control Comprobantes**; **`ANALISIS M.C.`** (`LineChart`) con **Margen Contribución** y **Costos Financieros**; por último **`ANALISIS DE PRECIOS`** (mismo contenido que antes en Gestión Productos; solo `editor` vía permisos de lista precios / tienda / comparación). `getOpenModule` prioriza **analisis-precios** si `getGpSidebarModule` lo indica; abre **BALANCE** para `/finanzas/balance/*`, **ANALISIS M.C.** para `/finanzas/analisis-mc/*` y **FINANZAS** para el resto de `/finanzas/*`. En **Estadísticas Productos** no hay módulos en sidebar todavía (mensaje vacío). En **Marketing** (`MARKETING_MODULES`, SSOT rutas `src/lib/marketingRoutes.ts`): módulo **`PUBLICACIONES`** (`Megaphone`) con **Calendario de Publicaciones** → `/marketing/publicaciones/calendario` (`CalendarRange`) e **Ideas** → `/marketing/publicaciones/ideas` (`Lightbulb`); permiso **`PERMISOS.marketing.acceso`** (solo `editor`). Pantallas placeholder **Módulo En Construcción.** hasta implementar dominio.
- **`SidebarMainAppArea`** (client): recibe **`esEditor`** desde `Sidebar` (`rol === "editor"`). Con **`showLogo` y `showLabel`**, el **nombre del área** no va dentro del botón del logo (ese botón usa `max-w-[45%]`): se renderiza **encima**, en un contenedor **`w-full min-w-0 px-1 text-center`** con `role="status"` + `aria-live="polite"`, y el `<span>` del nombre con **`w-full text-center`** + tipografía sidebar (`whitespace-nowrap`, `text-[13px]`, `leading-none`) para una sola línea centrada en todo el ancho del sidebar; el texto del nombre pasa por **`areaLabelMayusculas`**. **Rol `editor`:** el **logo** es `<button>` + `Image` (`alt=""`) con `aria-label` **Elegir Área De La Aplicación**; el click abre **`Dialog` + `AppModal`** **Áreas De La Aplicación** con las **cuatro** opciones (`MAIN_APP_AREAS`, `router.push` al `href` de cada área). **Rol vendedor (`simple`):** el logo es un **`<div>`** decorativo (sin modal): **Gestión Fin. & Adm.**, **Estadísticas Productos** y **Marketing** no se ofrecen — solo trabaja en **Gestión Del Vendedor**. Con **`showLabel` y sin logo** (`showLogo={false}`), el nombre sigue en `role="status"` con **`pb-0.5`** en el `<span>`. En el modal (solo editor), la opción de la ruta actual resaltada (`border-sidebar-indicator`, `bg-sidebar-accent/40`); íconos `lucide-react` por opción (`Boxes`, `Landmark`, `BarChart3`, `Megaphone`). Variantes **CVA**: `areaOptionVariants`, `areaTitleVariants` / `areaStatusVariants` (`context`: `sidebar` | `modal`).
- **`/finanzas`**: **`permanentRedirect`** a **`/finanzas/tesoreria`** (`src/app/finanzas/page.tsx`); no hay pantalla de resumen. La entrada del área **Gestión Fin. & Adm.** en el modal de áreas usa **`MAIN_APP_AREAS`** → mismo destino (`src/lib/main-app-areas.ts`). En sidebar, **Tesorería** se considera activa también si la ruta es exactamente **`/finanzas`** (hasta completar el redirect).
...
- **Estadísticas Productos** — SSOT `ESTADISTICAS_PRODUCTOS_ROUTES`; sidebar módulo **ESTADÍSTICAS** con **Ventas Por Producto** + **Categorizacion**. `/estadisticas-productos` → redirect a ventas.
- **`/estadisticas-productos/ventas-por-producto`**: **`ClassicFilteredTableLayout`** `contentWidth="full"`, título **ESTADÍSTICAS PRODUCTOS**, subtítulo **Ventas Por Producto**. Cascarón **`area-page-shell`**. **Filtros** en **`FilaFiltrosDesplegables`**: **MES** / **AÑO** / **SUCURSAL** (sucursales con `deposito` no nulo) + **`col-span-2`**: contador **REGISTRO(S)** + **`LimpiarFiltrosButton`**. Listado desde servidor (`listarEstPorProd`); filtrado y paginación en cliente (`PaginacionClient`, `PAGE_SIZE`). Solo `editor`: **`actions`** — **Importar Datos** abre **`ImportarEstPorProdModal`** (`size="xl"`, `max-w-3xl`): **mes/año/sucursal**, **Adjuntar Archivo** + **`ModalSiNoChoice`** encabezados, zona drag-and-drop, tabla **PRIMERA FILA / MAPEAR A** (muestra los **nombres de encabezado** de la primera fila de la planilla cuando hay encabezados; campos **COD. TIENDA***, **VTAS. EN UN.*** e **IGNORAR**; badges de campos requeridos como **Importar Lista De Precios**); parseo en `@/lib/parseEstPorProdExcelClient.ts` (`FILAS_OMITIR_INICIO_EST_POR_PROD = 0`); persiste vía **`POST /api/import-est-por-prod`** (no confundir con el poll **`/api/import-lista-precios/status`** del sidebar). Tabla **Registros Importados**: **PERIODO**, **SUCURSAL**, **COD. TIENDA**, **DESCRIPCIÓN**, **VTAS. EN UN.**, **IMPORTADO**, **ACCIONES** (eliminar). `PERMISOS.estadisticasProductos.acceso`.
- **`/estadisticas-productos/categorizacion`**: **`ClassicFilteredTableLayout`** título **ESTADÍSTICAS PRODUCTOS**, subtítulo **Categorizacion**. Listado `prod_tienda` (`listarProdTiendaCategorizacion`): **DESCRIPCIÓN**, **MARCA**, **RUBRO**, **SUB RUBRO**, **COLOR**, **TERMINACION**, **LTS** (catálogo conversión + fallback «N LTS»). **Filtros** fila 1: `FilaFiltrosDesplegables` **`columnas={6}`** — Marca / Rubro / Sub Rubro / Color / Terminacion / Lts; fila 2: `flex items-center gap-3` con `FilterRowSearch` `flex-1` + `FiltroBusquedaInput` + `LimpiarFiltrosButton` + contador `FILTER_COUNT_CLASS` `ml-auto` (mismo orden que **Px. Vta. Sugerido**). Paginación cliente. Solo `editor`: **`actions`** — **Gestion Colores** (`Palette`) · **Gestion Conversion** (`Beaker` → **`GestionarEstPorProdLtsConversionModal`**: texto → litros) · **Gestion Terminacion** (`Paintbrush` → **`GestionarEstPorProdTerminacionModal`**: catálogo `est_por_prod_terminacion`; `onCatalogoChanged` → `router.refresh()`).
...
- **Jerarquía canónica de URLs (2026-06-19):** SSOT en **`src/lib/gestionProductosRoutes.ts`** (`GP_ROUTES`). Prefijo **`/gestion-productos/{módulo}/{agrupador}/{submódulo}`** alineado al sidebar. Rewrites en `next.config.ts` sirven páginas internas (`src/app/pedidos`, `proveedores`, `tienda`, …); redirects permanentes desde URLs legacy y canónicas anteriores.
  ...
  - Entrada por defecto (`/` y modal área Gestión Del Vendedor): **`GP_ROUTES.defaultEntry`** → Px. Vta. Sugerido.
...
### Orden y labels — Sidebar Gestión Del Vendedor / Gestión Fin. & Adm. · Análisis (`Sidebar.tsx`)
