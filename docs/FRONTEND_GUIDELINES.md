...
10. **Al terminar un cambio**  
   ...
   - Si ajustas elementos de **slidenav/sidebar**, mantener componentes compactos y consistentes:
    - **Ritmo vertical** (`Sidebar.tsx`): **navegación** arriba (`sidebar-nav-scroll`, `pt-3 px-4 pb-2`). Abajo (`mt-auto`, `px-4 pb-3`, **`gap-2` uniforme**): **regla horizontal** (`h-px w-[80%] bg-sidebar-foreground/85`) → **sync/import** → **`SidebarAreaSwitcher`** (wrapper `rounded-lg px-2`) → **logo** (`SidebarMainAppArea`, decorativo). Sin paddings apilados entre bloques.
...
*Última actualización (2026-08-07): **Sidebar · orden pie** — línea → sync → switcher de módulos → logo.
