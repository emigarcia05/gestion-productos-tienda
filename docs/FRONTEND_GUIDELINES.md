...
- **Navegación lateral por área activa** (`Sidebar.tsx`): en **Vendedor** el orden de módulos es **PEDIDO MERCADERIA** → **AYUDA VENDEDOR** → **ASISTENTE IA** (todos accesibles al rol vendedor/`simple`; CRUD de prompts del Asistente IA solo `editor`). Dentro de **PEDIDO MERCADERIA**: **Generar Pedido** → agrupador **Conf. Pedido** (sin ruta; ícono `ListChecks`) con **Urgente**, **Tintométrico** y **Reposición** → **Recepcion Pedido** (`/gestion-productos/pedidos/historial`; subtítulo de página; antes **Historial Pedidos**). El agrupador **Conf. Pedido** auto-expande si la ruta activa es un hijo. Módulos retirados (no existen más): **LISTA TIENDA** y **Control Aumentos** (este último eliminado por completo el 2026-05-28; será reimplementado más adelante). Con área **Administración** se muestran, en este orden: **`BALANCE`** (`Scale`) con submódulos **Balance Mensual**, **Gastos**, **Catálogo Gastos** y **Ventas Mensuales**; **`FINANZAS`** (`Landmark`) con **Tesorería**, **Posición de IVA**, **Flujo De Fondo**, **Venc. Provee. Merc.**, **Venc. Provee. Gastos** y **Control Comprobantes**; **`ANALISIS M.C.`** (`LineChart`) con **Margen Contribución** y **Costos Financieros**; **`ANALISIS DE PRECIOS`** (mismo contenido que antes en Gestión Productos; solo `editor` vía permisos de lista precios / tienda / comparación); por último **`ESTADÍSTICAS`** (`BarChart3`) con **Carga de Datos**, **Categorizacion** y **Estadísticas Vtas** (`PERMISOS.estadisticasProductos.acceso`). `getOpenModule` prioriza **analisis-precios** si `getGpSidebarModule` lo indica; abre **BALANCE** para `/finanzas/balance/*`, **ANALISIS M.C.** para `/finanzas/analisis-mc/*`, **est-productos** para `/estadisticas-productos/*` y **FINANZAS** para el resto de `/finanzas/*`. En **Marketing** (`MARKETING_MODULES`, SSOT rutas `src/lib/marketingRoutes.ts`): módulo **`PUBLICACIONES`** (`Megaphone`) con **Calendario de Publicaciones** → `/marketing/publicaciones/calendario` (`CalendarRange`) e **Ideas** → `/marketing/publicaciones/ideas` (`Lightbulb`); permiso **`PERMISOS.marketing.acceso`** (solo `editor`). Pantallas placeholder **Módulo En Construcción.** hasta implementar dominio.
