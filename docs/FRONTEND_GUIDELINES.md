...
- **Reposición:** desplegables SUCURSAL → PROVEEDOR → MARCA → RUBRO → CONFIGURADO. Bloque secundario STOCK / CANT. A PEDIR. En este módulo, PROVEEDOR solo muestra proveedores de mercadería con `global_proveedores.es_fabrica = false` (también en el modal **Generar Pedido** cuando el origen es Reposición), y la grilla solo muestra productos que tengan al menos un vínculo habilitado con proveedor no fábrica. `ConfigurarReposicionModal`: punto/cant vacíos (no `0`) si no hay regla. FORMA PEDIR (vendedor): **UN. MÁXIMAS** (`UNIDADES_MAX`) / **BULTO** (`POR_BULTO`; **siempre** en el select, aunque no haya fila en `prod_tienda_bultos`). Layout de campos: con **UN. MÁXIMAS** mostrar `FORMA PEDIR | PUNTO REPOSICIÓN | UN. MÁXIMAS`; con **BULTO** mostrar `FORMA PEDIR | PUNTO REPOSICIÓN (EN UN.) | BULTOS REPOSICIÓN | UN. TOTALES` (`UN. TOTALES` solo lectura = `BULTOS REPOSICIÓN × Un. que viene en un bulto cerrado`) y, debajo del grid, **UN. QUE VIENE EN UN BULTO CERRADO** (etiqueta + input en la misma fila, centrados; entero ≥ 1; al Guardar crea o actualiza `prod_tienda_bultos` del ítem principal). En ambos casos, las etiquetas del bloque usan altura uniforme para mantener una fila horizontal de labels y otra de inputs. En la grilla, **CANT. REPOSIC.** muestra la cantidad de bultos (`reposicion_cant_conf`); **CANT. A PEDIR** y el pedido generado van en unidades (`cantConf × prod_tienda_bultos`). Encima de la tabla del modal va el botón ícono `+` centrado y con estilo `default` (fondo `primary`, ícono `primary-foreground`) para abrir el selector de productos adicionales (con **BULTO**, exige el input de unidades y filtra por ese valor). No CANT. FIJA POR UNID.
