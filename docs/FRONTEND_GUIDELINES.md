...
- **Texto**: **nombre de usuario** en MAYÚSCULAS (`global_personal.nombre_personal`). Sin sucursal ni módulo en este bloque (la sucursal queda en `sessionStorage` y alimenta el indicador compacto).
- **Cambiar Módulo**: solo si `modulos_permitidos.length > 1`. Línea blanca `h-px w-[80%] bg-sidebar-foreground/85` + botón **Cambiar Módulo** → modal con `Select` de los módulos permitidos de ese usuario + **Aplicar**.
