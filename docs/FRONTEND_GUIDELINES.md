...
- **Administración · pilares** (SSOT `administracionNav.ts`): **FINANZAS** · **LISTA PRECIOS** · **PEDIDO A FÁBRICA** · **ESTADÍSTICAS** · **CONFIGURACION**. **Estadísticas Productos** (URLs `/estadisticas-productos/*`, SSOT `ESTADISTICAS_PRODUCTOS_ROUTES`): **ESTADÍSTICAS** → **Vtas Por. Prod.**; **CONFIGURACION** → **Carga Datos** + **Categorizacion**. **Pedido A Fábrica** (SSOT `pedidoAFabricaRoutes.ts`, `/pedido-a-fabrica`; legacy `/estadisticas-productos/est-para-compra` → redirect). `/estadisticas-productos` → redirect a carga.
