...
- **Prueba PUT ajuste stock DUX** (no reemplaza el Excel): mismos ítems con variación que Exportar Excel. UI `PruebaPutStockDuxButton` → `probarPutAjusteStockDuxAction` (`requireStockAcceso`, sin `esEditor` — excepción **§1.2.3**). Zod `pruebaPutAjusteStockDuxSchema`. Servicio `enviarPruebaPutAjusteStockDux` (`duxAjusteStock.service.ts`) mapea sucursal → `global_sucursales.id_deposito` (`obtenerIdDepositoPorCodigoSucursal`; fallback `DUX_ID_STOCK_*`). `putAjusteStockItemV2` (`duxItemsV2Api.ts`): GET v1 `/items?codigoItem=` ([consultar-items](https://duxsoftware.readme.io/reference/consultar-items-1); auth **sin** Bearer, igual que la sync; `stock[].id` = depósito) y, si falla, GET v2 `/v2/items?cod_item=` ([listar_items](https://developers.duxsoftware.com.ar/reference/listar_items)) → pausa `DUX_API_BATCH_INTERVAL_MS` → PUT `/v2/items/{cod}?id_empresa=` ([actualizar_item](https://developers.duxsoftware.com.ar/reference/actualizar_item); `id_empresa` = `getDuxIdEmpresaCompras()`, default 2482; Bearer `DUX_API_TOKEN`). El PUT es **snake_case** OpenAPI (`id_personal`, `tipo_producto`, `cod_item`, `item`, `id_moneda`, `porc_iva`, `costo_compra`, `id_unidad_medida`, `sucursales_habilitadas[].id_sucursal` = **todas**: 1, 2, 4 o `DUX_SUCURSALES_HABILITADAS`). Body mínimo + `stock[{ id_deposito, ctd_disponible }]` de **todos** los depósitos del GET; solo se cambia el de la sucursal del filtro. Ítems con talle/color se rechazan (el PUT de prueba no arma `variantes[]`). `tipoProducto` / `idMoneda` / `idUnidadMedida` salen del GET si vienen; si no, env `DUX_TIPO_PRODUCTO` (SIMPLE), `DUX_ID_MONEDA`, `DUX_ID_UNIDAD_MEDIDA`. Un ítem por Action; `maxDuration` 60 en `/stock`. No escribe `ultimaExportacionExcel`. La sincro completa sigue en v1 `GET /items`.
...
- Reposición: `reposicion.ts` (Prisma parcial en Action). `reposicion_forma_pedido`: `UNIDADES_MAX` | `POR_BULTO` | `UNIDADES_FIJAS`. Vendedor (upsert regla): solo `UNIDADES_MAX` + `POR_BULTO`; `POR_BULTO` exige fila en `prod_tienda_bultos`. Solo se permiten productos con vínculo habilitado a proveedor no fábrica (`global_proveedores.es_fabrica = false`) tanto en grilla de Reposición como en selector de productos adicionales por bulto; `existeListaPrecioParaReposicionCodTienda` debe respetar esa misma condición. Listados de proveedor en Reposición (filtro de página y modal Generar Pedido origen reposición): solo proveedores de mercadería con `global_proveedores.es_fabrica = false`. Pedido A Fáb. (filas `A FÁBRICA`): solo `POR_BULTO` + `UNIDADES_FIJAS`. Cálculo `cantPedirReposicionMerc2` (pedir solo si `stock <= punto`): UNIDADES_MAX → `cantConf - stock` (unidades); UNIDADES_FIJAS → `cantConf` (unidades); POR_BULTO → `cantConf × bulto` (`cantConf` = cantidad de bultos persistida; `bulto` = unidades por bulto de `prod_tienda_bultos`; sin bulto válido → 0). Helper `cantConfReposicionAUnidades`. No reintroducir `CANT_MAX` / `CANT_FIJA_POR_BULTO` como valores persistidos.
>>>>>>> main
