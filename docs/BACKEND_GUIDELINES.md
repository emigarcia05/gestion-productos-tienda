...
- **Pantalla** `/finanzas/balance/gastos` (no confundir con el catálogo):
  ...
  - **Página** `src/app/finanzas/balance/gastos/page.tsx`: `searchParams` opcionales `mes` / `anio`. **`mes=todos`**: lista todo el año (`listarImputacionesMensualesBalance({ anio, mes: null })`). Si no, `mesAnioQuerySchema` (**mes** 1–12, **anio** 2026–2046). Si **no** vienen `mes` ni `anio` en la URL, **`redirect`** a `?mes=&anio=` con **mes y año calendario actuales en Argentina**. Si el parse falla, **`redirect`** al mismo default. `BalanceGastoMensualFila` incluye `mes`/`anio` de la imputación.
