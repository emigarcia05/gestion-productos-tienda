...
- **BULTO** (`prod_tienda_bultos`): 1:1 con `prod_tienda` (PK/FK `cod_tienda`). Sin fila = vacío. Entero positivo; `null` en Action borra la fila. Lectura: `buildMapBultosProdTienda` (Cx Compra `getTiendaPageData` y Pedido A Fáb. `listarProductosPorProveedorFabrica`). Mutación: `guardarBultoTiendaAction` (`tienda.acceso` + `esEditor()`). **No** entra aún en `cantPedirReposicionMerc2` (CANT_FIJA_POR_BULTO sigue usando `cantConf`).
...
- **Pedido A Fáb.:** `pedidoAFabrica.ts` — lectura `estadisticasProductos.acceso` (auth **antes** de Zod); upsert con pedidos **o** estadísticas. Métricas sucursales `genera_est`. Cálculos UI en `@/lib/pedidoAFabricaPromVta`. FORMA PEDIR en `reposicion_forma_pedido` de filas `A FÁBRICA` (no CANT_MAX). UI fábrica: labels **Bulto** / **Unidad**. Columna BULTO = `prod_tienda_bultos` solo si forma es `CANT_FIJA_POR_BULTO`.
