...
Tablas: `clientes` (catálogo; `nombre_completo` **en mayúsculas** `es-AR` al persistir; `tipo` = `CONSUMIDOR_FINAL` | `PINTOR`; **`pintor_asociado`** FK opcional a otro `clientes`), `envios_direcciones` (**`persona_id`** FK a cliente; en el alta, al `CONSUMIDOR_FINAL`), `envios_final` (envío). IDs CUID.
