...
- **Salida de modo editor**: no hay Server Action de “volver a simple”. Al cerrar el navegador, la cookie de arranque (`tienda-app-arranque`, sin `maxAge`) + middleware fuerzan rol `simple` en la siguiente sesión (ver párrafo siguiente). `activarModoEditor` sigue siendo la única Action de sesión.
