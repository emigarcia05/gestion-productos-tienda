...
- **Estadísticas Vtas** (`/estadisticas-productos/estadisticas-vtas`): reutiliza `listarProdTiendaCategorizacion` + `listarSucursalesParaEstPorProd` + periodos `listarPeriodosCargaEstPorProd`. UI **`EstVtasPageClient`**: filtros de producto (igual Categorizacion) + sucursal / fecha / modo unidad (`UNIDAD` \| `SUMA DE UNIDADES`). Gráficos: pendiente. Ruta SSOT `ESTADISTICAS_PRODUCTOS_ROUTES.estadisticasVtas`.
