...
- **Estadísticas Vtas** (`/estadisticas-productos/estadisticas-vtas`): `listarProductosEstVtas` (categorización + `factorSuma`) + `listarVentasEstVtas` (`est_por_prod` desde `EST_POR_PROD_CARGA_DESDE`) + `listarSucursalesParaEstPorProd`. Agregación cliente **`agregarUnidadesPorEjeY`**: eje Y = `variante` \| `marca` \| `rubro` \| `subRubro` \| `color` \| `terminacion`; opcional **`filtroPadre`** `{ ejeY, etiqueta }` para el gráfico 2 dependiente. **`agregarUnidadesPorSucursal`**: desglose legado de una categoría a barras por sucursal. **`agregarUnidadesPorEjeYDesgloseSucursal`**: desglose jerárquico del gráfico 1 (grupos `EstVtasGrupoDimension`: categoría → sucursales). **`agregarUnidadesMensualesAnio`**: serie de 12 meses del año de `fechaClave`, acotada por **`filtros`** (categorías de gráficos 1 y 2). Modo `unidad` = `vtas_en_un`, modo `suma` = `vtas_en_un * factorSuma`. UI **`EstVtasPageClient`** + dos **`EstVtasGraficoVarianteBarras`** + **`EstVtasGraficoBarrasMensual`** (barras verticales mensuales). Ruta SSOT `ESTADISTICAS_PRODUCTOS_ROUTES.estadisticasVtas`.
