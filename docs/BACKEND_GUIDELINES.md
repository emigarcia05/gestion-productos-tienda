...
- **BULTO** (`prod_tienda_bultos`): 1:1 con `prod_tienda` (PK/FK `cod_tienda`). Sin fila = vacío. Entero positivo; `null` en Action borra la fila. Lectura Cx Compra: `buildMapBultosProdTienda` en `getTiendaPageData`. Pedido A Fáb.: join `prod_precios_provee.cod_tienda` → `prod_tienda` → `prod_tienda_bultos` (sin vínculo = vacío). Mutación: `guardarBultoTiendaAction` (`tienda.acceso` + `esEditor()`). **No** entra aún en `cantPedirReposicionMerc2` (CANT_FIJA_POR_BULTO sigue usando `cantConf`).
