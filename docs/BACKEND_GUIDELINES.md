...
- **Prueba PUT ajuste stock DUX** (no reemplaza el Excel): mismos ítems con variación que Exportar Excel. UI `PruebaPutStockDuxButton` → `probarPutAjusteStockDuxAction` (`requireStockAcceso`, sin `esEditor` — excepción **§1.2.3**). Zod `pruebaPutAjusteStockDuxSchema`. Servicio `enviarPruebaPutAjusteStockDux` (`duxAjusteStock.service.ts`) mapea sucursal → depósito (`getIdDepositoGuaymallen` / `getIdDepositoMaipu`) y sucursales DUX (`global_sucursales.id_dux`). `putAjusteStockItemV2` (`duxItemsV2Api.ts`): GET `/v2/items?cod_item=` ([listar_items](https://duxsoftware.readme.io/reference/listar_items)) → pausa `DUX_API_BATCH_INTERVAL_MS` → PUT `/v2/items/{cod}?id_empresa=` ([actualizar_item](https://duxsoftware.readme.io/reference/actualizar_item); `id_empresa` = `getDuxIdEmpresaCompras()`, default 2482; Bearer `DUX_API_TOKEN`). El PUT **no es PATCH** y WSERP valida **camelCase** (`idPersonal`, `tipoProducto`=`SIMPLE`, `codItem`, `item`, `idMoneda`, `porcIva`, `costoCompra`, `idUnidadMedida`, `sucursalesHabilitadas[].idSucursal`). El body de prueba es **mínimo** (sin precios/códigos/rubro: esos extra suelen dar 500) + `stock[{ idDeposito, ctdDisponible }]`. (se actualiza el depósito de la sucursal del filtro; el resto se copia del GET). Defaults opcionales: `DUX_ID_MONEDA`, `DUX_ID_UNIDAD_MEDIDA`, `DUX_TIPO_PRODUCTO` (SIMPLE). Un ítem por Action; `maxDuration` 60 en `/stock`. No escribe `ultimaExportacionExcel`. La sincro completa sigue en v1 `GET /items`.
