...
- Indicador slidenav: `contarItemsPedidoPorTipoParaSlidenav` (mismos ítems que Generar Pedido, sucursal preferida) agrupa por proveedor; solo proveedores con cant. pedir > 0 y `global_proveedores.es_fabrica = false`. `getIndicadorSlidenavAction` expone `proveedoresPedido` + totales por tipo + `hayTransfOrigen` (`stock.acceso`; filas de `stock_trasn_depositos` con esa sucursal como origen; pedidos y transf. en paralelo). Aviso al login: `hayPendientesTransfOrigenAction` (solo el COUNT de origen; no arma Generar Pedido). Permiso `pedidos.acceso`; sin permiso el bloque de pedido va en 0 / lista vacía.
