...
*Última actualización (2026-07-27): **Asistente IA · cuentagotas** — el muestreo RGB es **solo cliente** (`colorMuestraImagen.ts` + canvas); no hay action ni persistencia de imagen. El prompt viene de **`prod_ia_diseno_promp.promp`**; en cliente `aplicarRgbAlPromptBuscarColor` reemplaza **`(R,G,B)`**. `url_redireccion` sigue en la misma tabla.*
