...
- **Estadísticas Vtas** (`/estadisticas-productos/estadisticas-vtas`): `listarProductosEstVtas` (categorización + `factorSuma`) + `listarVentasEstVtas` (`est_por_prod` desde `EST_POR_PROD_CARGA_DESDE`) + `listarSucursalesParaEstPorProd`. Agregación cliente **`agregarUnidadesPorVariante`**: variante = presentación; modo `unidad` = `vtas_en_un`, modo `suma` = `vtas_en_un * factorSuma`. UI **`EstVtasPageClient`** + **`EstVtasGraficoVarianteBarras`**. Ruta SSOT `ESTADISTICAS_PRODUCTOS_ROUTES.estadisticasVtas`.
