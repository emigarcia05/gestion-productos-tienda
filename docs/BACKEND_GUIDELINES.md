...
- **Catálogo unidades de presentación** (`est_por_prod_un_presentacion`, Prisma `EstPorProdUnPresentacion`): `unidad` (MAYÚSCULAS, único), `posicion_unidad` (`PREFIJO` \| `SUFIJO`), `suma` (boolean). Prefijo = unidad antes del número (`Nº 20`); Sufijo = después (`20 LTS`). `suma = true` permite agregar el valor numérico en estadísticas. Migración **`20260805190000_est_por_prod_presentacion_unidades`**. CRUD: `src/services/estPorProdUnPresentacion.service.ts` + `src/actions/estPorProdUnPresentacion.ts` + Zod `@/lib/validations/estPorProdUnPresentacion.ts`. UI: **Gestion Unidades**.
- **Catálogo presentaciones** (`est_por_prod_presentacion`, Prisma `EstPorProdPresentacion`; renombre de `est_por_prod_lts_conversion`): `texto` (MAYÚSCULAS, único, match regex; **derivado en servicio** con `formatearPresentacionConUnidad(presentacion_numerica, unidad_medida)` — el cliente **no** envía `texto`), `unidad_medida_id` → `est_por_prod_un_presentacion`, `presentacion_numerica`, `conversion_a_unidad_id` → `est_por_prod_un_presentacion` (**debe ser distinta** de `unidad_medida_id`), `conversion_a_unidad_presentacion`. Sin `conversion_lts`. Match: `matchPresentacionEnDescripcion` (texto más largo). CRUD: `src/services/estPorProdPresentacion.service.ts` + `src/actions/estPorProdPresentacion.ts` + Zod `@/lib/validations/estPorProdPresentacion.ts` (create/edit sin campo `texto`; refine unidades distintas). UI: **Gestion Presentacion** (form en 2 secciones: Presentacion + Conversion; **Unidad completa** / **Unidad convertida completa** solo lectura).
