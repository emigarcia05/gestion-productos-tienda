...
- **Catálogo presentaciones** (`est_por_prod_presentacion`, Prisma `EstPorProdPresentacion`; renombre de `est_por_prod_lts_conversion`): `texto` (MAYÚSCULAS, único, match regex), `unidad_medida_id` → `est_por_prod_un_presentacion`, `presentacion_numerica`, `conversion_a_unidad_id` → `est_por_prod_un_presentacion`, `conversion_a_unidad_presentacion`. Sin `conversion_lts`. Match: `matchPresentacionEnDescripcion` (texto más largo). CRUD: `src/services/estPorProdPresentacion.service.ts` + `src/actions/estPorProdPresentacion.ts` + Zod `@/lib/validations/estPorProdPresentacion.ts`. UI: **Gestion Presentacion**.
