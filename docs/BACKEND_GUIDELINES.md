...
- **Estadísticas Vtas** (`/estadisticas-productos/estadisticas-vtas`): `listarProductosEstVtas` (categorización + `factorSuma`) + `listarVentasEstVtas` (`est_por_prod` desde `EST_POR_PROD_CARGA_DESDE`) + `listarSucursalesParaEstPorProd`. Agregación cliente **`agregarUnidadesPorEjeY`**: eje Y = `variante` \| `marca` \| `rubro` \| `subRubro` \| `color` \| `terminacion` (default `variante` = presentación); modo `unidad` = `vtas_en_un`, modo `suma` = `vtas_en_un * factorSuma`. UI **`EstVtasPageClient`** + **`EstVtasGraficoVarianteBarras`** (Select de eje Y en el título). Ruta SSOT `ESTADISTICAS_PRODUCTOS_ROUTES.estadisticasVtas`.
