...
- **Prueba PUT ajuste stock DUX** (no reemplaza el Excel): mismos ítems con variación que Exportar Excel. UI `PruebaPutStockDuxButton` → `probarPutAjusteStockDuxAction` (`requireStockAcceso`, sin `esEditor` — excepción **§1.2.3**). Zod `pruebaPutAjusteStockDuxSchema`. Servicio `enviarPruebaPutAjusteStockDux` (`duxAjusteStock.service.ts`) mapea sucursal → depósito (`getIdDepositoGuaymallen` / `getIdDepositoMaipu`) y sucursales DUX (`global_sucursales.id_dux` **con `deposito` no vacío**). `putAjusteStockItemV2` (`duxItemsV2Api.ts`): GET v1 `/items?codigoItem=` ([consultar-items](https://duxsoftware.readme.io/reference/consultar-items-1); auth **sin** Bearer, igual que la sync; `stock[].id` = depósito) y, si falla, GET v2 `/v2/items?cod_item=` ([listar_items](https://duxsoftware.readme.io/reference/listar_items)) → pausa `DUX_API_BATCH_INTERVAL_MS` → PUT `/v2/items/{cod}?id_empresa=` ([actualizar_item](https://duxsoftware.readme.io/reference/actualizar_item); `id_empresa` = `getDuxIdEmpresaCompras()`, default 2482; Bearer `DUX_API_TOKEN`). El PUT **no es PATCH** y WSERP valida **camelCase** (`idPersonal`, `tipoProducto`, `codItem`, `item`, `idMoneda`, `porcIva`, `costoCompra`, `idUnidadMedida`, `sucursalesHabilitadas[].idSucursal`). Body mínimo (sin precios/códigos/rubro) + `stock[{ idDeposito, ctdDisponible }]` de **todos** los depósitos del GET; solo se cambia el de la sucursal del filtro. Ítems con talle/color se rechazan (el PUT de prueba no arma `variantes[]`). `tipoProducto` / `idMoneda` / `idUnidadMedida` salen del GET si vienen; si no, env `DUX_TIPO_PRODUCTO` (SIMPLE), `DUX_ID_MONEDA`, `DUX_ID_UNIDAD_MEDIDA`. Un ítem por Action; `maxDuration` 60 en `/stock`. No escribe `ultimaExportacionExcel`. La sincro completa sigue en v1 `GET /items`.
