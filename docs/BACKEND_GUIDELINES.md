...
- **Salida del modo editor**: no hay Server Action de “volver a simple”. El modo editor termina al **cerrar el navegador** (cookie de sesión `tienda-app-arranque` + middleware; ver siguiente viñeta). `activarModoEditor` sigue siendo la única Action de `sesion.ts`.
...
### 1.2.9 Auditoría backend — limpieza (2026-08-13)

**Cierre:** `node scripts/audit-actions-usage.mjs` → **0 huérfanas**. ESLint `src/actions src/services src/lib --max-warnings 0` ✓. **Sin cambios funcionales ni de contratos intencionales** (solo dead code, aliases deprecados y docs alineadas a la realidad).
...
| `finAnaMargenContribucion.ts` | `listarDescuentosFpMargenContribucionAction`, `listarFormulasMargenContribucionAction`, `actualizarFormulaMargenContribucionAction`, `crearFinAnaMcCategoriaAction`, `editarFinAnaMcCategoriaAction`, `eliminarFinAnaMcCategoriaAction` | Lecturas RSC/servicio; categorías vía `reemplazarFinAnaMcCategoriasAction`; descuentos vía `actualizarDescuentoFpMargenContribucionAction`. |
| `mktColoresMarca.ts` / `mktContenidoUrlDrive.ts` / `mktPublicaciones.ts` / `mktPublicacionesIdeas.ts` | `listarMktColoresMarcaAction`, `listarMktContenidoUrlDriveAction`, `listarMktPublicacionesCalendarioAction`, `listarMktIdeasJerarquiaAction` | RSC importa servicios. |
| `googleSheets.ts` | `probarConexionGoogleSheetsAction` | `probarConexionGoogleSheets` + `npm run test:google-sheets`. |
| `descEspecialReglas.ts` | `obtenerReglaDescEspecialPorCodExtAction` | Detalle por id: `obtenerReglaDescEspecialDetalleAction`. |
| `sesion.ts` | `volverModoSimple` | Cookie de sesión de navegador + middleware (§1.2.1). |

**Servicios / libs / validaciones eliminados:**

| Eliminado | Motivo |
|-----------|--------|
| `src/lib/syncProgressStore.ts` | Progreso DUX en `syncDuxStatusDb` / `sync_dux_status`. Incluía leftover de debug ingest. |
| `src/lib/importProgressStore.ts` | Progreso import lista en `importProgressDb`. |
| `listarEstPorProd`, `eliminarEstPorProd` + `eliminarEstPorProdSchema` | UI/API operan por periodo × sucursal. |
| `getListaPreciosConTienda` | Grilla usa `getListaPreciosConTiendaFiltrada`. |
| `listarFinTesoreriaTipoCaja`, `listarCajasTesoreriaPorTipoValor` | Tipos de caja vía enum Zod/UI; listado de cajas `listarCajasTesoreria` / `listarCajasTesoreriaPorTipoCaja`. |
| `getPrecioLista` / `getPrecioListaPrincipal` / `getPreciosPorCodTienda` | UI usa `buildMapPrecioListaPrincipal`. |
| `getStockMaipu` / `getStockGuaymallen` | UI/pedidos usan `getStockReal` + mapas por depósito. |
| `crearFinAnaMcCategoria` / `editarFinAnaMcCategoria` / `eliminarFinAnaMcCategoria` + Zod CRUD 1:1 | UI usa `reemplazarFinAnaMcCategorias`. |
| `agregarPedidoHistoriaItem` / `actualizarPedidoHistoriaItemCantRecibida` / `reabrirPedidoHistoriaRecepcion` | Recepción consolidada: `guardarRecepcionPedidoHistoria`. |
| `obtenerReglaDescEspecialPorCodExt` + `reglaDescEspecialPorCodExtSchema` | Sin callers. |
| `listarMktIdeaSecciones` / `listarMktIdeaDetalles` | RSC usa `listarMktIdeasJerarquia`. |
| `buildMapPxVtaSugeridoPorCodTienda` | Usar `buildMapPxSugeridoCompetenciaPorCodTienda`. |
| `listarMontosBrutosFinBalIvaDebPorAnio` / `importarCsvIvaDebitoMes` | Aliases deprecados; SSOT `listarIvaDebitoFinBalPorAnio` / `importarTxtIvaDebitoMes`. |
| `fechaHastaArgentinaComoDux` | Sync compras usa `fechaArgentinaMasDiasComoDux` / `fechaArgentinaMenosDiasComoDux`. |
| `MARKETING_ROUTE_BASE_MULTIMEDIA_LEGACY` | Usar `MARKETING_ROUTES.baseMultimedia.contenido`. |
| `ASISTENTE_IA_SUBMODULO_BUSCAR_COLOR_IMAGEN` (alias deprecado) | Canónico `ASISTENTE_IA_SUBMODULO_BUSCAR_CODIGO_IMAGEN`; lookup legacy sigue por `…_LEGACY`. |

**Seeds MC internos (ya no exportados):** `ensureFinAnaMcCategoriasSeed`, `ensureFinAnaMcFormulasSeed`, `ensureFinAnaMcDescuentoFpSeed`, `ensureFinAnaMcConfig` — se invocan desde `listar*` / `get*` del mismo archivo.

**No reintroducir** stores in-memory de progreso (`syncProgressStore` / `importProgressStore`) ni Actions paralelas a Route Handlers de Carga de Datos.
...
- **Lecturas precio tienda en UI**: **`prodTiendaPrecios.service.ts`** — `buildMapPrecioListaPrincipal` (mapa por lote; no hay wrapper 1:1 `getPrecioListaPrincipal`).
...
- **Entrada HTTP (sin Server Actions de este módulo)**: importación y borrado van por Route Handlers (ver siguientes viñetas). No existe `src/actions/estPorProd.ts`.
...
- **Borrado HTTP (grilla Carga de Datos)**: **`POST /api/est-por-prod/eliminar-periodo`** (mismo gate editor; body `eliminarEstPorProdPorPeriodoSchema`). La UI de la grilla usa esta ruta para feedback confiable + actualización optimista de celdas.
...
- **Servicio** (`src/services/mktPublicacionesIdeas.service.ts`): `listarMktIdeasJerarquia` (secciones + detalles anidados; RSC), CRUD sección/detalle. **Crear detalle**: `seccionId` + `tituloIdea` + `detalle` (opcional). Orden de detalles: libres primero (sin publicación) + **`titulo_idea` A–Z** (`es`).
...
- **Actions**: `src/actions/cajasTesoreria.ts`
  ...
  - `listarCajasTesoreriaTipoBancoAction` (destino UI **Acreditar cheque**), mutaciones de caja con `esEditor()` donde aplique.
...
- `competencia.service.ts` — CRUD + `normalizeWebUrlOptional` (alta/edición) y `normalizeWebUrl` (scripts seed; lanza si la URL queda vacía). Incluye `idProveedor` opcional.
...
*Última actualización (2026-07-15): **Google Sheets — probe de conexión** — ENV service account + `probarConexionGoogleSheets` (servicio; script `npm run test:google-sheets`); escribe `A1`. Export UI: `exportarMktGoogleSheetsAction`.*
...
*Última actualización (2026-08-13): **Auditoría backend — limpieza** — §1.2.9: Actions huérfanas (estPorProd duplicado de API, listados RSC de marketing/MC, probe Sheets, `volverModoSimple`); stores in-memory de progreso; wrappers/aliases deprecados. `audit-actions-usage.mjs` → 0 huérfanas. Sin cambios de reglas de negocio.*
