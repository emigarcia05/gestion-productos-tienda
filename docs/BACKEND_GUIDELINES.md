...
- **Anti-duplicado**: ventana **`TRANSF_DEPOSITOS_VENTANA_DUPLICADO_DIAS = 7`** (`src/lib/transfDepositosControl.ts`). Duplicado = mismo `cod_tienda` + origen + destino + cantidad con `created_at` ≥ ahora − 7 días.
- **Servicio** `src/services/transfDepositos.service.ts`: `listarControlesRecientesTransfDepositos`, `registrarControlTransfDepositos` (`forzar` para confirmar tras advertencia).
- **Action** `registrarControlTransfDepositosAction` + listado en `getTransfDepositos(origen, destino, params)` (`controlesRecientes`). Zod: `@/lib/validations/transfDepositos.ts`.
...
*Última actualización (2026-08-12): **Stock · Trans. Depósitos** — `prod_stock_transf_dep` + `registrarControlTransfDepositosAction`; anti-duplicado 7 días (mismo ítem/origen/destino/cantidad); listado sin `stock_real`.*
